...
## <a name="using-azure-cosmos-db-with-f"></a>С помощью Azure Cosmos DB с помощью F # #
...
1. Посредством создания функций Azure F # которой реагировать на них или вызывают изменения в коллекциях Azure Cosmos DB. См. в разделе [привязки Azure Cosmos DB для функций Azure](/azure/azure-functions/functions-bindings-cosmosdb), или
...
Аналогичным образом из скриптов F# можно развернуть и контролировать и другие ресурсы Azure. Например можно создавать учетные записи хранения, развертывать облачные службы Azure, создавать экземпляры Azure Cosmos DB и программно управлять концентраторами уведомлений Azure из скриптов F #.
