...
`CloudTableClient` Класс позволяет извлекать таблицы и сущности в хранилище таблиц. Вот один из способов создания клиента службы:
...
Чтобы запросить таблицу для всех сущностей в секции, используйте `TableQuery` объект. Здесь выполняется фильтрация для сущностей, где "Иванов" — ключ секции.
...
Можно написать запрос для получения одной конкретной сущности. Здесь `TableOperation` для указания клиента «Бен Смит» используется. Вместо коллекции вы получаете обратную передачу `Customer`. Указание как ключа секции, так и ключа строки в запросе — самый быстрый способ извлечь одну сущность из службы таблиц.
