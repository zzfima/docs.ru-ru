...
В примере функция `printTotalFileBytes` имеет тип `string -> Async<unit>`. Вызов функции фактически не выполняет асинхронное вычисление. Вместо этого он возвращает `Async<unit>`, который выступает в качестве *спецификации* работы, выполняемой асинхронно. Он вызывает `Async.AwaitTask` в своем тексте, который преобразует результат <xref:System.IO.File.ReadAllBytesAsync%2A> в соответствующий тип.
