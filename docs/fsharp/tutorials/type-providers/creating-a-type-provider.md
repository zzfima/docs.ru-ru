...
- [FSharp.Data.SqlClient](https://fsprojects.github.io/FSharp.Data.SqlClient/) имеет набор поставщиков типов для com, накопление во время проверки внедрение T-SQL в языке F #
...
Сделать указанных членов, (, подписи включают типы массивов, типы byref и создание экземпляров универсальных типов) с помощью нормали `MakeArrayType`, `MakePointerType`, и `MakeGenericType` на любом экземпляре System.Type, включая `ProvidedTypeDefinitions`.

Примечание: В некоторых случаях может потребоваться использовать вспомогательный модуль `ProvidedTypeBuilder.MakeGenericType`.  См. в документации пакета SDK поставщика типа для получения дополнительных сведений.
