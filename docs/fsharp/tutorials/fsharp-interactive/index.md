...
Чтобы открыть F# Interactive из консоли, запустите fsi.exe.  Вы найдете fsi.exe в «c:\Program файлы (x86) \Microsoft SDKs\F#\<версии > \Framework\<версии >\". Дополнительные сведения о доступных параметрах командной строки см. в статье [Параметры F# Interactive](fsharp-interactive-options.md).
