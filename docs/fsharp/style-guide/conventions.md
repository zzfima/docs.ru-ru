...
В F # так как можно скрыть элементы, открывается в область видимости другие уже присутствует. Это означает, что переупорядочение `open` инструкции можно изменить значение кода. В результате сортировки алфавитно-цифровом порядке (или pseudorandomly) обычно не рекомендуется, иначе создать другое поведение, которое могут рассчитывать.
