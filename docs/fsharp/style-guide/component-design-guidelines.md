...
Этот документ представляет собой набор рекомендации по проектированию компонентов для F# программирования, на основе F# рекомендации по проектированию компонентов, 14, перестанут, Microsoft Research и [другая версия](https://fsharp.org/specs/component-design-guidelines/) изначально проверенный и обслуживается F# Software Foundation.
...
Методы, описанные в этой статье выполните [хорошее пять принципов F# кода](index.md#five-principles-of-good-f-code)и таким образом использовать функций и объектов, программирование соответствующим образом.
...
Независимо от типа объекта F# кодирования вы выполняете, важно иметь опыт работы с [рекомендации по разработке библиотек .NET](../../standard/design-guidelines/index.md). Большинство других F# программистов .NET будет ознакомиться с рекомендациями, и ожидать, что код .NET в соответствии с их.
...
В этом разделе представлены рекомендации по разработке общедоступных F#-с выходом библиотеки; то есть библиотеки, предоставляя открытый API-интерфейсы, которые должны использоваться F# разработчиков. Существуют различные рекомендации по проектированию библиотеки, применимо специально к F#. В случае отсутствия определенные рекомендации, выполните рекомендации по разработке библиотек .NET являются резервной рекомендации.
...
| Объединения тегов     | PascalCase | Существительное | Некоторые из них, добавить, успех | Не используйте префикс общедоступных интерфейсов API. При необходимости используйте префикс при внутренней, такие как `type Teams = TAlpha | TBeta | TDelta.` |
...
#### <a name="use-either-pascalcase-or-camelcase-for-public-functions-and-values-in-f-modules"></a>Использовать для открытых функций и значения в PascalCase или camelCase F# модулей
...
* Пространства имен не может содержать F# функции, если они находятся в внутренним
...
В F#, это действие необходимо выполнить где что состояние не уже инкапсулируется другой языковой конструкции, такие как замыкание, выражения последовательности или асинхронное вычисление.
...
#### <a name="use-asynct-for-async-programming-at-f-api-boundaries"></a>Используйте `Async<T>` для асинхронного программирования в F# границе API
...
#### <a name="carefully-apply-f-extension-members-in-f-to-f-components"></a>Тщательно применить F# члены расширений в F#- к -F# компоненты
...
Можно имитировать с помощью «типизации по признакам» F# ограничениями членов. Однако члены, которые делают с помощью этого следует использовать не в общие в F#- к -F# библиотеки макетов. Это обусловлено тем, как правило, вызвать пользовательский код стать неудобными и привязаны к одной конкретной платформой шаблон библиотеки макетов на основе незнакомых или нестандартные неявное ограничений.
...
#### <a name="carefully-use-units-of-measure-for-added-type-safety-in-f-code"></a>Внимательно использовать единицы измерения для добавленных строгой типизации в F# кода
...
#### <a name="carefully-use-type-abbreviations-to-simplify-f-code"></a>Внимательно использовать сокращенные формы типов для упрощения F# кода
...
#### <a name="use-f-record-types-in-vanilla-net-apis-if-the-design-of-the-types-wont-evolve"></a>Используйте F# типах записей в обычный API-интерфейсы .NET, если не будет развиваться разработки типов
...
#### <a name="hide-the-representation-of-f-union-types-in-vanilla-net-apis"></a>Скрыть представление F# типам объединений в обычный API-интерфейсы .NET
...
#### <a name="use-net-delegate-types-instead-of-f-function-types"></a>Используйте типы делегата .NET вместо F# типы функций
...
#### <a name="use-the-trygetvalue-pattern-instead-of-returning-f-option-values-and-prefer-method-overloading-to-taking-f-option-values-as-arguments"></a>Использование шаблона TryGetValue вместо возвращения F# значения параметров, а предпочитаете перегрузка методов для ведения F# параметр значения в качестве аргументов
...
### <a name="end-to-end-example-of-designing-f-code-for-use-by-other-net-languages"></a>End-to-end примером проектирование F# код для использования с другими языками .NET
