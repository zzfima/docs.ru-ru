...
Для целевых объектов `assembly` атрибута и `module`атрибуты применяются к привязке верхнего уровня `do` в сборке. В объявление атрибута можно включить слово `assembly` или `module` следующим образом:
