...
description: Найти F# сведения о возможностях языка из этой ссылки для маркеров языка, основные понятия, типы, выражения и разделы конструкции, поддерживаемые компилятором.
ms.date: 05/16/2016
...
ms.locfileid: "57846640"
---
# <a name="f-language-reference"></a>Справочник по языку F#

В этом разделе является ссылкой на F# язык, мультипарадигматическим языком программирования, предназначенных для .NET. Язык F# поддерживает функциональные, объектно-ориентированные и императивные модели программирования.
...
|[Сигнатуры](signatures.md)|Описывает сигнатуры и их файлы. В файле сигнатур содержатся сведения об открытых сигнатурах набора элементов программы на языке F#, таких как типы, пространства имен и модули. Файл сигнатур можно использовать для указания доступности этих элементов программы.|
...
|[Базовые типы](basic-types.md)|Описывает основные базовые типы, которые используются в F# языка. Кроме того, указывает соответствующие типы .NET и минимальное и максимальное значения для каждого из них.|
...
|[Циклы. `for...to` Выражение](loops-for-to-expression.md)|Описывает выражение `for...to`, которое используется для циклической итерации по диапазону значений переменной цикла.|
|[Циклы. `for...in` Выражение](loops-for-in-expression.md)|Описывает выражение `for...in` — циклическую конструкцию, которая используется для итерации по совпадениям с шаблоном в перечислимой коллекции, такой как выражение диапазона, последовательность, список, массив или другая конструкция, поддерживающая перечисление.|
|[Циклы. `while...do` Выражение](loops-while-do-expression.md)|Описывает выражение `while...do`, используемое для итеративного выполнения (выполнения в цикле), пока заданное условие теста истинно.|
...
|[Отложенная выражения](lazy-expressions.md)|Описывает отложенной выражения, которые выполняются не сразу, но когда фактически требуется результат.|
