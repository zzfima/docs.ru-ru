...
|[Отложенные выражения](lazy-expressions.md)|Описывает отложенной выражения, которые выполняются не сразу, но когда фактически требуется результат.|
