...
Чтобы получить значение ByRef, переменная, которая содержит значение, должна находиться дольше текущей области.
Кроме того, чтобы возвратить ByRef, используйте & value (где value — это переменная, которая находится дольше текущей области).
