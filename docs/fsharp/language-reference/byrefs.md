...
Однако для F# типы значения, которые **являются** неизменяемым, `this` указатель определяется как `inref`.
...
`inref<'T>` Определить тип F# компилятора в следующих случаях:
...
Возвраты ByRef из F# функциям или членам можно создать и использовать. При использовании `byref`-возвращать метод, значение неявным образом не имеет. Пример:
