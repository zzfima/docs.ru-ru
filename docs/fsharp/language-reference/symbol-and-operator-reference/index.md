...
|`%`|[Арифметические операторы](arithmetic-operators.md)<br /><br />[Цитирование кода](../code-quotations.md)|<ul><li>Вычисляет целочисленный модуль.<br /></li><li>Используется для объединения выражений в типизированные цитаты кода.<br /></li><ul/>|
|`%%`|[Цитирование кода](../code-quotations.md)|<ul><li>Используется для объединения выражений в нетипизированные цитаты кода.<br /></li><ul/>|
|`%?`|[Операторы, допускающие значение NULL](nullable-operators.md)|<ul><li>Вычисляет целочисленный модуль, если правая часть является типом, допускающим значение NULL.<br /></li><ul/>|
...
|`:?`|[Выражения match](../match-expressions.md)|<ul><li>Возвращает `true`, если значение соответствует указанному типу; в противном случае возвращает `false` (оператор проверки типа).<br /></li><ul/>|
...
|`>?`|[Операторы, допускающие значение NULL](nullable-operators.md)|<ul><li>Вычисляет значение операции "больше чем", если правая часть является типом, допускающим значение NULL.<br /></li><ul/>|
...
|`>=`|[Арифметические операторы](arithmetic-operators.md)|<ul><li>Возвращает значение `true`, если правая часть больше или равна левой части; в противном случае возвращает значение `false`.<br /></li><ul/>|
...
|`as`|Справа|
|`when`|Правый|
|<code>&#124;</code>(вертикальная черта)|Левый|
|`;`|Справа|
|`let`|Неассоциативный|
...
|`&`, `&&`|По левому краю|
|`:>`, `:?>`|Правый|
|`!=`*OP*, `<` *op*, `>` *op*, `=`, <code>&#124;</code> *op*, `&`  *OP*,`&`<br /><br />(включая `<<<`, `>>>`, <code>&#124;&#124;&#124;</code>, `&&&`)|Левый|
...
|`*`*op*, `/`*op*, `%`*op*|Левый|
|`**`*op*|Правый|
|`f x` (применение функции)|Левый|
|<code>&#124;</code>(соответствие шаблону)|Правый|
|Префиксные операторы (`+`*op*, `-`*op*, `%`, `%%`, `&`, `&&`, `!`*op*, `~`*op*)|Левый|
|`.`|Слева|
|`f(x)`|Левый|
|`f<`*типы*`>`|Левый|
