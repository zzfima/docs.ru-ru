...
|`%?`|[Операторы, допускающие значение NULL](nullable-operators.md)|<ul><li>Вычисляет целочисленный остаток, если правая часть является тип nullable.<br /></li><ul/>|
