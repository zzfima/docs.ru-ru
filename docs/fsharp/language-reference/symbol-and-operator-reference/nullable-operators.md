...
В следующей таблице перечислены допускает значения NULL операторами, поддерживаемыми F# языка.
