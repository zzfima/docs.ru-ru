...
Объект `let` привязки на уровне модуля в типе или в вычислительном выражении может иметь явные параметры типов. Привязку let в выражении, например в определении функции не может иметь параметров типа. Дополнительные сведения см. в разделе [Универсальные типы](../generics/index.md).
