...
Передача по ссылке значения F # включает в себя `byref` ключевое слово, которое указывает, что параметр является фактически указатель на значение, передается по ссылке. Любое значение, передаваемое в метод с `byref` как аргумент должен быть `mutable`.
...
Можно сделать то же самое с [ссылочные ячейки](reference-cells.md), но важно обратить внимание, что **ссылочные ячейки и `byref`s — не то же самое**. Является контейнером для значения, можно проверить и изменить содержимое, но это значение хранится в куче и эквивалентно наличию записи с изменяемого значения, содержащиеся в нем не ссылочную ячейку. Объект `byref` является фактический указателем, поэтому различную семантику базовой и правила использования (которые могут быть довольно строгие).

Следующие примеры иллюстрируют использование `byref` ключевое слово. Обратите внимание, что при использовании ссылочной ячейки в качестве параметра, необходимо создать ссылочную ячейку как именованное значение и использовать его в качестве параметра, не просто добавить `ref` оператор, как показано в первом вызове `Increment` в следующем коде. Так как при создании ссылочной ячейки создается копия базового значения, первый вызов увеличивает только временное значение.
