...
|`and`|[`let`Привязки](./functions/let-bindings.md)<br /><br />[Записи](records.md)<br /><br />[Члены](./members/index.md)<br /><br />[Ограничения](./generics/constraints.md)|Используется в взаимно рекурсивных привязках и записях, в объявлениях свойств и с несколькими ограничениями на универсальные параметры.|
...
|`do`|[Привязки do](./functions/do-bindings.md)<br /><br />[Бираются `for...to`Выражение](loops-for-to-expression.md)<br /><br />[Бираются `for...in`Выражение](loops-for-in-expression.md)<br /><br />[Бираются `while...do`Выражение](loops-while-do-expression.md)|Используется в циклических конструкциях или для выполнения императивного кода.|
...
|`downto`|[Бираются `for...to`Выражение](loops-for-to-expression.md)|`for` В выражении используется при подсчете в обратную.|
...
|`false`|[Типы-примитивы](primitive-types.md)|Используется в качестве логического литерала.|
|`finally`|[Исключения. `try...finally` Выражение](/.exception-handling/the-try-finally-expression.md)|Используется вместе с `try` , чтобы ввести блок кода, который выполняется независимо от того, возникает ли исключение.|
|`fixed`|[Префикс](fixed.md)|Используется для «закрепления» указателя на стеке, чтобы предотвратить сбор мусора.|
|`for`|[Бираются `for...to`Выражение](loops-for-to-expression.md)<br /><br />[Циклы: выражение for...in](loops-for-in-expression.md)|Используется в циклических конструкциях.|
...
|`not`|[Справочник символов и операторов](./symbol-and-operator-reference/index.md)<br /><br />[Ограничения](./generics/constraints.md)|В действительности не является ключевым словом. `not struct` Однако в сочетании используется как ограничение универсального параметра.|
...
|`or`|[Справочник символов и операторов](./symbol-and-operator-reference/index.md)<br /><br />[Ограничения](./generics/constraints.md)|Используется с логическими условиями в качестве `or` логического оператора. Аналогично параметру `||`.<br /><br />Также используется в ограничениях элементов.|
...
|`to`|[Бираются `for...to`Выражение](loops-for-to-expression.md)|Используется в `for` циклах для обозначения диапазона.|
|`true`|[Типы-примитивы](primitive-types.md)|Используется в качестве логического литерала.|
...
|`use`|[Управление ресурсами: Ключевое слово `use`](resource-management-the-use-keyword.md)|Используется вместо `let` для значений `Dispose` , которые необходимо вызывать для освобождения ресурсов.|
|`use!`|[Выражения вычисления](computation-expressions.md)<br /><br />[Асинхронные рабочие потоки](asynchronous-workflows.md)|Используется вместо `let!` в асинхронных рабочих процессах и других вычислительных выражениях `Dispose` для значений, которые необходимо вызывать для освобождения ресурсов.|
|`val`|[Явные поля. Ключевое слово `val`](./members/explicit-fields-the-val-keyword.md)<br /><br />[Сигнатуры](signatures.md)<br /><br />[Члены](./members/index.md)|Используется в сигнатуре для обозначения значения или в типе для объявления члена в ограниченных ситуациях.|
|`void`|[Типы-примитивы](primitive-types.md)|Указывает тип .NET `void` . Используется при взаимодействии с другими языками .NET.|
...
|`while`|[Бираются `while...do`Выражение](loops-while-do-expression.md)|Вводит циклическую конструкцию.|
