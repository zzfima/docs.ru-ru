---
title: "Ссылки на XML-сущности не поддерживаются"
ms.date: 07/20/2015
ms.prod: .net
ms.reviewer: 
ms.suite: 
ms.technology: devlang-visual-basic
ms.topic: article
f1_keywords:
- vbc31180
- bc31180
helpviewer_keywords: BC31180
ms.assetid: 2a393327-d8e2-4187-85b1-642b4f53b4ae
caps.latest.revision: "6"
author: dotnet-bot
ms.author: dotnetcontent
ms.openlocfilehash: 3a264f8b5ded454efb86eec07cff378e89b87fec
ms.sourcegitcommit: 4f3fef493080a43e70e951223894768d36ce430a
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 11/21/2017
---
# <a name="xml-entity-references-are-not-supported"></a>Ссылки на XML-сущности не поддерживаются
Ссылка на сущность (например, `©`), не определенные в XML 1.0 спецификация включена как значение XML-литерала. Только `&`, `"`, `<`, `>`, и `'` в XML-литералах поддерживаются ссылки на объект XML.  
  
 **Идентификатор ошибки:** BC31180  
  
## <a name="to-correct-this-error"></a>Исправление ошибки  
  
-   Удалите ссылки на сущности не поддерживается.  
  
## <a name="see-also"></a>См. также  
 [XML-литералы и спецификация XML 1.0](../../../visual-basic/programming-guide/language-features/xml/xml-literals-and-the-xml-1-0-specification.md)  
 [XML-литералы](../../../visual-basic/language-reference/xml-literals/index.md)  
 [XML](../../../visual-basic/programming-guide/language-features/xml/index.md)
