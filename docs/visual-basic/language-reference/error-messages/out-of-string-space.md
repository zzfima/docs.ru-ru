---
title: Отсутствует место в строке (Visual Basic)
ms.date: 07/20/2015
ms.prod: .net
ms.reviewer: ''
ms.suite: ''
ms.technology:
- devlang-visual-basic
ms.topic: article
f1_keywords:
- vbrID14
ms.assetid: 16681c75-a400-422d-9351-c691d3c7614e
caps.latest.revision: 10
author: dotnet-bot
ms.author: dotnetcontent
ms.openlocfilehash: d820bdfd7c66ecbe81f8cb75ada2374045257598
ms.sourcegitcommit: 4f3fef493080a43e70e951223894768d36ce430a
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 11/21/2017
---
# <a name="out-of-string-space-visual-basic"></a>Отсутствует место в строке (Visual Basic)
С помощью Visual Basic можно использовать очень больших строк. Однако требования других программ и способы работы со строками по-прежнему может вызвать эту ошибку.  
  
## <a name="to-correct-this-error"></a>Исправление ошибки  
  
1.  Убедитесь, что требует создания временной строки во время вычисления выражения, не вызывает ошибку.  
  
2.  Удалите все ненужные приложения из памяти, чтобы освободить место.  
  
## <a name="see-also"></a>См. также  
 [Типы ошибок](../../../visual-basic/programming-guide/language-features/error-types.md)  
 [Сводка по работе со строками](../../../visual-basic/language-reference/keywords/string-manipulation-summary.md)
