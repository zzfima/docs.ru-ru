...
|<xref:Microsoft.VisualBasic.Strings.Format%2A>|Возвращает строку, отформатированную в соответствии с инструкциями, содержащимися в формате `String` Expression.|  
...
В этом примере используется функция `LTrim` для удаления начальных пробелов и функции `RTrim` для удаления конечных пробелов из строковой переменной. Для удаления обоих типов пробелов используется функция `Trim`.  
  
[!code-vb[VbVbalrStrings#25](~/samples/snippets/visualbasic/VS_Snippets_VBCSharp/VbVbalrStrings/VB/Class1.vb#25)]  
  
## <a name="example-mid"></a>Пример: mid

В этом примере используется функция `Mid` для возврата указанного числа символов из строки.  
...
В этом примере показаны различные варианты использования функции `Format` для форматирования значений с помощью форматов `String` и пользовательских форматов. Для разделителя даты (`/`), разделителя времени (`:`) и индикаторов AM/PM (`t` и `tt`), фактически отформатированные выходные данные, отображаемые системой, зависят от параметров языкового стандарта, используемых кодом. Если время и даты отображаются в среде разработки, используются короткий формат времени и короткий формат даты языкового стандарта кода.  
