...
|`datatype`|Необязателен при [ `Option Infer` ](option-infer-statement.md) (по умолчанию) или `element` уже объявлено; требуется, если `Option Infer` отключен и `element` уже не объявлен. Тип данных `element`.|  
...
Если [Option Infer](option-infer-statement.md) является (Настройка его по умолчанию), компилятор Visual Basic может вывести тип данных `element`. Если он выключен и `element` еще не был объявлен вне цикла, необходимо объявить ее в `For Each` инструкции. Чтобы объявить тип данных `element` явно, используйте `As` предложение. Если тип данных элемента определен вне данного `For Each`... `Next` конструкция, его область представляет собой тело цикла. Обратите внимание, что нельзя объявлять `element` и снаружи цикла.
...
Среда выполнения должна быть возможность преобразовать элементы в `group` для `element`. [`Option Strict`] Инструкция определяет, разрешено ли расширяющих и сужающих преобразований (`Option Strict` выключен, его значение по умолчанию), или, разрешена ли только расширяющие преобразования (`Option Strict` включен). Дополнительные сведения см. в разделе [сужающие преобразования](#narrowing-conversions).
