...
|`datatype`|Необязательный. Тип данных `counter`. Дополнительные сведения см. в подразделе [аргумент Counter](#BKMK_Counter) далее в этой статье.|
...
|`step`|Необязательный. Числовое выражение. Величина, на которую `counter` увеличивается каждый раз с помощью цикла.|
|`statements`|Необязательный. Одна или несколько инструкций между `For` и `Next`, которые выполняются указанное число раз.|
|`Continue For`|Необязательный. Передает управление в следующую итерацию цикла.|
|`Exit For`|Необязательный. Передает управление за пределы цикла `For`.|
|`Next`|Обязательный. Завершает определение цикла `For`.|
...
> [!TIP]
> Ответ [... Конец оператора while](../../../visual-basic/language-reference/statements/while-end-while-statement.md) или [Do... Оператор Loop](../../../visual-basic/language-reference/statements/do-loop-statement.md) хорошо работает, если заранее неизвестно, сколько раз нужно выполнять инструкции в цикле. Однако, если вы планируете запускать цикл определенное число раз, `For`... лучше выбрать цикл `Next`. Число итераций определяется при первом входе в цикл.

## <a name="nesting-loops"></a>Вложенные циклы

Можно вкладывать `For` циклы, помещая один цикл внутрь другого. В следующем примере демонстрируется вложенная `For`... `Next` структуры, которые имеют разные значения шага. Внешний цикл создает строку для каждой итерации цикла. Внутренний цикл уменьшает переменную счетчика цикла для каждой итерации цикла.
...
`Exit For` часто используется после вычисления некоторого условия (например, в структуре `If`... `Then`... `Else`). @No__t_0 может потребоваться использовать для следующих условий:
...
- @No__t_0... `Catch`... Оператор `Finally` перехватывает исключение. В конце блока `Finally` можно использовать `Exit For`.
...
|Имеется `datatype`?|@No__t_0 уже определена?|Результат (`counter` определяет новую локальную переменную, ограниченную всем циклом `For...Next`)|
|----------------------------|-----------------------------------|-------------------------------------------------------------------------------------------------------------|
...
- @No__t_0, `SByte`, `UShort`, `Short`, `UInteger`, `Integer`, `ULong`, `Long`, `Decimal`, `Single` или 0.
