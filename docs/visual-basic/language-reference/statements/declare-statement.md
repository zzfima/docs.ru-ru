...
|`libname`|Обязательный. Имя файла, которая содержит объявленные процедуру.|  
|`Alias`|Необязательный параметр. Указывает, что процедуры невозможно определить в файле с именем, указанным в `name`. Необходимо указать его идентификацию в `aliasname`.|  
