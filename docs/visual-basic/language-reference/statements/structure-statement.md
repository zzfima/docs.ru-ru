...
|`Partial`|Необязательный параметр. Указывает частичное определение структуры. См. в разделе [частичного](../../../visual-basic/language-reference/modifiers/partial.md).|  
...
|`Of`|Необязательный параметр. Указывает, что это структурой универсального типа.|  
...
|`Implements`|Необязательный параметр. Указывает, что эта структура реализует члены один или несколько интерфейсов. См. в разделе [реализует оператор](../../../visual-basic/language-reference/statements/implements-statement.md).|  
...
|`methodmemberdeclarations`|Необязательный параметр. Ноль или несколько объявлений `Function`, `Operator`, `Property`, или `Sub` процедуры, которые служат в качестве *метода члены* структуры.|  
