...
-   `Partial`  
     ...
     Необязательно. Указывает определение разделяемого метода. В разделе [разделяемые методы](../../../visual-basic/programming-guide/language-features/procedures/partial-methods.md).  
...
-   `Shared`  
     ...
     Необязательно. В разделе [общих](../modifiers/shared.md).  
  
-   `Shadows`  
  
     Необязательно. В разделе [Shadows](../modifiers/shadows.md).  
  
-   `Async`  
  
     Необязательно. В разделе [Async](../modifiers/async.md).  
...
-   `Implements`  
     ...
     Необязательно. Указывает, что эта процедура реализует один или несколько `Sub` процедуры, каждая из которых определена в интерфейс, реализуемый этой процедуры классом или структурой. В разделе [реализует оператор](implements-statement.md).  
...
-   `statements`  
     ...
     Необязательно. Блок операторов для работы в рамках этой процедуры.  
...
>  `Async` Процедура возвращает вызывающий объект при обнаружении либо первый ожидаемый объект, пока не завершена или в конце `Async` достигается процедуры, какое событие происходит раньше.  
 ...
 `Async``Sub` процедуры в основном используются для обработчиков событий, где не может быть возвращено значение. `Async``Sub` Процедуру нельзя ожидать и код, вызывающий `Async``Sub` процедуры не могут перехватывать исключения, `Sub` процедура вызывает исключение.  
