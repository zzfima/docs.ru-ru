...
## <a name="if-operator-called-with-three-arguments"></a>Оператор If с тремя аргументами  
 Если `If` вызывается с помощью трех аргументов, первый аргумент должен иметь значение, которое можно привести к типу `Boolean`. Значение `Boolean` определяет, какое из двух других аргументов вычисляется и возвращается. Следующий список применяется только в том случае, если оператор `If` вызывается с тремя аргументами.  
...
|Термин|Определение|  
|---|---|  
|`argument2`|Обязательный. `Object`. Должен быть ссылкой или типом, допускающим значение null. Вычисляется и возвращается, если он равен любому чему, кроме `Nothing`.|  
|`argument3`|Обязательный. `Object`. Вычисляется и возвращается, если `argument2` принимает значение `Nothing`.|  
  
 Если аргумент `Boolean` опущен, первый аргумент должен быть ссылкой или типом, допускающим значение null. Если первый аргумент принимает значение `Nothing`, возвращается второй аргумент. Во всех остальных случаях возвращается значение первого аргумента. В следующем примере показано, как работает эта оценка.  
