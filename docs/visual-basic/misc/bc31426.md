---
title: "Имена типов, отличных от встроенных, не допустимы в выражениях условной компиляции | Microsoft Docs"
ms.custom: ""
ms.date: "12/15/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "devlang-visual-basic"
ms.tgt_pltfrm: ""
ms.topic: "article"
f1_keywords: 
  - "bc31426"
  - "vbc31426"
helpviewer_keywords: 
  - "BC31426"
ms.assetid: 73deeafa-4dbe-4c27-aff7-c6bea0e37d5a
caps.latest.revision: 7
caps.handback.revision: 7
author: "stevehoag"
ms.author: "shoag"
manager: "wpickett"
---
# Имена типов, отличных от встроенных, не допустимы в выражениях условной компиляции
Выражение условной компиляции ссылается на невстроенный тип, а допустимы только встроенные типы.  
  
 Дополнительные сведения о встроенных типах в [!INCLUDE[vbprvb](../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)] см. в разделе [Сводка по типам данных](../../visual-basic/language-reference/keywords/data-types-summary.md).  
  
 **Идентификатор ошибки:** BC31426  
  
### Исправление ошибки  
  
-   Замените невстроенный тип на встроенный.  
  
## См. также  
 [НЕ В СБОРКЕ. Условная компиляция \(Visual Basic\)](http://msdn.microsoft.com/ru-ru/ad1e35e0-935e-4a35-a2ae-738bcf2a9240)   
 [Сводка по типам данных](../../visual-basic/language-reference/keywords/data-types-summary.md)