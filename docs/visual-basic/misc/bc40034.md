...
Класс помечен как `<CLSCompliant(True)>`, но содержит процедуру или свойство `MustOverride` , помеченное как `<CLSCompliant(False)>` или не помеченное вообще.  
 ...
 Если класс совместим с [независимость от языка и независимые от языка компоненты](../../../docs/standard/language-independence-and-language-independent-components.md) (CLS), приложение, использующее этот класс обращается только к членам, которые также помечены как `<CLSCompliant(True)>` и игнорирует члены, не. Однако приложение не может игнорировать процедуру или свойство `MustOverride` , поскольку доступ к этому объекту необходим для его переопределения.  
