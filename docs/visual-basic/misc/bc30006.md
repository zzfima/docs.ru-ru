...
Требуется ссылка на модуль "\<`modulename`\>", содержащий определение события "\<`eventname`\>". Добавьте эту ссылку в проект.  
 ...
 Событие определяется в модуле, на который в проекте нет прямой ссылки. Компилятор [!INCLUDE[vbprvb](../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] требует ссылки в целях устранения неоднозначности, если событие определено в нескольких модулях.  
