...
|Атрибут|Описание|
|---|---|
|`ID`|Обязательный. Уникальный идентификатор для расширения. Если расширение, которое имеет этот идентификатор уже был добавлен в проект, он будет не предложено добавить его снова.|
|`Version`|Обязательный. Номер версии для шаблона элемента.|
|`AssemblyFullName`|Необязательный параметр. Имя сборки. При добавлении в проект ссылку на эту сборку, пользователю будет предложено добавить `My` расширения на основе этого шаблона элемента.|
