...
## <a name="types-of-streams"></a>Типы потоков  
 В [!INCLUDE[dnprdnshort](../../../../csharp/getting-started/includes/dnprdnshort_md.md)] поток представлен классом <xref:System.IO.Stream>, который образует абстрактный класс для всех других потоков. Вы не можете непосредственно создать экземпляр класса <xref:System.IO.Stream>. Вместо него вы должны использовать один из реализуемых им классов.  
