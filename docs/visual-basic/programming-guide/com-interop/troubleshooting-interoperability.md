...
## <a name="overloaded-methods"></a>Перегруженные методы  
 Хотя можно создать перегруженные методы с помощью [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)], они не поддерживаются COM. Когда класс, содержащий перегруженные методы предоставляется как объект COM, новые имена методов создаются для перегруженных методов.  
