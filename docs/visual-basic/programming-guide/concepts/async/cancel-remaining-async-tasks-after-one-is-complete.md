...
---
# <a name="cancel-remaining-async-tasks-after-one-is-complete-visual-basic"></a>Отмена оставшихся асинхронных задач после одной из них завершения (Visual Basic)
...
## <a name="building-the-example"></a>Построение примера  
 Пример в этом разделе добавляет в проект, который разработан в [Отмена асинхронной задачи или список задач](http://msdn.microsoft.com/library/d6e4e801-df64-4705-98fc-df725a577fb0) отменить список задач. В примере используется тот же пользовательский интерфейс, хотя кнопка **Отмена** не используется явно.  
 ...
 В файле MainWindow.xaml.vb **CancelAListOfTasks** проекта, запустить перехода, перемещая шагам обработки для каждого веб-сайта из цикла в `AccessTheWebAsync` следующие асинхронного метода.  
