...
## <a name="how-to-create-and-use-constants"></a>Создание и использование констант  
 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]содержит ряд предопределенных констант, главным образом с помощью для печати и отображения. Можно также создать собственные константы с `Const` инструкция, использующая те же правила, что для создания имени переменной. Если `Option Strict` — `On`, необходимо явно объявить тип константы.  
...
|[Типы данных констант и литералов](../../../../visual-basic/programming-guide/language-features/constants-enums/constant-and-literal-data-types.md)|Предоставляет сведения о том, как [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] компилятор инициализирует константы при `Option Explicit` отключена.|  
...
|Определение|Термин|  
|---|---|  
|[Константы и перечисления](../../../../visual-basic/language-reference/constants-and-enumerations.md)|Список предопределенных констант в [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)].|  
