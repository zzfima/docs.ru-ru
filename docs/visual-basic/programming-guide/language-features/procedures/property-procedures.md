...
Процедура свойства — это последовательность [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] операторы, которые управляют пользовательских свойств для модуля, класса или структуры. Процедуры свойств также называются *свойствам*.  
  
 [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]предоставляет следующие процедуры свойств:  
