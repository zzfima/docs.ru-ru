...
Элементы массива students  
 ...
 Массив `students` в предыдущем примере имеет один индекс и считается одномерным. Массив, имеющий более одного индекса, называется многомерным. Дополнительные сведения см далее в этом статье, а также в статье[Array Dimensions in Visual Basic](../../../../visual-basic/programming-guide/language-features/arrays/array-dimensions.md) (Размерность массивов в Visual Basic).  
...
##  <a name="BKMK_Iterating"></a> Перебор массива  
 ...
 Приведенный выше пример кода выполняет перебор одномерного массива с помощью [оператора For...Next](../../../../visual-basic/language-reference/statements/for-next-statement.md). Метод <xref:System.Array.GetUpperBound%2A> возвращает наибольшее значение, которое может иметь индекс. Наименьшее значение индекса всегда равно 0.  
  
 [!code-vb[VbVbalrArrays#41](../../../../visual-basic/programming-guide/language-features/arrays/codesnippet/VisualBasic/index_13.vb)]  
  
 В приведенном ниже примере выполняется перебор многомерного массива с помощью оператора `For...Next` . Метод <xref:System.Array.GetUpperBound%2A> имеет параметр, который обозначает порядковый номер измерения. `GetUpperBound(0)` возвращает максимальное значение индекса для первого измерения, а `GetUpperBound(1)` возвращает максимальное значение индекса для второго измерения.  
...
-   Чтобы не создавать риска исключений <xref:System.NullReferenceException>, код приложения должен использовать члены класса <xref:System.Array>, например <xref:System.Array.Length%2A> или <xref:System.Array.Rank%2A>, или вызвать функцию [!INCLUDE[vbprvb](../../../../csharp/programming-guide/concepts/linq/includes/vbprvb_md.md)], например <xref:Microsoft.VisualBasic.Information.UBound%2A>.  
...
|Ограничения длины|Длина каждого измерения массива ограничена максимальным значением типа данных `Integer`, которое составляет (2 ^ 31) - 1. Однако общий размер массива также ограничен доступной памятью в системе. Если вы попытаетесь инициализировать массив, размер которого превышает объем доступной оперативной памяти, среда CLR создаст исключение <xref:System.OutOfMemoryException>.|  
...
##  <a name="BKMK_ArrayTypes"></a> Типы массивов и другие типы  
 ...
 Каждый массив наследует от класса <xref:System.Array?displayProperty=fullName>. Вы можете объявить переменную типа `Array`, но не можете создать массив типа `Array`. Кроме того, [оператор ReDim`Array` не может работать с переменной, объявленной с типом ](../../../../visual-basic/language-reference/statements/redim-statement.md). По этой причине и в целях строгой типизации рекомендуется объявлять каждый массив как конкретный тип, например `Integer` , как в предыдущем примере.  
  
 Выяснить тип данных массива или его элементов можно несколькими способами.  
  
-   Во время выполнения вы можете вызвать для переменной метод <xref:System.Object.GetType%2A?displayProperty=fullName>, чтобы получить объект <xref:System.Type>, содержащий тип переменной. Объект <xref:System.Type> содержит в свойствах и методах довольно подробные сведения.  
...
-   Можно передать переменную в функцию <xref:Microsoft.VisualBasic.Information.VarType%2A>, чтобы получить объект `VariantType`, представляющий классификацию типа этой переменной.  
...
### <a name="example"></a>Пример  
 Следующий пример кода использует универсальный класс <xref:System.Collections.Generic.List%601?displayProperty=fullName> платформы [!INCLUDE[dnprdnshort](../../../../csharp/getting-started/includes/dnprdnshort_md.md)] для создания коллекции-списка объектов `Customer`.  
...
|Термин|Определение|  
|----------|----------------|  
|[Array Dimensions in Visual Basic](../../../../visual-basic/programming-guide/language-features/arrays/array-dimensions.md) (Размерность массивов в Visual Basic)|Объяснение ранга и измерений в массиве.|  
