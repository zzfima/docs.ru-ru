...
Оператор в [!INCLUDE[vbprvb](../../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] — это полная инструкция.  В инструкциях содержатся ключевые слова, операторы, переменные, константы и выражения.  Каждая инструкция принадлежит к одной из следующих двух категорий:  
