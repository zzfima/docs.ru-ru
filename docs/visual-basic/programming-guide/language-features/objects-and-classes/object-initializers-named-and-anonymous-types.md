...
## <a name="named-types"></a>Именованные типы  
 ...
 Каждое из этих объявлений эквивалентно в следующем примере, который создает `Customer` объекта с помощью конструктора без параметров, а затем определяет начальные значения для `Name` и `City` свойств с помощью `With` инструкция.  
