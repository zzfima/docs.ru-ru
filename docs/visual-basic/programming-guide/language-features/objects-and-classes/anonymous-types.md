...
## <a name="declaring-an-anonymous-type"></a>Объявление анонимного типа  
 ...
 Дополнительные сведения о других способах указания свойств анонимного типа см. [в разделе как Выводит имена и типы свойств в объявлениях](../../../../visual-basic/programming-guide/language-features/objects-and-classes/how-to-infer-property-names-and-types-in-anonymous-type-declarations.md)анонимного типа.  
...
### <a name="read-only-values"></a>Значения только для чтения  
 Значения ключевых свойств нельзя изменить. Например, `prod8` в в предыдущем примере поля и `Price` имеют `read-only`значение `Name` , но `OnHand` могут быть изменены.  
  
 [!code-vb[VbVbalrAnonymousTypes#8](~/samples/snippets/visualbasic/VS_Snippets_VBCSharp/VbVbalrAnonymousTypes/VB/Class1.vb#8)]  
  
## <a name="anonymous-types-from-query-expressions"></a>Анонимные типы из выражений запросов  
 Выражениям запросов не всегда требуется создавать анонимные типы. По возможности они используют существующий тип для хранения данных столбца. Это происходит, когда запрос возвращает либо целые записи из источника данных, либо только одно поле из каждой записи. В следующих примерах `customers` кода — это коллекция объектов `Customer` класса. Класс имеет много свойств, и вы можете включить один или несколько из них в результат запроса в любом порядке. В первых двух примерах анонимные типы не требуются, поскольку запросы выбирают элементы именованных типов:  
  
- `custs1`содержит коллекцию строк, поскольку `cust.Name` является строкой.  
  
     [!code-vb[VbVbalrAnonymousTypes#30](~/samples/snippets/visualbasic/VS_Snippets_VBCSharp/VbVbalrAnonymousTypes/VB/Class2.vb#30)]  
  
- `custs2`содержит коллекцию `Customer` объектов, поскольку каждый `customers` элемент объекта является `Customer` объектом, а весь элемент выбирается запросом.  
...
## <a name="an-anonymous-type-definition"></a>Определение анонимного типа  
 ...
 Если анонимный тип содержит по крайней мере одно ключевое свойство, определение переопределяет три <xref:System.Object>члена <xref:System.Object.Equals%2A>, <xref:System.Object.GetHashCode%2A>наследуемые от:, и <xref:System.Object.ToString%2A>. Код, созданный для проверки равенства и определения значения хэш-кода, рассматривает только ключевые свойства. Если анонимный тип не содержит свойств ключа, переопределяется только <xref:System.Object.ToString%2A> . Явно именованные свойства анонимного типа не могут конфликтовать с этими созданными методами. То есть нельзя использовать `.Equals`, `.GetHashCode`или `.ToString` для именования свойства.  
  
 Определения анонимных типов, имеющие по крайней мере одно ключевое <xref:System.IEquatable%601?displayProperty=nameWithType> свойство, также `T` реализуют интерфейс, где — это тип анонимного типа.  
