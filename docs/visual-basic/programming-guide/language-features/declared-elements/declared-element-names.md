...
## <a name="rules"></a>Правила  
 Имя элемента в [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] необходимо соблюдать следующие правила:  
...
> [!CAUTION]
>  Имена элементов, начинающиеся с символа подчеркивания (`_`) которые не являются частью [независимость от языка и независимые от языка компоненты](https://msdn.microsoft.com/library/12a7a7h3) (CLS), поэтому CLS-совместимом коде нельзя использовать компонент, определяющий такие имена. Тем не менее символ подчеркивания в любом другом месте в имени элемента является CLS-совместимым.  
...
## <a name="escaped-names"></a>Дублирующие имена  
 Как правило, имя элемента должно соответствует любые ключевые слова, зарезервированные [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)], такие как `Case` или `Friend`. Тем не менее, можно определить *escape-последовательность имя*, что заключено в квадратные скобки (`[ ]`). Escape-имя может совпадать с любым [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] ключевое слово, так как любая неопределенность в скобки. Скобки также используются при обращении к имени в коде.  
  
 В общем случае следует использовать escape-имена только если:  
  
-   Выполнена миграция кода из предыдущей версии [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] не было зарезервировано ключевое слово, используемого в качестве имени; или  
...
## <a name="case-sensitivity-in-names"></a>Учет регистра в именах  
 В именах элементов [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)] регистр не учитывается. Это означает, что при сравнении двух имен, которые различаются только регистром букв, он интерпретирует их как тем же именем. Например, он считает, что `ABC` и `abc` являются одним и тем же объявленным элементом.  
