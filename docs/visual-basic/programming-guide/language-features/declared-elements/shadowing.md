...
#### <a name="shadowing-and-access-level"></a>Затенение и уровень доступа  
 ...
 В следующем примере показана иерархия наследования трех классов. Каждый класс определяет процедуру `Sub`, `display`, и каждый производный класс затеняет процедуру `display` в своем базовом классе.  
...
```  
 ...
 В предыдущем примере производный класс `secondClass` затеняет `display` с помощью процедуры `Private`. Когда Module `callDisplay` вызывает `display` в `secondClass`, вызывающий код находится за пределами `secondClass` и поэтому не может получить доступ к процедуре Private `display`. Затенение нарушается, и компилятор разрешает ссылку на базовый класс `display`.  
...
## <a name="accessing-a-shadowed-element"></a>Доступ к затененному элементу  
 ...
 Пример доступа к затененному элементу см. в разделе [How to: Доступ к переменной, скрытой производным классом @ no__t-0.  
