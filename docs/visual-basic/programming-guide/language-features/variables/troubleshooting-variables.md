...
На этой странице приведены некоторые общие проблемы, которые могут возникнуть при работе с переменными в [!INCLUDE[vbprvb](~/includes/vbprvb-md.md)].  
...
## <a name="other-components-cannot-access-your-variable"></a>Другие компоненты не могут получить доступ к переменной  
 В именах[!INCLUDE[vbprvb](~/includes/vbprvb-md.md)]  *регистр не учитывается*. Если два имени отличаются только регистром букв, компилятор интерпретирует их как одно и то же. Например, он считает, что `ABC` и `abc` являются одним и тем же объявленным элементом.  
