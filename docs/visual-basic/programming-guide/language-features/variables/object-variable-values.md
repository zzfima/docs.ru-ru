...
## Функции классификатора объектов  
 [!INCLUDE[vbprvb](../../../../csharp/programming-guide/concepts/linq/includes/vbprvb-md.md)] предоставляет функции, которые возвращают сведения о том, на что ссылается переменная типа `Object`, как показано в следующей таблице:  
