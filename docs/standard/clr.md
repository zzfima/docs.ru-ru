---
title: "среда CLR | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net"
ms.reviewer: ""
...
ms.topic: "article"
helpviewer_keywords: 
  - "платформа .NET Framework, CLR - среда"
  - "код, выполнение"
  - "код, функциональные возможности среды выполнения"
  - "CLR - среда"
  - "компиляция исходного кода, функциональные возможности среды выполнения"
  - "компиляторы языка"
  - "управляемый код"
  - "управляемые данные"
  - "метаданные, функциональные возможности среды выполнения"
  - "среда выполнения"
...
> [!WARNING]
>  Компиляторы и инструменты могут создавать выходные данные для общеязыковой среды выполнения \(CLR\), поскольку система типов, формат метаданных и среда выполнения \(виртуальная система выполнения\) определяются открытым стандартом — спецификацией общеязыковой инфраструктуры ECMA.  Дополнительные сведения см. в статье [ECMA C\# and Common Language Infrastructure Specifications \(Спецификации ECMA для C\# и общеязыковой инфраструктуры \(CLI\)\)](http://go.microsoft.com/fwlink/?LinkId=99212).  
...
|[NIB: Overview of the .NET Framework](http://msdn.microsoft.com/ru-ru/ea38ac1e-92af-4d1b-8db1-e8a5ea10ed85)|Описание ключевых понятий платформы .NET Framework: системы общих типов CTS, межъязыкового взаимодействия, управляемого выполнения, доменов приложений и сборок.|  
