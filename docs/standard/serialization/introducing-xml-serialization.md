...
- Класс должен иметь конструктор без параметров, который сериализуется с помощью **XmlSerializer**.
