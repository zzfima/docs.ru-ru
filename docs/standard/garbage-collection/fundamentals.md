...
-   Недостаточно физической памяти в системе. Это обнаруживается, либо уведомление о нехватке памяти от операционной системы или нехватки памяти, указанный узлом.
...
-   Сборка мусора рабочей станции, предназначенная для всех клиентских рабочих станций и автономных ПК. Это значение по умолчанию для [ \<gcServer > элемент](../../../docs/framework/configure-apps/file-schema/runtime/gcserver-element.md) в схеме конфигурации времени выполнения.  
...
-   Сборка мусора сервера, предназначенная для серверных приложений, которым нужны высокое быстродействие и масштабируемость. Серверная сборка мусора может быть непараллельной или фоновой.  
 ...
 ![Потоки сборки мусора сервера](../../../docs/standard/garbage-collection/media/gc-server.png "GC_Server")  
Серверная сборка мусора  
  
### <a name="configuring-garbage-collection"></a>Настройка сборки мусора  
 Можно использовать [ \<gcServer > элемент](../../../docs/framework/configure-apps/file-schema/runtime/gcserver-element.md) схемы конфигурации времени выполнения для задания типа сборки мусора, которую должна выполнять среда CLR. Когда для атрибута `enabled` этого элемента установлено значение `false` (по умолчанию), среда CLR выполняет сборку мусора рабочей станции. Когда для атрибута `enabled` задано значение `true`, среда CLR выполняет сборку мусора сервера.  
  
 Параллельная сборка мусора задается с [ \<gcConcurrent > элемент](../../../docs/framework/configure-apps/file-schema/runtime/gcconcurrent-element.md) схемы конфигурации времени выполнения. Значением по умолчанию является `enabled`. Этот параметр контролирует одновременную и фоновую сборку мусора.  
...
-   На компьютере, который имеет только один процессор, независимо от значения всегда используется сборка мусора рабочей станции [ \<gcServer >](../../../docs/framework/configure-apps/file-schema/runtime/gcserver-element.md) параметр. Если задана сборка мусора сервера, среда CLR использует сборку мусора рабочей станции с выключенным параллелизмом.  
...
## <a name="concurrent-garbage-collection"></a>Параллельная сборка мусора  
 ...
 Чтобы повысить быстродействие во время выполнения нескольких процессов, отключите параллельную сборку мусора. Это можно сделать, добавив [ \<gcConcurrent > элемент](../../../docs/framework/configure-apps/file-schema/runtime/gcconcurrent-element.md) в файл конфигурации приложения и установив для его `enabled` атрибут `"false"`.  
 ...
 ![Потоки параллельной сборки мусора](../../../docs/standard/garbage-collection/media/gc-concurrent.png "GC_Concurrent")  
...
## <a name="background-server-garbage-collection"></a>Фоновая сборка мусора сервера  
 Начиная с .NET Framework 4.5 фоновая сборка мусора сервера является режимом по умолчанию для сборки мусора сервера. Чтобы выбрать этот режим, установите `enabled` атрибут [ \<gcServer > элемент](../../../docs/framework/configure-apps/file-schema/runtime/gcserver-element.md) для `true` в схеме конфигурации времени выполнения. Этот режим функционирует аналогично фоновой сборке мусора рабочей станции, описанной в предыдущем разделе, однако с некоторыми отличиями. Для фоновой сборки мусора рабочей станции используется один выделенной поток фоновой сборки мусора, тогда как для фоновой сборки мусора сервера используется несколько потоков, — обычно по одному выделенному потоку для каждого логического процессора. В отличие от потока фоновой сборки мусора рабочей станции у этих потоков время ожидания не истекает.  
