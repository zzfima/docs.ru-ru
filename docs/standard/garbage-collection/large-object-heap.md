...
Большим считается объект не менее 85 000 байтов. Это число рассчитано путем настройки производительности. Если запрашивается выделение 85 000 или более байтов, среда выполнения отправляет объект в кучу больших объектов.
...
В ходе сборки мусора поколения 1 или 2 сборщик мусора отдает ОС сегменты, в которых нет живых объектов (вызывая [функцию VirtualFree](https://msdn.microsoft.com/library/windows/desktop/aa366892(v=vs.85).aspx)). Свободное место после последнего живого объекта до конца сегмента освобождается (за исключением временных сегментов с объектами поколения 0 и 1, в которых сборщик мусора сохраняет свободное пространство, поскольку вскоре приложение будет размещать в него объекты). А свободные пространства остаются выделенными, хотя и сбрасываются, освобождая ОС от необходимости записывать данные с них обратно на диск.

Поскольку куча больших объектов собирается только во время сборки мусора поколения 2, сегмент этой кучи можно освободить только во время этой сборки мусора. На рисунке 3 показан сценарий, где сборщик мусора возвращает ОС один сегмент (сегмент 2) и освобождает дополнительное место в оставшихся сегментах. Если освободившееся пространство в конце сегмента необходимо использовать для удовлетворения запросов на выделение памяти для большого объекта, он фиксирует память снова. (Дополнительные сведения о фиксации и освобождении см. в документации по [VirtualAlloc](https://msdn.microsoft.com/library/windows/desktop/aa366887(v=vs.85).aspx).
...
1. Исследовать другие известные области и не найти в них причину проблемы с производительностью.
...
Распространенным средством просмотра счетчиков производительности является монитор производительности (perfmon.exe). Нажмите "Добавить счетчики", чтобы добавить нужный счетчик для интересующих вас процессов. Данные счетчика производительности можно сохранить в файл журнала, как показано на рисунке 4.
...
- [События сборки мусора в трассировке событий Windows — 1](http://blogs.msdn.com/b/maoni/archive/2014/12/22/gc-etw-events.aspx)

- [События сборки мусора в трассировке событий Windows — 2](http://blogs.msdn.com/b/maoni/archive/2014/12/25/gc-etw-events-2.aspx)

- [События сборки мусора в трассировке событий Windows — 3](http://blogs.msdn.com/b/maoni/archive/2014/12/25/gc-etw-events-3.aspx) 

- [События сборки мусора в трассировке событий Windows — 4](http://blogs.msdn.com/b/maoni/archive/2014/12/30/gc-etw-events-4.aspx)
...
> [!NOTE]
> Команды отладки, описанные в этом разделе, применимы к [отладчикам Windows](http://www.microsoft.com/whdc/devtools/debugging/default.mspx).
...
Чтобы проверить, вызывается ли фрагментация виртуальной памяти кучей больших объектов, можно установить точку останова на функциях [VirtualAlloc](https://msdn.microsoft.com/library/windows/desktop/aa366887(v=vs.85).aspx) и [VirtualFree](https://msdn.microsoft.com/library/windows/desktop/aa366892(v=vs.85).aspx), чтобы увидеть, кто ее вызывает. Например, чтобы узнать, кто пытался выделить блоки виртуальной памяти ОС размером более 8 МБ, можно установить точку останова следующим образом:

```console
bp kernel32!virtualalloc "j (dwo(@esp+8)>800000) 'kb';'g'"
```

Эта команда останавливает отладчик и показывает стек вызова только в том случае, если [VirtualAlloc](https://msdn.microsoft.com/library/windows/desktop/aa366887(v=vs.85).aspx) вызывается с объемом выделенной памяти более 8 МБ (0x800000).
