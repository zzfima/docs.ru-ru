...
ms.locfileid: "44186192"
---
# <a name="implementing-a-dispose-method"></a>Реализация метода Dispose

Вы реализуете метод <xref:System.IDisposable.Dispose%2A> для освобождения неуправляемых ресурсов, которые используются вашим приложением. Сборщик мусора .NET не выделяет и не выпускает неуправляемую память.  
