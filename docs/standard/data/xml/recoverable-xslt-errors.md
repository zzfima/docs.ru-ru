...
-   Раздел ссылок можно найти в [рекомендация преобразований XSL (XSLT) W3C версии 1.0](http://go.microsoft.com/fwlink/?LinkId=49919) и [преобразований XSL (XSLT) W3C версии 1.0 спецификации ошибки](http://go.microsoft.com/fwlink/?LinkId=49917).  
...
|Второй аргумент node-set функции документа пуст, поэтому URI-ссылка является относительной.|поправка 14|Восстановление|  
 ...
 <sup>*</sup>Это поведение отличается от <xref:System.Xml.Xsl.XslTransform> класса. Дополнительные сведения см. в разделе [реализацию из поведения по выбору в классе XslTransform](../../../../docs/standard/data/xml/implementation-of-discretionary-behaviors-in-the-xsltransform-class.md).  
