---
title: "Включение или импорт XML-схем | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net"
ms.reviewer: ""
...
## Включение или импорт схемы XML  
 В следующем примере кода пользовательская схема, созданная в разделе [Построение XML\-схем](../../../../docs/standard/data/xml/building-xml-schemas.md), дополняется схемой адресов.  Добавление к пользовательской схеме схемы адресов делает типы адресов доступными в пользовательской схеме.  
...
```  
 ...
 Дополнительные сведения об элементах `<xs:import />`, `<xs:include />` и `<xs:redefine />` и классах <xref:System.Xml.Schema.XmlSchemaImport>, <xref:System.Xml.Schema.XmlSchemaInclude> и <xref:System.Xml.Schema.XmlSchemaRedefine> см. в разделе [Схема XML W3C](http://go.microsoft.com/fwlink/?LinkId=45242) и в справочной документации по классам пространств имен <xref:System.Xml.Schema?displayProperty=fullName>.  
