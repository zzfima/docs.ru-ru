---
title: "XmlSchemaSet для компиляции схемы | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net"
ms.reviewer: ""
...
## Класс XmlSchemaSet  
 ...
 В <xref:System.Xml?displayProperty=fullName> версии 1.0 схемы XML загружались в класс <xref:System.Xml.Schema.XmlSchemaCollection> как в библиотеку схем.  В <xref:System.Xml?displayProperty=fullName> версии 2.0 классы <xref:System.Xml.XmlValidatingReader> и <xref:System.Xml.Schema.XmlSchemaCollection> устарели и были заменены методами <xref:System.Xml.XmlReader.Create%2A> и классом <xref:System.Xml.Schema.XmlSchemaSet>, соответственно.  
...
## Добавление и получение схем  
 ...
 В следующем примере схема `books.xsd` из пространства имен `http://www.contoso.com/books` добавляется в класс <xref:System.Xml.Schema.XmlSchemaSet>, затем из класса <xref:System.Xml.Schema.XmlSchemaSet> получаются все схемы, принадлежащие к пространству имен `http://www.contoso.com/books`, и записываются в объект <xref:System.Console>.  
...
## Проверка XML\-документов  
 ...
 Дополнительные сведения о проверке XML\-документов с использованием класса <xref:System.Xml.Schema.XmlSchemaSet> см. в разделе [Проверка по XML\-схеме \(XSD\) с помощью XmlSchemaSet](../../../../docs/standard/data/xml/xml-schema-xsd-validation-with-xmlschemaset.md).  
