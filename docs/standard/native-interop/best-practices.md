...
- ✔️ РЕКОМЕНДУЕТСЯ использовать такие же имена и написание для методов и параметров, как и для метода машинного кода, который вы хотите вызвать.
- ✔️ ДОПУСТИМО использовать такое же именование и написание для констант.
- ✔️ РЕКОМЕНДУЕТСЯ использовать типы .NET, которые наиболее сопоставимы с собственным типом. Например, если собственный тип в C# — `unsigned int`, используйте `uint`.
- ✔️ РЕКОМЕНДУЕТСЯ использовать только атрибуты `[In]` и `[Out]`, если реакция на событие, которую нужно реализовать, отличается от реакции по умолчанию.
- ✔️ ДОПУСТИМО использовать <xref:System.Buffers.ArrayPool%601?displayProperty=nameWithType> для заполнения буферов собственного массива.
- ✔️ ДОПУСТИМО создавать оболочку для объявлений P/Invoke в классе с таким же именем и написанием, как в собственной библиотеке.
...
| Параметр | Значение по умолчанию | Рекомендация | Сведения |
|---------|---------|----------------|---------|
...
## <a name="string-parameters"></a>Параметры строки

Если выбрана кодировка Юникод или аргумент явно обозначен как `[MarshalAs(UnmanagedType.LPWSTR)]` _и_ строка передается по значению (не по `ref` или `out`), строка будет закреплена, чтобы использоваться непосредственно в машинном коде (вместо копирования).
...
❌НЕ используйте `[Out] string` параметры. Если эта строка интернирована, строковые параметры, передаваемые по значению с атрибутом `[Out]`, могут дестабилизировать среду выполнения. См. дополнительные сведения в документации по методу интернирования строк <xref:System.String.Intern%2A?displayProperty=nameWithType>.

❌ПАРАМЕТРЫ `StringBuilder` AVOID. `StringBuilder` во время маршалинга *всегда* создает копию собственного буфера. Таким образом, маршалинг может оказаться крайне неэффективным. Выполните обычный сценарий вызова API Windows, который принимает строку:

1. Создание SB нужной емкости (распределяет управляемую емкость)**{1}**
2. Invoke
   1. Выделяет родной буфер**{2}**
   2. Копируется содержимое, если задано значение `[In]` _(значение по умолчанию для параметра `StringBuilder`)_.
   3. В только что выделенный управляемый массив копируется собственный буфер, если задано значение `[Out]` **{3}** _(также устанавливается по умолчанию для `StringBuilder`)_.
3. `ToString()`выделяет еще один управляемый массив**{4}**

Это *{4}* выделение средств для получения строки из родного кода. Чтобы уменьшить их количество, повторно используйте `StringBuilder` в другом вызове. Но это позволяет сэкономить только *одно* распределение. Гораздо лучше использовать и кэшировать буфер символов из `ArrayPool` — так вы сможете ограничиться распределением для `ToString()` при последующих вызовах.
...
✔️ ДОПУСТИМО использовать массивы `char[]` из `ArrayPool`.
...
> __Специфические Windows__ Для `[Out]` строк CLR будет `CoTaskMemFree` использоватьпо умолчанию `SysStringFree` свободные строки или `UnmanagedType.BSTR`для строк, которые помечены как.
> **Для большинства AA с буфером строки вывода:** Пройденное в коле символов должно включать null. Если возвращаемое значение меньше, чем количество переданных символов, значит, вызов завершился успешно. В таком случае значение — это количество символов *без* NULL в конце. В противном случае это требуемый размер буфера, *включая* нуль-символ.
>
> - Передайте в 5, получить 4: строка 4 символов длиной с задней нулю.
> - Передайте в 5, получить 6: строка 5 символов длиной, необходимо 6 символов буфера провести null.
...
❌НЕ используйте `[MarshalAs(UnmanagedType.LPStruct)]` ни `ref` для чего, кроме параметров GUID.
...
- [Тип Маршалинга](type-marshaling.md)
...
[`HandleRef`](xref:System.Runtime.InteropServices.HandleRef)позволяет маршалу сохранять объект живым в течение всего срока действия P/Invoke. Его можно использовать вместо `IntPtr` в сигнатурах метода. `SafeHandle` фактически заменяет этот класс и должен использоваться вместо него.

[`GCHandle`](xref:System.Runtime.InteropServices.GCHandle)позволяет прикрепить управляемый объект и получить родной указатель на него. Базовый шаблон приведен ниже:
...
| Ширина | Windows          | C (Windows)          | C#       | Альтернатива                          |
|:------|:-----------------|:---------------------|:---------|:-------------------------------------|
...
❌AVOID `System.Delegate` использует `System.MulticastDelegate` или поля для представления полей указателей функций в структурах.

<xref:System.Delegate?displayProperty=fullName> Поскольку <xref:System.MulticastDelegate?displayProperty=fullName> и не имеют необходимой подписи, они не гарантируют, что делегат, сданный в будет соответствовать подписи родной код ожидает. Кроме того, в .NET Framework и .NET Core, `System.Delegate` `System.MulticastDelegate` перестраивание структуры, содержащей или от его родного представления к управляемому объекту, может дестабилизировать время выполнения, если значение поля в родном представлении не является указателем функции, который обертывает управляемого делегата. В .NET 5 и более `System.Delegate` поздних версиях не поддерживается перенаправление или `System.MulticastDelegate` поля от родного представления к управляемому объекту. Используйте определенный тип `System.Delegate` `System.MulticastDelegate`делегата вместо или .

### <a name="fixed-buffers"></a>Фиксированные буферы
