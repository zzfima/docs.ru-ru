...
| Параметр | Значение по умолчанию | Рекомендация | Подробные сведения |
|---------|---------|----------------|---------|
...
**❌ НЕЛЬЗЯ** использовать параметры `[Out] string`. Если эта строка интернирована, строковые параметры, передаваемые по значению с атрибутом `[Out]`, могут дестабилизировать среду выполнения. См. дополнительные сведения в документации по методу интернирования строк <xref:System.String.Intern%2A?displayProperty=nameWithType>.

**❌ НЕЖЕЛАТЕЛЬНО** использовать параметры `StringBuilder`. Во время маршалинга `StringBuilder` *всегда* создается копия собственного буфера. Таким образом, маршалинг может оказаться крайне неэффективным. Выполните обычный сценарий вызова API Windows, который принимает строку:
...
Дополнительные сведения см. в статье [Маршалинг по умолчанию для строк](../../framework/interop/default-marshaling-for-strings.md) и разделе [Customizing string parameters](customize-parameter-marshalling.md#customizing-string-parameters) (Настройка строковых параметров).

> __Особые параметры Windows__  
> Для строк `[Out]` в среде выполнения по умолчанию используется `CoTaskMemFree` (для свободных строк) или `SysStringFree` (для строк, обозначенных как `UnmanagedType.BSTR`).  
**В большинстве API с буфером выходной строки:**  
> Количество переданных символов должно включать значение NULL. Если возвращаемое значение меньше, чем количество переданных символов, значит, вызов завершился успешно. В таком случае значение — это количество символов *без* NULL в конце. В противном случае это требуемый размер буфера, *включая* нуль-символ.  
> - Передается 5 символов, возвращается 4 символа: строка содержит 4 символа со значением NULL в конце.
> - Передается 5 символов, возвращается 6 символов: строка содержит 5 символов, и для хранения значения NULL в буфере требуется 6 символов.  
...
| Идентификатор GUID | Идентификатор GUID для передачи по ссылке |
|------|-------------|
| `KNOWNFOLDERID` | `REFKNOWNFOLDERID` |

**❌ НЕЛЬЗЯ** использовать `[MarshalAs(UnmanagedType.LPStruct)]` ни для каких параметров, за исключением параметров GUID `ref`.
...
Ниже приведен список типов данных, часто используемых в API Win32, и типов данных C#, используемых при вызове в виде кода Win32.

Указанные ниже типы имеют одинаковый размер в 32-разрядной и 64-разрядной версиях Windows независимо от их имен.

| По ширине | Windows          | C (Windows)          | C#       | Альтернатива                          |
|:------|:-----------------|:---------------------|:---------|:-------------------------------------|
