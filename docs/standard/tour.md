...
Сведения о том, как настроить среду разработки для выполнения примеров кода, представлены в разделе [о начале работы](getting-started.md).  Можно скопировать примеры кода с этой страницы и вставить их в вашу среду для выполнения. 
...
Правила для такого действия имеют несколько иную семантику на языке F#.  Чтобы получить дополнительные сведения об управлении ресурсами на языке F#, см. статью [Resource Management: The `use` Keyword ](../fsharp/language-reference/resource-management-the-use-keyword.md) (Управление ресурсами: ключевое слово use (F#)).
...
Если вы хотите приступить к написанию собственного кода, ознакомьтесь с [руководством по началу работы](getting-started.md).
