...
keywords: ".NET, .NET Standard, .NET Core, .NET Framework, Mono, Xamarin, MSBuild, C#, F#, VB, компиляторы"
author: cartermp
ms.author: mairaw
ms.date: 08/10/2017
ms.topic: article
...
.NET Core — это кроссплатформенная реализация .NET, оптимизированная для серверных рабочих нагрузок. Будучи современной и эффективной, она предназначена для обработки серверных и облачных рабочих нагрузок большого масштаба. Она реализует .NET Standard, а значит, любой код, предназначенный для .NET Standard, может работать на .NET Core. ASP.NET Core выполняется в .NET Core.
