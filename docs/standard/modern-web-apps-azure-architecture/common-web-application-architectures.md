...
Гораздо более простой [пример приложения eShopOnWeb](https://github.com/dotnet-architecture/eShopOnWeb) поддерживает использование одного монолитного контейнера. Приложение включает в себя два веб-приложения: одно использует традиционный MVC, а другое — Razor Pages. Оба могут запускаться из корня решения с помощью команд `docker-compose build` и `docker-compose up`. Эта команда настраивает отдельные контейнеры для каждого веб-экземпляра с помощью `Dockerfile` из корневого каталога каждого веб-проекта и выполняет каждый контейнер в отдельном порте. Вы можете скачать исходный код этого приложения из GitHub и запустить его в локальной системе. Даже такое монолитное приложение выигрывает от развертывания в контейнерной среде.
...
Файл `docker-compose.yml` на уровне решения содержит сведения о том, какие образы необходимо создать и какие контейнеры запустить. Этот файл позволяет использовать команду `docker-compose` для запуска обеих версий веб-приложения одновременно. Вы также можете с его помощью настроить зависимости, например отдельный контейнер базы данных.
...
Файл `docker-compose.yml` ссылается на `Dockerfile` в проектах `Web` и `WebRazorPages`. С помощью `Dockerfile` можно указать, какой базовый контейнер будет использоваться и как приложение будет настроено на нем. `Dockerfile` `WebRazorPages`:
