...
* Чтобы получить сведения о дате и времени из источников вне .NET, обычно тогда, когда сведения о дате и времени хранятся в простой тип данных.
...
Тип <xref:System.DateTimeOffset> включает все функциональные возможности типа <xref:System.DateTime> , а также сведения о часовом поясе. Это вполне подходит для приложений, сделайте следующее:
...
* для измерения прошедшего времени. Например <xref:System.Diagnostics.Stopwatch.Elapsed%2A?displayProperty=nameWithType> возвращает <xref:System.TimeSpan> значение, которое отражает интервал времени, которое прошло с момента вызова одного из <xref:System.Diagnostics.Stopwatch> методов, который начинает измерение прошедшего времени.

Значение <xref:System.TimeSpan> также может использоваться для замены значения <xref:System.DateTime> , если это значение отражает время без указания времени суток. Такое использование аналогичен <xref:System.DateTime.TimeOfDay%2A?displayProperty=nameWithType> и <xref:System.DateTimeOffset.TimeOfDay%2A?displayProperty=nameWithType> свойства, которые возвращают <xref:System.TimeSpan> значение, представляющее время безотносительно к дате. Например, структуру <xref:System.TimeSpan> можно использовать для представления ежедневного времени открытия или закрытия магазина или времени, в которое происходит любое регулярное событие.
