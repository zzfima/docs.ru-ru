...
* Чтобы получить сведения о дате и времени из источников вне .NET, обычно где хранятся сведения о дате и времени в простой тип данных.
...
Включает в себя .NET <xref:System.DateTime>, <xref:System.DateTimeOffset>, <xref:System.TimeSpan>, и <xref:System.TimeZoneInfo> типы, которые можно использовать для создания приложений, работающих с датами и временем.
...
Значение <xref:System.DateTime> определяет конкретную дату и время. Он включает <xref:System.DateTime.Kind%2A> свойства, которое предоставляет ограниченные сведения о часовом поясе, к которому указанной даты и времени принадлежит. <xref:System.DateTimeKind> Значения, возвращенного <xref:System.DateTime.Kind%2A> свойство указывает, является ли <xref:System.DateTime> значение представляет собой местное время (<xref:System.DateTimeKind.Local?displayProperty=nameWithType>), по Гринвичу (UTC) (<xref:System.DateTimeKind.Utc?displayProperty=nameWithType>), или неопределенное время (<xref:System.DateTimeKind.Unspecified?displayProperty=nameWithType>).
...
* Получить сведения о дате и времени из источников вне .NET, таких как базы данных SQL. (как правило, информация о дате и времени хранится в этих источниках в простом формате, который совместим со структурой <xref:System.DateTime> );
...
Тип <xref:System.DateTimeOffset> включает все функциональные возможности типа <xref:System.DateTime> , а также сведения о часовом поясе. Это вполне подходит для приложений, которые выполняют следующее:
...
* для измерения прошедшего времени. Например <xref:System.Diagnostics.Stopwatch.Elapsed%2A?displayProperty=nameWithType> возвращает <xref:System.TimeSpan> значение, которое отражает интервал времени, прошедшего с момента вызова одного из <xref:System.Diagnostics.Stopwatch> методов, который начинает измерение прошедшего времени.

Значение <xref:System.TimeSpan> также может использоваться для замены значения <xref:System.DateTime> , если это значение отражает время без указания времени суток. Данный подход аналогичен <xref:System.DateTime.TimeOfDay%2A?displayProperty=nameWithType> и <xref:System.DateTimeOffset.TimeOfDay%2A?displayProperty=nameWithType> свойства, которые возвращают <xref:System.TimeSpan> значение, представляющее время без указания даты. Например, структуру <xref:System.TimeSpan> можно использовать для представления ежедневного времени открытия или закрытия магазина или времени, в которое происходит любое регулярное событие.
...
В некоторых случаях использование всех преимуществ класса <xref:System.TimeZoneInfo> может потребовать дальнейших усилий по разработке. Если значения даты и времени не связаны тесно с часовыми поясами, к которым они принадлежат, дополнительная не требуется. Если приложение не предоставляет некоторый механизм для связывания даты и времени с соответствующим часовым поясом, можно легко для определенной даты и времени значение утратить связь с его часовым поясом. Одним из способов связывания этой информации является определение класса или структуры, содержащих значение даты и времени и связанный с ним часовой пояс.
