---
title: "Независимость от языка и независимые от языка компоненты | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net"
ms.reviewer: ""
...
helpviewer_keywords: 
  - "CLS"
  - "CLR - среда, взаимодействие между языками"
  - "Спецификация CLS"
  - "взаимодействие разных языков"
  - "взаимодействие между языками"
  - "среда выполнения, взаимодействие между языками"
...
-   [Взаимная совместимость кодов на разных язык](#CrossLang)  
  
<a name="Rules"></a>   
## Правила CLS\-совместимости  
 В этом разделе рассматриваются правила создания CLS\-совместимого компонента.  Полный список правил см. в документе [Стандарт ECMA\-335: общеязыковая инфраструктура \(CLI\)](http://go.microsoft.com/fwlink/?LinkID=116487) \(пункт 11 раздела I\).  
...
>  Правила CLS\-совместимости применяются только к открытому интерфейсу компонента и не применяются к закрытой реализации.  
 ...
 Чтобы класс `Person` удовлетворял требованиям CLS, можно изменить тип свойства `Age` с <xref:System.UInt16> на <xref:System.> Int16?qualifyHint=False&autoUpgrade=True — 16\-битное целое число со знаком, которое является CLS\-совместимым.  Тип закрытого поля `personAge` изменять не требуется.  
...
-   параметров и возвращаемых типов открытых методов открытых классов, а также параметров и возвращаемых типов методов, доступных производным классам.  
 ...
 В следующей таблице представлены правила CLS\-совместимости.  Это дословные выдержки из документа [Стандарт ECMA\-335: общеязыковая инфраструктура \(CLI\)](http://go.microsoft.com/fwlink/?LinkID=116487) © Ecma International, 2012 г.  Дополнительные сведения об этих правилах вы найдете в следующих разделах.  
...
|Перечисления|[Перечисления](#enums)|Базовый тип перечисления должен быть встроенным целочисленным типом CLS, имя поля должно быть"value\_\_" и это поле должно быть отмечено атрибутом `RTSpecialName`.|7|  
...
|Общие правила|[CLS\-совместимость: правила](#Rules)|Правила CLS\-совместимости применяются только к тем составляющим типа, которые доступны или видны за пределами определяющей сборки.|1|  
|Общие правила|[CLS\-совместимость: правила](#Rules)|Члены типов, не соответствующих правилам CLS\-совместимости, не должны быть отмечены как CLS\-совместимые.|2|  
...
|Члены|[Обзор членов типов](#members)|Глобальные статические \(static\) поля и методы не совместимы с CLS.|36|  
...
|Члены|[Обзор членов типов](#members)|Ограничение vararg не входит в спецификацию CLS. В спецификации поддерживается только стандартное соглашение об управляемых вызовах.|15|  
|Соглашения об именах|[Соглашения об именах](#naming)|Допустимые в сборках открывающие и внутренние символы идентификаторов указаны в приложении 7 технического отчета 15 стандарта Юникода версии 3.0. См. http:\/\/www.unicode.org\/unicode\/reports\/tr15\/tr15\-18.html.  Идентификаторы должны иметь канонический формат, определяемый формой нормализации Юникода C \(NFC\).  В рамках спецификации CLS два идентификатора считаются одинаковыми, если их записи в нижнем регистре совпадают \(т. е. при взаимно\-однозначном сопоставлении символов Юникода нижнего регистра без учета языкового стандарта\).  Таким образом в рамках спецификации CLS отличия регистра недостаточно для различения идентификаторов.  Однако чтобы переопределить наследуемое определение, требуется использовать точную кодировку исходного объявления.|4|  
|Перегрузка|[Соглашения об именах](#naming)|Все имена, которые вводятся в CLS\-совместимой области, должны быть различными. Это правило не относится к одинаковым именам, конфликт между которыми разрешается во время перегрузки.  Другими словами, в системе общих типов CTS разрешено давать одинаковое имя методу и полю, а в спецификации CLS — нет.|5|  
|Перегрузка|[Соглашения об именах](#naming)|Поля и вложенные типы должны отличаться только идентификаторами, хотя в системе общих типов CTS могут отличаться сигнатуры.  Методы, свойства и события с одинаковыми именами \(при сравнении идентификаторов\) должны отличаться не только возвращаемым типом, за исключением случаев из правила 39 спецификации CLS.|6|  
|Перегрузка|[Overloads](#overloads)|Перегружать можно только свойства и методы.|37|  
|Перегрузка|[Overloads](#overloads)|Свойства и методы могут перегружаться только на основе количества и типов их параметров, за исключением операторов преобразования `op_Implicit` и `op_Explicit`, которые также могут перегружаться на основе возвращаемого типа.|38|  
...
|Типы|[Сигнатуры типов и членов типа](#Types)|Тип <xref:System.Object?displayProperty=fullName> является CLS\-совместимым.  Все другие CLS\-совместимые классы должны наследоваться от CLS\-совместимого класса.|23|  
...
|Типы|[Сигнатуры типов и членов типа](#Types)|Упакованные типы значений не являются CLS\-совместимыми.|3|  
|Типы|[Сигнатуры типов и членов типа](#Types)|Все типы сигнатуры должны быть CLS\-совместимыми.  Все типы, составляющие экземпляры универсального типа, должны быть CLS\-совместимыми.|11|  
|Типы|[Сигнатуры типов и членов типа](#Types)|Типизированные ссылки несовместимы с CLS.|14|  
|Типы|[Сигнатуры типов и членов типа](#Types)|Типы неуправляемых указателей не являются CLS\-совместимыми.|17|  
|Типы|[Сигнатуры типов и членов типа](#Types)|CLS\-совместимые классы, типы значений и интерфейсы должны реализовываться без членов, не соответствующих правилам CLS\-совместимости.|20|  
  
<a name="Types"></a>   
### Сигнатуры типов и членов типов  
 ...
 Все типы в сигнатурах членов, в том числе тип свойства и возвращаемый тип метода, должны быть CLS\-совместимыми.  Кроме того, в отношении универсальных типов применяются следующие правила:  
...
|<xref:System.TypedReference>|Указатель на объект и тип среды выполнения|Нет|  
...
-   Методы, которые не являются CLS\-совместимыми.  Например, в следующем определении интерфейса есть метод `INumber.GetUnsigned`.  Компилятор выдает предупреждение о том, что он не является CLS\-совместимым.  
 ...
 Кроме того, CLS\-совместимые языковые компиляторы должны разрешать классам иметь отдельные реализации членов с одинаковыми именами и сигнатурами в нескольких интерфейсах.  Языки C\# и Visual Basic поддерживают [явные реализации интерфейса](../Topic/Explicit%20Interface%20Implementation%20\(C%23%20Programming%20Guide\).md), с помощью которых можно определить разные реализации методов с одинаковыми именами.  Язык Visual Basic поддерживает ключевое слово `Implements`, позволяющее явно указать интерфейс и член, которые реализует конкретный член.  Этот сценарий представлен в следующем примере, где определяется класс `Temperature`, который явно реализует интерфейсы `ICelsius` и `IFahrenheit`.  
...
-   Тип перечисления должен иметь поле `Value__` с одним экземпляром, отмеченное атрибутом <xref:System.Reflection.FieldAttributes?displayProperty=fullName>.  Это позволяет неявно ссылаться на значение поля.  
...
### Универсальные типы и члены  
 ...
 В следующем примере определяется универсальный тип `C1<T>` \(`C1(Of T)` в Visual Basic\) и защищенный класс `C1<T>.N` \(`C1(Of T).N` в Visual Basic\).  Объект `C1<T>` имеет два метода: `M1` и `M2`.  Однако `M1` не является CLS\-совместимым, поскольку он возвращает объект `C1<int>.N` \(или `C1(Of Integer).N`\) из C1\<T\> \(или `C1(Of T)`\).  Второй класс `C2` является производным от  `C1<long>` \(или `C1(Of Long)`\).  Он имеет два метода: `M3` и `M4`.  `M3` не является CLS\-совместимым, поскольку он возвращает объект `C1<int>.N` \(или `C1(Of Integer).N`\) из подкласса `C1<long>`.  Обратите внимание, что языковые компиляторы могут накладывать даже более строгие ограничения.  В этом примере на языке Visual Basic при компиляции `M4` возникает ошибка.  
...
-   Тип свойства, возвращаемый тип метода получения значения свойства и тип последнего аргумента метода задания значения должны совпадать.  Все эти типы должны быть CLS\-совместимыми, и аргументы не могут присваиваться свойству по ссылке \(т. е. не могут быть управляемыми указателями\).  
...
-   Метод добавления обработчика событий с именем `add_`*EventName*.  Например, метод подписки для события <xref:System.AppDomain.AssemblyResolve?displayProperty=fullName> называется `add_AssemblyResolve`.  
  
-   Метод удаления обработчика событий с именем `remove_`*EventName*.  Например, метод удаления для события <xref:System.AppDomain.AssemblyResolve?displayProperty=fullName> называется `remove_AssemblyResolve`.  
  
-   Метод, который указывает, что возникло событие, с именем `raise_`*EventName*.  
...
```  
 ...
 Подробнее о синтаксисе командной строки для компилятора Visual Basic см. в разделе [Построение из командной строки](../Topic/Building%20from%20the%20Command%20Line%20\(Visual%20Basic\).md).  
...
```  
 ...
 Подробнее о синтаксисе командной строки для компилятора C\# см. в разделе [Построение из командной строки с помощью csc.exe](../../ocs/csharp/language-reference/compiler-options/command-line-building-with-csc-exe.md).  
  
 Затем вызовите [Компоновщик \(Link.exe\)](../Topic/Linker%20Options.md), чтобы скомпилировать оба модуля в сборку.  
...
```  
 ...
 Чтобы скомпилировать C\#, измените имя компилятора с **vbc** на **csc** и измените расширение файла на с CS на VB.  
