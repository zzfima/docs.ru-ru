...
Объект *дочерняя задача* (или *вложенная задача*) является <xref:System.Threading.Tasks.Task?displayProperty=nameWithType> экземпляр, создаваемый в пользовательском делегате другой задачи, которая называется *родительская задача*. Дочерняя задача может быть отсоединенной или присоединенной. Объект *отсоединенная дочерняя задача* — это задача, которая выполняется независимо от родительского. *Присоединенная дочерняя задача* — это вложенная задача, созданная с <xref:System.Threading.Tasks.TaskCreationOptions.AttachedToParent?displayProperty=nameWithType> , родительский объект которой не явно или по умолчанию запрещает ее присоединения. Задача может создавать любое количество присоединенных и отсоединенных дочерних задач, ограничиваемое только системными ресурсами.  
