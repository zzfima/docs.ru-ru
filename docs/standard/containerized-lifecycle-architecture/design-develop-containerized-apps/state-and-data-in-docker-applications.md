...
- **Тома** хранятся в той части в файловой системе узла под управлением Docker.
...
Как показано на рисунке 4-5, обычные тома Docker могут храниться за пределами самих контейнерами, но в физических границах сервера узла или виртуальной машины. Тем не менее контейнеры Docker с одного сервера узла или виртуальной машины не могут обращаться к тому на другом сервере узла или виртуальной машине. Другими словами с этих томов, это не позволяет управлять данными контейнеров, которые выполняются на разных узлах Docker, несмотря на то, что его можно достичь с помощью драйвера тома, который поддерживает удаленными узлами.
