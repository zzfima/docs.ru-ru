...
## <a name="determining-the-options"></a>Определение параметров  
 ...
 В следующих разделах перечислены параметры, поддерживаемые регулярными выражениями в .NET.  
...
-   Пробел не допускается в квантификаторах, окруженных, таких как `{`  *n*  `}`, `{`  *n*  `,}`и `{`  *n*  `,` *m*`}`. Например, шаблон регулярного выражения `\d{1. 3}` не сопоставляет последовательности цифр из одной до трех цифр, так как он содержит пробел.  
  
-   Пробел не допускается в последовательности символов, предоставляющей языковой элемент. Пример:  
  
    -   Языковой элемент `(?:`*subexpression*`)` представляет незахватываемую группу, а часть `(?:` элемента не может включать пробелы. Шаблон `(? :` *часть выражения* `)` вызывает <xref:System.ArgumentException> во время выполнения, поскольку обработчик регулярных выражений не удается проанализировать шаблон и шаблон `( ?:` *часть выражения*  `)` не удается сопоставить *часть выражения*.  
 ...
 Этот шаблон аналогичен шаблону, заданному в [только явные захваты](#Explicit) статьи, за исключением того, что он использует <xref:System.Text.RegularExpressions.RegexOptions.IgnorePatternWhitespace?displayProperty=nameWithType> возможность пропуска пробелов.  
