...
---
# <a name="character-encoding-in-net"></a>Кодировки в .NET
...
## <a name="encodings-in-net"></a>Кодировки в .NET  
 Классы кодировок в .NET наследуются от всех символьных <xref:System.Text.Encoding?displayProperty=nameWithType> класс, который является абстрактным классом, который определяет функции, общие для всех кодировок. Для доступа к отдельным объектам кодировок, реализованным в .NET, можно сделать следующее:  
  
-   Использовать статические свойства <xref:System.Text.Encoding> класса, который возвращает объекты, которые представляют стандартные кодировки, доступные в .NET (ASCII, UTF-7, UTF-8, UTF-16 и UTF-32). Например, свойство <xref:System.Text.Encoding.Unicode%2A?displayProperty=nameWithType> возвращает объект <xref:System.Text.UnicodeEncoding>. Каждый объект использует резервную стратегию замены для обработки строк, которые он не может закодировать, и байтов, которые не может декодировать. (Дополнительные сведения см. в разделе [Replacement Fallback](../../../docs/standard/base-types/character-encoding.md#Replacement) .)  
...
-   Вызовите <xref:System.Text.Encoding.GetEncoding%2A?displayProperty=nameWithType> метод, возвращающий любой стандартный, кодовую страницу или кодировку DBCS в .NET. Перегрузки позволяют задать резервный объект как для кодировщика, так и для декодера.  
...
>   
>  Платформа .NET поддерживает три кодировки, определенные стандартом Юникод: UTF-8, UTF-16 и UTF-32. Дополнительные сведения см. в описании стандарта Юникод на [домашней странице Юникода](http://go.microsoft.com/fwlink/?LinkId=37123).  
  
 Сведения обо всех доступных в .NET кодировках можно получить, вызвав <xref:System.Text.Encoding.GetEncodings%2A?displayProperty=nameWithType> метод. Платформа .NET поддерживает системы кодирования символов, перечисленные в таблице ниже.  
...
### <a name="platform-notes-includenetcoreincludesnet-core-mdmd"></a>Заметки о платформе: [!INCLUDE[net_core](../../../includes/net-core-md.md)]  
 По умолчанию [!INCLUDE[net_core](../../../includes/net-core-md.md)] не предоставляет доступ к кодировкам кодовых страниц, кроме кодовой страницы 28591 и кодировок Юникода, например UTF-8 и UTF-16. Тем не менее можно добавить кодировок кодовых страниц, найден в стандартных приложений Windows, ориентированных на .NET в приложение. Подробнее см. в разделе <xref:System.Text.CodePagesEncodingProvider> .  
  
<a name="Selecting"></a>   
## <a name="selecting-an-encoding-class"></a>Выбор класса кодировки  
 Если у вас есть возможность выбрать кодировку для использования в приложении, следует использовать Юникод, предпочтительно <xref:System.Text.UTF8Encoding> или <xref:System.Text.UnicodeEncoding>. (.NET также поддерживает третью кодировку Юникод, <xref:System.Text.UTF32Encoding>.)  
...
> [!NOTE]
>  В теории кодировку Юникод классы .NET, предоставленный в (<xref:System.Text.UTF8Encoding>, <xref:System.Text.UnicodeEncoding>, и <xref:System.Text.UTF32Encoding>) поддерживают все символы всех наборов символов, поэтому их можно использовать для устранения проблем со стратегией наилучшего соответствия.  
