...
|`\u` *nnnn*|Единица кода UTF-16, значение которого является соответствует  *nnnn*  шестнадцатеричное. **Примечание:** escape символ из Perl 5, который используется для указания Юникода не поддерживается в .NET. Escape-символ из Perl 5 имеет форму `\x{`*####*`…}`, где *####*`…` — последовательность шестнадцатеричных цифр. Вместо этого используйте `\u`  *nnnn* .|  
