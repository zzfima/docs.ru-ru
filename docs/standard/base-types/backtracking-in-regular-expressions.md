...
### <a name="nonbacktracking-subexpression"></a>Невозвращающиеся подвыражения  
 Элемент языка `(?>` *часть выражения*`)` подавляет обратное отслеживание в части выражения. Это полезно для предотвращения проблем с производительностью, связанных с неуспешным совпадением.  
