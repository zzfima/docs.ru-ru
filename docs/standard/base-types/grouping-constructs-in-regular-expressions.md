...
-   Извлечение отдельных частей выражения из свойства <xref:System.Text.RegularExpressions.Match.Groups%2A?displayProperty=nameWithType> и обработка их по отдельности от всего текста.  
 ...
 В следующей таблице перечислены конструкции группирования, поддерживаемые обработчик регулярных выражений .NET и указывает, является ли они захватываемыми или незахватываемыми.  
...
> [!NOTE]
>  По умолчанию языковой элемент `(`*часть выражения*`)` выделяет сопоставленную часть выражения. Но если <xref:System.Text.RegularExpressions.RegexOptions> параметр метода сопоставления шаблона регулярного выражения включает <xref:System.Text.RegularExpressions.RegexOptions.ExplicitCapture?displayProperty=nameWithType> флаг, или если `n` к части выражения применен параметр (см. [группы параметров](#group_options) далее в этом разделе), сопоставленная часть выражения не выделяется.  
...
-   С помощью `$` *номер* последовательность замены в <xref:System.Text.RegularExpressions.Regex.Replace%2A?displayProperty=nameWithType> или <xref:System.Text.RegularExpressions.Match.Result%2A?displayProperty=nameWithType> вызова метода, где *номер* — это порядковый номер захватываемой части выражения.  
...
> [!NOTE]
>  Если <xref:System.Text.RegularExpressions.RegexOptions> параметр метода сопоставления шаблона регулярного выражения включает <xref:System.Text.RegularExpressions.RegexOptions.ExplicitCapture?displayProperty=nameWithType> флаг, или если `n` к части выражения применен параметр (см. [группы параметров](#group_options) далее в этом разделе), единственным часть выражения можно для явного имени захватываемых групп.  
...
-   С помощью `${` *имя* `}` последовательность замены в <xref:System.Text.RegularExpressions.Regex.Replace%2A?displayProperty=nameWithType> или <xref:System.Text.RegularExpressions.Match.Result%2A?displayProperty=nameWithType> вызова метода, где *имя* имя захватываемой части выражения.  
...
|3|`(((?'Open'<)`|Найдена левая угловая скобка в "\<abc >» и присваивает его `Open` группы.|  
...
|6|`((?'Close-Open'>)`|Сопоставляется правая угловая скобка в "\<abc >», затем «abc», подстрока между `Open` группы и правой угловой скобкой, назначается `Close` группы и удаляет текущее значение («<») из `Open` группы, Если оставить его пустым.|  
...
|10|`(((?'Open'<)`|Найдена левая угловая скобка в "\<mno >» и присваивает его `Open` группы. Теперь ее коллекция <xref:System.Text.RegularExpressions.Group.Captures%2A?displayProperty=nameWithType> содержит один символ, "<".|  
...
|13|`(((?'Open'<)`|Найдена левая угловая скобка в "\<xyz >» и присваивает его `Open` группы. <xref:System.Text.RegularExpressions.Group.Captures%2A?displayProperty=nameWithType> Коллекцию `Open` содержит два выделения: левую угловую скобку из "\<mno >» и левую угловую скобку из"\<xyz >».|  
...
|16|`((?'Close-Open'>)`|Сопоставляется правая угловая скобка в "\<xyz >». "xyz" задает подстроку между группой `Open` и правой угловой скобкой группы `Close` и удаляет текущее значение группы `Open` . Значение предыдущего выделения (левая угловая скобка в "\<mno >») становится текущим значением из `Open` группы. <xref:System.Text.RegularExpressions.Group.Captures%2A> Коллекцию `Open` содержит один символ, левую угловую скобку из "\<xyz >».|  
...
|19|`((?'Close-Open'>)`|Сопоставляется последняя правая угловая скобка в «xyz >>», назначает «mno\<xyz >» (подстрока между `Open` группы и правой угловой скобкой) для `Close` группы и удаляет текущее значение `Open` группы. Теперь группа `Open` пуста.|  
