...
|13|`(((?'Open'<)`|Сопоставляет левую угловую скобку в строке "\<xyz>" и назначает ее группе `Open`. Коллекция <xref:System.Text.RegularExpressions.Group.Captures%2A?displayProperty=nameWithType> группы записи `Open` теперь содержит два захвата: левую угловую скобку из "\<mno>" и правую угловую скобку из "\<xyz>".|  
