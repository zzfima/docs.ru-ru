---
title: "Конструкции группировки в регулярных выражениях | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
ms.prod: ".net"
ms.reviewer: ""
...
ms.topic: "article"
helpviewer_keywords: 
  - "регулярные выражения .NET Framework, конструкции группирования"
  - "конструкции, группирование"
  - "конструкции группирования"
  - "поиск вперед"
  - "поиск назад"
  - "регулярные выражения, конструкции группирования"
...
-   Применение квантификатору к части выражения с несколькими языковыми элементами регулярных выражений. Дополнительные сведения о квантификаторах см. в разделе [Кванторы](../../../docs/standard/base-types/quantifiers-in-regular-expressions.md).  
  
-   Добавление в строку части выражения, которая возвращается методами <xref:System.Text.RegularExpressions.Regex.Replace%2A?displayProperty=fullName> и <xref:System.Text.RegularExpressions.Match.Result%2A?displayProperty=fullName>.  
  
-   Извлечение отдельных частей выражения из свойства <xref:System.Text.RegularExpressions.Match.Groups%2A?displayProperty=fullName> и обработка их по отдельности от всего текста.  
  
 В следующей таблице перечислены конструкции группирования, поддерживаемые механизмом регулярных выражений .NET Framework, и показано, являются ли они захватываемыми или незахватываемыми.  
...
|[Невозвращающиеся части выражения](#nonbacktracking_subexpression)|Незахватываемая|  
...
> [!NOTE]
>  По умолчанию языковой элемент `(`*subexpression*`)` выделяет сопоставленную часть выражения. Но если параметр <xref:System.Text.RegularExpressions.RegexOptions> метода сопоставления шаблона регулярного выражения содержит флаг <xref:System.Text.RegularExpressions.RegexOptions?displayProperty=fullName> или к части выражения применен параметр `n` \(см. [Параметры группы](#group_options) далее в этой статье\), сопоставленная часть выражения не выделяется.  
...
-   Используя последовательность замены `$`*number* в вызове метода <xref:System.Text.RegularExpressions.Regex.Replace%2A?displayProperty=fullName> или <xref:System.Text.RegularExpressions.Match.Result%2A?displayProperty=fullName>, где *number* — порядковый номер захватываемой части выражения.  
  
-   Программным способом, используя объект <xref:System.Text.RegularExpressions.GroupCollection>, возвращаемый свойством <xref:System.Text.RegularExpressions.Match.Groups%2A?displayProperty=fullName>. Член коллекции в нулевой позиции представляет все сопоставление регулярного выражения. Каждый последующий член представляет сопоставленную часть выражения. Дополнительные сведения см. в разделе [Конструкции группировки и объекты регулярных выражений](#Objects).  
...
> [!NOTE]
>  Если параметр <xref:System.Text.RegularExpressions.RegexOptions> метода сопоставления шаблона регулярного выражения содержит флаг <xref:System.Text.RegularExpressions.RegexOptions?displayProperty=fullName> или к части выражения применен параметр `n` \(см. [Параметры группы](#group_options) далее в этой статье\), выделить часть выражения можно только с помощью явного имени захватываемых групп.  
...
-   Используя последовательность замены `${`*name*`}` в вызове метода <xref:System.Text.RegularExpressions.Regex.Replace%2A?displayProperty=fullName> или <xref:System.Text.RegularExpressions.Match.Result%2A?displayProperty=fullName>, где *name* — это имя захватываемой части выражения.  
...
|3|`(((?'Open'<)`|Найдена левая угловая скобка в строке "\<abc\>", которая назначается группе `Open`.|  
...
|9|`)*`|Значение первой захватываемой группы — "\<abc\>".<br /><br /> Следующий символ во входной строке является левой угловой скобкой, поэтому механизм регулярных выражений возвращается к подшаблону `(((?'Open'<)`.|  
|10|`(((?'Open'<)`|Найдена левая угловая скобка в строке "\<mno\>", которая назначается группе `Open`. Теперь ее коллекция <xref:System.Text.RegularExpressions.Group.Captures%2A?displayProperty=fullName> содержит один символ, "\<".|  
...
|13|`(((?'Open'<)`|Найдена левая угловая скобка в строке "\<xyz\>", которая назначается группе `Open`. Коллекция <xref:System.Text.RegularExpressions.Group.Captures%2A?displayProperty=fullName> группы `Open` содержит два выделения: левую угловую скобку из "\<mno\>" и правую угловую скобку из "\<xyz\>".|  
...
|16|`((?'Close-Open'>)`|Сопоставляет правую угловую скобку в строке "\<xyz\>". "xyz" задает подстроку между группой `Open` и правой угловой скобкой группы `Close` и удаляет текущее значение группы `Open`. Значение предыдущего выделения \(левая угловая скобка в "\<mno\>"\) становится текущим значением группы `Open`. Коллекция <xref:System.Text.RegularExpressions.Group.Captures%2A> группы `Open` содержит один символ, левую угловую скобку из "\<xyz\>".|  
...
|22|`)*`|Значение первой захватываемой группы — "\<mno\<xyz\>\>".<br /><br /> Следующий символ во входной строке не является левой угловой скобкой, поэтому механизм регулярных выражений не возвращается к подшаблону `(((?'Open'<)`.|  
...
> [!NOTE]
>  Вы можете указать параметры, которые применяются ко всему регулярному выражению, а не его части, используя конструктор класса <xref:System.Text.RegularExpressions.Regex?displayProperty=fullName> или статичный метод. Также можно указать встроенные параметры, которые применяются после определенной точки в регулярном выражении с использованием языковой конструкции `(?imnsx-imnsx)`.  
...
## Невозвращающиеся части выражения  
...
## Конструкции группировки и объекты регулярных выражений  
 Подстроки, сопоставляемые захватываемой группой регулярного выражения, представлены объектами <xref:System.Text.RegularExpressions.Group?displayProperty=fullName>, которые можно извлечь из объекта <xref:System.Text.RegularExpressions.GroupCollection?displayProperty=fullName>, возвращаемого свойством <xref:System.Text.RegularExpressions.Match.Groups%2A?displayProperty=fullName>. Объект <xref:System.Text.RegularExpressions.GroupCollection> заполняется следующим образом:  
...
-   Следующий набор объектов <xref:System.Text.RegularExpressions.Group> представляет неименованные \(нумерованные\) захватываемые группы. Они указываются в том порядке, в котором они определены в регулярном выражении, слева направо. Значения индекса этих групп лежат в диапазоне от 1 до числа неименованных захватываемых групп в коллекции. \(Индекс определенной группы эквивалентен нумерованной обратной ссылке. Дополнительные сведения об обратных ссылках см. в разделе [Конструкции обратных ссылок](../../../docs/standard/base-types/backreference-constructs-in-regular-expressions.md).\)  
...
-   Последний набор объектов <xref:System.Text.RegularExpressions.Group> представляет именованные захватываемые группы. Они указываются в том порядке, в котором они определены в регулярном выражении, слева направо. Значение индекса первой именованной захватываемой группы на единицу больше индекса, последней неименованной захватываемой группы. Если в регулярном выражении нет неименованных захватываемых групп, индекс первой именованной захватываемой группы равен единице.  
 ...
 Если к захватываемой группе применяется квантификатор, соответствующие свойства <xref:System.Text.RegularExpressions.Group>, <xref:System.Text.RegularExpressions.Capture.Value%2A?displayProperty=fullName> и <xref:System.Text.RegularExpressions.Capture.Index%2A?displayProperty=fullName> объекта <xref:System.Text.RegularExpressions.Capture.Length%2A?displayProperty=fullName> отражают последнюю подстроку, выделяемую захватываемой группой. Вы можете получить полный набор подстрок, выделяемые группами с квантификаторами, из объекта <xref:System.Text.RegularExpressions.CaptureCollection>, возвращаемого свойством <xref:System.Text.RegularExpressions.Group.Captures%2A?displayProperty=fullName>.  
