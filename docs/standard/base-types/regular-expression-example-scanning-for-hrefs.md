...
|`\s*`|Соответствует нулю или нескольким символам пробела.|  
|`(?:["'](?<1>[^"']*)"&#124;(?<1>\S+))`|Соответствует одному из следующих символов, не назначая результат группе записи.<br /><br /> — Кавычки или апостроф, за которыми следует ноль или несколько любых символов, кроме кавычек или апострофа, за которыми следуют кавычки или апостроф. В этот шаблон включена группа с именем `1`.<br />— Один или более символов, которые не являются пробелами. В этот шаблон включена группа с именем `1`.|  
