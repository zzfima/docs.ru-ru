...
>  Для делегата, имеющего более одного метода в списке вызовов, возвращаемое значение не определяется, даже если у делегата есть возвращаемый тип.  
 ...
 Для делегатов, представляющих несколько методов, .NET предоставляет методы <xref:System.Delegate> и <xref:System.MulticastDelegate> делегировать классы для поддержки операций, таких как добавление методов к списку вызовов делегата ( <xref:System.Delegate.Combine%2A?displayProperty=nameWithType> метод), удаление метода ( <xref:System.Delegate.Remove%2A?displayProperty=nameWithType> метод) и получение списка вызовов ( <xref:System.Delegate.GetInvocationList%2A?displayProperty=nameWithType> метода).  
...
## <a name="see-also"></a>См. также  
 [Библиотека классов .NET](http://go.microsoft.com/fwlink/?LinkID=217856)  
