...
> [!NOTE]
>  Помимо реализации шаблона разработки наблюдателя, возможно, вас заинтересуют библиотеки, создаваемые с помощью интерфейсов <xref:System.IObservable%601> и <xref:System.IObserver%601>. Например, [Реактивные расширения для .NET (Rx)](https://msdn.microsoft.com/library/hh242985.aspx) — это набор методов расширения и стандартных операторов последовательности LINQ для поддержки асинхронного программирования.  
