...
Например, за исключением чисел типа [Byte](xref:System.Byte) целые числа без знака не соответствуют CLS. В следующем примере класс `Person` предоставляет свойство `Age`, имеющее тип [UInt16](xref:System.UInt16), поэтому в коде отображается предупреждение компилятора.
...
Исключения | [Исключения](#exceptions) | Объекты исключений должны иметь тип [System.Exception](xref:System.Exception) или унаследованный от него тип. Однако не требуется, чтобы соответствующие CLS методы блокировали распространение других типов исключений. | 40
Общие сведения | [Правила CLS-совместимости](#cls-compliance-rules) | Правила CLS-совместимости применяются только к тем составляющим типа, которые доступны или видны за пределами определяющей сборки. | 1
Общие сведения | [Правила CLS-совместимости](#cls-compliance-rules) | Члены типов, не соответствующих CLS, не должны быть отмечены как соответствующие CLS. | 2
...
Универсальные шаблоны | [Универсальные типы и члены](#generic-types-and-members) | Типы, используемые в качестве ограничений универсальных параметров, должны соответствовать CLS. | 45
...
Участники | [Обзор членов типов](#type-members-in-general) | Глобальные статические (static) поля и методы не соответствуют CLS. | 36
...
Типы | [Сигнатуры типов и членов типов](#types-and-type-member-signatures) | Упакованные типы значений не соответствуют CLS. | 3
...
Типы | [Сигнатуры типов и членов типов](#types-and-type-member-signatures) | Типизированные ссылки не соответствуют CLS. | 14
Типы | [Сигнатуры типов и членов типов](#types-and-type-member-signatures) | Типы неуправляемых указателей не соответствуют CLS. | 17
Типы | [Сигнатуры типов и членов типов](#types-and-type-member-signatures) | соответствующие CLS классы, типы значений и интерфейсы должны реализовываться без членов, не соответствующих CLS. | 20
Типы | [Сигнатуры типов и членов типов](#types-and-type-member-signatures) | [System.Object](xref:System.Object) соответствует CLS. Все другие соответствующие CLS классы должны наследоваться от соответствующего CLS класса. | 23

### <a name="types-and-type-member-signatures"></a>Сигнатуры типов и членов типов

Тип [System.Object](xref:System.Object) соответствует CLS. Это базовый тип всех типов объектов в системе типов .NET Framework. В платформе .NET Framework существует два вида наследования: неявное (например, класс [String](xref:System.String) неявно наследуется от класса `Object`) или явное (например, класс [CultureNotFoundException](xref:System.Globalization.CultureNotFoundException) явно наследуется от класса [ArgumentException](xref:System.ArgumentException), который явно наследуется от класса [Exception](xref:System.Exception)). Чтобы производный тип был CLS-совместимым, его базовый тип должен быть CLS-совместимыми.
...
[Double](xref:System.Double) | Значение двойной точности с плавающей запятой
...
[UInt64](xref:System.UInt64) | 64-разрядное целое число без знака | [Int64](xref:System.Int64) (возможно переполнение), [BigInteger](xref:System.Numerics.BigInteger) или [Double](xref:System.Double)
...
* Базовый тип перечисления должен быть соответствующим CLS встроенным целочисленным типом ([Byte](xref:System.Byte), [Int16](xref:System.Int16), [Int32](xref:System.Int32) или [Int64](xref:System.Int64)). Например, при определении перечисления с базовым типом [UInt32](xref:System.UInt32) компилятор выдает предупреждение.
...
В следующем примере определяется универсальный тип `C1<T>` и защищенный класс `C1<T>.N`. Объект `C1<T>` имеет два метода: `M1` и `M2`. Однако `M1` не соответствует CLS, поскольку он возвращает объект `C1<int>.N` из `C1<T>`. Второй класс `C2` является производным от `C1<long>`. Он имеет два метода: `M3` и `M4`. `M3` не является CLS-совместимым, поскольку он возвращает объект `C1<int>.N` из подкласса `C1<long>`. Обратите внимание, что языковые компиляторы могут накладывать даже более строгие ограничения. В этом примере на языке Visual Basic при компиляции `M4` возникает ошибка.
...
Это правило не выполняется в следующем примере. В нем определяется класс `NumericAttribute`, который не является производным от `System.Attribute`. Обратите внимание, что компилятор выдает ошибку не при определении класса, а когда используется атрибут, который не соответствует CLS.
...
* [Тип](xref:System.Type)

* Любой тип перечисления с базовым типом `Byte`, `Int16`, `Int32` или `Int64`.

В следующем примере определяется класс `DescriptionAttribute`, производный от [Attribute](xref:System.Attribute). Конструктор класса имеет параметр типа `Descriptor`, поэтому класс не соответствует CLS. Обратите внимание, что компилятор C# выдает предупреждение, но при этом успешно завершает компиляцию.
...
> [!WARNING]
> В некоторых случаях языковые компиляторы применяют правила соответствия CLS вне зависимости от того, используется ли атрибут `CLSCompliantAttribute`. Например, когда в интерфейсе определяется член `*static`, нарушается правило CLS-совместимости. Однако если определить в интерфейсе член `*static`, компилятор C# выдает сообщение об ошибке и не компилирует приложение.
