...
> SourceLink автоматически добавляет метаданные `RepositoryUrl` и `RepositoryType` в пакет NuGet.
> SourceLink также добавляет сведения о точном исходном коде, из которого создан пакет.
> Например, к пакету, созданному из репозитория Git, будет добавлен хэш фиксации в качестве метаданных.
...
> Внедрение файлов символов в главный пакет NuGet упрощает отладку по умолчанию. Разработчикам не нужно находить и настраивать сервер символов NuGet в своей интегрированной среде разработки, чтобы получить файлы символов.
>
