...
![Согласованность между агрегатами достигается за счет событий предметной области, агрегат заказа отправляет событие предметной области OrderStarted, которое обрабатывается для обновления агрегата покупателя. ](./media/image15.png)
...
3. Обработка событий предметной области (в рамках текущего процесса), которые будут выполнять открытое количество побочных эффектов в нескольких агрегатах или действиях приложения. Например:
...
![Может существовать несколько обработчиков для одного события предметной области на прикладном уровне: один обработчик может решать согласованность между агрегатами, а другой обработчик может публиковать событие интеграции, чтобы другие микрослужбы могли отреагировать.](./media/image16.png)
...
![Внедрение зависимостей можно использовать для связывания событий с обработчиками событий — этот подход использует MediatR](./media/image17.png)
