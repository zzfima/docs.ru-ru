...
## <a name="structuring-aggregates-in-a-custom-net-standard-library"></a>Структура агрегатов в пользовательской библиотеке .NET Standard
...
Согласованность транзакций гарантирует согласованность и актуальность агрегата по завершении бизнес-действия. Например, агрегат заказа из модели предметной области микрослужбы размещения заказов eShopOnContainers состоит из компонентов, показанных на рисунке 9-11.

![](./media/image12.png)

**Рис. 9-11**. Агрегат заказа в решении Visual Studio
...
## <a name="implementing-domain-entities-as-poco-classes"></a>Реализация сущностей предметной области в виде классов POCO
...
## <a name="encapsulating-data-in-the-domain-entities"></a>Инкапсуляция данных в сущностях предметной области
...
Например, согласно шаблонам DDD *не* следует выполнять следующую операцию из любого обработчика команд или класса прикладного уровня:
...
При использовании EF Core 1.0 в классе DbContext необходимо сопоставлять свойства, определенные только с методами задания, с фактическими полями в таблице базы данных. Для этого служит метод HasField класса PropertyBuilder.
...
[Назад] (microservice-domain-model.md) [Далее] (seedwork-domain-model-base-classes-interfaces.md)
