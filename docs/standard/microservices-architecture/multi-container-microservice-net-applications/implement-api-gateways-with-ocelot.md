...
В примере приложения для микрослужб [eShopOnContainers](https://github.com/dotnet-architecture/eShopOnContainers) используется [Ocelot](https://github.com/ThreeMammals/Ocelot), поскольку Ocelot — это простой и легкий шлюз API, который можно развернуть где угодно вместе с вашими микрослужбами и контейнерами, например в любой из следующих сред, используемых eShopOnContainers.

- Узел Docker, на локальном компьютере разработчика, в локальной среде или в облаке.
- Кластер Kubernetes, локально или в управляемой облачной среде, например в службе Azure Kubernetes (AKS).
- Кластер Service Fabric, локально или в облаке.
- Сетка Service Fabric, как PaaS или бессерверное решение в Azure.

## <a name="architect-and-design-your-api-gateways"></a>Разработка архитектуры и проектирование шлюзов API

На следующей архитектурной схеме показано, как реализуются шлюзы API с помощью Ocelot в eShopOnContainers.

![Схема архитектуры eShopOnContainers с клиентскими приложениями, микрослужбами и шлюзами API между ними](./media/image28.png)

**Рис. 8-27.** Архитектура eShopOnContainers со шлюзами API
...
Дополнительные сведения о службах с составным пользовательским интерфейсом см. в разделе [Создание составного пользовательского интерфейса на основе микрослужб](https://docs.microsoft.com/dotnet/standard/microservices-architecture/architect-microservice-container-applications/microservice-based-composite-ui-shape-layout).
...
Например, у `eShopOnContainers` есть около шести типов внутренних микрослужб, которые необходимо публиковать через шлюзы API, как показано на следующем рисунке.
 
![](./media/image29.png)

**Рис. 8-28.** Папки микрослужб в решении eShopOnContainers в Visual Studio
...
**Рис. 8-29.** Пример микрослужбы веб-API (микрослужба каталога)
...
```
HTTP-запрос в итоге запустит такой код C#, который получает доступ к базе данных микрослужбы и выполняет какое-либо действие.
...
Порт 80, показанный в коде, находится внутри узла Docker, поэтому клиентские приложения не имеют к нему доступ. Клиентские приложения имеют доступ только к внешним портам (если таковые имеются), которые публикуются при развертывании с помощью `docker-compose`.
...
Запустите микрослужбу каталога в локальном узле Docker. Для этого запустите полное решение eShopOnContainers из Visual Studio (оно запустит все службы в файлах docker-compose) или просто запустите микрослужбу каталога с помощью следующей команды docker-compose в CMD или PowerShell в папке, где находятся `docker-compose.yml` и docker-compose.override.yml.
...
Затем вы можете напрямую обратиться к микрослужбе каталога и просмотреть ее методы в пользовательском интерфейсе Swagger напрямую через "внешний" порт, в данном случае `http://localhost:5101`. 

![](./media/image31.png)

**Рис. 8-30.** Тестирование микрослужбы каталога с помощью ее пользовательского интерфейса Swagger
...
Но такого прямого взаимодействия с микрослужбой, в этом случае через внешний порт 5101, как раз следует избегать в приложении. Для этого необходимо задать дополнительный уровень косвенного обращения через шлюз API (в данном случае Ocelot). Таким образом, клиентское приложение не будет обращаться к микрослужбе напрямую.
...
В eShopOnContainers его реализация шлюза API представляет собой простой проект ASP.NET Core WebHost, и ПО промежуточного слоя Ocelot обрабатывают все функции шлюза API, как показано на следующем рисунке:

![](./media/image32.png)

**Рис. 8-31.** Базовый проект OcelotApiGw в eShopOnContainers
...
Существует два раздела конфигурации: массив ReRoute и GlobalConfiguration. ReRoute — это объекты, которые указывают Ocelot, как обрабатывать вышестоящий запрос. Глобальная конфигурация может переопределять конкретные параметры объектов ReRoute. Это полезно, если вы не хотите управлять большим числом параметров ReRoute.

Ниже приведен упрощенный пример файла [конфигурации ReRoute](https://github.com/dotnet-architecture/eShopOnContainers/blob/dev/src/ApiGateways/Web.Bff.Shopping/apigw/configuration.json) из одного из шлюзов API в eShopOnContainers.
...
Основная функция шлюзов API Ocelot — принимать входящие HTTP-запросы и перенаправлять их подчиненной службе, в настоящее время в виде другого HTTP запроса. Ocelot описывает направление одного запроса в другой как перенаправление ReRoute.

Давайте сосредоточимся на одном из перенаправлений в файле configuration.json из примера выше, где содержится конфигурация для микрослужбы корзины.
...
Проект в eShopOnContainers реализует единый образ контейнера Docker с помощью шлюза API Ocelot, но затем при развертывании в Docker он создает различные службы и контейнеры для каждого типа шлюза API/BFF, предоставляя отдельный файл configuration.json для каждого контейнера.

![](./media/image33.png)

**Рис. 8-32.** Повторное использование одного образа Ocelot Docker в нескольких типах шлюзов API
...
Кроме того, как видно из файла docker-compose.override.yml, единственное различие между этими контейнерами в шлюзах API — файл конфигурации Ocelot, который отличается для каждого контейнера службы и указывается во время выполнения с помощью тома Docker, как показано в следующем файле docker-compose.override.yml.
...
**Рис. 8-33.** Для определения каждого шлюза API/BFF с помощью Ocelot требуется только файл конфигурации 
...
Например, при посещении вышестоящего URL-адреса http://localhost:5202/api/v1/c/catalog/items/2/, обслуживаемого шлюзом API webshoppingapigw, вы получаете результат от внутреннего подчиненного URL-адреса http://catalog.api/api/v1/2 в узле Docker, как показано ниже.

![](./media/image35.png)

**Рис. 8-34.** Доступ к микрослужбе с помощью URL-адреса, предоставленного шлюзом API 

В связи с тестированием или отладкой, если вы захотите получить прямой доступ к контейнеру Docker Catalog (только в среде разработки) без передачи через шлюз API, так как catalog.api является разрешением DNS внутри узла Docker (обнаружение служб, обрабатываемое именами служб docker-compose), это возможно только через внешний порт, опубликованный в файле docker-compose.override.yml, который предоставляется только для тестирования, например http://localhost:5101/api/v1/Catalog/items/1 в следующем примере.

![](./media/image36.png)

**Рис. 8-35.** Прямой доступ к микрослужбе для тестирования 
...
Как уже было сказано, объединять запросы удобнее всего с помощью настраиваемых служб, через код. Вы также можете объединять запросы с помощью специальной функции в Ocelot, но этот метод может оказаться недостаточно гибким. Поэтому рекомендуется реализовывать объединение в eShopOnContainers с помощью явных служб веб-API ASP.NET Core для каждого агрегатора. 
...
**Рис. 8-36.** Архитектура eShopOnContainers со службами агрегатора

На следующей картинке детально показана часть этой схемы. Вы видите, как в области покупок можно улучшить клиентские приложения, сократив обмен данных с микрослужбами при использовании служб агрегатора в области шлюзов API. 

 ![](./media/image38.png)

**Рис. 8-37.** Службы агрегатора на схеме

Можно заметить, что, когда на схеме показаны возможные запросы, поступающие из шлюзов API, она значительно усложняется. И хотя видно, что синие стрелки можно упростить, с точки зрения клиентских приложений, использование шаблона агрегатора сокращает число вызовов и задержки при обмене данными и, в конечном счете, значительно повышает удобство работы пользователей, особенно с удаленными приложениями (мобильными и одностраничными приложениями). 

В области и микрослужбах маркетинга схема очень проста, и можно обойтись без агрегаторов, но их допустимо использовать при необходимости.

### <a name="authentication-and-authorization-in-ocelot-api-gateways"></a>Проверка подлинности и авторизация в шлюзах API Ocelot

В шлюзе API Ocelot можно разместить службу проверки подлинности, например службу веб-API ASP.NET Core, с помощью [IdentityServer](http://identityserver.io/), предоставив маркер проверки подлинности снаружи или внутри шлюза API.
...
**Рис. 8-38.** Положение службы идентификации в eShopOnContainers
...
**Рис. 8-39.** Проверка подлинности в шлюзе API Ocelot

Поскольку приложение eShopOnContainers разделило шлюз API на несколько BFF (Backend for Frontend) и шлюзов API для областей бизнеса, для решения общих задач также можно было бы создать дополнительный шлюз API. Такой вариант подойдет для более сложной архитектуры на базе микрослужб с несколькими микрослужбами, выполняющими общие задачи. Так как в eShopOnContainers есть только одна общая задача, для простоты было решено обрабатывать службу безопасности вне области шлюза API.
...
На следующем шаге при попытке получить доступ к любой защищенной микрослужбе, например микрослужбе корзины с URL-адресом перенаправления на основе шлюза API, например http://localhost:5202/api/v1/b/basket/1, возникнет ошибка 401 (неавторизованный доступ), если вы не предоставите допустимый токен. С другой стороны, если URL-адрес перенаправления прошел проверку подлинности, Ocelot будет вызывать связанную с ним подчиненную схему (URL-адрес внутренней микрослужбы).

**Авторизация на уровне перенаправлений Ocelot.**  Ocelot поддерживает авторизацию на основе утверждений, которая вычисляется после проверки подлинности. Вы задаете авторизацию на уровне маршрута, добавляя следующий код в конфигурацию перенаправления. 
...
Если в Kuberentes вы не используете точку входа, IP ваших служб и модулей pod может перенаправляться только сетью кластера. 
...
Однако при нацеливании на "рабочую" среду на основе Kuberentes eShopOnContainers использует точку входа перед шлюзами API. Таким образом, клиенты по-прежнему вызывают тот же базовый URL-адрес, но запросы направляются к нескольким шлюзам API или BFF. 

Обратите внимание, что шлюзы API — это внешний интерфейс, взаимодействующий только со службами, но не с веб-приложениями, которые обычно находятся вне области их действия. Кроме того, шлюзы API могут скрывать некоторые внутренние микрослужбы. 

Но точка входа просто перенаправляет запросы HTTP и не пытается скрыть микрослужбы или веб-приложения.

В идеальной архитектуре есть входящий уровень Nginx в Kuberentes перед веб-приложениями, а также несколько шлюзов API Ocelot/BFF, как показано на следующей схеме.

 ![](./media/image41.png)

**Рис. 8-40.** Входящий уровень в eShopOnContainers при развертывании в Kubernetes

При развертывании eShopOnContainers в Kubernetes он предоставляет всего несколько служб или конечных точек через _точку входа_, в основном только следующий список постфиксов в URL-адресах:
...
-   **Обнаружение службы на стороне клиента через Ocelot с Consul или Eureka** 
...
[Назад] (background-tasks-with-ihostedservice.md) [Далее] (../microservice-ddd-cqrs-patterns/index.md)
