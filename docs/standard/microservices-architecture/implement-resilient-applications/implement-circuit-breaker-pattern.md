...
Таким образом, вам необходим некий защитный барьер, чтобы повторные запросы прекращались, если пытаться не стоит. Этим защитным барьером и является размыкатель цепи.
...
В приведенном выше примере кода политика размыкателя цепи настроена таким образом, что прерывает или размыкает цепь после пяти исключений при повторной отправке HTTP-запросов. Перерыв будет длиться 30 секунд.
