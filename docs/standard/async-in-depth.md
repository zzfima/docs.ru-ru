...
description: "Подробное описание работы асинхронного кода в .NET"
...
Писать асинхронный код, связанный с операциями ввода-вывода и задействованием ЦП, очень просто с помощью асинхронной модели .NET на базе класса Task. Эта модель основывается на типах `Task` и `Task<T>` и ключевых словах языка `async` и `await`. В этой статье описывается использование асинхронного кода в .NET и объясняется базовая асинхронная платформа.

## <a name="task-and-tasklttgt"></a>Task и Task&lt;T&gt;

Task (задача) — это конструкция, реализующая [модель параллельной обработки на основе обещаний (Promise)](https://en.wikipedia.org/wiki/Futures_and_promises).  Если в двух словах, она "обещает", что работа будет выполнена позже, позволяя взаимодействовать с помощью обещания с чистым API.
...
Важно рассматривать задачи как абстракции асинхронных операций, но *не* как абстракции поверх потоков. По умолчанию задачи выполняются в текущем потоке и при необходимости делегируют работу операционной системе. Либо для задач может явно запрашиваться запуск в отдельном потоке через API `Task.Run`.
...
Использование `await` позволяет приложению или службе выполнять полезную работу во время выполнения задачи за счет передачи управления вызывающему объекту задачи до ее завершения. В коде не нужно использовать обратные вызовы или события для продолжения выполнения после завершения задачи. Это берут на себя язык и интеграция API-интерфейса задач. Если вы используете `Task<T>`, ключевое слово `await` будет дополнительно "разворачивать" значение, возвращаемое после завершения Task.  Ниже приводится подробное описание того, как это работает.

Дополнительные сведения о задачах и разных способах взаимодействия с ними см. в статье [Асинхронный шаблон, основанный на задачах (TAP)](https://msdn.microsoft.com/library/hh873175.aspx).
...
Вызов `GetStringAsync()` обращается к библиотекам .NET более низкого уровня (возможно с вызовом других асинхронных методов), пока не достигнет вызова взаимодействия P/Invoke собственной сетевой библиотеки. Собственная библиотека может впоследствии воспользоваться вызовом API System (например, выполнить запись `write()` в сокет в системе Linux). Объект задачи будет создан на собственной/управляемой границе, возможно, с использованием [TaskCompletionSource](xref:System.Threading.Tasks.TaskCompletionSource%601.SetResult(%600)). Объект задачи будет передаваться вверх через уровни, возможно, с некоторой обработкой или напрямую, и в результате возвратится к исходному вызывающему объекту. 
