...
|--------------|------------|---------|
|**C#**        |ОБЩЕДОСТУПНОЙ ВЕРСИИ          |Предварительный просмотр  |
|**JavaScript**|ОБЩЕДОСТУПНОЙ ВЕРСИИ          |Предварительный просмотр  |
...
В этом примере функция активируется изменением в хранилище BLOB-объектов в `images` контейнере. Информация для файла передается, поэтому триггер также выступает в качестве привязки. Существует другая привязка для размещения информации в очереди с именем `images`.
...
`Domain Redirect` Прокси-сервер принимает сокращенный маршрут и сопоставляет его с более длинным ресурсом функции. Преобразование выглядит следующим образом:

`https://--shorturl--/123` -> `https://--longurl--.azurewebsites.net/api/UrlRedirect/123`

Прокси-сервер принимает все данные, отправленные в`https://--shorturl--/`корневой URL-адрес (), и перенаправляет его на сайт документации. `Root`

Пример использования прокси-серверов показан в видео [Azure: Перенесите свое приложение в облако с помощью безсерверных](https://channel9.msdn.com/events/Connect/2017/E102)функций Azure. В режиме реального времени приложение ASP.NET Core, работающее на локальном SQL Server, переносится в облако Azure. Прокси-серверы используются для рефакторинга традиционного проекта Web API для использования функций.
