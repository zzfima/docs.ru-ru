...
В предыдущем примере `CallActivityAsync` кода функция отвечает за выполнение данного действия на виртуальной машине в центре обработки данных. Когда функция await возвращает результат и базовая задача завершается, выполнение будет записано в таблицу журнала. Код в функции Orchestrator может использовать любую из привычных конструкций библиотеки параллельных задач и ключевых слов async/await.

Следующий код является упрощенным примером `ProcessPayment` того, как может выглядеть метод:
...
В этом сценарии `DurableOrchestrationClient`возможность проверки состояния выполняющегося рабочего процесса может оказаться полезной. При использовании `HttpTrigger` для запуска рабочего процесса `CreateCheckStatusResponse` метод можно использовать `HttpResponseMessage`для возврата экземпляра. Этот ответ предоставляет клиенту URI в полезных данных, который можно использовать для проверки состояния выполняющегося процесса.
...
`DurableOrchestrationContext``CreateTimer` настраивает расписание следующего вызова цикла для проверки изменений цен на акции. `DurableOrchestrationContext`также имеет `CurrentUtcDateTime` свойство для получения текущего значения DateTime в формате UTC. Лучше использовать это свойство вместо, `DateTime.UtcNow` поскольку оно легко макетирование для тестирования.
