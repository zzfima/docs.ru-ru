...
[Решение портфолиоссампле](https://github.com/dotnet-architecture/grpc-for-wcf-developers/tree/master/PortfoliosSample/wcf/TraderSys) включает в себя простую службу портфеля "запрос-ответ" для скачивания одного портфеля или всех портфелей для конкретного торговца. Служба определена в интерфейсе `IPortfolioService` атрибутом `ServiceContract`:
...
Модель `Portfolio` — это простой C# класс, помеченный как [DataContract](xref:System.Runtime.Serialization.DataContractAttribute), включая список объектов `PortfolioItem`. Эти модели определяются в проекте `TraderSys.PortfolioData` вместе с классом репозитория, представляющим абстракцию доступа к данным.
...
## <a name="convert-the-datacontracts-to-grpc-messages"></a>Преобразование DataContract в сообщения gRPC

Сначала класс `PortfolioItem` будет преобразован в сообщение protobuf, так как от него зависит класс `Portfolio`. Класс очень прост, и три свойства сопоставляются непосредственно с типами данных gRPC. Свойство `Cost`, представляющее цену, уплаченное для общих ресурсов по закупке, является `decimal` полем, а gRPC поддерживает только `float` или `double` для реальных чисел, которые не подходят для валюты. Так как цены на общий ресурс отличаются по меньшей мере одним увеличением, стоимость может быть выражена в `int32` центов.

> [!NOTE]
> Не забывайте использовать `camelCase` для имен полей в файле `.proto`; Генератор C# кода преобразует их в `PascalCase`, и пользователи других языков будут поблагодарить вас за соблюдение разных стандартов кодирования.
...
Теперь у нас есть сообщения о данных. можно объявить конечные точки RPC службы.

## <a name="convert-the-servicecontract-to-a-grpc-service"></a>Преобразование ServiceContract в службу gRPC

Метод `Get` WCF принимает два параметра: `Guid traderId` и `int portfolioId`. методы службы gRPC могут принимать только один параметр, поэтому необходимо создать сообщение для хранения двух значений. Обычно имена этих объектов запроса должны совпадать с именами метода и суффикса `Request`. Опять же, `string` используется для поля `traderId`, а не `Guid`.

Служба может просто вернуть сообщение `Portfolio` напрямую, но опять же это может повлиять на обратную совместимость в будущем. Рекомендуется определять отдельные `Request` и `Response` сообщения для каждого метода в службе, даже если многие из них одинаковы, поэтому объявите `GetResponse` сообщение с одним полем `Portfolio`.

В следующем примере показано объявление метода службы gRPC с помощью сообщения `GetRequest`:
...
> [!WARNING]
> Возможно, вы можете создать `PortfolioList` сообщение или аналогичное ему и использовать его в нескольких методах службы, но вы должны быть бесискушенияными. Невозможно понять, как различные методы в службе могут развиваться в будущем, так что их сообщения должны быть специфичными и четко разделяться.
...
На этом этапе проекту требуется репозиторий портфеля и модели, содержащиеся в библиотеке классов `TraderSys.PortfolioData` в решении WCF. Самый простой способ сделать это — создать новую библиотеку классов с помощью диалогового окна **Новый проект** Visual Studio с шаблоном *библиотеки классов (.NET Standard)* или из командной строки с помощью .NET Core CLI, выполнив следующие команды из каталога, содержащего файл `TraderSys.sln`.
...
Проекты .NET в стиле пакета SDK автоматически включают в себя любые файлы `.cs` в или в своем каталоге, поэтому нет необходимости явно добавлять их в проект. Осталось только удалить `DataContract` и `DataMember` атрибуты из классов `Portfolio` и `PortfolioItem`, чтобы они были простыми старыми C# классами.
...
Теперь можно добавить ссылку на эту библиотеку в проект приложения gRPC и использовать класс `PortfolioRepository`, используя внедрение зависимостей в реализации службы gRPC. В приложении WCF внедрение зависимостей было предоставлено контейнером IoC Autofac. ASP.NET Core помогут внедрения зависимостей в; репозиторий можно зарегистрировать в методе `ConfigureServices` класса `Startup`.
...
Начните с реализации метода `Get`. Переопределение по умолчанию выглядит, как в следующем примере:
...
Первая причина заключается в том, что `request.TraderId` является строкой, а для службы требуется `Guid`. Хотя ожидаемый формат строки является `UUID`, код должен иметь дело с возможностью, чтобы вызывающий объект отправлял недопустимое значение и отвечал соответствующим образом. Служба может реагировать на ошибки, вызывая `RpcException`, и использовать стандартный код состояния `InvalidArgument` для выражения проблемы.
...
После получения правильного `Guid` значения для `traderId`репозиторий можно использовать для извлечения портфеля и возврата его клиенту.
...
Предыдущий код не работает, так как репозиторий возвращает собственную модель POCO `Portfolio`, но для *gRPC требуется собственное* сообщение protobuf `Portfolio`. Что очень похоже на сопоставление типов Entity Framework с типами передачи данных, лучшим решением является преобразование между ними. Хорошим местом для размещения этого кода является класс, созданный protobuf, который объявляется как класс `partial`, чтобы его можно было расширить.
...
После ввода кода преобразования можно завершить реализацию метода `Get`.
...
Реализация метода `GetAll` аналогична. Обратите внимание, что поля `repeated` в сообщениях protobuf создаются как `readonly` свойства типа `RepeatedField<T>`, поэтому необходимо добавить элементы в них с помощью метода `AddRange`, как показано в следующем примере:
...
В Visual Studio 2019 можно добавить ссылки на службы gRPC, аналогичные способу добавления ссылок на службы в проекты WCF в более ранних версиях Visual Studio. Ссылки на службы и Подключенные службы управляются в одном пользовательском интерфейсе. доступ к нему можно получить, щелкнув правой кнопкой мыши узел **зависимости** в проекте `TraderSys.Portfolios.Client` в Обозреватель решений и выбрав пункт **Добавить подключенную службу**. В появившемся окне инструментов выберите раздел ссылки на **службы** и щелкните **Добавить новую ссылку на службу gRPC**.
...
Перейдите к файлу `portfolios.proto` в проекте `TraderSys.Portfolios`, оставьте **тип класса, который будет создан** как **клиент**, и нажмите кнопку **ОК**.
...
> [!TIP]
> Если вы не используете Visual Studio или предпочитаете работать из командной строки, можно использовать глобальное средство **DotNet-GRPC** для управления ссылками protobuf в проекте .NET GRPC. [Дополнительные сведения см. в документации по **DotNet-GRPC** ](https://docs.microsoft.com/aspnet/core/grpc/dotnet-grpc).
...
Теперь вы перенесли базовое приложение WCF в службу ASP.NET Core gRPC и создали клиент для использования службы из приложения .NET. В следующем разделе будут рассмотрены более под«Дуплексные» службы.
