...
Это руководством было написано для разработчиков, работающих в .NET Framework или .NET Core, которые ранее использовали WCF и которым требуется перенести свои приложения в современное окружение RPC для .NET Core 3,0 и более поздних версий. Кроме того, это средство может быть использовано для разработчиков, которые обычно используются для обновления или обновления до .NET Core 3,0, которые хотят использовать встроенные средства gRPC.
