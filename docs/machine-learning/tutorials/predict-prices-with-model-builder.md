...
1. В раскрывающемся списке *Прогнозируемый столбец (метка)* выберите *fare_amount* и перейдите к шагу обучения в построителе моделей.
1. Разверните раскрывающийся список *Входные столбцы (признаки)* и снимите флажок в столбце *trip_time_in_secs*, чтобы исключить его в качестве признака во время обучения.
