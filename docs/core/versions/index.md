...
(\*) В этой таблице в качестве примера используется будущая версия 2.2 среды выполнения .NET Core из-за исторического артефакта: первый пакет SDK для .NET Core 2.1 имел версию 2.1.300. Дополнительные сведения см. в [выбранной версии .NET Core](selection.md).
