...
> [!IMPORTANT]
> Приведенные в этой статье сведения об управлении версиями не относятся к текущей версии пакета SDK для .NET Core и среды выполнения.
> Схема управления версиями изменится в будущих выпусках. Текущее предложение можно найти в репозитории [dotnet/designs](https://github.com/dotnet/designs/pull/29).

## <a name="versioning-details"></a>Сведения об управлении версиями

Начиная с версии .NET Core 2.0 в имени скачиваемых файлов указывается номер версии. Были объединены следующие номера версии:
...
.NET Core применяет набор политик, определяющих, какие версии среды выполнения .NET Core и пакета SDK используются в различных сценариях. Эти сценарии и политики подробно рассматриваются в этой статье о [выборе версии](selection.md).

Можно представить, что эти политики выполняют следующие роли:

* Обеспечивают простое и эффективное развертывание .NET Core, включая обновления безопасности и надежности.
* Позволяют разработчикам использовать новейшие средства и команды независимо от целевой среды выполнения.

### <a name="installers"></a>Установщики

Начиная с .NET Core 2.0 скачиваемые файлы [ежедневных сборок](https://github.com/dotnet/core-setup#daily-builds) и [выпусков](https://www.microsoft.com/net/download/core) соответствуют новой схеме именования, которая проще для понимания.
Пользовательский интерфейс установщика этих сборок и выпусков также изменен. Теперь в нем четко обозначены имена и версии устанавливаемых компонентов. В частности, названия теперь содержат тот же номер версии, что и имя скачиваемого файла.
...
Возможны ситуации, когда исправленные средства .NET Core CLI (входят в состав пакета SDK) поставляются с существующей средой выполнения. В этом случае номер версии пакета SDK увеличивается (например, до 2.1.2), а для среды выполнения это изменение учитывается при последующей поставке (например, в следующий раз среда выполнения и пакет SDK поставляются с номером версии 2.1.3).
...
- Узел: либо *dotnet.exe* для приложений, зависящих от платформы, либо  *\<имя_приложения>.exe* для автономных приложений.
...
Имя скачиваемого файла .NET Core содержит номер версии, например `dotnet-sdk-2.0.4-win10-x64.exe`.
