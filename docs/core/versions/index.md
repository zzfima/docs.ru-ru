...
description: "Сведения о схеме управления версиями для .NET Core."
...
- Описывает пакеты, которые считаются необходимыми для разработки современных приложений и которые должны реализовывать реализации .NET, чтобы считаться платформами [.NET Standard](../../standard/net-standard.md).
...
Платформа `netcoreapp` связана с распространяемым пакетом .NET Core. Ее номера версий соответствуют версиям .NET Core. Например, когда будет выпущена версия .NET Core 2.0, она будет предназначена для `netcoreapp2.0`. Для платформы `netstandard` не используется схема управления версиями какой-либо реализации .NET, так как она в равной степени применяется ко всем средам.
