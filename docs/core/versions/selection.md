...
Данный пакет SDK поддерживает фиксированный набор платформ, вплоть до целевой платформы среды выполнения, с которой он поставляется. Например, пакет SDK для .NET Core 3.0 включает среду выполнения .NET Core 3.0, которая представляет собой реализацию целевой платформы `netcoreapp3.0`. Пакет SDK для .NET Core 3.0 поддерживает `netcoreapp2.1`, `netcoreapp2.2`, `netcoreapp3.0`, но не `netcoreapp3.1` (или более поздней версии). Установите пакет SDK для .NET Core 3.1 для сборки `netcoreapp3.1`.

Целевая платформа .NET Standard также ограничена целевой платформой среды выполнения, с которой поставляется пакет SDK. Пакет SDK для .NET Core 3.1 ограничен версией `netstandard2.1`. Дополнительные сведения см. в статье [.NET Standard](../../standard/net-standard.md).
...
Узел выбирает версию с последними исправлениями, установленную на компьютере. Например, если вы указали `netcoreapp3.0` в файле проекта, а `3.0.4` является последней установленной версией среды выполнения .NET, будет использоваться среда выполнения `3.0.4`.

Если допустимая версия `3.0.*` не найдена, используется новая версия `3.*`. Например, если вы указали `netcoreapp3.0` и только `3.1.0` установлена, приложение будет работать в среде выполнения `3.1.0`. Это поведение называется накатом до дополнительной версии. Более ранние версии не будут рассматриваться. Если допустимая среда выполнения не установлена, приложение не будет работать.

Несколько примеров для демонстрации такого поведения, если вы используете версию 3.0:

- ✔️ Указана версия 3.0. Последняя установленная версия с исправлениями — 3.0.5. Используется версия 3.0.5.
- ❌ Указана версия 3.0. Нет установленных версий 3.0.*. Последняя установленная версия среды выполнения — 2.1.1. Отображается сообщение об ошибке.
- ✔️ Указана версия 3.0. Нет установленных версий 3.0.*. Последняя установленная версия среды выполнения — 3.1.0. Используется версия 3.1.0.
- ❌ Указана версия 2.0. Нет установленных версий 2.x. Последняя установленная версия среды выполнения — 3.0.0. Отображается сообщение об ошибке.

Накат до дополнительной версии имеет побочный эффект для конечных пользователей. Рассмотрим следующий сценарий.

1. Приложение требует установки версии 3.0.
2. При запуске версия 3.0.* не установлена, зато установлена версия 3.1.0. Будет использоваться версия 3.1.0.
3. Позже пользователь устанавливает 3.0.5 и запускает приложение еще раз. Теперь будет использоваться 3.0.5.

Версии 3.0.5 и 3.1.0 могут вести себя по-разному, особенно в таких сценариях, как сериализация двоичных данных.
...
Вы можете опубликовать приложение как [**автономный дистрибутив**](../deploying/index.md#publish-self-contained). При таком подходе среда выполнения .NET Core и библиотеки объединяются с вашим приложением. Автономные развертывания не зависят от среды выполнения. Выбор версии среды выполнения происходит во время публикации, а не запуска.

Процесс публикации выбирает последнюю версию исправления определенного семейства сред выполнения. Например, `dotnet publish` выберет .NET Core 3.0.4, если это последняя версия исправления семейства среды выполнения .NET Core 3.0. Целевая платформа (включая последние установленные обновления безопасности) упаковывается с приложением.
