...
[Скачайте и начните работу с .NET Core 3.0 (предварительная версия 5)](https://aka.ms/netcore3download) прямо сейчас в Windows, Mac и Linux.
...
<xref:System.Text.Json.Serialization.JsonSerializer?displayProperty=nameWithType> создается на основе <xref:System.Text.Json.Utf8JsonReader> и <xref:System.Text.Json.Utf8JsonWriter> и позволяет при работе с JSON-документами и фрагментами выполнять быструю сериализацию с минимальной нагрузкой на ресурсы памяти.
...
Тип <xref:System.Net.Http.HttpClient?displayProperty=nameWithType> поддерживает протокол HTTP/2. Сейчас поддержка отключена, но ее можно включить, вызвав `AppContext.SetSwitch("System.Net.Http.SocketsHttpHandler.Http2Support", true);` перед использованием <xref:System.Net.Http.HttpClient>. Поддержку HTTP/2 также можно включить, если перед запуском приложения присвоить переменной среды `DOTNET_SYSTEM_NET_HTTP_SOCKETSHTTPHANDLER_HTTP2SUPPORT` значение `true`.

Если протокол HTTP/2 включен, версия протокола HTTP согласуется через TLS/ALPN, а HTTP/2 используется только в том случае, если его выбрал сервер.
