...
- В противном случае производится JIT-компиляция. Как правило, эти методы являются универсальными для типов значений.
  ...
  - JIT-компиляция с полной оптимизацией создает высококачественный (более оптимизированный) код медленнее. Для методов, где быстрая JIT-компиляция не будет применяться (например, если метод имеет атрибут <xref:System.Runtime.CompilerServices.MethodImplOptions.AggressiveOptimization?displayProperty=nameWithType>), используется JIT-компиляция с полной оптимизацией.
