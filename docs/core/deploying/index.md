...
1. Создайте каталог для вашего проекта, а затем введите `[dotnet new console](../tools/dotnet-new.md)` в командной строке, чтобы создать проект консольного приложения C#.
...
5. Отладив и протестировав программу, вы можете создать файлы, которые будут развертываться с приложением, с помощью команды `dotnet publish -f netcoreapp1.1 -c release`. При этом будет создана версия выпуска приложения (а не отладочная версия).
