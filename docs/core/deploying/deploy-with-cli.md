...
>[!IMPORTANT]
>\*При использовании пакета SDK версии 3.0 режим публикации по умолчанию при выполнении базовой команды `dotnet publish` — зависящий от платформы исполняемый файл. Это относится только к проектам, предназначенным для **.NET Core 2.1** или **.NET Core 3.0**.
...
Если приложение имеет машинные зависимости, оно не может выполняться в другой операционной системе. Например, если приложение использует машинный интерфейс Win32 API, оно не будет работать в macOS или Linux. Необходимо написать код для конкретной платформы и скомпилировать исполняемый файл для каждой платформы. 
...
В случае командной строки пакета SDK для .NET Core 3.x зависящий от платформы исполняемый файл (FDE) является режимом по умолчанию для базовой команды `dotnet publish`. Указывать другие параметры, если вы будете работать в текущей операционной системе, не нужно.
...
- `-r <RID>`  
  Этот параметр использует идентификатор (RID) для указания целевой платформы. Список идентификаторов сред выполнения см. в [каталоге идентификаторов сред выполнения](../rid-catalog.md).

- `--self-contained false`  
  Этот параметр указывает пакету SDK для .NET Core, что исполняемый файл нужно создать как FDE.
...
При публикации автономного развертывания (SCD) пакет SDK для .NET Core создает исполняемый файл для конкретной платформы. Публикация SCD включает в себя все необходимые файлы .NET Core, чтобы запустить приложение, но она не включает [машинные зависимости .NET Core](https://github.com/dotnet/core/blob/master/Documentation/prereqs.md). Эти зависимости должны присутствовать в системе перед запуском приложения. 
...
- `--self-contained true`  
  Этот параметр указывает пакету SDK для .NET Core, что исполняемый файл нужно создать как SCD.
