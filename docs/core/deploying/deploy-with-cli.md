---
title: "Развертывание приложений .NET Core с помощью средств командной строки | Документы Майкрософт"
...
1. Обновите зависимости и средства проекта.
   ...
   Выполните команду [dotnet restore](../tools/dotnet-restore.md), чтобы восстановить зависимости, указанные в проекте.
...
Для зависящего от платформы развертывания с одной или более зависимостями сторонних разработчиков требуется, чтобы эти зависимости были доступны в проекте. Перед выполнением команды `dotnet restore` следует выполнить два дополнительных действия.
...
1. Скачайте пакет NuGet, содержащий зависимость стороннего разработчика, если вы этого еще не сделали. Чтобы скачать пакет, выполните команду `dotnet restore` после добавления зависимости. Так как зависимость разрешается из локального кэша NuGet во время публикации, она должна быть доступна в системе.
...
Выполнение автономного развертывания с одной или несколькими зависимостями сторонних разработчиков предполагает добавление этих зависимостей. Перед выполнением команды `dotnet restore` следует выполнить два дополнительных действия.
