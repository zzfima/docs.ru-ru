...
Все доступные события и виртуальные функции описаны в статьях [Managed assembly loading algorithm](loading-managed.md) (Алгоритм загрузки управляемых сборок), [Satellite assembly loading algorithm](loading-resources.md) (Алгоритм загрузки вспомогательных сборок) и [Unmanaged (native) library loading algorithm](loading-unmanaged.md) (Алгоритм загрузки неуправляемых (собственных) сборок).  В тех статьях указаны относительные положения всех событий и функций в алгоритмах загрузки. Мы не будем дублировать эту информацию в этой статье.
