...
В этой статье приводится обзор действий, необходимых для запуска среды выполнения .NET Core из машинного кода, создания исходного домена приложений (@System.AppDomain) и выполнения в нем управляемого кода.
...
Пример основного приложения, демонстрирующий описанные здесь шаги, доступен в нашем репозитории [примеров .NET Core](https://github.com/dotnet/docs/tree/master/samples/core/hosting). Комментарии в файле host.cpp примера четко соответствуют пронумерованным шагам в этом учебнике.
