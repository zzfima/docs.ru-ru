...
1. В Visual Studio последовательно щелкните **Файл**, **Создать**, **Проект**. В диалоговом окне **Новый проект** разверните узел **Visual C#**, выберите узел **.NET Standard**, а затем выберите шаблон **Библиотека классов (.NET Standard)**. При этом будет создана библиотека .NET Standard, использующая .NET Core а также любую другую реализацию .NET, которая поддерживает [.NET Standard](../../standard/net-standard.md) версии 2.0.
