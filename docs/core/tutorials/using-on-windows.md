...
> [!WARNING]
> Эта статья применима к инструментам .NET Core (предварительная версия 2) для Visual Studio 2015. Сведения для версии-кандидата 4 средств .NET Core см. в разделе [Начало работы с .NET Core в Windows с помощью Visual Studio 2017](../preview3/tutorials/using-on-windows-vs-2017.md).
