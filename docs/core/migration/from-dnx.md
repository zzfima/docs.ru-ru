...
Поддержка DNVM прекращена, так как его функциональность стала избыточной вследствие изменений, реализованных в средствах CLI .NET Core.
...
Начиная с версии 1, в средствах CLI .NET Core нет команды `install` для установки зависимостей. Для установки пакета из NuGet, необходимо добавить его в качестве зависимостей для вашей `project.json` файла, а затем запустите `dotnet restore` ([см. Примечание](#dotnet-restore-note)). 
...
При использовании других целевых платформ `dnx`, например `dnx451`, их также необходимо изменить. `net451` следует изменить на `dnx451`. Дополнительные сведения см. в разделе о [.NET Standard](../../standard/net-standard.md). 
