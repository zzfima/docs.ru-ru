...
1. Создайте новую папку и в этой папке выполните `dotnet new classlib`.
2. Откройте `Class1.cs`.
3. Добавьте `using System.Runtime.InteropServices;` в начало файла.
4. Создайте интерфейс с именем `IServer`. Пример: [!code-csharp[The IServer interface](~/samples/core/extensions/COMServerDemo/COMContract/IServer.cs)]
