...
Если у вас есть код, работающий в .NET Framework, возможно, вам будет интересно запустить его в .NET Core.  В этой статье приводятся общие сведения о процессе переноса и перечислены средства, которые могут оказаться полезными при переносе кода в .NET Core.
...
2. Перенацельте все переносимые проекты на .NET Framework последней версии.
...
4. Перенести код тестов.
   ...
   Так как перенос в .NET Core является важным изменением для базы кода, настоятельно рекомендуется перенести тесты, чтобы можно было выполнять их в процессе переноса кода.  Платформу .NET Core сейчас поддерживают все основные средства: MSTest, xUnit и NUnit.
