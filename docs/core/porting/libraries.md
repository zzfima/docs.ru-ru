...
В этой статье рассматривается официальная спецификация API-интерфейсов .NET, которые должны быть доступны во всех средах выполнения .NET.

[Пакеты, метапакеты и платформы](~/docs/core/packages.md)   
Из этой статьи вы узнаете, как .NET Core определяет и использует пакеты и как пакеты поддерживают выполнение кода в нескольких средах выполнения .NET.
...
Целевые объекты проекта PCL можно преобразовать в формат .NET Standard. Для этого загрузите библиотеку в Visual Studio 2017 и сделайте следующее:

1. Щелкните файл проекта правой кнопкой мыши и выберите пункт **Свойства**.
1. В разделе **Библиотека** выберите **Нацелить на стандартную платформу .NET**.

Если пакеты поддерживают NuGet 3.0, целевая платформа проекта изменяется на .NET Standard.

В противном случае в Visual Studio откроется диалоговое окно с предложением удалить текущие пакеты. При получении этого уведомления сделайте следующее:

1. Щелкните проект правой кнопкой мыши и выберите **Управление пакетами NuGet**.
1. Запишите имена пакетов проекта.
1. Удалите пакеты по одному.
1. Возможно, потребуется перезапустить Visual Studio, чтобы завершить удаление. В этом случае в окне **Диспетчер пакетов NuGet** отобразится кнопка **Перезапустить**.
1. После перезапуска целевой платформой проекта будет .NET Standard. Добавьте пакеты, которые требовалось удалить.

## <a name="retargeting-your-net-framework-code-to-net-framework-462"></a>Изменение целевой платформы для кода .NET Framework на .NET Framework 4.6.2

Если ваш код не предназначен для .NET Framework 4.6.2, рекомендуется изменить его целевую платформу на эту версию. Это обеспечит доступность альтернативных API-интерфейсов последних версий в случаях, когда .NET Standard не поддерживает существующие API-интерфейсы.
...
1. В раскрывающемся списке **Требуемая версия .NET Framework** выберите значение **.NET Framework 4.6.2**.
1. Перекомпилируйте проекты.

Так как ваши проекты теперь предназначены для .NET Framework 4.6.2, используйте эту версию .NET Framework в качестве основы для переноса кода.
