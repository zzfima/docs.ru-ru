...
После переноса проекта в формат `PackageReference` необходимо проверить, совместимы ли пакеты с .NET Core.
