...
Файл `botsay.1.0.0.nupkg` создается в папке, которую указывает значение XML `<PackageOutputPath>` из файла `botsay.csproj`. В нашем примере это папка `./nupkg`. Это упрощает установку и тестирование. Если вы решите опубликовать это средство, передайте его в [https://www.nuget.org](https://www.nuget.org).
