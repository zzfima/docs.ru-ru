...
Оба решения допустимые. С помощью скрипта ручной настройки можно управлять версиями средства, полученными в процессе сборки. Сборка выполняется с помощью скрипта, который необходимо создать. В этом разделе описывается только создание вручную. Дополнительные сведения о создании сборки с помощью задач сборки VSTS см. в статье [Непрерывная интеграция и развертывание](https://docs.microsoft.com/vsts/build-release/index).
