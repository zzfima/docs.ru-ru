...
В этом разделе приведены сведения о ручной настройке с помощью скрипта PowerShell или bash, а также описано несколько решений SaaS для непрерывной интеграции, таких как [Travis CI](https://travis-ci.org/), [AppVeyor](https://www.appveyor.com/) и [Build](https://docs.microsoft.com/azure/devops/build-release/index).
...
Оба решения допустимые. С помощью скрипта ручной настройки можно управлять версиями средства, полученными в процессе сборки. Сборка выполняется с помощью скрипта, который необходимо создать. В этом разделе описывается только создание вручную. Дополнительные сведения о создании сборки с помощью задач сборки Azure DevOps Services см. в статье о [непрерывной интеграции и развертывании](https://docs.microsoft.com/azure/devops/build-release/index).
