...
Все наборы инструментов теперь используют общий компонент SDK и его целевые объекты, в том числе интерфейс командной строки. Например, следующей версии Visual Studio не будет вызывать в `dotnet restore` ([см. Примечание](#dotnet-restore-note)) для восстановления зависимостей для проектов .NET Core, он будет использовать целевой объект «Restore» непосредственно. Так как это целевые объекты MSBuild, их можно выполнить с помощью команды [dotnet msbuild](dotnet-msbuild.md) в самом модуле MSBuild. 
