...
- Метод `SetPrice`, который задает значения для полей `bookPrice` и `ISOCurrencySymbol`. Эти значения возвращаются свойствами `Price` и `Currency`.
