...
> [!NOTE]
> В финальном выпуске C# 8 (не предварительная версия) элемент `Nullable` будет добавляться с помощью шаблонов новых проектов. До того времени вам потребуется добавлять его вручную.
