...
В предыдущем разделе вы использовали интерполяцию строк для вставки одной строки внутрь другой. Однако интерполированное выражение может относиться к любому типу данных. Давайте включим в интерполированную строку значения разных типов данных.
...
Затем создайте экземпляр класса `Vegetable` с именем `item`. Для этого воспользуйтесь [ключевым словом`new`](../../language-reference/keywords/new-operator.md) и укажите параметр имени для конструктора `Vegetable`.
...
Теперь вы можете выполнить задачи руководства по [коллекциям в виде списков](arrays-and-collections.md) в своей среде разработки.
