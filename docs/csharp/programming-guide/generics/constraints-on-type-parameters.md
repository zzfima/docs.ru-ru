...
Некоторые ограничения являются взаимоисключающими. Все типы значений должны иметь доступ к конструктору без параметров. Ограничение `struct` подразумевает ограничение `new()`, а ограничение `new()` не может использоваться с ограничением `struct`. Ограничение `unmanaged` подразумевает ограничение `struct`. Ограничение `unmanaged` не может использоваться с ограничениями `struct` или `new()`.
