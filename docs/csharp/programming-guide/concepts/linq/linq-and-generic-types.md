---
title: "LINQ и универсальные типы (C#) | Документы Майкрософт"
ms.custom: 
ms.date: 2015-07-20
ms.prod: .net
ms.reviewer: 
...
2.  <xref:System.Collections.Generic.IEnumerable%601> — это интерфейс, поддерживающий перечисление универсальных классов коллекций с помощью оператора `foreach`. Универсальные классы коллекций поддерживают интерфейс <xref:System.Collections.Generic.IEnumerable%601> так же, как неуниверсальные классы коллекций, такие как <xref:System.Collections.ArrayList>, поддерживают <xref:System.Collections.IEnumerable>.  
...
## <a name="ienumerablet-variables-in-linq-queries"></a>Переменные IEnumerable<T\> в запросах LINQ  
 Переменные запроса [!INCLUDE[vbteclinq](../../../../csharp/includes/vbteclinq_md.md)] типизируются как <xref:System.Collections.Generic.IEnumerable%601> или производный тип, например <xref:System.Linq.IQueryable%601>. Если появляется переменная запроса, которая типизируется как `IEnumerable<Customer>`, это просто означает, что запрос при выполнении выведет последовательность из нуля или более объектов `Customer`.  
