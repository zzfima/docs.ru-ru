...
Как и в предыдущем примере <xref:System.Threading.Tasks.Task%601>, вы можете отделить вызов `Task_MethodAsync` от применения инструкции await, как показывает следующий код. Однако следует помнить, что `Task` не содержит свойство `Result`, и при применении оператора await к `Task` никакое значение не создается.  
...
Начиная с версии 7, в языке C# асинхронные методы могут возвращать любой тип с доступным методом `GetAwaiter`.
