...
Если ваша задача **ограничена производительностью ввода-вывода**, используйте `async` и `await` *без* конструкции `Task.Run`.  Библиотеку параллельных задач использовать *не следует*.  Причина этого указана в статье [Подробный обзор асинхронного программирования](../standard/async-in-depth.md).
...
Может возникнуть ситуация, когда несколько фрагментов данных должны извлекаться одновременно.  `Task` API содержит два метода `Task.WhenAll` и `Task.WhenAny` позволяют написать асинхронный код, который выполняет ожидание без блокировки на нескольких фоновых заданий.
