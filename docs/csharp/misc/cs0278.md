...
"тип" не реализует шаблон "имя шаблона". "имя метода" является неоднозначным с "имя метода".  
  
В C# существует несколько операторов, основывающихся на заданных шаблонах, таких как `foreach` и `using`. Например [ `foreach` инструкции](../language-reference/keywords/foreach-in.md) основывается на классе коллекции, реализующий шаблон «перечисление».
