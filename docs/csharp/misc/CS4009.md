...
Нельзя использовать ключевое слово `async` в точке входа приложения (обычно это метод `Main`).
...
Чтобы ожидать метод, возвращающий <xref:System.Threading.Tasks.Task%601>, извлеките значение свойства <xref:System.Threading.Tasks.Task%601.Result>, как показано в следующем примере:
