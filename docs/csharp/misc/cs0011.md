...
Не удается разрешить базовый класс или интерфейс "класс" в сборке "сборка", на которую ссылается тип "тип"  
 ...
 Дополнительные сведения см. в разделе [диалоговое окно "Добавление ссылки"](/visualstudio/ide/how-to-add-or-remove-references-by-using-the-reference-manager) и [-Reference (C# параметры компилятора)](../language-reference/compiler-options/reference-compiler-option.md).  
