---
title: "Новые возможности C# 7 — руководство по C#"
...
---
# <a name="whats-new-in-c-7"></a>Новые возможности C# 7

В C# 7 язык C# получает ряд новых возможностей:
...
* [Операции удаления](#discards)
    - Удаляет являются временный, только для записи переменные, используемые в назначения, если вас не интересует присвоенное значение. Они особенно полезны при разбора кортежи и определяемых пользователем типов, а также при вызове методов с `out` параметров.
...
* [Локальные переменные и возвращаемые значения `ref`](#ref-locals-and-returns)
    - Аргументы и локальные переменные метода могут быть ссылками на другое хранилище.
...
> [!NOTE]
> Кортежи, существовавшие до 7 C#, но они были неэффективным и бы язык не поддерживается.
> Это означает, что элементы кортежа может ссылаться только как `Item1`, `Item2` и т. д. C# 7 появилась поддержка языка для кортежей, позволяющий семантической имена для полей кортежа, с помощью нового, более эффективно кортежа типов.

Кортеж можно создать, назначив каждый элемент определенному значению:

[!code-csharp[UnnamedTuple](../../../samples/snippets/csharp/new-in-7/program.cs#04_UnnamedTuple "Unnamed tuple")]

Что назначения создает кортеж, членами которой являются `Item1` и `Item2`, который можно использовать так же, как <xref:System.Tuple> можно изменить синтаксис для создания кортежа, предоставляющего семантической имена для каждого из элементов кортежа:

[!code-csharp[NamedTuple](../../../samples/snippets/csharp/new-in-7/program.cs#05_NamedTuple "Named tuple")]

Кортеж `namedLetters` содержит поля, которые называются `Alpha` и `Beta`. Эти имена не сохраняются для примера при проверке кортежа, с помощью отражения во время выполнения и существует только во время компиляции.

В назначении кортежа можно также указать имена полей в правой части назначения:

[!code-csharp[ImplicitNamedTuple](../../../samples/snippets/csharp/new-in-7/program.cs#06_ImplicitNamedTuple "Implicitly named tuple")]

Можно указать имена для полей на левую и правую части операции присваивания:
...
Отдельные поля можно извлекать, назначая кортеж методу `Point`:
...
## <a name="discards"></a>Отменяет

Часто при разбора кортеж или вызов метода с `out` параметров, приходится определение переменной, значение которого не интересует и не собираетесь использовать. C# добавляет поддержку для *отменяет* для обработки этого сценария. Отклонения является переменной только для записи, имя которого `_` (символ подчеркивания); можно назначить все значения, которые вы собираетесь отменить для одной переменной. Отменить аналогичен неприсвоенную переменную; Помимо оператора присваивания отклонения не может использоваться в коде.

Удаляет поддерживаются в следующих сценариях:

* Когда разбора кортежей или определяемые пользователем типы.

* При вызове методов с [out](../language-reference/keywords/out.md) параметров.

* В выражении шаблона операции с [—](../language-reference/keywords/is.md) и [переключения](../language-reference/keywords/switch.md) инструкции.

* Как автономный идентификатор при необходимо явно определите значение как отменить назначение.

В следующем примере определяется `QueryCityDataForYears` метод, который возвращает 6-кортеж, содержащий данные для города для двух разных лет. Вызов метода в примере отвечает только за заполнение два значения, возвращаемые методом и поэтому рассматривает остальные значения в кортеже как отменяет при его деконструирование кортежа.

[!code-csharp[Tuple-discard](../../../samples/snippets/csharp/programming-guide/deconstructing-tuples/discard-tuple1.cs)]

Дополнительные сведения см. в разделе [отменяет](../discards.md).
...
### <a name="is-expression"></a>`is`выражение

`is` Выражение шаблона расширяет привычную `is` оператор, чтобы запросить объект за пределами своего типа.
...
Чтобы добавить такой кубик в коллекцию, в первую очередь определите тип, представляющий процентный кубик:
...
Язык C# имеет три другие правила, которые можно защитить неправильного использования `ref` локальные переменные и возвращает:

* Невозможно присвоить возвращаемое значение стандартный метод для `ref` локальной переменной.
...
* Переменную `ref` нельзя возвращать переменной, которая продолжает существовать даже после того, как метод будет выполнен.
    - Это означает, что не может вернуть ссылку на локальную переменную или переменную с аналогичные области.
* `ref`локальные переменные и возвращает не может использоваться с асинхронными методами.
    - Компилятор не знает, если упоминаемой переменной передано в свое последнее значение при возвращении асинхронного метода.
...
В C# версии 6 появились [элементы, воплощающие выражение](csharp-6.md#expression-bodied-function-members), для функций-членов и свойств, доступных только для чтения. В C# версии 7 список допустимых членов, которые могут быть реализованы как выражения, был расширен. В C# версии 7 можно реализовать *конструкторы*, *методы завершения*, а также методы доступа `get` и `set` для *свойств* и *индексаторов*. В следующем коде показаны примеры каждого из них:
...
> [!NOTE]
> Необходимо добавить пакет NuGet [ `System.Threading.Tasks.Extensions` ](https://www.nuget.org/packages/System.Threading.Tasks.Extensions/) для использования <xref:System.Threading.Tasks.ValueTask%601> типа.
...
Неправильное толкование числовых констант затрудняет понимание кода при первом прочтении. Это часто случается, когда эти числа используются как битовые маски или иные символьные, а не числовые значения. C# версии 7 включает две новые возможности для упрощения записи чисел в удобочитаемом виде: *двоичные литералы* и *разделители между цифрами*.
