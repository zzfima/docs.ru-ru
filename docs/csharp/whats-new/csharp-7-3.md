...
## <a name="enabling-more-performant-safe-code"></a>Повышение производительности безопасного кода
...
В более ранних версиях C# переменную необходимо закрепить, чтобы получить доступ к целым числам, входящим в `myFixedField`. Теперь такой код компилируется в безопасном контексте:
...
Дополнительные сведения см. в статье [о возвращаемых значениях `ref` и локальных переменных `ref`](../programming-guide/classes-and-structs/ref-returns.md).
...
Типы кортежей в C# теперь поддерживают `==` и `!=`. Дополнительные сведения см. в разделе о [равенстве](../tuples.md#equality-and-tuples) в статье про [кортежи](../tuples.md).
