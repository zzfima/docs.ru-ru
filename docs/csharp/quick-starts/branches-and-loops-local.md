...
Из этого краткого руководства вы узнаете, как написать код, который проверяет значения переменных и на их основе изменяет путь выполнения программы. Вы напишете код C# и сможете просмотреть результаты его компиляции и выполнения. Это краткое руководство содержит упражнения, в которых рассматриваются конструкции ветвления и циклов в C#. В рамках этих занятий вы ознакомитесь с основами языка C#.
...
Создайте каталог с именем **branches-quickstart**. Откройте этот каталог и выполните команду `dotnet new console -n BranchesAndLoops -o .`. Эта команда создает консольное приложение .NET Core в текущем каталоге.
...
> В языке C# не обязательно ставить отступы или пробелы. Операторы после ключевого слова `if` или `else` будут выполняться на основе условия. В этом кратком руководстве во всех строках в примерах кода отступы традиционно соответствуют управляющему потоку операторов.
...
> [!TIP]
> Все примеры кода в следующих разделах краткого руководства содержат фигурные скобки в соответствии с общепринятыми принципами.
...
Попробуйте самостоятельно. Затем проверьте результат. Вы должны получить ответ "63". Один из возможных ответов можно увидеть в [полном примере кода в GitHub](https://github.com/dotnet/docs/tree/master/samples/csharp/branches-quickstart/Program.cs#L46-L54).

Вы ознакомились с кратким руководством по ветвям и циклам.

Теперь вы можете выполнить руководство [Интерполированные строки](interpolated-strings-local.md) в своей среде разработки.
