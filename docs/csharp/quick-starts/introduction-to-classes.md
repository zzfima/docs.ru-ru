...
Инструкция [`throw`](../language-reference/throw.md) **вызывает** исключение. Когда будет найден соответствующий блок `catch`, выполнение текущего метода завершится и возобновится. Вы добавите блок `catch` для тестирования этого кода немного позже.
