...
- **Ссылка может иметь значение NULL**. Предупреждения не выдаются, если ссылочный тип инициализируется со значением NULL либо позднее получает это значение.
