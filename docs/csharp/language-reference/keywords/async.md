...
## Пример  
 В следующем примере показана структура и поток управления между обработчиком асинхронных событий `StartButton_Click` и асинхронным методом `ExampleMethodAsync`.  Результатом выполнения асинхронного метода является длина загруженного веб\-сайта.  Код подходит для приложения Windows Presentation Foundation \(WPF\) или приложения для Магазина Windows Presentation Foundation \(WPF\), которые создается в [!INCLUDE[vs_dev12](../../../csharp/getting-started/includes/vs-dev12-md.md)]; см. комментарии к коду для настройки приложения.  
