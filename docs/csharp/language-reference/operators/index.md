...
[x > y](comparison-operators.md#greater-than-operator-) — больше чем (возвращает true, если x больше y).
...
## <a name="logical-and-operator"></a>Логический оператор И
...
[x <<= y](bitwise-and-shift-operators.md#compound-assignment) — назначение со сдвигом влево. Сдвиньте значение `x` влево на `y` позиций, сохраните результат в `x` и возвратите новое значение.
