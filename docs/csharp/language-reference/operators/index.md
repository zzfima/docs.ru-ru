...
## <a name="null-coalescing-operator"></a>Оператор объединения с NULL  
 ...
 [x ?? y](../../../csharp/language-reference/operators/null-coalescing-operator.md) — возвращает `x`, если значение отличается от `null`; в противном случае возвращает `y`.  
