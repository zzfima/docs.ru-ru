...
Оператор "оператор" не может применяться к операнду типа "тип" и "тип"
 ...
 Бинарный оператор применяется к типам данных, которые не поддерживают его. Например, оператор [&#124;&#124;](../operators/boolean-logical-operators.md#conditional-logical-or-operator-) невозможно применять к строкам, операторы [+](../operators/addition-operator.md), [-](../operators/subtraction-operator.md), [\<](../operators/comparison-operators.md#less-than-operator-) или [>](../operators/comparison-operators.md#greater-than-operator-) — к переменным типа [bool](../keywords/bool.md), а оператор [==](../operators/equality-operators.md#equality-operator-) — с типом `struct`, если этот тип не перегружает оператор явно.
...
## <a name="example"></a>Пример
 ...
 В следующем примере ошибка CS0019 возникает в двух местах, поскольку тип [bool](../keywords/bool.md) в C# не допускает преобразование в [int](../builtin-types/integral-numeric-types.md). Кроме того, ошибка CS0019 возникает при попытке применить оператор вычитания `-` к строке. Оператор сложения `+` можно использовать со строковыми операндами, поскольку он перегружен в классе `String`, где реализует объединение строк.
