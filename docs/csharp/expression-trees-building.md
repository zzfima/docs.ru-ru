...
Процесс построения деревьев выражений усложняется тем фактом, что эти деревья выражений являются неизменяемыми. Неизменяемость означает, что дерево необходимо создать, начиная с листьев и заканчивая корнем. В API-интерфейсах, с помощью которых вы будете создавать деревья выражений, это учтено: Методы создания узла принимают все его дочерние элементы в качестве аргументов. Рассмотрим несколько примеров, демонстрирующих способы создания.
