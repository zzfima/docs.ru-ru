...
- передачи выполняемого кода в асинхронные методы, такие как <xref:System.Threading.Tasks.Task.Run(System.Action)>;
