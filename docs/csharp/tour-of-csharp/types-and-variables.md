...
description: "Узнайте, как определяются типы и объявляются переменные в языке C#"
...
Типы классов, `struct`, `interface` и `delegate` поддерживают универсальные шаблоны, которые позволяют передавать им другие типы в качестве параметров.
