...
Это вторая задача быстрого запуска службы WCF Data Services. В этой задаче вы запустите службы данных WCF из Visual Studio и дополнительно отключим чтение канала в веб-браузере. Можно затем получить документ определения службы также доступ к ресурсам службы данных путем отправки запросов HTTP GET через веб-браузер к доступным ресурсам.

> [!NOTE]
> По умолчанию Visual Studio автоматически приписывает для URI на компьютере номер порта `localhost`. В данной задаче в примерах URI используется порт номер `12345`. Дополнительные сведения о том, как задать конкретный номер порта в проекте Visual Studio см. в разделе [Создание службы данных](../../../../docs/framework/data/wcf/creating-the-data-service.md).
...
1. В Internet Explorer из **средства** меню, выберите **обозревателя**, нажмите кнопку **содержимого** щелкните **параметры**и снимите  **Включить просмотр каналов**.
...
2. В Visual Studio нажмите клавишу **F5** клавишу, чтобы начать отладку приложения.
