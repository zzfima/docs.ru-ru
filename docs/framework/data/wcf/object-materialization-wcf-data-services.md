...
После того как библиотека получает ответное сообщение запроса, она преобразует возвращенные данные из [!INCLUDE[ssODataShort](../../../../includes/ssodatashort-md.md)] канала в экземпляры классов клиентских служб данных, которые имеют тип запроса. Обычно общий процесс материализации этих объектов происходит следующим образом.
