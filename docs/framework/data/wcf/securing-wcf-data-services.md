...
При планировании безопасности службы OData на основе службы данных WCF, необходимо решить, процесс обнаружения и проверки удостоверения участника, проверки подлинности и авторизации, процесс определения, является ли авторизованный участник разрешен доступ к запрошенным ресурсам. Следует также рассмотреть необходимость шифрования сообщения с помощью протокола SSL.  
  
## <a name="authenticating-client-requests"></a>Проверка подлинности клиентских запросов  
Службы WCF Data Services не реализует любого вида проверки подлинности собственными, а использует условия проверки подлинности узла службы данных. Это означает, что служба предполагает, что любой запрос, который он получает уже прошел проверку подлинности на сетевом узле и что узел правильно определил участника, передавшего запрос соответствующим образом через интерфейсы, предоставляемые службами WCF Data Services. Эти параметры проверки подлинности и методики, подробно изложены в multipart [OData and Authentication серии](https://devblogs.microsoft.com/odata/?s=%22OData+and+authentication%22).  
...
```  
 ...
 Дополнительные сведения см. в разделе [Практическое руководство. Укажите учетные данные клиента для запроса службы данных](../../../../docs/framework/data/wcf/specify-client-creds-for-a-data-service-request-wcf.md).  
  
 Когда служба данных требует учетные данные входа в систему, которые нельзя указать с помощью объекта <xref:System.Net.NetworkCredential>, например маркер на базе утверждений или файл cookie, необходимо вручную задать заголовки в HTTP-запросе, обычно это заголовки `Authorization` и `Cookie`. Дополнительные сведения об этом сценарии проверки подлинности см. в записи блога [ OData и обработчики проверки подлинности — часть 3 — на стороне клиента](https://devblogs.microsoft.com/odata/odata-and-authentication-part-3-clientside-hooks/). Пример задания HTTP-заголовков в сообщении запроса, см. в разделе [как: Задать заголовки в клиентском запросе](../../../../docs/framework/data/wcf/how-to-set-headers-in-the-client-request-wcf-data-services.md).  
...
### <a name="restrict-access-to-data-service-resources"></a>Ограничение доступа к ресурсам службы данных  
 По умолчанию WCF Data Services позволяет предоставлять общий доступ чтение и запись к ресурсам службы данных (entity задать и операции службы) любому пользователю, который может получить доступ к службе данных. Правила, определяющие доступ для чтения и записи, можно задавать отдельно для каждого набора сущностей, предоставляемого службой данных, а также для любой операции службы. Рекомендуется ограничить доступ для чтения и записи только теми ресурсами, которые необходимы клиентскому приложению. Дополнительные сведения см. в разделе [минимальные требования к доступа ресурсов](../../../../docs/framework/data/wcf/configuring-the-data-service-wcf-data-services.md#accessRequirements).  
...
Протокол OData основан на протоколе HTTP. Заголовок HTTP-сообщения может содержать полезные учетные данные пользователя в зависимости от типа проверки подлинности, реализуемого службой данных. Текст сообщения может также содержать полезные пользовательские данные, которые могут требовать защиты. В обоих случаях рекомендуется защищать передачу этих сведений по сети с помощью SSL.  
...
### <a name="ignored-message-headers-and-cookies"></a>Пропускаемые заголовки сообщений и файлы cookie  
 ...
 Файлы cookie можно использовать как часть проверки подлинности, такие как с помощью проверки подлинности форм ASP.NET. Тем не менее все файлы cookie HTTP, задайте во входящем запросе игнорируются WCF DataServices. Узел службы данных может обрабатывать файл cookie, но среда выполнения службы данных WCF никогда не анализирует и не возвращает файлы cookie. Клиентская библиотека службы WCF Data Services также не обрабатывает файлы cookie, отправленные в ответе.  
  
### <a name="custom-hosting-requirements"></a>Требования к пользовательскому размещению  
 По умолчанию WCF Data Services создается как приложения ASP.NET, размещенного в службах IIS. Это позволяет службе данных использовать методы защиты этой платформы. Вы можете определить, размещаемых на пользовательский узел службы WCF Data Services. Дополнительные сведения см. в разделе [размещение служб данных](../../../../docs/framework/data/wcf/hosting-the-data-service-wcf-data-services.md). Компоненты и платформа, на которой размещается служба данных, должны поддерживать приведенные ниже методы защиты для предотвращения атак на службу данных.  
...
- Ограничить размер заголовков HTTP и их значений и предоставляют службы WCF Data Services доступ к данным заголовка.  
...
### <a name="values-are-not-further-encoded"></a>Отсутствие дальнейшего кодирования значений  
 Значения свойств, отправляемые службе данных не выполняет дальнейшего кодирования средой выполнения службы данных WCF. Например, если строковое свойство сущности содержит отформатированное HTML-содержимое, теги не кодируются службой данных в HTML. Кроме того, служба данных не выполняет дальнейшего кодирования значений свойств в ответе. Клиентская библиотека также не выполняет дополнительного кодирования.  
  
### <a name="considerations-for-client-applications"></a>Замечания по клиентским приложениям  
 Следующие рекомендации по безопасности применяются к приложениям, использующим клиента служб данных WCF для доступа к службам OData.  
