...
## <a name="querying-datasets-using-linq-to-dataset"></a>Запросы к наборам данных с помощью LINQ to DataSet  
 ...
 Помимо стандартных операторов запросов реализованным в библиотеке System.Core.dll, LINQ to DataSet добавляет некоторые <xref:System.Data.DataSet>-специальные расширения, которые упрощают запрос через набор <xref:System.Data.DataRow> объектов. Эти специфичные для <xref:System.Data.DataSet> расширения включают в себя операторы для сравнения последовательностей строк, а также методы, обеспечивающие доступ к значениям столбцов <xref:System.Data.DataRow>.  
...
## <a name="n-tier-applications-and-linq-to-dataset"></a>Многоуровневые приложения и LINQ to DataSet  
 ...
 В многоуровневых приложениях объект <xref:System.Data.DataSet> часто используется на среднем уровне, чтобы кэшировать данные для веб-приложений. LINQ to DataSet, функциональные возможности отправки запросов реализуется с помощью методов расширения и расширяет возможности существующего в ADO.NET 2.0 <xref:System.Data.DataSet>.  
