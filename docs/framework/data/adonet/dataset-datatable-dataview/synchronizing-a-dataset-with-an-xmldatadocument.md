...
В этом разделе демонстрируется один шаг обработки заказа на покупку с использованием <xref:System.Data.DataSet> со строгой типизацией, синхронизированного с <xref:System.Xml.XmlDataDocument>. В приведенных ниже примерах создается **набор данных** с минимальной схемой, которая соответствует только части исходного XML-документа. В примерах объект **XmlDataDocument** используется для сохранения точности исходного XML-документа, что позволяет использовать **набор данных** для представления подмножества XML-документа.  
...
```  
 ...
 Обратите внимание, что в схему **набора данных**включаются только сведения из элементов **OrderDetails** и **Products** исходного XML-документа. Синхронизация **набора данных** с **XmlDataDocument** гарантирует, что элементы, не входящие в **набор данных** , будут сохраняться в XML-документе.  
  
 Благодаря строго типизированному **набору данных** , созданному из схемы XML (с пространством имен **Northwind. филлордер**), часть исходного XML-документа может быть предоставлена путем синхронизации **набора данных** с загруженным объектом **XmlDataDocument** . из исходного XML-документа. Обратите внимание, что **набор данных** , сформированный из схемы, содержит структуру, но не имеет данных. Данные заполняются при загрузке XML в объект **XmlDataDocument**. При попытке загрузить объект **XmlDataDocument** , который был синхронизирован с **набором данных** , который уже содержит данные, будет создано исключение.  
