...
```  
 ...
 Обратите внимание, что только сведения из **OrderDetails** и **продуктов** из исходного XML-документа включены в схему для **набора данных**. Синхронизация **DataSet** с **XmlDataDocument** гарантирует, что элементы, не включенные в **DataSet** будут сохраняться с XML-документа.  
  
 Со строгой типизацией **DataSet** созданные из схемы XML (с пространством имен **Northwind.FillOrder**), часть исходного XML-документа может предоставляться синхронизацией  **Набор данных** с **XmlDataDocument** загружаются из исходного XML-документа. Обратите внимание, что **DataSet** созданные из схемы содержит структуру, но не данные. Данные заполняются при загрузке XML в **XmlDataDocument**. При попытке загрузить **XmlDataDocument** синхронизированной с **набора данных** , который уже содержит данные, будет вызвано исключение.  
  
 После **DataSet** (и **XmlDataDocument**) будет обновлен, **XmlDataDocument** может записать измененный XML-документ с элементами, которые пропустил **DataSet** остаться без изменений, как показано ниже. В сценарии заказа на покупку после заполнения элементов заказа измененный XML-документ может быть передан на следующий шаг обработки заказа, например в отдел доставки компании.  
