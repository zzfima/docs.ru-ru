...
|**hasChanges**|Определяет строку в  **\<**  ***DataInstance***  **>**  блокировать как измененный. **HasChanges** заметка может иметь одно из следующих двух значений:<br /><br /> **inserted**<br /> Идентифицирует **Added** строки.<br /><br /> **modified**<br /> Идентифицирует **Modified** строку, содержащую **исходного** версию строки в  **\<diffgr: перед >** блока. Обратите внимание, что **Deleted** строки будут иметь **исходного** версию строки в  **\<diffgr: перед >** блок, но будет нет элемент с заметкой в  **\<**  ***DataInstance***  **>**  блока.|  
