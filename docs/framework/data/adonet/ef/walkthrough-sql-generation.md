...
## <a name="first-phase-of-sql-generation-visiting-the-expression-tree"></a>Первый этап формирования SQL: Обход дерева выражения  
 ...
 Элемент управления возвращается к обработке Join4, родительского элемента Join1. Поскольку дочерний элемент повторно использовал инструкцию Select, области Join1 замещаются одним <joinSymbol_Join1> символа Join. Кроме того, в символьную таблицу добавляется новая запись, чтобы связать Join1 с <joinSymbol_Join1>.  
 ...
 Рассмотрим порядок посещения «Var(Join2).Extent4.OrderID». Сначала посещается свойство экземпляра «Var(Join2).Extent4», которое является очередным DbPropertyExpression и посещает в первую очередь свой экземпляр «Var(Join2)». В самой верхней области таблицы символов «Join2» разрешается в <joinSymbol_join2>. В методе посещения для обработки DbPropertyExpression «Var(Join2).Extent4» отмечает, что при посещении экземпляра был возвращен символ соединения и требуется объединение.  
  
 Поскольку это вложенное соединение, в словаре NameToExtent символа соединения следует найти свойство «Extent4», разрешить его в «symbol_Extent4» и возвратить новый SymbolPair(<joinSymbol_join2>, <symbol_Extent4>). Поскольку обработка экземпляра «Var(Join2).Extent4.OrderID» возвращает пару символов, свойство «OrderID» разрешается из ColumnPart этой пары символов (<symbol_Extent4>), где есть список столбцов представляемой области. Таким образом, «Var(Join2).Extent4.OrderID» разрешается в { <joinSymbol_Join2>, ".", <symbol_OrderID>}.  
