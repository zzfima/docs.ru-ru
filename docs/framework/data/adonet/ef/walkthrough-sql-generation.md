...
Элемент управления возвращается к обработке Join4, родительского элемента Join1. Так как дочерний элемент повторно использовал инструкцию Select, области join1 заменяются символа Join \<joinSymbol_Join1 >. Также добавляется новая запись в таблицу символов, чтобы связать Join1 с \<joinSymbol_Join1 >.
...
Поскольку это вложенное соединение, мы поиска в словаре NameToExtent символа соединения свойство «Extent4», разрешить его в \<symbol_Extent4 > и возвратить новый SymbolPair (\<joinSymbol_join2 >, \<symbol_Extent4 >). Так как пару символов возвращается из-за обработки экземпляра «Var(Join2). Extent4.OrderID», свойство «OrderID» разрешается из ColumnPart этой пары символов (\<symbol_Extent4 >), который содержит список столбцов представляемой. Таким образом, «Var(Join2). Extent4.OrderID» разрешается в { \<joinSymbol_Join2 >, «.», \<symbol_OrderID >}.
