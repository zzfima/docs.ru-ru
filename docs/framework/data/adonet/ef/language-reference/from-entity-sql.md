...
Указывает коллекцию, используемую в инструкциях [SELECT](../../../../../../docs/framework/data/adonet/ef/language-reference/select-entity-sql.md) .  
...
> [!NOTE]
> В отличие от Transact-SQL, нет необходимости в [!INCLUDE[esql](../../../../../../includes/esql-md.md)]явном развложенном шаге.  
  
> [!NOTE]
> `CROSS`операторы `OUTER APPLY` и появились в SQL Server 2005. В некоторых случаях конвейер запросов может сформировать код Transact-SQL, который содержит операторы `CROSS APPLY` и `OUTER APPLY`. Поскольку некоторые серверные поставщики, включая версии SQL Server более ранние, чем SQL Server 2005, не поддерживают эти операторы, такие запросы не могут выполняться на этих внутренних поставщиках.  
>   
...
## <a name="multiple-collections-in-the-from-clause"></a>Несколько коллекций в предложении FROM  
 ...
 В следующем примере `C` и `D` являются независимыми коллекциями `C`, `c.Names` но зависит от.  
