...
При работе с путями в приложениях ASP.NET, необходимо учитывать следующее.  
  
#### <a name="verify-whether-your-host-performs-path-checks"></a>Убедиться в том, что узел выполняет проверку путей  
 Когда `|DataDirectory|` (заключаемой в символы конвейера) используется строка подстановки, ADO.NET проверяет, что поддерживается разрешенный путь. Например, недопустима подстрока ".." после `DataDirectory`. Та же проверка разрешения оператор задания корневого каталога приложения Web (`~`) выполняется процессом размещения ASP.NET. Эту проверку выполняют службы IIS, однако узлы, отличные от IIS, могут не проверять, поддерживается ли преобразованный путь. Необходимо знать, как действует узел, на котором развертывается приложение [!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)].  
