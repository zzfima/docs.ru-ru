...
### <a name="writing-a-provider-with-symmetric-type-mapping"></a>Разработка поставщика с симметричным сопоставлением типов  
 Можно написать поставщик для, [!INCLUDE[adonet_ef](../../../../../includes/adonet-ef-md.md)] где каждый тип хранилища сопоставляется с одним типом EDM, независимо от направления сопоставления. Для типа поставщика, имеющего простое сопоставление с типом модели EDM, можно использовать симметричное решение, поскольку система типов проста или совпадает с системой типов модели EDM.  
...
- Список типов поставщиков, выраженных в терминах модели EDM и определяющий сопоставление для обоих направлений: EDM-to-provider и provider-to-EDM.  
...
#### <a name="using-a-data-store-connection"></a>Использование соединения с хранилищем данных  
 Если подключение к хранилищу данных доступно, вызовите <xref:System.Data.Common.DbProviderServices.GetProviderManifestToken%2A?displayProperty=nameWithType> метод, чтобы вернуть маркер, который передается в этот <xref:System.Data.Common.DbProviderManifest> <xref:System.Data.Common.DbProviderServices.GetProviderManifest%2A> способ, который возвращает. Этот метод делегирует реализацию `GetDbProviderManifestToken`поставщика.  
...
|Имя атрибута|Тип данных|Обязательно|Default Value|Описание|  
|--------------------|---------------|--------------|-------------------|-----------------|  
