...
В этом разделе рассматривается поддержка асинхронного программирования в поставщик данных .NET Framework для SQL Server (SqlClient) включая усовершенствования для поддержки асинхронного программирования функций, появившихся в .NET Framework 4.5.

## <a name="legacy-asynchronous-programming"></a>Асинхронное программирование прежних версий

До .NET Framework 4.5, асинхронное программирование с помощью SqlClient выполнялось с помощью следующих методов и `Asynchronous Processing=true` свойство соединения:
...
Эти функциональные возможности остаются в SqlClient в .NET Framework 4.5.

> [!TIP]
> Начиная с .NET Framework 4.5, эти устаревшие методы больше не требуют `Asynchronous Processing=true` в строке подключения.

## <a name="asynchronous-programming-features-added-in-net-framework-45"></a>Возможности асинхронного программирования, добавленных в .NET Framework 4.5

Новые функции асинхронного программирования предоставляют простой метод создания асинхронного кода.

Дополнительные сведения о возможности асинхронного программирования, которая была представлена в .NET Framework 4.5 см. в разделе:
...
- [Применение новых асинхронных методов SqlDataReader в .NET 4.5 (часть 1)](https://blogs.msdn.microsoft.com/adonet/2012/04/20/using-sqldatareaders-new-async-methods-in-net-4-5/)

- [Применение новых асинхронных методов SqlDataReader в .NET 4.5 (часть 2)](https://blogs.msdn.microsoft.com/adonet/2012/07/15/using-sqldatareaders-new-async-methods-in-net-4-5-part-2-examples/)
...
Модификатор `async` указывает, что метод является асинхронным. При вызове метода `async` происходит возврат задачи. Когда `await` оператор применяется к задаче, текущий метод немедленно завершает работу. После завершения задачи выполнение возобновляется в том же методе.
...
В .NET Framework 4.5 для поддержки асинхронного программирования, были добавлены следующие методы:
...
Для использования распределенных транзакций необходимо включить на сервере MSDTC. Сведения о включении MSDTC см. в разделе [как включить MSDTC на веб-сервере](https://docs.microsoft.com/previous-versions/commerce-server/dd327979(v=cs.90)).
...
Режим MARS позволяет использовать соединение как для операций чтения, так и для операций языка DML более чем с одной отложенной операцией. Эта возможность избавляет приложение от необходимости решения ошибок занятости соединения. Кроме того, режим MARS позволяет заменить серверные курсоры, которые, как правило, потребляют много ресурсов. Наконец, так как несколько операций могут выполняться в одном соединении, они могут совместно использовать тот же контекст транзакции, устраняя необходимость использования **sp_getbindtoken** и **sp_bindsession** системные хранимые процедуры.
