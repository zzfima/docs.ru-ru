...
До .NET Framework 4,5 асинхронное программирование с помощью SqlClient выполнялось со следующими методами и `Asynchronous Processing=true` свойством Connection:
...
> [!TIP]
> Начиная с .NET Framework 4,5, эти устаревшие методы больше не требуются `Asynchronous Processing=true` в строке подключения.
...
- [Использование новых асинхронных методов SqlDataReader в .NET 4,5 (часть 1)](https://blogs.msdn.microsoft.com/adonet/2012/04/20/using-sqldatareaders-new-async-methods-in-net-4-5/)

- [Использование новых асинхронных методов SqlDataReader в .NET 4,5 (часть 2)](https://blogs.msdn.microsoft.com/adonet/2012/07/15/using-sqldatareaders-new-async-methods-in-net-4-5-part-2-examples/)
...
Модификатор `async` указывает, что метод является асинхронным. При вызове метода `async` происходит возврат задачи. `await` Когда оператор применяется к задаче, текущий метод немедленно завершает работу. После завершения задачи выполнение возобновляется в том же методе.
...
> [!TIP]
> Новые асинхронные методы не нуждаются `Asynchronous Processing=true` в строке подключения.
