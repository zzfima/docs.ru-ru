...
---
# <a name="how-to-implement-copytodatatablelttgt-where-the-generic-type-t-is-not-a-datarow"></a>Как реализовать метод CopyToDataTable&lt;T&gt;, если универсальный тип T не является DataRow
...
1.  Реализуйте класс `ObjectShredder<T>` для создания объекта <xref:System.Data.DataTable> из источника <xref:System.Collections.Generic.IEnumerable%601>:  
    ...
    В предыдущем примере предполагается, что свойства `DataColumn` не являются типами, допускающими значение null. Для обработки свойств с типами, допускающими значение null, используйте следующий код:
