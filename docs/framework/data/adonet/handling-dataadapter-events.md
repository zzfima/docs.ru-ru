---
title: "Обработка событий DataAdapter | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
|`SkipAllRemainingRows`|Прервать операцию обновления без вызова исключения.|  
 ...
 В следующем образце кода показано, как добавлять и удалять обработчики событий.  Обработчик события `RowUpdating` ведет журнал всех удаленных записей с временной меткой.  Обработчик события `RowUpdated` добавляет сведения об ошибке в свойство `RowError` строки в `DataSet`, запрещает исключение и продолжает обработку \(зеркально отображая поведение `ContinueUpdateOnError` \= `true`\).  
