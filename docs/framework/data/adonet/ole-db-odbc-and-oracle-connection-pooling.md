...
```  
 ...
 Дополнительные сведения о сеансе OLE DB или Создание пулов ресурсов, а также способах отключения пулов путем переопределения стандартных настроек служб поставщика OLE DB см. в разделе [Руководство программиста OLE DB](http://go.microsoft.com/fwlink/?linkid=45232).  
...
## <a name="connection-pooling-for-odbc"></a>Пулы соединений для Odbc  
 ...
 Дополнительные сведения о пулах соединений ODBC см. в разделе [INFO: часто задаваемые вопросы о пулах соединений ODBC](http://support.microsoft.com/kb/169470).  
...
### <a name="transaction-support"></a>Поддержка транзакций  
 Соединения выбираются из пула и назначаются в зависимости от контекста транзакции. Контексты запрашивающего потока и назначенного соединения должны совпадать. Поэтому каждый пул соединений фактически делится на соединения без контекста транзакции, связанные с ними, а также в *N* подразделов, каждый из которых содержит соединения с определенным контекстом транзакции.  
