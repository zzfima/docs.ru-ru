...
> Формат даты ГДМ не поддерживается при преобразовании из строкового формата в формат `date`, `time`, `datetime2` или `datetimeoffset`.  
 ...
 Дополнительные сведения о том, как SQL Server интерпретирует данные даты и времени, см. в разделе [использование данных даты и времени](https://go.microsoft.com/fwlink/?LinkID=98361) в электронной документации по SQL Server 2008.  
...
Тип <xref:System.Data.SqlClient.SqlParameter> данных можно указать с помощью одного из описанных выше <xref:System.Data.SqlDbType> перечислений. 

> [!NOTE]
> Нельзя задать `DbType` `SqlParameter` для свойствазначение.`SqlDbType.Date`
...
|<xref:System.Data.SqlClient.SqlParameter.Scale%2A>|Возвращает или задает число десятичных разрядов, до которого разрешается временная часть значения для `Time`, `DateTime2`и `DateTimeOffset`. Значение по умолчанию — 0, означающее, что фактический масштаб выводится из значения и отправляется на сервер.|  
...
### <a name="creating-parameters"></a>Создание параметров  
 ...
 В следующих подразделах данного раздела приведены примеры указания параметров даты и времени. Дополнительные примеры работы с параметрами см. в разделе [Настройка параметров и типов данных](../../../../../docs/framework/data/adonet/configuring-parameters-and-parameter-data-types.md) параметров и [параметров DataAdapter](../../../../../docs/framework/data/adonet/dataadapter-parameters.md).  
...
```  
 ...
 Параметр может сопоставляться `date`с типом данных `datetime`, или `datetime2` на сервере. `@date` При работе с новыми типами данных `datetime` необходимо явным образом присвоить свойству <xref:System.Data.SqlDbType> параметра тип данных экземпляра. Использование <xref:System.Data.SqlDbType.Variant> или неявных значений параметра может привести к проблемам с обратной совместимостью для типов данных `datetime` и `smalldatetime`.  
