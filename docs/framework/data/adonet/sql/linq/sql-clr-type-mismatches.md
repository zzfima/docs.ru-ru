---
title: "Несоответствия типов SQL-CLR | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
-   Несоответствия в пространстве имен CLR <xref:System>.  
    ...
    -   **TimeSpan**.  Этот тип представляет различие между двумя значениями `DateTime` и не соответствует типу `timestamp` SQL Server.  В некоторых случаях тип CLR <xref:System.Timespan?displayProperty=fullName> может также сопоставляться с типом `TIME` SQL Server.  Назначение типа SQL Server `TIME` состоит только в том, чтобы представлять положительные значения менее суток.  Диапазон типа CLR <xref:System.Timespan> значительно шире.  
  
    > [!NOTE]
    >  В данное сопоставление не включаются связанные с SQL Server типы платформы [!INCLUDE[dnprdnshort](../../../../../../includes/dnprdnshort-md.md)] в <xref:System.Data.SqlTypes>.  
  
-   Несоответствия в SQL Server.  
  
    -   **Символьные типы фиксированной длины**.  В языке Transact\-SQL категории, представленные в Юникоде и в кодировке, отличной от Юникода, являются различными. При этом в каждой категории выделяются три различных типа: типы фиксированной длины `nchar`\/`char`, типы переменной длины `nvarchar`\/`varchar` и типы больших размеров `ntext`\/`text`.  Символьные типы фиксированной длины могут быть сопоставлены с типом CLR <xref:System.Char?displayProperty=fullName> для получения символов, однако они не совсем соответствуют такому же типу в преобразованиях и поведении.  
  
    -   **Bit**.  Хотя домен `bit` имеет то же число значений, что и `Nullable<Boolean>`, это два различных типа.  Тип `Bit` принимает значения `1` и `0` \(а не `true` и `false`\) и не может быть использован в качестве эквивалента логических выражений.  
  
    -   **Отметка времени**.  В отличие от типа CLR <xref:System.Timespan?displayProperty=fullName> тип SQL Server `TIMESTAMP` представляет созданное базой данных 8\-разрядное число, уникальное для каждого обновления и не основанное на различии между значениями <xref:System.DateTime>.  
