...
|Реализация соединений|`Join` Используетпредложение/ `join`|`Join` Может использовать / <xref:System.Data.Linq.Mapping.AssociationAttribute> предложение, но более эффективно использует атрибут. `join` Дополнительные сведения см. в разделе [запросы по связям](querying-across-relationships.md).|  
|Удаленное или локальное выполнение||Дополнительные сведения см. в [разделе Удаленная и Локальное](remote-vs-local-execution.md)выполнение.|  
