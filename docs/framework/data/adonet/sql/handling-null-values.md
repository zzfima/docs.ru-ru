...
### <a name="understanding-the-ansinulls-option"></a>Основные сведения о параметре ANSI_NULLS  
 <xref:System.Data.SqlTypes> предоставляет ту же семантику, что и в случае, когда параметр ANSI_NULLS установлен в SQL Server. Все арифметические операторы (+, -, *, /, %), битовые операторы (~ &, &#124;), и большинство функций возвращают null, если любой из операндов или аргументов равны null, за исключением свойства `IsNull`.  
...
1.  Значение по умолчанию *по умолчанию* значение `DbNull.Value` для всех, за исключением строго типизированных нулевых столбцов, там, где это соответствующее строго типизированное значение null.  
