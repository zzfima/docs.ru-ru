---
title: "Поддержка SqlClient для высокого уровня доступности, аварийного восстановления | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
-   Используемый тип проверки подлинности \([!INCLUDE[ssNoVersion](../../../../../includes/ssnoversion-md.md)], Kerberos или Windows\) не влияет на работу приложения, использующего свойство соединения `MultiSubnetFailover`.  
...
## Переход от зеркального отображения базы данных к использованию кластеров с множеством подсетей  
 Если в строке подключения присутствуют ключевые слова `MultiSubnetFailover` и `Failover Partner` или если используется `MultiSubnetFailover=True` с любым другим протоколом, кроме TCP, то попытка подключения вернет ошибку \(<xref:System.ArgumentException>\).  Ошибка \(<xref:System.Data.SqlClient.SqlException>\) также возникает в том случае, если используется `MultiSubnetFailover` и [!INCLUDE[ssNoVersion](../../../../../includes/ssnoversion-md.md)] возвращает ответ партнера по обеспечению отработки отказа, указывающий на его участие в паре зеркального отображения базы данных.  
...
2.  Необходимо присвоить значение `ReadOnly` ключевому слову `ApplicationIntent` строки подключения.  
