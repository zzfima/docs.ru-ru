...
## <a name="statistical-values-available"></a>Доступные статистические значения  
 На данный момент существует 18 различных элементов, доступ к которым предоставляется поставщиком Microsoft SQL Server. Доступное число доступных элементов можно получить с помощью <xref:System.Collections.IDictionary> свойства **Count** ссылки на интерфейс, возвращаемой методом <xref:System.Data.SqlClient.SqlConnection.RetrieveStatistics%2A>. Все счетчики для статистики поставщика используют тип среды <xref:System.Int64> CLR (**Long** в и Visual Basic) C# , что составляет 64 бит в ширину. Максимальное значение типа данных **Int64** в соответствии с определением **Int64. Поле MaxValue** , имеет ((2 ^ 63)-1)). После достижения значениями счетчиков этого максимального значения их больше нельзя считать точными. Это означает, что **Int64. Значение MaxValue**-1 ((2 ^ 63)-2) фактически является максимальным допустимым значением для статистики.  
