...
> [!NOTE]
>  Использование базы данных Access (Jet) в качестве источника данных для многопоточных приложений, таких как ASP.NET, не рекомендуется. При Jet необходимо использовать в качестве источника данных для приложения ASP.NET, учтите, что ASP.NET-приложений, подключающихся к базе данных Access можно столкнуться с проблемами соединения.  
 ...
 Поставщик данных .NET Framework для OLE DB не работает с поставщиком OLE DB для ODBC (MSDASQL). Для доступа к источнику данных ODBC с помощью ADO.NET, используйте поставщик данных .NET Framework для ODBC.  
