...
> [!CAUTION]
> Настоятельно рекомендуется всегда закрывать соединение после его использования, чтобы оно вернулось в пул. Это `Close` можно сделать с помощью методов `Connection` или объекта `Dispose` или путем открытия всех C# `Using` соединений внутри `using` инструкции в или инструкции в Visual Basic. Соединения, которые явно не закрыты, нельзя добавить или вернуть в пул. Дополнительные сведения см. в разделе [оператор using](../../../csharp/language-reference/keywords/using-statement.md) или [инструкции. Удаление системного ресурса](../../../visual-basic/programming-guide/language-features/control-flow/how-to-dispose-of-a-system-resource.md) для Visual Basic.  
