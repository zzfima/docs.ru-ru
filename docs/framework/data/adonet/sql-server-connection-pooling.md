...
> [!CAUTION]
> Настоятельно рекомендуется всегда закрывать соединение после его использования, чтобы оно вернулось в пул. Это можно сделать `Close` с `Dispose` помощью `Connection` либо методов объекта, либо, открыв все соединения внутри `using` оператора в C- или `Using` оператора в Visual Basic. Соединения, которые явно не закрыты, нельзя добавить или вернуть в пул. Для получения дополнительной информации [How to: Dispose of a System Resource](../../../visual-basic/programming-guide/language-features/control-flow/how-to-dispose-of-a-system-resource.md) [см.](../../../csharp/language-reference/keywords/using-statement.md)  
...
Для получения дополнительной информации о событиях, связанных с [Audit Logout Event Class](/sql/relational-databases/event-classes/audit-logout-event-class) открытием и закрытием соединений, [см.](/sql/relational-databases/event-classes/audit-login-event-class)  
...
## <a name="clearing-the-pool"></a>Очистка пула  
 ADO.NET 2.0 ввели два новых метода очистки бассейна: <xref:System.Data.SqlClient.SqlConnection.ClearAllPools%2A> и <xref:System.Data.SqlClient.SqlConnection.ClearPool%2A>. Метод `ClearAllPools` очищает пулы соединений данного поставщика, а метод `ClearPool` - пул, связанный с конкретным соединением. Если во время вызова методов соединения использовались, они соответствующим образом помечаются. При их закрытии вместо возвращения в пул они удаляются.  
