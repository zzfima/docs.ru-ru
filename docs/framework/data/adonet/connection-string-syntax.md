...
## <a name="windows-authentication"></a>Проверка подлинности Windows  
 Рекомендуется использовать проверку подлинности Windows (иногда называется *встроенная система безопасности*) для подключения к источникам данных, которые ее поддерживают. Синтаксис строки подключения зависит от поставщика. В следующей таблице показан синтаксис проверки подлинности Windows, который используется с поставщиками данных платформы .NET Framework.  
...
> [!IMPORTANT]
>  По умолчанию `Persist Security Info` ключевое слово `false`. Значение `true` или `yes` позволяет получить из строки соединения конфиденциальные данные (в том числе идентификатор пользователя и пароль) после открытия соединения. Сохранить `Persist Security Info` присвоено `false` для то ненадежный источник не сможет получить доступ к конфиденциальным данным строки подключения.  
...
Когда подключение к базе данных SQL Azure или хранилище данных SQL Azure и предоставить имя входа в формате `user@servername`, убедитесь, что `servername` значение в имени входа соответствует значению, указанному в `Server=`.
...
Чтобы подключиться к именованному экземпляру SQL Server, используйте *имя_сервера\имя_экземпляра* синтаксис.  
...
|Нет|Н/Д|Да|Да|Шифрование производится всегда, однако может использовать самозаверяющий сертификат.|  
|Да|Нет|Не учитывается|Не учитывается|Шифрование выполняется только при наличии подтверждаемого сертификата сервера; в противном случае попытка соединения завершается.|  
|Да|Да|Нет (по умолчанию)|Не учитывается|Шифрование производится всегда, однако может использовать самозаверяющий сертификат.|  
|Да|Да|Да|Нет (по умолчанию)|Шифрование выполняется только при наличии подтверждаемого сертификата сервера; в противном случае попытка соединения завершается.|  
|Да|Да|Да|Да|Шифрование производится всегда, однако может использовать самозаверяющий сертификат.|  
