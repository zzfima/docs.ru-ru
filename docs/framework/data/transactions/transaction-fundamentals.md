---
title: "Основные сведения о транзакциях  | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
Транзакции позволяют объединить несколько задач в один логический блок.Например, пусть приложение выполняет две задачи.Сначала оно создает новую таблицу в базе данных.Затем оно вызывает специализированный объект для сбора, форматирования и вставки данных в новую таблицу.Эти две задачи являются связанными и даже взаимозависимыми: создание новой таблицы требуется, только если ее можно заполнить данными.Выполнение обеих задач в пределах области единой транзакции приводит к образованию связи между ними.Если выполнение второй задачи заканчивается сбоем, выполняется откат первой задачи в состояние до момента создания новой таблицы.  
 ...
 При создании транзакции можно указать уровень изоляции, применяемый к этой транзакции.Уровень изоляции, определенный классом <xref:System.Transactions.IsolationLevel>, задает уровень доступа других транзакций к данным, на которые воздействует транзакция пользователя.  
  
 Транзакции можно создавать с помощью ADO.NET, <xref:System.EnterpriseServices> или новой модели программирования транзакций, предоставляемой пространством имен <xref:System.Transactions>.В разделе [Функциональные возможности, предоставляемые пространством имен System.Transactions ](../../../../docs/framework/data/transactions/features-provided-by-system-transactions.md) рассматриваются функции, которые можно использовать для создания транзакционного приложения с помощью пространства имен <xref:System.Transactions>.  
