...
Каждым используемым в транзакции ресурсом управляет диспетчер ресурсов, действия которого координируются диспетчером транзакций. [Прикрепление ресурсов как участников в разделе транзакции](enlisting-resources-as-participants-in-a-transaction.md) описывает, как ресурс (или несколько ресурсов) можно прикрепить к транзакции. В этом разделе также описывается порядок координации процесса фиксации транзакции между зачисленными ресурсами.  
 ...
 Если транзакция включает несколько ресурсов, необходимо выполнить двухфазную фиксацию (2PC). Протокол двухфазной фиксации (фаза подготовки и фаза фиксации) гарантирует, что при завершении транзакции все изменения, произведенные над всеми ресурсами, либо полностью фиксируются, либо полностью откатываются. После завершения транзакции результат сообщается всем участникам. Подробное описание протокола двухфазной фиксации см. в книге "*обработка транзакций: Основные понятия и методики (серия Morgan Кауфманн в управление данными Systems) ISBN*: 1558601902 ", Джим серый.  
  
 Кроме того, оптимизировать производительность транзакции можно путем участия в протоколе однофазной фиксации. Дополнительные сведения см. в статье [Оптимизация с использованием однофазных фиксаций и уведомления с одним этапом продвижения](optimization-spc-and-promotable-spn.md).  
...
## <a name="single-phase-commit-optimization"></a>Оптимизация однофазной фиксации  
 Протокол однофазной фиксации наиболее эффективен при использовании во время выполнения, поскольку все изменения производятся без какой-либо явной координации. Дополнительные сведения об этом протоколе см. в статье [Оптимизация с использованием однофазных фиксаций и уведомления с одним этапом продвижения](optimization-spc-and-promotable-spn.md).  
