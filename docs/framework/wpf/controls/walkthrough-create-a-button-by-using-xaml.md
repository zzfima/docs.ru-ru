...
2.  **Создайте новый проект WPF:** на **файл** последовательно выберите пункты **New**, а затем нажмите кнопку **проекта**. Найти **приложения Windows (WPF)** шаблон и имя проекта «AnimatedButton». Будет создан каркас для приложения.  
...
1.  **Определение блока Application.Resources:** откройте файл app.xaml и добавьте выделенную ниже разметку в том случае, если он не существует:  
...
3.  **Добавление прозрачности в шаблон:** Далее вы добавите прозрачного стекла. Сначала создайте ресурсы, которые создают эффект градиента прозрачности. Добавьте эти градиентные ресурсы в любом месте в пределах `Application.Resources` блок:  
...
## <a name="create-button-interactivity"></a>Создать интерактивную кнопку  
 ...
 Простой способ добавления интерактивности (Наведение указателя мыши, оставьте мыши, нажмите кнопку и т. д.) является определение триггеров в шаблоне или стиле. Для создания <xref:System.Windows.Trigger>, вы определяете свойство «условие», например: кнопки <xref:System.Windows.UIElement.IsMouseOver%2A> значение свойства равно `true`. Затем предстоит методов задания (действия), которые выполняются, если условие триггера истинно.  
...
2.  **Добавление триггеров свойств:** добавьте выделенную разметку для `ControlTemplate.Triggers` блок:  
...
5.  **Добавление анимации для при нажатии кнопки:** последним шагом является добавление триггера для при нажатии кнопки. Добавьте следующую разметку в любое место `ControlTemplate.Triggers` блок:  
