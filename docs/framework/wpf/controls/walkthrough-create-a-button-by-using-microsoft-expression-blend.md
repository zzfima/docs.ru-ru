...
1. **Задайте для свойства фон окна значение черный:** Выберите окно, перейдите на **вкладку Свойства**и задайте <xref:System.Windows.Controls.Control.Background%2A> для `Black`свойства значение.  
     ...
     ![Как задать черный фон кнопки](./media/custom-button-blend-changebackground.png "custom_button_blend_ChangeBackground")  
...
3. **Округлить углы прямоугольника:** Либо перетащите контрольные точки прямоугольника, либо напрямую задайте <xref:System.Windows.Shapes.Rectangle.RadiusX%2A> свойства и. <xref:System.Windows.Shapes.Rectangle.RadiusY%2A> Установите значения <xref:System.Windows.Shapes.Rectangle.RadiusX%2A> и <xref:System.Windows.Shapes.Rectangle.RadiusY%2A> равными 20.  
     ...
     ![Как сделать углы прямоугольника] в круглом (./media/custom-button-blend-roundcorners.png "custom_button_blend_RoundCorners")  
  
4. **Замените прямоугольник на кнопку:** Выберите прямоугольник. В меню **Сервис** щелкните **создать**кнопку.  
  
     ![Создание фигуры в кнопке](./media/custom-button-blend-makebutton.png "custom_button_blend_MakeButton")  
...
3. **Измените прямоугольник таким образом, чтобы он был пустым внутри (например, кольцо):** Выберите **аутерректангле** и задайте <xref:System.Windows.Shapes.Shape.Fill%2A> для параметра значение прозрачный и <xref:System.Windows.Shapes.Shape.StrokeThickness%2A> 5.  
     ...
     ![Как сделать прямоугольник пустым](./media/custom-button-blend-changerectproperties.png "custom_button_blend_ChangeRectProperties")  
     ...
     ![Настройка использования цвета шаблона](./media/custom-button-blend-templatestroke.png "custom_button_blend_TemplateStroke")  
...
4. **Создайте внутренний прямоугольник:** Теперь создайте другой прямоугольник (назовите его "Иннерректангле") и разместите его симметрично внутри **аутерректангле** . Для такого рода работы вам, вероятно, потребуется увеличить масштаб, чтобы кнопка стала больше в области редактирования.  
    ...
    > Прямоугольник может выглядеть иначе, чем на рисунке (например, у него могут быть скругленные углы).  
     ...
     ![Создание прямоугольника внутри другого прямоугольника](./media/custom-button-blend-innerrectangleproperties.png "custom_button_blend_innerRectangleProperties")  
...
5. **Переместить ContentPresenter в начало:** На этом этапе текст "Кнопка" может не отображаться. Если это так, это связано с тем, что **иннерректангле** находится поверх **миконтентпресентер**. Чтобы устранить эту проблему, перетащите **миконтентпресентер** под **иннерректангле**. Переместите прямоугольники и **миконтентпресентер** , как показано ниже.  
    ...
    > Кроме того, можно также разместить **миконтентпресентер** в верхней части окна, щелкнув его правой кнопкой мыши и выбрав команду **отправить вперед**.  
     ...
     ![Перемещение одной кнопки поверх другой кнопки](./media/custom-button-blend-innerrectangle2.png "custom_button_blend_innerRectangle2")  
...
6. **Измените вид Иннерректангле:** Задайте значения <xref:System.Windows.Shapes.Rectangle.RadiusX%2A>, <xref:System.Windows.Shapes.Rectangle.RadiusY%2A> и<xref:System.Windows.Shapes.Shape.StrokeThickness%2A> равными 20. Кроме того, задайте для <xref:System.Windows.Shapes.Shape.Fill%2A> свойства значение фон шаблона, используя пользовательское выражение "{TemplateBinding фона}") и задайте <xref:System.Windows.Shapes.Shape.Stroke%2A> для параметра значение "прозрачный". Обратите внимание, что параметры <xref:System.Windows.Shapes.Shape.Fill%2A> для <xref:System.Windows.Shapes.Shape.Stroke%2A> и для **иннерректангле** являются противоположными для **аутерректангле**.  
     ...
     ![Изменение внешнего вида прямоугольника](./media/custom-button-blend-glassrectangleproperties1.png "custom_button_blend_glassRectangleProperties1")  
...
9. **Присвойте Гласскубе немного другой форме, чем Аутерректангле:** Измените свойства **гласскубе**. Начните с изменения <xref:System.Windows.Shapes.Rectangle.RadiusX%2A> <xref:System.Windows.Shapes.Rectangle.RadiusY%2A>свойстви на 10 и до2.<xref:System.Windows.Shapes.Shape.StrokeThickness%2A>  
     ...
     ![Параметры внешнего вида для гласскубе](./media/custom-button-blend-glasscubeappearance.gif "custom_button_blend_GlassCubeAppearance")  
...
10. **Сделать Гласскубе похожим на стекло:** Задайте для <xref:System.Windows.Shapes.Shape.Fill%2A> параметра значение прозрачность, используя линейный градиент, который равен 75% непрозрачен, и варианты между белым цветом и прозрачными для 6 приблизительно равными интервалами. Вот что нужно сделать для ограничителей градиента:  
    ...
    - Ограничитель градиента 6: Прозрачный  
     ...
     ![Прямоугольник, выглядящий как стекло](./media/custom-button-blend-glassrectangleproperties2.png "custom_button_blend_glassRectangleProperties2")  
...
2. **Сделайте свойство, используемое триггером:** Измените значение свойства на <xref:System.Windows.UIElement.IsMouseOver%2A>. Это приводит к активации триггера свойства, <xref:System.Windows.UIElement.IsMouseOver%2A> если свойство `true` имеет значение (когда пользователь наводит указатель мыши на кнопку).  
     ...
     ![Задание триггера для свойства](./media/custom-button-blend-ismousedoverpropertytrigger.png "custom_button_blend_IsMousedOverPropertyTrigger")  
...
3. **Параметр onmouseover активирует непрозрачность 100% для Гласскубе:** Обратите внимание, что **запись триггера включена** (см. предыдущий рисунок). Это означает, что любые изменения, вносимые в значения свойств **гласскубе** при записи, будут иметь действие, которое выполняется, когда <xref:System.Windows.UIElement.IsMouseOver%2A> имеет значение. `true` Во время записи измените значение <xref:System.Windows.UIElement.Opacity%2A> параметра **гласскубе** на 100%.  
     ...
     ![Как задать непрозрачность кнопки](./media/custom-button-blend-ismousedoverpropertytrigger2.gif "custom_button_blend_IsMousedOverPropertyTrigger2")  
...
4. **Параметр onmouseover инициирует изменение значения Stroke:** Давайте связываем некоторые другие действия с <xref:System.Windows.UIElement.IsMouseOver%2A> триггером. Пока запись продолжится, переключите выбранный вариант с **гласскубе** на **аутерректангле**. Затем задайте <xref:System.Windows.Shapes.Shape.Stroke%2A> для **аутерректангле** пользовательское выражение "{DynamicResource {x:Static SystemColors. хигхлигхтбрушкэй}}". Этот параметр задает <xref:System.Windows.Shapes.Shape.Stroke%2A> в качестве стандартного цвета выделения, используемого кнопками. Нажмите клавишу F5, чтобы увидеть результат при наведении указателя мыши на кнопку.  
     ...
     ![Как установить для штриха цвет выделения](./media/custom-button-blend-ismousedoverpropertytrigger3.png "custom_button_blend_IsMousedOverPropertyTrigger3")  
...
1. **Создайте триггер события MouseEnter:** Добавьте новый триггер события и выберите <xref:System.Windows.UIElement.MouseEnter> его в качестве события, которое будет использоваться в триггере.  
     ...
     ![Создание триггера событий MouseEnter](./media/custom-button-blend-mouseovereventtrigger.png "custom_button_blend_MouseOverEventTrigger")  
  
2. **Создать шкалу анимации:** Затем свяжите шкалу времени анимации с <xref:System.Windows.UIElement.MouseEnter> событием.  
  
     ![Добавление временной шкалы анимации в событие](./media/custom-button-blend-mouseovereventtrigger2.png "custom_button_blend_MouseOverEventTrigger2")  
    ...
    > Возможно, потребуется изменить размер окна и (или) панелей, чтобы увидеть отображение.  
     ...
     ![Панель временной шкалы](./media/custom-button-blend-mouseovereventtrigger3.png "custom_button_blend_MouseOverEventTrigger3")  
...
4. **Сжать Гласскубе в этом опорном кадре:** Выбрав второй ключевой кадр, уменьшите размер **гласскубе** до 90% от его полного размера с помощью **преобразования размера**.  
     ...
     ![Уменьшение размера кнопки](./media/custom-button-blend-sizetransform.png "custom_button_blend_SizeTransform")  
...
5. **Создайте другой триггер события и свяжите с ним другую анимацию:** Давайте добавим еще одну анимацию. Используйте аналогичную процедуру, которая использовалась для создания предыдущей анимации триггера событий.  
    ...
    2. Свяжите новую временную шкалу <xref:System.Windows.Controls.Primitives.ButtonBase.Click> с событием.  
     ...
     ![Создание новой временной шкалы](./media/custom-button-blend-clickeventtrigger1.png "custom_button_blend_ClickEventTrigger1")  
  
    1. Для этой временной шкалы создайте два опорных кадра, один из которых составляет 0,0 секунд, а второй — 0,3 секунд.  
  
    2. Выделив опорный кадр в 0,3 секунд, установите для параметра **угол поворота** значение 360 градусов.  
  
     ![Создание преобразования «Вращение] » (./media/custom-button-blend-rotatetransform.gif "custom_button_blend_RotateTransform")  
  
    1. Нажмите клавишу F5 для запуска приложения. Нажмите кнопку. Обратите внимание, что прозрачный слой вращается вокруг.  
...
## <a name="conclusion"></a>Заключение  
 ...
 ![Несколько кнопок, использующих один и тот же шаблон](./media/custom-button-blend-createmultiplebuttons.png "custom_button_blend_CreateMultipleButtons")  
