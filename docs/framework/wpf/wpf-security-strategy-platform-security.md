---
title: "Стратегия безопасности WPF — безопасность платформы | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "функции безопасности среды CLR"
  - "среда CLR - функции безопасности"
  - "Internet Explorer - функции безопасности"
  - "модель безопасности операционной системы"
  - "модель безопасности платформы"
  - "модель безопасности"
  - "модель безопасности, операционная система"
  - "модель безопасности, платформа"
  - "Метод критической безопасности"
  - "Windows Presentation Foundation (WPF), сведения о модели безопасности"
  - "WPF, сведения о модели безопасности"
...
Хотя [!INCLUDE[TLA#tla_wpf](../../../includes/tlasharptla-wpf-md.md)] предоставляет множество служб безопасности, предусмотрена также возможность использования функций безопасности базовой платформы, включая операционную систему, [!INCLUDE[TLA2#tla_clr](../../../includes/tla2sharptla-clr-md.md)] и [!INCLUDE[TLA2#tla_ie](../../../includes/tla2sharptla-ie-md.md)].  Эти уровни объединяются для обеспечения в [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)] надежной модели безопасности с глубокой защитой, которая пытается исключить любые точки сбоя, как показано на следующем рисунке.  
 ...
 ![Иллюстрация безопасности WPF](../../../docs/framework/wpf/media/windowplatformsecurity.PNG "windowplatformsecurity")  
...
#### Уровень глубокой защиты  
 ...
 См. статью [Использование учетной записи пользователя с минимальными привилегиями](http://technet.microsoft.com/library/cc700846.aspx).  
...
### Проверка и верификация  
 ...
 Чтобы обеспечить безопасность типов, помочь предотвратить общие проблемы безопасности \(например,  переполнение буфера\) и включить изолированную среду путем изоляции подпроцесса, безопасность [!INCLUDE[TLA2#tla_clr](../../../includes/tla2sharptla-clr-md.md)] использует концепцию верификации.  
...
-   **Интернет**.  Для приложений, запускаемых из зоны **Интернет** или **Доверенные веб\-сайты**.  Подмножество разрешений предоставляется для обеспечения ограниченного доступа к ресурсам на клиентском компьютере, включая изолированное хранилище, только открытие файлов и ограниченный пользовательский интерфейс.  В сущности, эти наборы разрешений изолируют приложения от клиентского компьютера.  
  
 Приложениям, которые идентифицированы как происходящие из зоны **Веб\-сайты без доверия**, [!INCLUDE[TLA2#tla_cas](../../../includes/tla2sharptla-cas-md.md)] не предоставляет разрешений вообще.  Следовательно, для них не существует предопределенный набор разрешений.  
  
 На следующем рисунке показано отношение между зонами, наборами разрешений, разрешениями и ресурсами.  
  
 ![наборы разрешений управления доступом для кода](../../../docs/framework/wpf/media/caspermissionsets.png "CASPermissionSets")  
...
-   загрузку шрифт Tahoma.  
 ...
 [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)] выполняет это с помощью метода **Assert** разрешения.  В следующем примере кода показано, как это происходит.  
  
 [!code-csharp[WPFPlatformSecuritySnippets#Permission](../../../samples/snippets/csharp/VS_Snippets_Wpf/WPFPlatformSecuritySnippets/CSharp/Page1.xaml.cs#permission)]
 [!code-vb[WPFPlatformSecuritySnippets#Permission](../../../samples/snippets/visualbasic/VS_Snippets_Wpf/WPFPlatformSecuritySnippets/VisualBasic/Page1.xaml.vb#permission)]  
  
 Метод **Assert**, в сущности, предотвращает ограничение неограниченных разрешений, необходимых [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)], разрешениями зоны Интернета [!INCLUDE[TLA2#tla_winfxwebapp](../../../includes/tla2sharptla-winfxwebapp-md.md)].  
  
 С точки зрения платформы [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)] несет ответственность за правильное использование **Assert**; неправильное использование **Assert** может привести к тому, что вредоносный код получит повышенные привилегии.  Следовательно, важно вызывать **Assert** только при необходимости и обеспечить сохранение ограничений песочницы.  Например, изолированному коду не разрешается открывать случайные файлы, но разрешается использовать шрифты.  [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)] позволяет изолированным приложениям использовать функциональность шрифтов путем вызова **Assert**, а также для чтения [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)] файлов, которые заведомо содержат эти шрифты, от имени изолированного приложения.  
  
<a name="ClickOnce_Deployment"></a>   
### Развертывание ClickOnce  
 [!INCLUDE[TLA#tla_clickonce](../../../includes/tlasharptla-clickonce-md.md)] — это комплексная технология развертывания, которая входит в состав [!INCLUDE[TLA2#tla_winfx](../../../includes/tla2sharptla-winfx-md.md)] и интегрируется с [!INCLUDE[TLA#tla_visualstu](../../../includes/tlasharptla-visualstu-md.md)] \(подробные сведения см. в разделе [Обзор развертывания ClickOnce](http://msdn.microsoft.com/library/142dbbz4.aspx)\).  Автономные приложения [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)] могут развертываться с помощью [!INCLUDE[TLA#tla_clickonce](../../../includes/tlasharptla-clickonce-md.md)], тогда как браузерные приложения необходимо развертывать с помощью [!INCLUDE[TLA2#tla_clickonce](../../../includes/tla2sharptla-clickonce-md.md)].  
...
### Методология, критическая с точки зрения безопасности  
 В отношении кода [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)], который использует разрешения для включения песочницы зоны Интернета для приложений [!INCLUDE[TLA2#tla_winfxwebapp](../../../includes/tla2sharptla-winfxwebapp-md.md)], должна поддерживаться наивысшая возможная степень аудита и контроля безопасности.  Для облегчения этого требования [!INCLUDE[TLA2#tla_winfx](../../../includes/tla2sharptla-winfx-md.md)] обеспечивает новую поддержку для управляемого кода, который повышает привилегии.  В частности, [!INCLUDE[TLA2#tla_clr](../../../includes/tla2sharptla-clr-md.md)] позволяет определить код, который повышает привилегии, и пометить его атрибутом <xref:System.Security.SecurityCriticalAttribute>; любой код, не помеченный <xref:System.Security.SecurityCriticalAttribute>, становится *прозрачным* с помощью этой методологии.  В свою очередь, управляемому коду, который не помечен атрибутом <xref:System.Security.SecurityCriticalAttribute>, запрещено повышать привилегии.  
  
 Методология, критическая с точки зрения безопасности, позволяет организацию кода [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)], который повышает привилегии в *ядре, критическом с точки зрения безопасности*, а остальное остается прозрачным.  Изоляция кода, критического с точки зрения безопасности, позволяет группе разработчиков [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)] сосредоточиться на дополнительном анализе безопасности и управлении исходным кодом в ядре, критическом с точки зрения безопасности, помимо стандартных методов безопасности \(см. раздел [Стратегия безопасности WPF — проектирование безопасности](../../../docs/framework/wpf/wpf-security-strategy-security-engineering.md)\).  
...
## См. также  
 ...
 [Общие сведения и работа в защищенном режиме Internet Explorer](http://msdn.microsoft.com/library/bb250462.aspx)   
