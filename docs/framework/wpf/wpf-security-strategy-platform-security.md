...
#### <a name="defense-in-depth-layer"></a>Уровень глубокой защиты  
 ...
 В разделе [под учетной записью пользователя с минимальными](http://technet.microsoft.com/library/cc700846.aspx).  
...
### <a name="validation-and-verification"></a>Проверка и верификация  
 ...
 Чтобы обеспечить безопасность типов, помочь предотвратить общие проблемы безопасности (например, переполнение буфера) и включить выполнение в изолированной среде путем изоляции подпроцесса [!INCLUDE[TLA2#tla_clr](../../../includes/tla2sharptla-clr-md.md)] безопасности использует концепцию проверки.  
...
-   не имеет бесконтрольный доступ к памяти.  
 ...
 Преимущества верифицируемого кода являются главной причиной того, что [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)] строится на [!INCLUDE[TLA2#tla_winfx](../../../includes/tla2sharptla-winfx-md.md)]. При условии, что используется верифицируемый код, вероятность использования возможных уязвимостей значительно ниже.  
  
<a name="Code_Access_Security"></a>   
### <a name="code-access-security"></a>Управление доступом для кода  
 Клиентский компьютер предоставляет широкий спектр ресурсов, к которым может получить доступ управляемое приложение, включая файловую систему, реестр, службы печати, пользовательский интерфейс, отражение и переменные среды. Управляемого приложения можно получить доступ к любому из ресурсов на клиентском компьютере, он должен иметь [!INCLUDE[TLA2#tla_winfx](../../../includes/tla2sharptla-winfx-md.md)] [!INCLUDE[TLA#tla_cas](../../../includes/tlasharptla-cas-md.md)] соответствующее разрешение. Разрешение в [!INCLUDE[TLA2#tla_cas](../../../includes/tla2sharptla-cas-md.md)] является подклассом <xref:System.Security.CodeAccessPermission>; [!INCLUDE[TLA2#tla_cas](../../../includes/tla2sharptla-cas-md.md)] реализует по одному подклассу для каждого ресурса, к которому управляемые приложения могут получать доступ.  
...
-   **Интернет**. Для приложений, запускаемых из **Internet** или **надежных узлов** зоны. Подмножество разрешений предоставляется для обеспечения ограниченного доступа к ресурсам на клиентском компьютере, включая изолированное хранилище, только открытие файлов и ограниченный пользовательский интерфейс. В сущности, эти наборы разрешений изолируют приложения от клиентского компьютера.  
  
 Приложения, как из **ненадежных веб-сайтов** зоны не предоставляется разрешений с [!INCLUDE[TLA2#tla_cas](../../../includes/tla2sharptla-cas-md.md)] вообще. Следовательно, для них не существует предопределенный набор разрешений.  
...
### <a name="security-critical-methodology"></a>Методология, критическая с точки зрения безопасности  
 В отношении кода [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)], который использует разрешения для включения песочницы зоны Интернета для приложений [!INCLUDE[TLA2#tla_winfxwebapp](../../../includes/tla2sharptla-winfxwebapp-md.md)], должна поддерживаться наивысшая возможная степень аудита и контроля безопасности. Для облегчения этого требования [!INCLUDE[TLA2#tla_winfx](../../../includes/tla2sharptla-winfx-md.md)] обеспечивает новую поддержку для управляемого кода, который повышает привилегии. В частности [!INCLUDE[TLA2#tla_clr](../../../includes/tla2sharptla-clr-md.md)] позволяет определить код, который повышает права и пометьте его атрибутом <xref:System.Security.SecurityCriticalAttribute>; любой код, не помеченный <xref:System.Security.SecurityCriticalAttribute> становится *прозрачный* с помощью этой методики. В свою очередь, управляемому коду, который не помечен атрибутом <xref:System.Security.SecurityCriticalAttribute>, запрещено повышать привилегии.  
  
 Методология критической безопасности позволяет организацию [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)] код, который повышает права доступа в *критическим с точки зрения безопасности ядра*, а остальные будут прозрачными. Изоляция критическим с точки зрения безопасности кода включает [!INCLUDE[TLA2#tla_wpf](../../../includes/tla2sharptla-wpf-md.md)] группе разработчиков сосредоточиться дополнительный анализ и источник контроль безопасности на ядре критическим с точки зрения безопасности, помимо стандартных методов безопасности (см. [стратегия безопасности WPF -Проектирование безопасности](../../../docs/framework/wpf/wpf-security-strategy-security-engineering.md)).  
  
 Обратите внимание, что [!INCLUDE[TLA2#tla_winfx](../../../includes/tla2sharptla-winfx-md.md)] позволяет доверенному коду расширять песочницу зоны Интернета [!INCLUDE[TLA2#tla_winfxwebapp](../../../includes/tla2sharptla-winfxwebapp-md.md)], разрешая разработчикам создавать управляемые сборки, которые помечены атрибутом <xref:System.Security.AllowPartiallyTrustedCallersAttribute> (APTCA) и развертываются в глобальном кэше сборок пользователя. Пометка сборки атрибутом APTCA является исключительно важной операцией безопасности, так как позволяет вызывать эту сборку любому коду, включая вредоносный код из Интернета. При этом необходимо использовать особую осторожность и рекомендации, и пользователи должны указывать, что доверяют этому программному обеспечению, чтобы установить его.  
...
## <a name="see-also"></a>См. также  
 [Общие сведения о безопасности в Microsoft Internet Explorer 6 в Windows XP SP2](http://www.microsoft.com/downloads/details.aspx?FamilyId=E550F940-37A0-4541-B5E2-704AB386C3ED&displaylang=en)  
 [Общие сведения о и работа в защищенный режим Internet Explorer](http://msdn.microsoft.com/library/bb250462.aspx)  
 [Windows XP с пакетом обновления 3](http://www.microsoft.com/windows/products/windowsxp/sp3/default.mspx)  
 [Руководство по безопасности Windows Vista](http://www.microsoft.com/downloads/details.aspx?familyid=a3d1bbed-7f35-4e72-bfb5-b84a526c1565&displaylang=en)  
