...
> Использование типов WPF в службе Windows настоятельно не рекомендуется. При попытке использовать эти возможности в службе Windows они могут не работать должным образом.  
 ...
 Для создания этого набора приложений в WPF реализован узел служб. Этот раздел предоставляет обзор этих служб и место, где искать дополнительные сведения.  

<a name="Application_Management"></a>   
## <a name="application-management"></a>Управление приложениями  
 Для исполняемых приложений WPF обычно требуется основной набор функций, включающий следующее:  
...
## <a name="windows-and-dialog-boxes"></a>Окна и диалоговые окна  
 Пользователи взаимодействуют с автономными приложениями WPF через Windows. Предназначением окна является размещение содержимого приложения и предоставление функциональных возможностей приложения, которые обычно позволяют пользователям взаимодействовать с содержимым. В WPF эти окна инкапсулированы классом <xref:System.Windows.Window>, который поддерживает:  
...
- отслеживание и взаимодействие со временем существования окна.  
 ...
 Для удобства и преимуществ повторного использования и согласованного взаимодействия с пользователем в приложениях WPF предоставляет три общих диалоговых окна Windows: <xref:Microsoft.Win32.OpenFileDialog>, <xref:Microsoft.Win32.SaveFileDialog>и <xref:System.Windows.Controls.PrintDialog>.  
...
## <a name="navigation"></a>Навигация в  
 WPF поддерживает навигацию в стиле веб-страниц, используя страницы (<xref:System.Windows.Controls.Page>) и гиперссылки (<xref:System.Windows.Documents.Hyperlink>). Навигация может быть реализована разнообразными способами, включая следующие:  
...
- Страницы, размещенные на кадре (<xref:System.Windows.Controls.Frame>), которые могут размещаться на автономной странице, или на странице, скомпилированной в XBAP или в автономное приложение.  
 ...
 Для упрощения навигации в WPF реализованы следующие возможности:  
