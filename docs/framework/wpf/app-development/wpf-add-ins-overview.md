...
В приведенных примерах ведущее приложение было установленным автономным приложением. Но приложения браузера XAML (XBAP) могут также размещать надстройки, хотя и следующие дополнительные требования к сборке и реализации:

- Манифест приложения XBAP должен быть настроен специально для загрузки конвейера (папок и сборок) и сборки надстройки в кэш приложения ClickOnce на клиентском компьютере в той же папке, что и XBAP.
...
- XBAP должен загрузить надстройку в специальный контекст безопасности, если надстройка ссылается на свободные файлы, расположенные на исходном узле. При размещении с помощью XBAP надстройки могут ссылаться только на свободные файлы, расположенные на исходном узле хост-приложения.
...
XBAP загружаются в защищенную папку в кэше развертывания ClickOnce и запускаются из нее. Чтобы XBAP мог разместить надстройку, сборка конвейера и надстройки также должна быть загружена в защищенную папку. Для этого нужно настроить манифест приложения для включения и конвейера и сборки надстройки для загрузки. Это проще всего сделать в Visual Studio, хотя сборка конвейера и надстройки должна находиться в корневой папке проекта приложения XBAP, чтобы Visual Studio смогла обнаружить сборки конвейера.

Следовательно, первый шаг состоит в создании сборки конвейера и надстройки в корне проекта XBAP путем установки выходных данных сборки для каждой сборки конвейера и проектов сборки надстройки. В следующей таблице показаны выходные пути сборки для проектов сборки конвейера и проекта сборки надстройки, которые находятся в том же решении и в корневой папке, что и проект приложения XBAP.
...
Следующий шаг — указать сборки конвейера и сборку надстройки как файлы содержимого XBAP в Visual Studio, выполнив следующие действия.
...
Последним шагом является настройка манифеста приложения для включения файлов сборки конвейера и файла сборки надстройки для загрузки. Файлы должны находиться в папках в корне папки в кэше ClickOnce, который занимает приложение XBAP. Конфигурацию можно добиться в Visual Studio, выполнив следующие действия.

1. Щелкните проект XBAP правой кнопкой мыши, выберите пункт **Свойства**, нажмите кнопку **опубликовать**, а затем нажмите кнопку **файлы приложения** .
