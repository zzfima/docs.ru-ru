...
Определение приложения обычно реализуется с помощью разметки и кода программной части с файлом разметки, настроенным в`ApplicationDefinition` качестве элемента MSBuild. Ниже приведено определение приложения для [!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)].
...
> [!IMPORTANT]
> Можно перейти к фрагментам на несвободных [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] страницах (файлы только [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] разметки с `Page` корневым элементом), только если страницы можно просматривать по протоколу HTTP.
>
...
Для [!INCLUDE[TLA2#tla_xbap#plural](../../../../includes/tla2sharptla-xbapsharpplural-md.md)] , размещенных в Internet Explorer, [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] интегрирует журнал в навигацию [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] по Internet Explorer. Это позволяет пользователям перемещаться по страницам [!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)] в с помощью кнопок **назад**, **вперед**и **Последние страницы** в Internet Explorer.

> [!IMPORTANT]
> В Internet Explorer, когда пользователь отключается от и обратно [!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)], только записи журнала для страниц, которые не были сохранены в активном состоянии, сохраняются в журнале. Сведения о сохранении страниц в активном состоянии см. в разделе [время существования страницы и журнал](#PageLifetime) далее в этой статье.

<xref:System.Windows.Controls.Page> По умолчанию текст для каждого, отображаемый в списке **Последние страницы** [!INCLUDE[TLA2#tla_uri](../../../../includes/tla2sharptla-uri-md.md)] Internet Explorer, <xref:System.Windows.Controls.Page>— для. В большинстве случаев это не особенно важно для пользователя. К счастью можно изменить текст, используя следующие параметры.
...
Хотя пользователь может перемещаться по журналу с помощью страниц « **назад**», « **вперед**» и « **последние** » в Internet Explorer, можно также перемещаться по журналу с помощью [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)]декларативных и программных механизмов, предоставляемых. Одной из причин для этого является предоставление настраиваемых переходов [!INCLUDE[TLA2#tla_ui#plural](../../../../includes/tla2sharptla-uisharpplural-md.md)] на страницах.
...
- В Internet Explorer политика P3P, относящаяся к файлам cookie, учитывается [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)]в, особенно в отношении первого и [!INCLUDE[TLA2#tla_xbap#plural](../../../../includes/tla2sharptla-xbapsharpplural-md.md)]стороннего производителей.
...
Обратите внимание, что записи журнала отображаются навигацией [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] <xref:System.Windows.Controls.Frame>в, а не Internet Explorer.
...
Как показано в этом разделе, можно также переходить к [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] свободным файлам, файлам HTML и объектам.
...
<xref:System.Windows.Controls.WebBrowser> Элемент управления поддерживает размещение документов HTML, навигацию и взаимодействие сценариев и управляемого кода. Подробные сведения об элементе управления <xref:System.Windows.Controls.WebBrowser> см. в <xref:System.Windows.Controls.WebBrowser>разделе.

Как <xref:System.Windows.Controls.Frame>и при переходе на HTML <xref:System.Windows.Controls.WebBrowser> с использованием, требуются специальные разрешения. Например, из приложения с частичным доверием можно переходить только к HTML-коду, находящемуся на исходном узле. Дополнительные сведения см. в разделе [Безопасность частичного доверия в WPF](../wpf-partial-trust-security.md).
