...
<xref:System.Windows.Controls.Page>, <xref:System.Windows.Documents.Hyperlink>, <xref:System.Windows.Navigation.NavigationService> и журнал формируют основу поддержки навигации, предоставляемой [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)]. В этом обзоре подробно рассматриваются эти функции, прежде чем будет рассмотрена поддержка расширенной навигации, включающая навигацию по свободным [!INCLUDE[TLA#tla_xaml](../../../../includes/tlasharptla-xaml-md.md)] файлам, HTML-файлам и объектам.

> [!NOTE]
> В этом разделе термин "браузер" относится только к браузерам, которые могут размещать [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] приложения, в которые в настоящее время входит Microsoft Internet Explorer и Firefox. Если конкретные функции [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] поддерживаются только определенным браузером, то ссылка на версию обозревателя называется.
...
> [!NOTE]
> При добавлении нового <xref:System.Windows.Controls.Page> в проект с помощью [!INCLUDE[TLA#tla_visualstu](../../../../includes/tlasharptla-visualstu-md.md)] <xref:System.Windows.Controls.Page> реализуется с помощью разметки и кода программной части и включает необходимую конфигурацию для создания связи между файлами разметки и файлов кода программной части, как описано здесь.
...
Определение приложения обычно реализуется с помощью разметки и кода программной части с файлом разметки, настроенным в качестве элемента `ApplicationDefinition` MSBuild. Ниже приведено определение приложения для [!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)].
...
@No__t_0 может использовать определение приложения для указания начального <xref:System.Windows.Controls.Page>, который является <xref:System.Windows.Controls.Page>, который автоматически загружается при запуске [!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)]. Это можно сделать, задав для свойства <xref:System.Windows.Application.StartupUri%2A> универсальный код ресурса (URI) для требуемого <xref:System.Windows.Controls.Page>.
...
В этом примере атрибуту `StartupUri` задается относительный URI типа Pack, идентифицирующий HomePage. XAML. При запуске [!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)] автоматически осуществляется переход на страницу HomePage. XAML и ее отображение. Это продемонстрировано на следующем рисунке, на котором показан [!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)], запущенный с веб-сервера.
...
На следующем рисунке показана [!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)] с <xref:System.Windows.Controls.Page> с <xref:System.Windows.Documents.Hyperlink>.
...
Как можно было бы догадаться, щелчок <xref:System.Windows.Documents.Hyperlink> приводит к тому, что [!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)] переходит к <xref:System.Windows.Controls.Page>, определяемому атрибутом `NavigateUri`. Кроме того, [!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)] добавляет запись для предыдущего <xref:System.Windows.Controls.Page> в список "Последние страницы" в Internet Explorer. Это показано на следующем рисунке.
...
При нажатии <xref:System.Windows.Documents.Hyperlink> [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] вызывает <xref:System.Windows.Navigation.NavigationService.Navigate%2A?displayProperty=nameWithType>, чтобы нахождение и скачивание <xref:System.Windows.Controls.Page> с указанным URI типа pack. Скачанный <xref:System.Windows.Controls.Page> преобразуется в дерево объектов, корневой объект которых является экземпляром скачанного <xref:System.Windows.Controls.Page>. Ссылка на корневой объект <xref:System.Windows.Controls.Page> хранится в свойстве <xref:System.Windows.Navigation.NavigationService.Content%2A?displayProperty=nameWithType>. URI типа "Pack" для содержимого, к которому осуществлялся переход, хранится в свойстве <xref:System.Windows.Navigation.NavigationService.Source%2A?displayProperty=nameWithType>, а <xref:System.Windows.Navigation.NavigationService.CurrentSource%2A?displayProperty=nameWithType> сохраняет URI типа "Pack" для последней страницы, к которой был выполнен переход.

> [!NOTE]
> Приложение [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] может иметь более одного активного <xref:System.Windows.Navigation.NavigationService>. Дополнительные сведения см. в подразделе [узлы навигации](#Navigation_Hosts) далее в этой статье.
...
@No__t_0, который переходит к <xref:System.Windows.Controls.Page> с конструктором без параметров, показан в следующей разметке и коде.
...
При регистрации обработчика с событием навигации из <xref:System.Windows.Controls.Page>, как показано в предыдущем примере, необходимо также отменить регистрацию обработчика событий. В противном случае могут возникнуть побочные эффекты в отношении того, как [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] переносить элементы навигации <xref:System.Windows.Controls.Page> навигации с помощью журнала.

<a name="NavigationHistory"></a>

### <a name="remembering-navigation-with-the-journal"></a>Запоминание перехода в журнале

[!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] использует два стека для запоминания страниц, с которых был осуществлен переход: стек "Назад" и стек "Вперед". При переходе от текущего <xref:System.Windows.Controls.Page> к новому <xref:System.Windows.Controls.Page> или пересылке к существующему <xref:System.Windows.Controls.Page>у, текущий <xref:System.Windows.Controls.Page> добавляется в *стек назад*. При переходе от текущего <xref:System.Windows.Controls.Page> к предыдущему <xref:System.Windows.Controls.Page> текущий <xref:System.Windows.Controls.Page> добавляется в *стек вперед*. Стек "Назад", стек "Вперед" и функциональные возможности для управления ими в совокупности называются журналом. Каждый элемент в стеке заднего и прямого стека является экземпляром класса <xref:System.Windows.Navigation.JournalEntry> и называется *записью в журнале*.
...
Для [!INCLUDE[TLA2#tla_xbap#plural](../../../../includes/tla2sharptla-xbapsharpplural-md.md)], размещенных в Internet Explorer, [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] интегрирует журнал в [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] навигации Internet Explorer. Это позволяет пользователям перемещаться по страницам в [!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)] с помощью кнопок **назад**, **вперед**и **Последние страницы** в Internet Explorer.

> [!IMPORTANT]
> В Internet Explorer, когда пользователь отменяет и возвращается к [!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)], в журнал сохраняются только записи журнала для страниц, которые не были сохранены в активном состоянии. Сведения о сохранении страниц в активном состоянии см. в разделе [время существования страницы и журнал](#PageLifetime) далее в этой статье.
...
Во-вторых, можно указать, что [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] хранить экземпляр <xref:System.Windows.Controls.Page> в журнале, установив для свойства <xref:System.Windows.Controls.Page.KeepAlive%2A> значение `true` (значение по умолчанию — `false`). Как показано в следующем примере, можно задать <xref:System.Windows.Controls.Page.KeepAlive%2A> декларативно в разметке.
...
2. @No__t_0 обновляется состояние, хранящееся в записи журнала для <xref:System.Windows.Controls.Page>.
...
- [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] автономные приложения и [!INCLUDE[TLA2#tla_xbap#plural](../../../../includes/tla2sharptla-xbapsharpplural-md.md)] могут создавать файлы cookie и управлять ими.
...
- [!INCLUDE[TLA2#tla_xbap#plural](../../../../includes/tla2sharptla-xbapsharpplural-md.md)] и HTML-страницы из одного домена могут создавать файлы cookie и предоставлять к ним общий доступ.

- Файлы cookie отправляются, когда [!INCLUDE[TLA2#tla_xbap#plural](../../../../includes/tla2sharptla-xbapsharpplural-md.md)] и свободные [!INCLUDE[TLA2#tla_xaml](../../../../includes/tla2sharptla-xaml-md.md)] страницы выполняют веб-запросы.

- Как [!INCLUDE[TLA2#tla_xbap#plural](../../../../includes/tla2sharptla-xbapsharpplural-md.md)] верхнего уровня, так и [!INCLUDE[TLA2#tla_xbap#plural](../../../../includes/tla2sharptla-xbapsharpplural-md.md)], размещенные в IFRAME, могут получить доступ к файлам cookie.
...
- В Internet Explorer политика P3P, относящаяся к файлам cookie, учитывается [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)], особенно в отношении сторонних и [!INCLUDE[TLA2#tla_xbap#plural](../../../../includes/tla2sharptla-xbapsharpplural-md.md)] сторонних разработчиков.

<a name="Structured_Navigation"></a>

### <a name="structured-navigation"></a>Структурная навигация

Если необходимо передать данные из одного <xref:System.Windows.Controls.Page> в другой, данные можно передать как аргументы в конструктор <xref:System.Windows.Controls.Page> без параметров. Обратите внимание, что при использовании этого метода необходимо поддерживать <xref:System.Windows.Controls.Page> в активном состоянии; в противном случае при следующем переходе к <xref:System.Windows.Controls.Page> [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] повторно создает экземпляр <xref:System.Windows.Controls.Page> с помощью конструктора без параметров.
...
Чтобы включить навигацию в стиле браузера в автономные приложения, можно использовать класс <xref:System.Windows.Navigation.NavigationWindow>. <xref:System.Windows.Navigation.NavigationWindow> является производным от <xref:System.Windows.Window> и расширяет его с помощью той же поддержки навигации, которую [!INCLUDE[TLA2#tla_xbap#plural](../../../../includes/tla2sharptla-xbapsharpplural-md.md)] предоставить. @No__t_0 можно использовать как главное окно автономного приложения или как дополнительное окно, такое как диалоговое окно.
...
Кроме того, можно использовать поддержку настройки в [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)], чтобы заменить [!INCLUDE[TLA2#tla_ui](../../../../includes/tla2sharptla-ui-md.md)] <xref:System.Windows.Navigation.NavigationWindow>.

<a name="Frame_in_Standalone_Applications"></a>

## <a name="the-frame-class"></a>Класс Frame

Браузер и <xref:System.Windows.Navigation.NavigationWindow> являются окнами, в которых размещается содержимое для навигации. В некоторых случаях приложения имеют содержимое, которое не обязательно должно размещаться в целом окне. Такое содержимое помещается внутрь другого содержимого. Можно вставить содержимое для навигации в другое содержимое с помощью класса <xref:System.Windows.Controls.Frame>. <xref:System.Windows.Controls.Frame> предоставляет ту же поддержку, что и <xref:System.Windows.Navigation.NavigationWindow> и [!INCLUDE[TLA2#tla_xbap#plural](../../../../includes/tla2sharptla-xbapsharpplural-md.md)].
...
Эта разметка задает атрибут `Source` элемента `Frame` с URI типа Pack для <xref:System.Windows.Controls.Page>, к которому <xref:System.Windows.Controls.Frame> следует первоначально переходить. На следующем рисунке показана [!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)] с <xref:System.Windows.Controls.Page> с <xref:System.Windows.Controls.Frame>, который перешел между несколькими страницами.
...
По сути это позволяет <xref:System.Windows.Navigation.NavigationWindow> и <xref:System.Windows.Controls.Frame> обеспечивать ту же поддержку навигации, которую [!INCLUDE[TLA2#tla_xbap](../../../../includes/tla2sharptla-xbap-md.md)] предоставляет при размещении в браузере.
...
В этом разделе <xref:System.Windows.Controls.Page> и Pack [!INCLUDE[TLA2#tla_xbap#plural](../../../../includes/tla2sharptla-xbapsharpplural-md.md)] использовались для демонстрации различных возможностей навигации [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)]. Однако <xref:System.Windows.Controls.Page>, компилируемый в приложение, — это не единственный тип содержимого, к которому можно осуществлять переход, а [!INCLUDE[TLA2#tla_xbap#plural](../../../../includes/tla2sharptla-xbapsharpplural-md.md)] Pack не единственный способ определить содержимое.
...
Поддержка навигации [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] позволяет переходить к [!INCLUDE[TLA2#tla_xbap#plural](../../../../includes/tla2sharptla-xbapsharpplural-md.md)] через Интернет, что позволяет приложениям размещать сторонние материалы. Чтобы защитить как приложения, так и пользователей от вредоносного поведения, [!INCLUDE[TLA2#tla_wpf](../../../../includes/tla2sharptla-wpf-md.md)] предоставляет разнообразные функции безопасности, обсуждаемые в разделе [Безопасность](../security-wpf.md) и [Безопасность частичного доверия в WPF](../wpf-partial-trust-security.md).
