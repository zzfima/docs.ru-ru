...
- `dragSource` — ссылка на объект зависимости, являющийся источником передаваемых данных; Обычно это источник события <xref:System.Windows.UIElement.MouseMove>.  
  
- `data` — объект, содержащий перенесенные данные, заключенные в <xref:System.Windows.DataObject>.  
  
- `allowedEffects` — одно из значений перечисления <xref:System.Windows.DragDropEffects>, которое указывает разрешенные эффекты операции перетаскивания.  
