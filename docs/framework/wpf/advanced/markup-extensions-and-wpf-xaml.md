---
title: Расширение разметки и XAML
ms.date: 03/30/2017
helpviewer_keywords:
- brace character [WPF]
...
- `RelativeSource`предоставляет исходную <xref:System.Windows.Data.Binding> информацию для того, кто может перемещаться по нескольким возможным отношениям в дереве объектов времени выполнения. Это обеспечивает специализированные источники для привязок, которые создаются в шаблонах для многократного использования или в коде без полных сведений об окружающем дереве объектов. Дополнительные сведения см. в разделе [Расширение разметки RelativeSource](relativesource-markupextension.md).  
...
## <a name="extension-classes"></a>\*Классы расширения  
 Как для общего языка XAML, так и для расширения разметки, специфичных для WPF, поведение каждого расширения разметки идентифицируется с процессором XAML через `*Extension` класс, который вытекает из, <xref:System.Windows.Markup.MarkupExtension>и обеспечивает реализацию <xref:System.Windows.Markup.StaticExtension.ProvideValue%2A> метода. Этот метод в каждом расширении предоставляет объект, который возвращается при вычислении расширения разметки. Возвращаемый объект обычно вычисляется, исходя из различных токенов строк, передаваемых в расширение разметки.  
  
 Например, <xref:System.Windows.StaticResourceExtension> класс обеспечивает поверхностную реализацию фактического поиска <xref:System.Windows.Markup.MarkupExtension.ProvideValue%2A> ресурсов, чтобы его реализация возвращала запрашиваемый объект, при этом вход этой конкретной реализации является строкой, которая используется для поиска ресурса. `x:Key` Большая часть подробностей этой реализации не важна при использовании существующего расширения разметки.  
 ...
 Шаблон именования `*Extension` принят для удобства и согласованности. Он не требуется, чтобы обработчик XAML определил этот класс как поддерживающий расширение разметки. До тех пор, пока ваша кодовая база включает в себя System.Xaml и использует реализации .NET Framework XAML Services, все, что необходимо для признания в качестве расширения разметки XAML, состоит в том, чтобы получить от <xref:System.Windows.Markup.MarkupExtension> и поддержать синтаксис строительства. WPF определяет классы, способствуют ездое расширение разметки, которые не соответствуют шаблону `*Extension` именования, например. <xref:System.Windows.Data.Binding> Обычно причина заключается в том, что класс поддерживает не только расширение разметки, но и сценарии. В <xref:System.Windows.Data.Binding>случае, что класс поддерживает время выполнения доступа к методам и свойствам объекта для сценариев, которые не имеют ничего общего с XAML.  
...
- Если отдельные разделенные токены не содержат знаки равенства, каждый токен считается аргументом конструктора. Каждый параметр конструктора должен передаваться в качестве типа, ожидаемого сигнатурой, и в соответствующем порядке, ожидаемом сигнатурой.  
    ...
    > [!NOTE]
    > Обработчик XAML должен вызвать конструктор, который соответствует количеству аргументов числа пар. По этой причине, если вы реализуете пользовательское расширение разметки, не предоставляйте несколько конструкторов с одинаковым количеством аргументов. Поведение для обработчика XAML при наличии нескольких путей конструктора расширения разметки с одинаковым количеством параметров не определено, но следует помнить, что обработчику XAML разрешено создавать исключение для использования при возникновении такой ситуации в определениях типов расширения разметки.  
  
- Если отдельные разделенные токены содержат равные знаки, то процессор XAML сначала вызывает беспараметрыный конструктор для расширения разметки. Затем каждая пара «имя = значение» интерпретируется как имя свойства, которое существует в расширении разметки, и значение для присвоения этому свойству.  
  
- Если имеется параллельный результат между поведением конструктора и поведением установки свойства в расширении разметки, не имеет значения, какое поведение использовать. Чаще всего используются пары*значений* *свойств*`=`для расширений разметки, которые имеют более чем одно свойство установки, только потому, что это делает вашу разметку более преднамеренной, и вы менее склонны случайно перенести параметры конструктора. (При указании пар свойств и значений эти свойства могут быть в любом порядке.) Кроме того, нет никакой гарантии, что расширение разметки поставляет параметр конструктора, который устанавливает каждый из его установленных свойств. Например, <xref:System.Windows.Data.Binding> это расширение разметки, со многими свойствами, которые <xref:System.Windows.Data.Binding> устанавливаются через расширение в форме*значения* *свойства,*`=`но поддерживает только два конструктора: без параметров конструктора, и тот, который устанавливает начальный путь.  
