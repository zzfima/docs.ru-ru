...
Кэширование позволяет хранить данные в памяти для быстрого доступа. При повторном доступе к данным приложения могут получать их из кэша вместо извлечения из исходного источника. Это может повысить производительность и масштабируемость. Кроме того, кэширование обеспечивает доступность данных при временной недоступности источника данных.
 ...
 .NET Framework предоставляет классы, позволяющие использовать кэширование в приложениях .NET Framework. Эти классы находятся в <xref:System.Runtime.Caching> пространстве имен.

> [!NOTE]
> <xref:System.Runtime.Caching> Пространство имен является новым в .NET Framework 4. Это пространство имен делает кэширование доступным для всех .NET Framework приложений. В предыдущих версиях .NET Framework кэширование было доступно только в <xref:System.Web> пространстве имен и, следовательно, требовало зависимости от классов ASP.NET.
...
2. В области **элементов**в разделе **Общие элементы управления WPF**перетащите `Button` элемент управления в `MainWindow` окно.

3. В окне **Свойства** задайте `Content` для свойства `Button` элемента управления значение **получить кэш**.
...
- Создайте экземпляр класса Cache, то есть будет создан экземпляр нового <xref:System.Runtime.Caching.MemoryCache> объекта.

- Указывает, что кэш использует <xref:System.Runtime.Caching.HostFileChangeMonitor> объект для отслеживания изменений в текстовом файле.
...
2. В верхней части файла (перед объявлением класса) добавьте следующие `Imports` операторы (Visual Basic) или `using` (C#):
...
3. В обработчике событий добавьте следующий код для создания экземпляра объекта кэша:
    ...
    ```
     ...
     <xref:System.Runtime.Caching.ObjectCache> Класс является встроенным классом, который предоставляет кэш объектов в памяти.
...
5. Добавьте следующий код, чтобы проверить, существует ли запись кэша `filecontents` с именем:
...
6. В блоке добавьте следующий код, чтобы создать новый <xref:System.Runtime.Caching.CacheItemPolicy> объект, указывающий, что срок действия записи кэша истекает через 10 секунд. `if/then`
...
7. `if/then` В блоке и после кода, добавленного на предыдущем шаге, добавьте следующий код, чтобы создать коллекцию для путей к файлам, которые требуется отслеживать, а также для добавления пути к текстовому файлу в коллекцию:
    ...
    > [!NOTE]
    > Если нужный текстовый файл не `c:\cache\cacheText.txt`используется, укажите путь, по которому следует использовать текстовый файл.

8. После кода, добавленного на предыдущем шаге, добавьте следующий код, чтобы добавить новый <xref:System.Runtime.Caching.HostFileChangeMonitor> объект в коллекцию мониторов изменений для записи кэша:
    ...
    ```
     ...
     <xref:System.Runtime.Caching.HostFileChangeMonitor> Объект отслеживает путь к текстовому файлу и уведомляет кэш, если происходят изменения. В этом примере срок действия записи кэша истечет при изменении содержимого файла.
...
10. После кода, добавленного на предыдущем шаге, добавьте следующий код, чтобы вставить содержимое файла в объект кэша в качестве <xref:System.Runtime.Caching.CacheItem> экземпляра:
    ...
    ```
     ...
     Сведения о том, как следует удалить запись кэша, можно указать, передав <xref:System.Runtime.Caching.CacheItemPolicy> созданный ранее объект в качестве параметра.

11. `if/then` После блока добавьте следующий код для вывода содержимого кэшированного файла в окне сообщения:
