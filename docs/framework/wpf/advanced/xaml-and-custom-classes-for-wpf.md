...
### <a name="structures"></a>Структуры  
 Структуры, определяемые в качестве пользовательских типов, всегда могут быть созданы в XAML в [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] . Это обусловлено тем [!INCLUDE[TLA2#tla_clr](../../../../includes/tla2sharptla-clr-md.md)] , что компиляторы неявно создают конструктор без параметров для структуры, которая инициализирует все значения свойств значениями по умолчанию. В некоторых случаях поведение конструктора по умолчанию или использование объектных элементов для структуры является нежелательным. Это возможно в тех случаях, когда структура используется для заполнения значений и функционирует в качестве объединения, в котором хранятся взаимоисключающие значения, и поэтому ни одному из свойств нельзя присвоить значение. Примером такой структуры является <xref:System.Windows.GridLength>. [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] Как правило, в таких структурах необходимо реализовать преобразователь типов, чтобы значения можно было представить в виде атрибутов, используя преобразования строк для создания различных интерпретаций или режимов значений структуры. Структура должна также предоставлять аналогичное поведение для построения кода с помощью конструктора без параметров.  
...
## <a name="requirements-for-properties-of-a-custom-class-as-xaml-attributes"></a>Требования к свойствам пользовательского класса как атрибутам XAML  
 Свойства должны ссылаться на тип по значению (например, на примитив) или использовать класс для типа, имеющего либо конструктор без параметров, либо специальный преобразователь типов, к которому может получить доступ обработчик XAML. В реализации CLR XAML обработчики XAML либо находят такие конвертеры с помощью встроенной поддержки примитивов языка, либо посредством <xref:System.ComponentModel.TypeConverterAttribute> применения к типу или члену в определениях резервных типов.  
...
## <a name="requirements-for-xaml-event-handler-attribute-syntax-on-events-of-a-custom-class"></a>Требования к синтаксису атрибутов обработчиков событий XAML пользовательского класса  
 Чтобы можно было использовать в [!INCLUDE[TLA2#tla_clr](../../../../includes/tla2sharptla-clr-md.md)] качестве события, событие должно быть предоставлено как открытое событие в классе, поддерживающем конструктор без параметров, или на абстрактном классе, где событие может быть получено в производных классах. Для удобства использования в [!INCLUDE[TLA2#tla_clr](../../../../includes/tla2sharptla-clr-md.md)] качестве перенаправленного события в событии должны быть реализованы явные `add` методы и `remove` [!INCLUDE[TLA2#tla_clr](../../../../includes/tla2sharptla-clr-md.md)] , которые добавляют и удаляют обработчики для сигнатуры событий и передают эти обработчики в <xref:System.Windows.UIElement.AddHandler%2A>методы <xref:System.Windows.UIElement.RemoveHandler%2A> и. Эти методы добавляют или удаляют обработчики из хранилища обработчиков перенаправленных событий экземпляра, к которому присоединено событие.  
  
> [!NOTE]
>  Можно зарегистрировать обработчики напрямую для перенаправленных событий с помощью <xref:System.Windows.UIElement.AddHandler%2A>, а также намеренно не [!INCLUDE[TLA2#tla_clr](../../../../includes/tla2sharptla-clr-md.md)] определять событие, предоставляющее перенаправленное событие. Обычно это не рекомендуется, так как событие не включит синтаксис атрибута XAML для подключения обработчиков и результирующий класс предложит менее прозрачное представление XAML возможностей этого типа.  
...
- Является производным <xref:System.Array> от (Дополнительные сведения о массивах в XAML см. в разделе [расширение разметки x:Array](../../xaml-services/x-array-markup-extension.md).)  
  
- Реализует <xref:System.Windows.Markup.IAddChild> (интерфейс, определенный параметром [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]).  
...
## <a name="declaring-xaml-content-properties"></a>Объявление свойств содержимого XAML  
 ...
 Некоторые существующие свойства содержимого XAML используют тип свойства `Object`. Это включает свойство содержимого XAML, которое может принимать примитивные значения, такие <xref:System.String> как, а также принимает одно значение ссылочного объекта. Если следовать этой модели, то тип будет отвечать как за определение типа, так и за обработку возможных типов. Типичная причина для <xref:System.Object> типа содержимого — поддержка простого способа добавления содержимого объекта в виде строки (которая получает обработку представления по умолчанию) или расширенного способа добавления содержимого объекта, которое указывает представление, отличное от представления по умолчанию или Дополнительные данные.  
