...
```  
 ...
 При вызове `new MyClass(objectvalue)`кода приложения вызывает конструктор без параметров и конструкторы базового класса. Затем он задает `Property1 = object1`, который вызывает виртуальный метод `OnPropertyChanged` для владельца `MyClass` <xref:System.Windows.DependencyObject>.  Переопределение ссылается на `_myList`, который еще не был инициализирован.  
...
#### <a name="setting-dependency-properties-with-setvalue"></a>Установка свойств зависимостей с помощью SetValue  
 Те же шаблоны применяются, если вы устанавливаете свойство, которое не имеет оболочки для удобства настройки свойств, и устанавливает значения с помощью <xref:System.Windows.DependencyObject.SetValue%2A>. Вызовы <xref:System.Windows.DependencyObject.SetValue%2A> , передаваемые через параметры конструктора, должны также вызвать конструктор класса "без параметров" для инициализации.  
