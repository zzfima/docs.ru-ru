...
Ссылка на символ дает модуль кода UTF16 конкретного [!INCLUDE[TLA#tla_unicode](../../../../includes/tlasharptla-unicode-md.md)] символов, оно представляет в десятичное или шестнадцатеричное. В следующем примере показано ссылку десятичных цифр по ЗАГЛАВНАЯ буква КОПТСКИЙ или «Ϩ»:

```
&#1000;
```

Следующий пример показывает ссылку шестнадцатеричных цифр. Обратите внимание, что он имеет **x** перед шестнадцатеричное число.
...
### <a name="language-attribute"></a>Атрибут Language  
 ...
 Следующие [!INCLUDE[TLA#tla_xaml](../../../../includes/tlasharptla-xaml-md.md)] примере `fr-CA` атрибут language, чтобы указать французский (Канада).  
...
## <a name="designing-an-international-user-interface-with-xaml"></a>Проектирование международного интерфейса пользователя с XAML  
 В этом разделе описываются [!INCLUDE[TLA#tla_ui](../../../../includes/tlasharptla-ui-md.md)] функций, которые следует учитывать при написании приложений.  
  
<a name="intl_text"></a>   
### <a name="international-text"></a>Международный текст  
 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] включает встроенные возможности обработки для систем письма все поддерживаемые Microsoft .NET Framework.  
