...
- **Свойство зависимостей:** Свойство, которое поддерживается <xref:System.Windows.DependencyProperty>объектом.

- **Идентификатор свойства зависимости:** <xref:System.Windows.DependencyProperty> Экземпляр, который получается как возвращаемое значение при регистрации свойства зависимости, а затем сохраняется как статический член класса. Этот идентификатор используется в качестве параметра для многих API, взаимодействующих с системой свойств WPF.

- **Оболочка CLR:** Фактические реализации метода Get и Set для свойства. Эти реализации включают идентификатор свойства зависимостей, используя его в вызовах <xref:System.Windows.DependencyObject.GetValue%2A> и <xref:System.Windows.DependencyObject.SetValue%2A> и, таким образом, обеспечивая поддержку для свойства с помощью системы свойств WPF.
...
### <a name="setting-properties-in-code"></a>Задание свойств с помощью кода
 Установка значений свойств зависимостей в коде обычно осуществляется просто путем вызова реализации набора, предоставляемой оберткой CLR.
...
Также можно вызывать API <xref:System.Windows.DependencyObject.GetValue%2A> системы свойств и <xref:System.Windows.DependencyObject.SetValue%2A> напрямую. Обычно это не требуется, если используются существующие свойства (оболочки более удобны и предоставляют лучшую раскрытие свойств для средств разработчика), но вызов интерфейсов API напрямую подходит для определенных сценариев.
