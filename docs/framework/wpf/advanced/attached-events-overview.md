...
-   Метод **добавить*EventName*обработчик** с двумя параметрами. Первый параметр должен определять событие, и имя определенного события должно совпадать с ***EventName*** в имени метода. Вторым параметром является добавляемый обработчик. Этот метод должен быть `public` и `static`, не возвращая значения.  
  
-   Метод **удалить*EventName*обработчик** с двумя параметрами. Первый параметр должен определять событие, и имя определенного события должно совпадать с ***EventName*** в имени метода. Вторым параметром является удаляемый обработчик. Этот метод должен быть `public` и `static`, не возвращая значения.  
