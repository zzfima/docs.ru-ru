...
В следующем примере показано, что <xref:System.Windows.Controls.Image> объект не может наследовать объект <xref:System.Windows.Controls.StackPanel> <xref:System.Windows.FlowDirection> из, который содержит его.
