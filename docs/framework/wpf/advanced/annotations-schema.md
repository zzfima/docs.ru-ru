...
---
# <a name="annotations-schema"></a><span data-ttu-id="65971-102">Схема примечаний</span><span class="sxs-lookup"><span data-stu-id="65971-102">Annotations Schema</span></span>
<span data-ttu-id="65971-103">В этом разделе описывается определение схемы XML (XSD), используемое структурой заметок для сохранения и извлечения данных заметок пользователей.</span><span class="sxs-lookup"><span data-stu-id="65971-103">This topic describes the XML schema definition (XSD) used by the Microsoft Annotations Framework to save and retrieve user annotation data.</span></span>  
  
 <span data-ttu-id="65971-104">[!INCLUDE[TLA2#tla_caf](../../../../includes/tla2sharptla-caf-md.md)] сериализует данные заметки из внутреннего представления в формат XML.</span><span class="sxs-lookup"><span data-stu-id="65971-104">The [!INCLUDE[TLA2#tla_caf](../../../../includes/tla2sharptla-caf-md.md)] serializes annotation data from an internal representation to an XML format.</span></span>  <span data-ttu-id="65971-105">Формат XML, используемый для этого преобразования, описывается XSD-схемой [!INCLUDE[TLA2#tla_caf](../../../../includes/tla2sharptla-caf-md.md)].</span><span class="sxs-lookup"><span data-stu-id="65971-105">The XML format used for this conversion is described by the [!INCLUDE[TLA2#tla_caf](../../../../includes/tla2sharptla-caf-md.md)] XSD Schema.</span></span>  <span data-ttu-id="65971-106">Схема определяет независимый от реализации формат XML, который может использоваться для обмена данными заметок между приложениями.</span><span class="sxs-lookup"><span data-stu-id="65971-106">The schema defines the implementation-independent XML format that can be used to exchange annotation data between applications.</span></span>  
  
 <span data-ttu-id="65971-107">Определение схемы XML [!INCLUDE[TLA2#tla_caf](../../../../includes/tla2sharptla-caf-md.md)] состоит из двух подсхем:</span><span class="sxs-lookup"><span data-stu-id="65971-107">The [!INCLUDE[TLA2#tla_caf](../../../../includes/tla2sharptla-caf-md.md)] XML schema definition consists of two subschemas</span></span>  
  
-   <span data-ttu-id="65971-108">основная схема XML-заметок (основная схема);</span><span class="sxs-lookup"><span data-stu-id="65971-108">The Annotations XML Core Schema (Core Schema).</span></span>  
  
-   <span data-ttu-id="65971-109">базовая схема XML-заметок (базовая схема).</span><span class="sxs-lookup"><span data-stu-id="65971-109">The Annotations XML Base Schema (Base Schema).</span></span>  
  
 <span data-ttu-id="65971-110">Основная схема определяет первичный XML-структуру <xref:System.Windows.Annotations.Annotation>.</span><span class="sxs-lookup"><span data-stu-id="65971-110">The Core Schema defines the primary XML structure of an <xref:System.Windows.Annotations.Annotation>.</span></span>  <span data-ttu-id="65971-111">Большая часть XML-элементов, определенных в схеме Core соответствуют типам в <xref:System.Windows.Annotations> пространства имен.</span><span class="sxs-lookup"><span data-stu-id="65971-111">The majority of XML elements defined in the Core Schema correspond to types in the <xref:System.Windows.Annotations> namespace.</span></span>  <span data-ttu-id="65971-112">Основная схема предоставляет три точки расширения, в которых приложения могут добавлять собственные XML-данные.</span><span class="sxs-lookup"><span data-stu-id="65971-112">The Core Schema exposes three extension points where applications can add their own XML data.</span></span>  <span data-ttu-id="65971-113">Эти точки расширения включают <xref:System.Windows.Annotations.Annotation.Authors%2A>, <xref:System.Windows.Annotations.ContentLocatorPart>и «Содержимое».</span><span class="sxs-lookup"><span data-stu-id="65971-113">These extension points include the <xref:System.Windows.Annotations.Annotation.Authors%2A>, <xref:System.Windows.Annotations.ContentLocatorPart>, and "Content".</span></span>  <span data-ttu-id="65971-114">(Элементы содержимого представлены в виде <xref:System.Xml.XmlElement> список.)</span><span class="sxs-lookup"><span data-stu-id="65971-114">(Content elements are provided in the form of an <xref:System.Xml.XmlElement> list.)</span></span>  
  
 <span data-ttu-id="65971-115">Базовая схема, описанная в этом разделе определяет расширения для <xref:System.Windows.Annotations.Annotation.Authors%2A>, <xref:System.Windows.Annotations.ContentLocatorPart>и типы содержимого, включая начальный [!INCLUDE[TLA#tla_wpf](../../../../includes/tlasharptla-wpf-md.md)] выпуска.</span><span class="sxs-lookup"><span data-stu-id="65971-115">The Base Schema described in this topic defines the extensions for the <xref:System.Windows.Annotations.Annotation.Authors%2A>, <xref:System.Windows.Annotations.ContentLocatorPart>, and Content types included with the initial [!INCLUDE[TLA#tla_wpf](../../../../includes/tlasharptla-wpf-md.md)] release.</span></span>  
  
<a name="CoreSchema"></a>   
## <a name="annotations-xml-core-schema"></a><span data-ttu-id="65971-116">Основная схема XML-заметок</span><span class="sxs-lookup"><span data-stu-id="65971-116">Annotations XML Core Schema</span></span>  
 <span data-ttu-id="65971-117">Основная схема XML примечаний определяет XML-структуру, которая используется для хранения <xref:System.Windows.Annotations.Annotation> объектов.</span><span class="sxs-lookup"><span data-stu-id="65971-117">The Annotations XML Core Schema defines the XML structure that is used to store <xref:System.Windows.Annotations.Annotation> objects.</span></span>  
...
<a name="BaseSchema"></a>   
## <a name="annotations-xml-base-schema"></a><span data-ttu-id="65971-118">Базовая схема XML-заметок</span><span class="sxs-lookup"><span data-stu-id="65971-118">Annotations XML Base Schema</span></span>  
 <span data-ttu-id="65971-119">Базовая схема определяет структуру XML для трех абстрактных элементов, определенных в схеме Core – <xref:System.Windows.Annotations.Annotation.Authors%2A>, <xref:System.Windows.Annotations.ContentLocatorPart>, и <xref:System.Windows.Annotations.AnnotationResource.Contents%2A>.</span><span class="sxs-lookup"><span data-stu-id="65971-119">The Base Schema defines the XML structure for the three abstract elements defined in the Core Schema – <xref:System.Windows.Annotations.Annotation.Authors%2A>, <xref:System.Windows.Annotations.ContentLocatorPart>, and <xref:System.Windows.Annotations.AnnotationResource.Contents%2A>.</span></span>  
...
<a name="SampleXML"></a>   
## <a name="sample-xml-produced-by-annotations-xmlstreamstore"></a><span data-ttu-id="65971-120">Пример XML-кода, созданный классом XmlStreamStore Annotations</span><span class="sxs-lookup"><span data-stu-id="65971-120">Sample XML Produced by Annotations XmlStreamStore</span></span>  
 <span data-ttu-id="65971-121">Кодом XML ниже показан результат выполнения заметки <xref:System.Windows.Annotations.Storage.XmlStreamStore> и организация образец файла, который содержит три заметки - выделения текста-записки и манипулятор заметки рукописного ввода.</span><span class="sxs-lookup"><span data-stu-id="65971-121">The XML that follows shows the output of an Annotations <xref:System.Windows.Annotations.Storage.XmlStreamStore> and the organization of a sample file that contains three annotations - a highlight, a text sticky-note, and an ink stick-note.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="65971-122">См. также</span><span class="sxs-lookup"><span data-stu-id="65971-122">See Also</span></span>  
 ...
 [<span data-ttu-id="65971-123">Общие сведения о заметках</span><span class="sxs-lookup"><span data-stu-id="65971-123">Annotations Overview</span></span>](../../../../docs/framework/wpf/advanced/annotations-overview.md)
