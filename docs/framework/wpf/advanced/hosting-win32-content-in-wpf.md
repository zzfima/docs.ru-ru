...
```  
 ...
 Оба msg имеют те же данные, но иногда бывает проще работать с неуправляемым определением, чтобы в этом примере можно было определить подпрограмма очевидным преобразования:  
