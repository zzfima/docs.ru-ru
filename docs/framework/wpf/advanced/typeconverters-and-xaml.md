...
### <a name="xaml-and-string-values"></a>XAML и строковые значения  
 Когда задается значение атрибута в файле XAML, начальный тип этого значения — строка чистого текста. Даже другие примитивные типы, такие как <xref:System.Double> изначально являются текстовыми строками для обработчика XAML.  
...
### <a name="using-existing-type-conversion-behavior-in-xaml"></a>Использование поведения преобразования существующего типа в XAML  
 В зависимости от уровня понимания концепций XAML, вы, возможно, уже используете преобразование типа в простом приложении XAML, не реализуя его. Например, WPF определяет сотни свойств, принимающих значение типа <xref:System.Windows.Point>. Объект <xref:System.Windows.Point> значение, которое описывает координаты в двухмерном пространстве координат, и это просто имеет два важных свойства: <xref:System.Windows.Point.X%2A> и <xref:System.Windows.Point.Y%2A>. При указании точки в XAML она задается в виде строки с разделителем (обычно запятую) между <xref:System.Windows.Point.X%2A> и <xref:System.Windows.Point.Y%2A> значения, указываемые. Например, `<LinearGradientBrush StartPoint="0,0" EndPoint="1,1">`.  
  
 Даже этот простой тип <xref:System.Windows.Point> и простое его использование в XAML используют преобразователь типа. В этом случае это класс <xref:System.Windows.PointConverter>.  
  
 Преобразователь типов для <xref:System.Windows.Point> определены на уровне класса и упрощает разметки всех свойств, которые принимают <xref:System.Windows.Point>. Без преобразователя типов для создания примера, показанного выше, требуется гораздо больше разметки.  
 ...
 Существующих преобразователей типов могут быть найдены для типов WPF и .NET Framework, проверив наличие примененного атрибута класса (или свойство) <xref:System.ComponentModel.TypeConverterAttribute>. Этот атрибут содержит имя класса, поддерживающего преобразование для данного типа, как для XAML, так потенциально и для других целей.  
...
-   <xref:System.ComponentModel.TypeConverter.ConvertFrom%2A>  
 ...
 Из них является самым важным методом <xref:System.ComponentModel.TypeConverter.ConvertFrom%2A>. Этот метод преобразует входную строку к требуемому типу объекта. Строго говоря <xref:System.ComponentModel.TypeConverter.ConvertFrom%2A> метод может быть реализован для преобразования более широкого диапазона типов в требуемый конечный тип преобразователя и таким образом используются для целей, которые выходят за пределы XAML, например для поддержки преобразований во время выполнения, но для нужд XAML он является только путь кода, который может обрабатывать <xref:System.String> ввода, который имеет значение.  
  
 Далее наиболее важный метод — <xref:System.ComponentModel.TypeConverter.ConvertTo%2A>. Если приложение преобразуется в представление разметки (например, если оно сохранено в файл XAML), <xref:System.ComponentModel.TypeConverter.ConvertTo%2A> отвечает за создание представления разметки. В этом случае путь кода, который имеет значение для XAML — при передаче `destinationType` из <xref:System.String> .  
...
### <a name="implementing-convertto"></a>Реализация ConvertTo  
 ...
 Для использования в качестве <xref:System.ComponentModel.TypeConverter> реализацию, поддерживающей XAML, <xref:System.ComponentModel.TypeConverter.ConvertTo%2A> метод данного преобразователя должен принимать экземпляр типа (или значение) поддерживается как `value` параметр. Когда `destinationType` параметр является типом <xref:System.String>, возвращаемый объект должен иметь возможность приведения как <xref:System.String>. Возвращаемая строка должна представлять сериализованное значение `value`. В идеальном случае выбранный формат сериализации должна быть возможность создания и то же значение, если были переданы строки <xref:System.ComponentModel.TypeConverter.ConvertFrom%2A> реализацию того же преобразователя без значительной потери информации.  
  
 Если значение не может быть сериализовано или преобразователь не поддерживает сериализацию, <xref:System.ComponentModel.TypeConverter.ConvertTo%2A> должен вернуть реализацию `null`и разрешено в этом случае создаст исключение. Но при создании исключения, следует сообщать о невозможности использовать это преобразование в рамках вашей <xref:System.ComponentModel.TypeConverter.CanConvertTo%2A> реализацию, чтобы рекомендуется проверять с помощью <xref:System.ComponentModel.TypeConverter.CanConvertTo%2A> поддерживается сначала Чтобы избежать исключений.  
  
 Если `destinationType` параметр не относится к типу <xref:System.String>, можно выбрать собственную обработку преобразователя. Как правило, будет возвращаться к базовой обработке реализации, которая в возвращаетесь <xref:System.ComponentModel.TypeConverter.ConvertTo%2A> вызывает определенное исключение.  
