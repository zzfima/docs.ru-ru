...
## <a name="mapping-to-custom-classes-and-assemblies"></a>Сопоставление пользовательских классов и сборок  
 ...
 `assembly=`Сборка, содержащая часть или все упоминаемое пространство имен CLR. Это значение обычно содержит только имя сборки, но не путь к ней, и не включает расширение имени файла (например, EXE или DLL). Путь к этой сборке должен быть задан в качестве ссылки проекта в файле проекта, который содержит код XAML, который следует сопоставить. Чтобы включить управление версиями и подписывание строгих имен, `assembly` значением может быть строка <xref:System.Reflection.AssemblyName>, определенная, а не простое строковое имя.  
...
4. Для вызова <xref:System.Reflection.Assembly.Load%28System.String%29?displayProperty=nameWithType>используйте короткое имя + токен открытого ключа.  
  
5. Если имя не уточнено, вызовите <xref:System.Reflection.Assembly.LoadWithPartialName%2A?displayProperty=nameWithType>.  
  
 Свободный XAML не использует шаг 3, так как нет сборки, из которой выполнялась загрузка.  
  
 Скомпилированный XAML для WPF (сформированный с помощью ксамлбуилдтаск) не использует уже загруженные сборки <xref:System.AppDomain> из (шаг 1). Кроме того, имя из выходных данных XamlBuildTask не должно быть неполным, поэтому шаг 5 не применяется.  
