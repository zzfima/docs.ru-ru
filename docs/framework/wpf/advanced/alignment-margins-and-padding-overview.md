...
<xref:System.Windows.FrameworkElement> Класс предоставляет несколько свойств, которые используются для точного позиционирования дочерних элементов. В этом разделе обсуждаются четыре наиболее важных <xref:System.Windows.FrameworkElement.HorizontalAlignment%2A>свойства:, <xref:System.Windows.FrameworkElement.Margin%2A>, <xref:System.Windows.Controls.Border.Padding%2A>и <xref:System.Windows.FrameworkElement.VerticalAlignment%2A>. Очень важно иметь представление о результатах применения этих свойств, поскольку они обеспечивают основу для управления положением элементов в приложениях [!INCLUDE[TLA#tla_winclient](../../../../includes/tlasharptla-winclient-md.md)].  

<a name="wcpsdk_layout_amp_introduction"></a>   
## <a name="introduction-to-element-positioning"></a>Введение в позиционирование элементов  
 Существуют разные способы позиционирования элементов с помощью [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)]. Однако достижение идеального макета выходит за рамки простого выбора правильного <xref:System.Windows.Controls.Panel> элемента. Точное управление размещением <xref:System.Windows.FrameworkElement.HorizontalAlignment%2A>требует понимания свойств, <xref:System.Windows.FrameworkElement.Margin%2A>, <xref:System.Windows.Controls.Border.Padding%2A>и <xref:System.Windows.FrameworkElement.VerticalAlignment%2A> .  
 ...
 В следующем примере демонстрируется создание макета, показанного на предыдущем рисунке. Элемент инкапсулирует родителя <xref:System.Windows.Controls.StackPanel>, значение которого равно 15 аппаратно- <xref:System.Windows.Controls.Border.Padding%2A> независимых пикселей. <xref:System.Windows.Controls.Border> Эти учетные записи для <xref:System.Windows.Media.Brushes.LightBlue%2A> узких полос, охватывающих <xref:System.Windows.Controls.StackPanel>дочерний элемент. Дочерние элементы <xref:System.Windows.Controls.StackPanel> элемента используются для иллюстрации каждого из различных свойств позиционирования, подробно описанных в этом разделе. Для <xref:System.Windows.Controls.Button> демонстрации <xref:System.Windows.FrameworkElement.Margin%2A> свойств и<xref:System.Windows.FrameworkElement.HorizontalAlignment%2A> используются три элемента.  
...
> [!NOTE]
> Явно заданный <xref:System.Windows.FrameworkElement.Height%2A> параметр <xref:System.Windows.FrameworkElement.Width%2A> и свойства <xref:System.Windows.HorizontalAlignment.Stretch> элемента имеют приоритет над значением свойства. Попытка установить <xref:System.Windows.FrameworkElement.Height%2A>, <xref:System.Windows.FrameworkElement.Width%2A>и <xref:System.Windows.FrameworkElement.HorizontalAlignment%2A> значение `Stretch` ,еслизапрос`Stretch` пропускается.  
...
|<xref:System.Windows.HorizontalAlignment.Stretch>Параметры|Дочерние элементы растягиваются для заполнения выделенного пространства макета родительского элемента. Явные <xref:System.Windows.FrameworkElement.Width%2A> значения <xref:System.Windows.FrameworkElement.Height%2A> и имеют приоритет.|  
 ...
 Приведенный выше код создает макет, похожий на следующий рисунок. На рисунке видны эффекты <xref:System.Windows.FrameworkElement.HorizontalAlignment%2A> позиционирования каждого значения.  
...
|<xref:System.Windows.VerticalAlignment.Stretch>Параметры|Дочерние элементы растягиваются для заполнения выделенного пространства макета родительского элемента. Явные <xref:System.Windows.FrameworkElement.Width%2A> значения <xref:System.Windows.FrameworkElement.Height%2A> и имеют приоритет.|  
 ...
 Приведенный выше код создает макет, похожий на следующий рисунок. На рисунке видны эффекты <xref:System.Windows.FrameworkElement.VerticalAlignment%2A> позиционирования каждого значения.  
...
## <a name="understanding-margin-properties"></a>Основные сведения о свойствах Margin  
 <xref:System.Windows.FrameworkElement.Margin%2A> Свойство описывает расстояние между элементом и его дочерними или одноранговыми узлами. <xref:System.Windows.FrameworkElement.Margin%2A>значения могут быть однородными с помощью синтаксиса, такого как `Margin="20"`. Благодаря этому синтаксису элементу <xref:System.Windows.FrameworkElement.Margin%2A> будет применено универсальное из 20 аппаратно независимых пикселей. <xref:System.Windows.FrameworkElement.Margin%2A>значения могут также иметь форму из четырех различных значений, каждое из которых описывает отдельное поле для применения к левым, верхним, правым и нижним (в этом порядке), `Margin="0,10,5,25"`например. Правильное использование <xref:System.Windows.FrameworkElement.Margin%2A> свойства обеспечивает очень точное управление позицией отрисовки элемента и позицией отрисовки его соседних элементов и потомков.  
  
> [!NOTE]
> Ненулевое поле применяет пространство за пределами <xref:System.Windows.FrameworkElement.ActualHeight%2A>элемента <xref:System.Windows.FrameworkElement.ActualWidth%2A> и.  
...
## <a name="understanding-the-padding-property"></a>Основные сведения о свойстве Padding  
 Заполнение аналогично <xref:System.Windows.FrameworkElement.Margin%2A> в большинстве случаев. Свойство Padding предоставляется только в нескольких классах, в <xref:System.Windows.Documents.Block>основном как удобство: <xref:System.Windows.Controls.Control>, <xref:System.Windows.Controls.Border>, и <xref:System.Windows.Controls.TextBlock> — примеры классов, предоставляющих свойство заполнения. Свойство увеличивает эффективный размер дочернего элемента на указанное <xref:System.Windows.Thickness> значение. <xref:System.Windows.Controls.Border.Padding%2A>  
...
## <a name="using-alignment-margins-and-padding-in-an-application"></a>Использование свойств Alignment, Margin и Padding в приложении  
 <xref:System.Windows.FrameworkElement.HorizontalAlignment%2A>,, и [!INCLUDE[TLA#tla_ui](../../../../includes/tlasharptla-ui-md.md)] <xref:System.Windows.FrameworkElement.Margin%2A> предоставляютэлементуправленияположением,необходимыйдлясоздания<xref:System.Windows.FrameworkElement.VerticalAlignment%2A>сложногоэлемента. <xref:System.Windows.Controls.Border.Padding%2A> Эффекты каждого свойства позволяют изменить размещение дочерних элементов, обеспечивая гибкость создания динамических приложений и интерфейсов пользователя.  
  
 В следующем примере демонстрируются понятия, описанные в этом разделе. Основываясь на инфраструктуре в первом примере этого раздела, в этом примере <xref:System.Windows.Controls.Grid> элемент добавляется <xref:System.Windows.Controls.Border> в качестве дочернего элемента в первом примере. <xref:System.Windows.Controls.Border.Padding%2A>применяется к родительскому <xref:System.Windows.Controls.Border> элементу. Используется для разделения пространства между тремя дочерними <xref:System.Windows.Controls.StackPanel> элементами. <xref:System.Windows.Controls.Grid> <xref:System.Windows.Controls.Button>элементы снова используются для отображения различных эффектов <xref:System.Windows.FrameworkElement.Margin%2A> и. <xref:System.Windows.FrameworkElement.HorizontalAlignment%2A> <xref:System.Windows.Controls.TextBlock>к каждому <xref:System.Windows.Controls.ColumnDefinition> элементу добавляются элементы для лучшего определения различных свойств, применяемых <xref:System.Windows.Controls.Button> к элементам в каждом столбце.  
