...
## <a name="cameras-and-projections"></a>Камеры и проекции  
 Разработчики, работающие в [!INCLUDE[TLA2#tla_2d](../../../../includes/tla2sharptla-2d-md.md)], привыкли к размещению графических примитивов на двумерном экране. При создании сцены [!INCLUDE[TLA2#tla_3d](../../../../includes/tla2sharptla-3d-md.md)] важно помнить, что фактически создается представление [!INCLUDE[TLA2#tla_2d](../../../../includes/tla2sharptla-2d-md.md)] объектов [!INCLUDE[TLA2#tla_3d](../../../../includes/tla2sharptla-3d-md.md)]. Поскольку сцена [!INCLUDE[TLA2#tla_3d](../../../../includes/tla2sharptla-3d-md.md)] выглядит по-разному в зависимости от точки наблюдения, необходимо указать эту точку наблюдения. <xref:System.Windows.Media.Media3D.Camera> Позволяет указать этой точки зрения для [!INCLUDE[TLA2#tla_3d](../../../../includes/tla2sharptla-3d-md.md)] сцены.  
  
 Другой способ понимания того, как представляется сцена [!INCLUDE[TLA2#tla_3d](../../../../includes/tla2sharptla-3d-md.md)] на поверхности [!INCLUDE[TLA2#tla_2d](../../../../includes/tla2sharptla-2d-md.md)], — это описание сцены как проекции на поверхность просмотра. <xref:System.Windows.Media.Media3D.ProjectionCamera> Можно указать различные проекции и их свойства для изменения как наблюдатель видит [!INCLUDE[TLA2#tla_3d](../../../../includes/tla2sharptla-3d-md.md)] моделей. Объект <xref:System.Windows.Media.Media3D.PerspectiveCamera> указывает проекцию камера сцены.  Другими словами <xref:System.Windows.Media.Media3D.PerspectiveCamera> предоставляет случайной точки зрения.  Можно указать положение камеры в пространстве координат сцены, направление и поле зрения камеры и вектор, определяющий направление "вверх" в сцене. На следующей схеме показана <xref:System.Windows.Media.Media3D.PerspectiveCamera>в проекции.  
  
 <xref:System.Windows.Media.Media3D.ProjectionCamera.NearPlaneDistance%2A> И <xref:System.Windows.Media.Media3D.ProjectionCamera.FarPlaneDistance%2A> свойства <xref:System.Windows.Media.Media3D.ProjectionCamera> ограничить диапазон проекции камеры. Поскольку камеры могут быть расположены в любом месте сцены, фактически можно расположить камеру внутри модели или очень близко от нее, что усложняет правильное распознавание объекта.  <xref:System.Windows.Media.Media3D.ProjectionCamera.NearPlaneDistance%2A> позволяет указать минимальное расстояние от камеры, за которой объекты не отображаются.  И наоборот <xref:System.Windows.Media.Media3D.ProjectionCamera.FarPlaneDistance%2A> задает расстояние от камеры, за которой объекты не отображается, что гарантирует, что объекты слишком далеко стать распознаваемыми не будут включены в сцене.  
  
 ![Настройка камеры](../../../../docs/framework/wpf/graphics-multimedia/media/coordsystem-6.png "CoordSystem 6")  
Позиция камеры  
  
 <xref:System.Windows.Media.Media3D.OrthographicCamera> Указывает ортогональной проекции [!INCLUDE[TLA2#tla_3d](../../../../includes/tla2sharptla-3d-md.md)] модели [!INCLUDE[TLA2#tla_2d](../../../../includes/tla2sharptla-2d-md.md)] визуальная поверхность. Подобно другим камерам, она указывает позицию, направление просмотра и направление "вверх". В отличие от <xref:System.Windows.Media.Media3D.PerspectiveCamera>, но при этом <xref:System.Windows.Media.Media3D.OrthographicCamera> описывает проекцию, которая не включает ракурс. Другими словами <xref:System.Windows.Media.Media3D.OrthographicCamera> описывает призму, стороны которого параллельны, вместо одной стороны которой сходятся в точке камеры. На следующем рисунке показана та же модель, как просмотреть с помощью <xref:System.Windows.Media.Media3D.PerspectiveCamera> и <xref:System.Windows.Media.Media3D.OrthographicCamera>.  
  
 ![Ортогональная и перспективная проекции](../../../../docs/framework/wpf/graphics-multimedia/media/camera-projections4.png "Camera_projections4")  
...
## <a name="model-and-mesh-primitives"></a>Примитивы модели и сетки  
 ...
 [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] [!INCLUDE[TLA2#tla_3d](../../../../includes/tla2sharptla-3d-md.md)] Системе в данный момент предоставляет <xref:System.Windows.Media.Media3D.MeshGeometry3D> класс, которой можно указать любой geometry; в настоящее время не поддерживает стандартные [!INCLUDE[TLA2#tla_3d](../../../../includes/tla2sharptla-3d-md.md)] примитивы, такие как сферы или кубические формы. Начать создание <xref:System.Windows.Media.Media3D.MeshGeometry3D> , указав список вершин треугольников в качестве его <xref:System.Windows.Media.Media3D.MeshGeometry3D.Positions%2A> свойство. Каждая вершина задается как <xref:System.Windows.Media.Media3D.Point3D>.  (В [!INCLUDE[TLA#tla_xaml](../../../../includes/tlasharptla-xaml-md.md)] укажите это свойство в виде списка чисел, сгруппированных тройками и представляющих координаты каждой вершины.) В зависимости от геометрического объекта сетка может состоять из множества треугольников, некоторые из которых совместно используют общие углы (вершины). Чтобы нарисовать сетку правильно, приложению [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] необходимы сведения о том, какие вершины треугольников являются общими. Предоставить эти сведения, указав список индексов треугольник с <xref:System.Windows.Media.Media3D.MeshGeometry3D.TriangleIndices%2A> свойство. Этот список определяет порядок, в котором точки, указанные в <xref:System.Windows.Media.Media3D.MeshGeometry3D.Positions%2A> будут определять треугольник.  
  
 [!code-xaml[basic3d#Basic3DXAML3DN3](../../../../samples/snippets/xaml/VS_Snippets_Wpf/Basic3D/XAML/Window1.xaml#basic3dxaml3dn3)]  
  
 В приведенном выше примере <xref:System.Windows.Media.Media3D.MeshGeometry3D.Positions%2A> восемь вершин для определения сетки кубической Указывает список. <xref:System.Windows.Media.Media3D.MeshGeometry3D.TriangleIndices%2A> Свойство указывает список двенадцати групп по три индекса.  Каждое значение в списке определяет смещение в <xref:System.Windows.Media.Media3D.MeshGeometry3D.Positions%2A> списка.  Например, первые три вершин, заданные <xref:System.Windows.Media.Media3D.MeshGeometry3D.Positions%2A> список являются вершин (1,1,0), (0,1,0) и (0,0,0). Первые три индексами, заданными <xref:System.Windows.Media.Media3D.MeshGeometry3D.TriangleIndices%2A> список: 0, 2 и 1, который соответствует первому, в-третьих и втором этапах <xref:System.Windows.Media.Media3D.MeshGeometry3D.Positions%2A> списка. В результате первый треугольник, формирующий модель куба, будет составлен из вершин (1, 1, 0), (0, 1, 0) и (0, 0, 0), а оставшиеся одиннадцать треугольников будут определяться аналогичным образом.  
...
-   <xref:System.Windows.Media.Media3D.DiffuseMaterial> Указывает, кисти, которые будут применяться к модели, как будто рассеянным были горит этой модели. Использование класса DiffuseMaterial больше всего напоминает применение кистей непосредственно в моделях [!INCLUDE[TLA2#tla_2d](../../../../includes/tla2sharptla-2d-md.md)]; поверхности модели не отражают свет, как блестящие поверхности.  
  
-   <xref:System.Windows.Media.Media3D.SpecularMaterial> Указывает, кисти, которые будут применяться к модели, как если бы поверхность модели были жесткого или блестящий, способной отражать выделения. Степень, в который предложит текстуры гладкости или «блеска», можно задать, указав значение для <xref:System.Windows.Media.Media3D.SpecularMaterial.SpecularPower%2A> свойства.  
  
-   <xref:System.Windows.Media.Media3D.EmissiveMaterial> позволяет указать, как будто модели были испускал свет, равный цвету кисти будет применяться текстуры. Это не делает модель светящейся; однако это иначе влияет на затенение, чем если бы текстура была создана с помощью класса DiffuseMaterial или SpecularMaterial.  
  
 Для повышения производительности противоположные поверхности объекта <xref:System.Windows.Media.Media3D.GeometryModel3D> (грани, которые невидимы, поскольку находятся на противоположной стороне модели относительно камеры) удаляются из сцены.  Чтобы указать <xref:System.Windows.Media.Media3D.Material> для применения к противоположной модели, например плоскости, задайте модели <xref:System.Windows.Media.Media3D.GeometryModel3D.BackMaterial%2A> свойство.  
  
 Для достижения некоторых свойств поверхности, таких как свечение или эффект отражения, можно последовательно применить к модели несколько различных кистей. Можно применить и повторно использовать несколько материалов с помощью <xref:System.Windows.Media.Media3D.MaterialGroup> класса. Дочерние элементы класса MaterialGroup применяются от первого к последнему в нескольких проходах отрисовки.  
...
## <a name="illuminating-the-scene"></a>Освещение сцены  
 ...
 Следующие индикаторы являются производными от базового класса <xref:System.Windows.Media.Media3D.Light>:  
  
-   <xref:System.Windows.Media.Media3D.AmbientLight>-Обеспечивает от освещенности, освещает все объекты одинаково, независимо от их расположения или ориентации.  
  
-   <xref:System.Windows.Media.Media3D.DirectionalLight>Создает освещение, как удаленному источнику света.  Направленные источники света имеют <xref:System.Windows.Media.Media3D.DirectionalLight.Direction%2A> указан как объект Vector3D, но без заданного местоположения.  
  
-   <xref:System.Windows.Media.Media3D.PointLight>Создает освещение, как Ближнего источника света. Источники света PointLights занимают определенное положение и испускают свет из этого положения. Объекты на сцене освещаются в зависимости от их положения и расстояния относительно источника света. <xref:System.Windows.Media.Media3D.PointLightBase> предоставляет <xref:System.Windows.Media.Media3D.PointLightBase.Range%2A> свойство, которое определяет расстояние, после которого модели не будут освещены светом. Класс PointLight также предоставляет свойства затухания, определяющие интенсивность ослабления источника света в зависимости от расстояния. Можно указать константу, линейную или квадратичную интерполяцию затухания источника света.  
  
-   <xref:System.Windows.Media.Media3D.SpotLight>Наследуется от исключения <xref:System.Windows.Media.Media3D.PointLight>. Источники света Spotlight освещают сцену подобно источникам света и также имеют расположение и направление. Они проектируют свет в конус область, задаваемую <xref:System.Windows.Media.Media3D.SpotLight.InnerConeAngle%2A> и <xref:System.Windows.Media.Media3D.SpotLight.OuterConeAngle%2A> свойства, заданные в градусах.  
  
 Индикаторы должны <xref:System.Windows.Media.Media3D.Model3D> объектов, поэтому можно преобразовывать и анимировать светлой свойств, включая положение, цвет, направление и диапазон.  
...
## <a name="animating-models"></a>Анимация моделей  
 Реализация модели [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] в приложении [!INCLUDE[TLA2#tla_3d](../../../../includes/tla2sharptla-3d-md.md)] участвует в той же системе анимации и времени, что и [!INCLUDE[TLA2#tla_2d](../../../../includes/tla2sharptla-2d-md.md)] графика. Другими словами, для анимации трехмерной сцены необходимо анимировать свойства ее моделей. Можно непосредственно анимировать свойства примитивов, но обычно проще анимировать преобразования, изменяющие позицию или внешний вид моделей. Поскольку преобразования можно применять к <xref:System.Windows.Media.Media3D.Model3DGroup> объектов и отдельных моделей можно применить одного набора анимаций к дочернему элементу Model3DGroup и другой набор дочерних объектов к группе. Также можно добиться разнообразных визуальных эффектов, анимируя свойства элементов освещения сцены. Наконец, можно анимировать саму проекцию, изменяя положение камеры или поле зрения. Дополнительные сведения о системе времени и анимации в приложении [!INCLUDE[TLA2#tla_winclient](../../../../includes/tla2sharptla-winclient-md.md)] см. в разделах [Общие сведения об эффектах анимации](../../../../docs/framework/wpf/graphics-multimedia/animation-overview.md), [Общие сведения о Storyboard](../../../../docs/framework/wpf/graphics-multimedia/storyboards-overview.md) и [Общие сведения об объектах класса Freezable](../../../../docs/framework/wpf/advanced/freezable-objects-overview.md).  
...
## <a name="add-3-d-content-to-the-window"></a>Добавление трехмерного содержимого в окно  
 Для отображения сцены добавьте модели и источники света для <xref:System.Windows.Media.Media3D.Model3DGroup>, установите <xref:System.Windows.Media.Media3D.Model3DGroup> как <xref:System.Windows.Media.Media3D.ModelVisual3D.Content%2A> из <xref:System.Windows.Media.Media3D.ModelVisual3D>. Добавить <xref:System.Windows.Media.Media3D.ModelVisual3D> для <xref:System.Windows.Controls.Viewport3D.Children%2A> коллекцию <xref:System.Windows.Controls.Viewport3D>. Добавьте камеры к <xref:System.Windows.Controls.Viewport3D> , установив его <xref:System.Windows.Controls.Viewport3D.Camera%2A> свойство.  
  
 Наконец, добавьте <xref:System.Windows.Controls.Viewport3D> в окно. Когда <xref:System.Windows.Controls.Viewport3D> включено как содержимое элемента макета как холст, укажите размер Viewport3D, задав его <xref:System.Windows.FrameworkElement.Height%2A> и <xref:System.Windows.FrameworkElement.Width%2A> свойства (наследуется от <xref:System.Windows.FrameworkElement>).  
...
## <a name="see-also"></a>См. также  
 ...
 [Разделы практического руководства](../../../../docs/framework/wpf/graphics-multimedia/3-d-graphics-how-to-topics.md)  
