...
> [!IMPORTANT]
>  Метод не может являться универсальным только потому, что он принадлежит универсальному типу и использует параметры этого типа.  Метод является универсальным только в том случае, если он имеет свой собственный список параметров типа.  Большинство методов в универсальных типах не являются универсальными, как в этом примере.  Пример выпуска универсального метода см. в разделе [How to: Define a Generic Method with Reflection Emit](../../../docs/framework/reflection-and-codedom/how-to-define-a-generic-method-with-reflection-emit.md).  
  
### Чтобы определить универсальный тип  
  
1.  Определите динамическую сборку с именем `GenericEmitExample1`.  В этом примере сборка выполняется и сохраняется на диске, так что указан флаг <xref:System.Reflection.Emit.AssemblyBuilderAccess?displayProperty=fullName>.  
...
4.  Определите параметры универсального типа метода `Sample` посредством передачи массива строк, содержащего имена параметров, в метод <xref:System.Reflection.Emit.TypeBuilder.DefineGenericParameters%2A?displayProperty=fullName>.  Этот класс станет универсального типа.  Возвращаемое значение — это массив объектов <xref:System.Reflection.Emit.GenericTypeParameterBuilder>, представляющих параметры типа, который может использоваться при выпуске кода.  
  
     В следующем коде `Sample` становится универсальным типом с параметрами типа `TFirst` и `TSecond`.  Чтобы сделать код более удобным для прочтения, каждый объект <xref:System.Reflection.Emit.GenericTypeParameterBuilder> помещается в переменную с тем же именем, что и параметр типа.  
...
6.  Дополнительно добавьте ограничения класса и интерфейса в параметры типа.  В этом примере параметр типа `TFirst` ограничен типами, которые являются производными из базового класса, представленного объектом <xref:System.Type>, содержащимся в переменной `baseType`, а также которые реализуют интерфейсы, типы которых содержатся в переменных `interfaceA` и `interfaceB`.  Объявление и назначение этих переменных см. в примере кода.  
...
11. Вызовите метод.  `ExampleMethod` не является универсальным, однако тип, к которому он принадлежит, является универсальным. Поэтому для получения объекта <xref:System.Reflection.MethodInfo>, который может быть вызван, необходимо создать сконструированный тип из определения типа для `Sample`.  Сконструированный тип использует класс `Example`, который согласуется с ограничениями `TFirst`, так как это ссылочный тип, имеющий конструктор без параметров, а также класс `ExampleDerived`, который соответствует ограничениям `TSecond`. \(Код для `ExampleDerived` можно найти в разделе примера кода.\) Эти два типа передаются в метод <xref:System.Type.MakeGenericType%2A> для создания сконструированного типа.  Затем получается объект <xref:System.Reflection.MethodInfo> с помощью метода <xref:System.Type.GetMethod%2A>.  
...
12. В следующем коде создается массив объектов `Example`, затем этот массив размещается в массиве типа <xref:System.Object>, представленном аргументами вызываемого метода, после чего они передаются в метод [Invoke\(Object, Object\<xref:System.Reflection.MethodBase.Invoke%28System.Object%2CSystem.Object%5B%5D%29>.  Первый аргумент метода <xref:System.Reflection.MethodBase.Invoke%2A> — это пустая ссылка, так как метод является `static`.  
...
## Пример  
 ...
 Параметр типа `TSecond` используется для демонстрации ограничений класса и интерфейса с помощью базовых классов и интерфейсов, а параметр типа `TFirst` используется для демонстрации специальных ограничений.  
 ...
 Программа сохраняет завершенный модуль на диске как `GenericEmitExample1.dll`, чтобы можно было открыть его с помощью [Ildasm.exe \(IL Disassembler\)](../../../docs/framework/tools/ildasm-exe-il-disassembler.md) и изучить MSIL для класса `Sample`.  
...
## См. также  
 ...
 [Reflection Emit Dynamic Assembly Scenarios](http://msdn.microsoft.com/ru-ru/e1cc6750-e20f-473b-bb4e-f43bc66aecce)