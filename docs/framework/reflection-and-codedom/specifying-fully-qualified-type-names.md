...
Для выполнения различных операций отражения необходимо задать имена типов.  Полное имя типа состоит из спецификации имени сборки, спецификации пространства имен и имени типа.  Спецификации имен типов используются такими методами, как <xref:System.Type.GetType%2A?displayProperty=fullName>, <xref:System.Reflection.Module.GetType%2A?displayProperty=fullName>, <xref:System.Reflection.Emit.ModuleBuilder.GetType%2A?displayProperty=fullName> и <xref:System.Reflection.Assembly.GetType%2A?displayProperty=fullName>.  
...
|ReflectionEmitDimension    :\=   '\*'<br /><br /> &#124;     Number '..' Числовой<br /><br /> &#124;     Number '…'<br /><br /> &#124;     ReflectionDimension ',' ReflectionDimension<br /><br /> &#124;     NOTOKEN|  
|Number                            :\=   \[0\-9\]\+|  
...
|\\\\|Обратная косая черта, если она используется в качестве строкового литерала.|  
 ...
 Классы отражения, такие как <xref:System.Type.FullName%2A?displayProperty=fullName>, возвращает добавленное имя, которое можно использовать при вызове метода <xref:System.Type.GetType%2A>, например `MyType.GetType(myType.FullName)`.  
...
|------------------|--------------|-------------------------|  
|**Версия**|Номер версии сборки|*Major.Minor.Build.Revision*, где *Major*, *Minor*, *Build* и *Revision* являются целочисленными значениями в диапазоне от 0 до 65535 включительно.|  
...
|**PublicKeyToken**|Маркер открытого ключа \(8\-байтный хэш\-код полного открытого ключа\)|Строковое значение маркера открытого ключа в шестнадцатеричном формате.  Чтобы явным образом описать сборку как закрытую, необходимо задать пустую ссылку \(**Nothing** в Visual Basic\).|  
...
|**Пользовательское**|Пользовательский BLOB\-объект.  В настоящее время используется только в сборках, созданных [генератором образов в машинном коде \(Ngen\)](../../../docs/framework/tools/ngen-exe-native-image-generator.md).|Настраиваемая строка, с помощью которой средство создания образов в машинном коде уведомляет кэш сборок о том, что устанавливаемая сборка является образом в машинном коде и должна быть установлена в кэш образа в машинном коде.  Также называется zap\-строкой.|  
...
## Указание массивов  
 В БНФ спецификация ReflectionEmitDimension применяется только в отношении неполных типов, извлеченных с использованием метода <xref:System.Reflection.Emit.ModuleBuilder.GetType%2A?displayProperty=fullName>.  К определениям неполных типов относятся объекты <xref:System.Reflection.Emit.TypeBuilder>, созданные с использованием метода [Reflection.Emit](frlrfSystemReflectionEmit), для которых не был осуществлен вызов <xref:System.Reflection.Emit.TypeBuilder.CreateType%2A?displayProperty=fullName>.  Спецификацию ReflectionDimension можно использовать для получения любого полного определения типа, то есть типа, который уже загружен.  
