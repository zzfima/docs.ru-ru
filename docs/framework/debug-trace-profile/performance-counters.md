...
ms.locfileid: "79400971"
---
# <a name="performance-counters-in-the-net-framework"></a>Счетчики производительности в рамочном поле .NET

Эта тема содержит список счетчиков производительности, которые вы можете найти в [мониторе производительности Windows.](https://docs.microsoft.com/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc749249%28v=ws.11%29)  
...
|Счетчик производительности|Описание|  
|-------------------------|-----------------|  
|**% времени на загрузку**|Зарезервировано для последующего использования.|  
|**Длительность поиска сборки**|Зарезервировано для последующего использования.|  
...
|**Указателей сборки мусора**|Отображение текущего числа используемых указателей сборки мусора. Указатели сборки мусора — это указатели на ресурсы, внешние для среды CLR и управляемой среды.|  
...
|**Размер кучи больших объектов**|Отображает текущий размер, в байтах, большой кучи объекта. Объекты, превышаюющие примерно 85 000 байтов, рассматриваются сборщиком мусора как крупные объекты и непосредственно распределяются в специальной куче. Они не продвигаются из поколения в поколение. Этот счетчик обновляется в конце сборки мусора, не при каждом выделении памяти.|  
|**ИД процесса**|Отображает идентификатор процесса отслеживаемого процесса среды CLR.|  
...
|**Установлено подключений**|Совокупное общее число объектов <xref:System.Net.Sockets.Socket> для сокетов потока, к которым когда-либо выполнялось подключение в <xref:System.AppDomain> с момента запуска процесса.|  
...
- **Сбоев запросов HttpWebRequest/сек**  
 ...
 Счетчики производительности сети необходимо включить для использования в файле конфигурации. Все счетчики производительности сети включаются и отключаются с помощью одного параметра в файле конфигурации. Включить или отключить отдельные счетчики производительности сети невозможно. Для получения дополнительной информации [ \<см. производительностьCounter> Элемент (Настройки сети)](../configure-apps/file-schema/network/performancecounter-element-network-settings.md).  
...
|**% проверки подлинности по временным сигналам**|Зарезервировано для последующего использования.|  
...
- [Профилирование времени выполнения](runtime-profiling.md)
