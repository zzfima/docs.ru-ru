...
## <a name="cause"></a>Причина  
 Смешанные сборки с управляемым и неуправляемым кодом C++ для .NET Framework версий 1.0 и 1.1 обычно пытаются выполнять управляемый код во время блокировки загрузчика, если не предпринималось специальных действий, например связывания с помощью **/NOENTRY**.  Подробное описание этих проблем см. в разделе "Проблема загрузки смешанных библиотек DLL" в библиотеке MSDN.  
