...
- Улучшенное тестирование: Контракты кода обеспечивают проверку статических контрактов, проверка во время выполнения и создания документации.

- Средства автоматического тестирования: Контракты для кода можно использовать для создания более осмысленных модульных тестов, отфильтровывая бессмысленных аргументов тестирования, не удовлетворяющие предусловиям.

- Статической проверки: Средство статической проверки можно определить, имеет ли нарушения контракта, без запуска программы. Оно проверяет неявные контракты, такие как пустые разыменования и границы массивов, а также явные контракты.

- Справочная документация по: Генератор документации расширяет существующие XML-файлов документации с помощью сведений о контракте. Со средством [Sandcastle](https://github.com/EWSoftware/SHFB) также можно использовать таблицы стилей, чтобы сформированные страницы документации содержали разделы контрактов.
...
Сведения о средствах и подробные инструкции по использованию контрактов кода см. на странице [Контракты кода](https://go.microsoft.com/fwlink/?LinkId=152461) на веб-сайте MSDN DevLabs.
...
Обратите внимание, что условие в предыдущем тесте является предусловием с отрицанием. (Фактическим предусловием было бы `x != null`.) Предусловие с отрицанием сильно ограничено: Оно должно быть написано как показано в предыдущем примере; то есть оно не должно содержать `else` предложений и текст `then` предложение должен представлять собой одно `throw` инструкции. Тест `if` подчиняется правилам чистоты и видимости (см. раздел [Правила использования](#usage_guidelines)), но выражение `throw` подчиняется только правилам чистоты. Однако тип вызываемого исключения должен быть так же видим, как и метод, в котором возникает контракт.
