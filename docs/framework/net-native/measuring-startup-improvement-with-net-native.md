...
- Когда приложение завершило синхронизацию новых историй.  
 ...
 Инструментирование приложения — это просто: Просто вызовите соответствующий метод в производном классе. С помощью `AppEventSource` из предыдущего примера можно инструментировать приложение следующим образом:  
...
- В поле **Фильтр типов событий** укажите `Process/Start | MyCompany-MyApp`. Это задает фильтр событий из MyCompany MyApp и события Windows Kernel/Process/Start (Ядро/процесс/запуск).  
 ...
 Повторяя процедуру для исходного приложения и версии, созданной с помощью цепочки инструментов .NET Native, можно сравнить разницу в производительности.   .NET Native приложения обычно запускаются быстрее, чем non-.NET собственные приложения. Если вы заинтересованы в более глубокой информации, то PerfView также может определить части кода, на выполнение которых уходит больше всего времени. Дополнительные сведения см. в [руководствах по PerfView](https://channel9.msdn.com/Series/PerfView-Tutorial) или в [записи в блоге Вэнса Моррисона](https://blogs.msdn.microsoft.com/vancem/2011/12/28/publication-of-the-perfview-performance-analysis-tool/).  
