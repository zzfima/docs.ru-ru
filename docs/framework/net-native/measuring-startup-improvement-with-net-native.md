...
## <a name="using-eventsource-to-emit-events"></a>Использование EventSource для порождения событий  
 <xref:System.Diagnostics.Tracing.EventSource> Предоставляет базовый класс для создания пользовательского поставщика событий. Как правило, создается подкласс <xref:System.Diagnostics.Tracing.EventSource> и методы `Write*` помещаются в оболочку с вашими собственными методами событий. Обычно используется единый шаблон для каждого <xref:System.Diagnostics.Tracing.EventSource>.  
...
-   В поле **Фильтр типов событий** укажите `Process/Start | MyCompany-MyApp`. Это задает фильтр событий из MyCompany MyApp и события Windows Kernel/Process/Start (Ядро/процесс/запуск).   
 ...
 Повторяя процедуры для исходного приложения и версии, построенных с использованием цепочки инструментов [!INCLUDE[net_native](../../../includes/net-native-md.md)], можно сравнить разницу в производительности.   [!INCLUDE[net_native](../../../includes/net-native-md.md)] приложения обычно запускаются быстрее, чем отличных[!INCLUDE[net_native](../../../includes/net-native-md.md)] приложений. Если вы заинтересованы в более глубокой информации, то PerfView также может определить части кода, на выполнение которых уходит больше всего времени. Дополнительные сведения см. в [руководствах по PerfView](https://channel9.msdn.com/Series/PerfView-Tutorial) или в [записи в блоге Вэнса Моррисона](https://blogs.msdn.com/b/vancem/archive/2011/12/28/publication-of-the-perfview-performance-analysis-tool.aspx).  
