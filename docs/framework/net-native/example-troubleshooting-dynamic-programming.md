...
>  В этом разделе рассматривается предварительная версия программного обеспечения для разработчиков машинного кода .NET. Предварительную версию можно скачать на [веб-сайте Microsoft Connect](https://go.microsoft.com/fwlink/?LinkId=394611) (требуется регистрация).  
 ...
 Не все метаданных поиска ошибок в приложениях разработаны с помощью .NET Native результат цепочки инструментов к исключению.  Некоторые могут проявляться в непредсказуемом виде в приложении.  В следующем примере показано нарушение доступа, вызванное ссылкой на пустой объект:  
...
```  
 ...
 В динамическом программировании хорошей практикой при использовании API в группе .NET Native отражения является использование <xref:System.Type.GetType%2A?displayProperty=nameWithType> перегрузки, которые выдают исключение в случае сбоя.  
