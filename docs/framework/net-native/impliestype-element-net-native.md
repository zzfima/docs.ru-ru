...
---
# <a name="impliestype-element-net-native"></a>\<ПодразумеваетТип> элемент (.NET Родной)
...
|[\<Метод>](method-element-net-native.md)|Применяет политику отражения к методу.|  
...
```  
 ...
 Ниже приведен реальный пример, который применяется по крайней мере к одному сериализатору. Директивы фиксируют требование, которое отражается на чем-то `IList<`набранном как *что-то,* `>` также включает в себя отражение соответствующего `List<` *типа,* `>` не требуя аннотации к приложению.  
