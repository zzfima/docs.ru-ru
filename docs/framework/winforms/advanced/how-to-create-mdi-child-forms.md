...
В следующей процедуре вы используете Visual Studio для создания дочерней формы MDI, отображающий <xref:System.Windows.Forms.RichTextBox> управления, аналогично большинству текстовых приложений. Замена элемента управления <xref:System.Windows.Forms> на другие элементы управления, такие как <xref:System.Windows.Forms.DataGridView>, или на сочетание элементов управления позволяет создавать дочерние окна MDI (а также приложения MDI) с различными возможностями.
...
9. Вставьте код, аналогичный приведенному ниже, чтобы создать новую дочернюю форму MDI, когда пользователь щелкает **New** пункта меню.
   ...
   В C++, добавьте следующий `#include` директиву в начало формы Form1.h:
