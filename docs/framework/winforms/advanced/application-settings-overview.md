...
В этом разделе описывается создание и хранение параметров приложения и пользователей.  
 ...
 Параметры приложения позволяют разработчикам сохранять состояние своих приложений с помощью очень небольшого объема кода и является заменой динамических свойств в предыдущих версиях платформы .NET Framework. Параметры приложения содержат много улучшений по сравнению с динамическими свойствами, которые доступны только для чтения. Кроме того, в динамических свойствах используется позднее связывание, и для них требуется дополнительное программирование. Классы динамических свойств были сохранены в [!INCLUDE[dnprdnext](../../../../includes/dnprdnext-md.md)], но они являются лишь классами-оболочками для классов параметров приложения.  
...
## <a name="limitations-of-application-settings"></a>Ограничения параметров приложения  
 Нельзя использовать параметры приложения в неуправляемых приложениях, на котором размещена в .NET Framework. Параметры не будут работать в таких средах, как надстройки Visual Studio, C++ для Microsoft Office, элементы управления, размещенные в Internet Explorer, или надстройки и проекты Microsoft Outlook.  
