...
|<xref:System.Windows.Forms.OpenFileDialog>|Не меняется; зависит от языка операционной системы.|Не влияет.|Нет|  
