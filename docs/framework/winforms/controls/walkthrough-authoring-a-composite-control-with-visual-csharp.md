...
2.  В списке проектов [!INCLUDE[csprcs](../../../../includes/csprcs-md.md)] выберите шаблон проекта **Библиотека элементов управления Windows Forms**, введите `ctlClockLib` в поле **Имя** и нажмите кнопку **ОК**.  
...
3.  В обозревателе решений щелкните файл **UserControl1.cs** правой клавишей мыши и выберите команду **Переименовать**. Измените имя файла на `ctlClock.cs`. Чтобы переименовать все ссылки на элемент кода UserControl1, в соответствующем запросе нажмите кнопку **Да**.  
    ...
    > [!NOTE]
    >  По умолчанию, составного элемента управления наследует от <xref:System.Windows.Forms.UserControl> класс, предоставляемый системой. <xref:System.Windows.Forms.UserControl> Класс обеспечивает функциональность, необходимую всем составным элементам управления и реализует стандартные методы и свойства.  
...
2.  На **панели элементов** разверните узел **Общие элементы управления**, а затем дважды щелкните пункт **Метка**.  
     ...
     Объект <xref:System.Windows.Forms.Label> управления с именем `label1` добавляется в элемент управления в рабочей области конструктора.  
...
5.  В **конструктор компонентов**, нажмите кнопку **timer1**и задайте <xref:System.Windows.Forms.Timer.Interval%2A> свойства `1000` и <xref:System.Windows.Forms.Timer.Enabled%2A> свойства `true`.  
     ...
     <xref:System.Windows.Forms.Timer.Interval%2A> Свойство управляет частотой <xref:System.Windows.Forms.Timer> компонент тактов. При каждом такте `timer1` оно запускает код в событии `timer1_Tick`. Интервал представляет время между тактами в миллисекундах.  
...
3.  Добавить <xref:System.Windows.Forms.Label> элемента управления в составной элемент управления.  
  
4.  С помощью мыши, перетащите <xref:System.Windows.Forms.Label> управления сразу под поле отображения. В окне "Свойства" задайте следующие свойства.  
...
7.  В **элементов**, найдите и дважды щелкните **DateTimePicker** добавление <xref:System.Windows.Forms.DateTimePicker> управления в форму, а затем добавьте <xref:System.Windows.Forms.Label> управления, дважды щелкнув **метка**.  
...
13. В меню **Отладка** щелкните **Начать отладку**.  
     ...
     Запустится тестовая программа. Обратите внимание, что текущее время обновляется в `ctlAlarmClock` управления и что время начала, отображаются в <xref:System.Windows.Forms.DateTimePicker> элемента управления.  
  
14. Нажмите кнопку <xref:System.Windows.Forms.DateTimePicker> которой отображаются минуты часа.  
