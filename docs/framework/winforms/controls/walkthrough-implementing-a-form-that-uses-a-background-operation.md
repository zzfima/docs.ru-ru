...
Если у вас есть операция будет выполняться долго для завершения, и требуется пользовательский интерфейс (UI) перестает отвечать на запросы или блокировать, можно использовать <xref:System.ComponentModel.BackgroundWorker> класс для выполнения операции в другом потоке.
...
## <a name="create-a-form-that-uses-a-background-operation"></a>Создать форму, которая выполняется фоновая операция

1. В Visual Studio создайте проект приложения на основе Windows с именем `BackgroundWorkerExample` (**файл** > **New** > **проекта**  >  **Visual C#**  или **Visual Basic** > **классический рабочий стол** > **Windows Forms Приложение**).
...
## <a name="create-a-backgroundworker-with-the-designer"></a>Создание BackgroundWorker с помощью конструктора
...
## <a name="add-asynchronous-event-handlers"></a>Добавление обработчиков асинхронных событий
...
### <a name="implement-progress-reporting"></a>Реализация отчетов о ходе выполнения
...
### <a name="implement-support-for-cancellation"></a>Реализация поддержки отмены
...
Нажмите клавишу **F5** для компиляции и запуска приложения.
