...
### <a name="identifier-name-is-not-a-valid-identifier"></a>"\<идентификатор имени >" не является допустимым идентификатором
...
Сообщение об ошибке: "Запрошенный анализатор языка не установлен. Имя синтаксического анализатора языка —{0}"".
...
Сообщение об ошибке: "Произошло исключение при попытке создания экземпляра"\<имя объекта > ". Исключением было "\<строка\>исключения".
...
Сообщение об ошибке: "Другой редактор содержит"\<имя документа > "открыт в несовместимом режиме. Закройте редактор и повторите операцию ".
...
### <a name="another-editor-has-made-changes-to-document-name"></a>Другой редактор внес изменения в "\<Document Name >"
...
Сообщение об ошибке: "Файл открыт в другом редакторе в несовместимом режиме. Закройте редактор и повторите операцию ".

Это сообщение похоже на "другой редактор имеет\<имя документа >" открыть в несовместимом режиме ", но Visual Studio не удается определить имя файла. Чтобы исправить эту ошибку, закройте редактор, в котором открыт файл, и повторите попытку.

### <a name="array-rank-rank-in-array-is-too-high"></a>Слишком высокий ранг\<массива "ранг в массиве >"
...
Сообщение об ошибке: Не удалось открыть\<сборку "имя сборки >". Убедитесь, что файл по-прежнему существует. "
...
### <a name="cannot-bind-an-event-handler-to-the-event-name-event-because-it-is-read-only"></a>Не удается привязать обработчик событий к событию "\<Event name >", так как оно доступно только для чтения
...
### <a name="cannot-name-the-object-name-because-it-is-already-named-name"></a>Не удается присвоить\<имя объекту "имя >", так как\<он уже назван "name >"
...
### <a name="category-toolbox-tab-name-does-not-have-a-tool-for-class-class-name"></a>Категория "\<имя вкладки панели элементов >" не содержит средство для класса "\<имя класса >"
...
### <a name="class-class-name-has-no-matching-constructor"></a>Класс "\<class name >" не имеет соответствующего конструктора
...
### <a name="component-component-name-did-not-call-containeradd-in-its-constructor"></a>Компоненту\<"имя компонента >" не удалось вызвать контейнер. Add () в его конструкторе
...
Чтобы исправить эту ошибку, обратитесь к поставщику компонента. Или, если это созданный компонент, вызовите `IContainer.Add` метод в конструкторе компонента.
...
### <a name="error-reading-resources-from-the-resource-file-for-the-culture-culture-name"></a>Ошибка при чтении ресурсов из файла ресурсов для языка и\<региональных параметров "имя культуры" > "
...
### <a name="error-reading-resources-from-the-resource-file-for-the-default-culture-culture-name"></a>Ошибка при чтении ресурсов из файла ресурсов для языка и региональных\<параметров по умолчанию "имя культуры >"
...
### <a name="failed-to-parse-method-method-name"></a>Не удалось выполнить синтаксический\<анализ метода "имя метода >"
...
При определении класса в нескольких файлах с помощью ключевого слова [partial](/dotnet/csharp/language-reference/keywords/partial-type) можно иметь только одно частичное определение в каждом файле.
...
Эта ошибка похожа на "тип"\<имя типа > "не найден", но эта ошибка обычно возникает из-за атрибута метаданных. Чтобы исправить эту ошибку, убедитесь, что имеются ссылки на все сборки, используемые атрибутами.
...
### <a name="the-base-class-class-name-cannot-be-designed"></a>Базовый класс "\<имя класса >" не может быть спроектирован
...
### <a name="the-component-cannot-be-added-because-it-contains-a-circular-reference-to-reference-name"></a>Невозможно добавить компонент, так как он содержит циклическую ссылку на "\<Reference Name >"
...
Эта ошибка возникла из-за того, что базовый класс объекта, переданного в конструктор, является [абстрактным](/dotnet/csharp/language-reference/keywords/abstract), что недопустимо.
...
Сообщение об ошибке: "Класс средства синтаксического анализа языка\<" имя класса > "не реализован должным образом. Обратитесь к поставщику за обновленным модулем синтаксического анализатора.
...
Visual Studio попытался создать компонент, но созданный объект не реализует <xref:System.ComponentModel.IComponent> интерфейс. Обратитесь к поставщику компонента за исправлением.
...
### <a name="the-service-instance-must-derive-from-or-implement-interface-name"></a>Экземпляр службы должен быть производным от или реализовывать "\<Interface Name >"
...
### <a name="the-toolbox-item-for-component-name-could-not-be-retrieved-from-the-toolbox"></a>Не удалось извлечь элемент панели\<элементов для "имя компонента >" из панели элементов
...
### <a name="there-is-already-a-command-handler-for-the-menu-command-menu-command-name"></a>Уже существует обработчик команд для команды меню "\<имя команды меню >"
...
### <a name="there-is-already-a-component-named-component-name"></a>Уже существует компонент с именем "\<Component name >"

Сообщение об ошибке: "Уже существует компонент с именем"\<Component name > ". Компоненты должны иметь уникальные имена, а имена не должны быть чувствительны к регистру. Имя также не может конфликтовать с именем любого компонента в унаследованном классе. "
...
### <a name="there-is-already-a-toolbox-item-creator-registered-for-the-format-format-name"></a>Для формата "\<имя формата >" уже зарегистрирован создатель элемента панели элементов
...
Сообщение об ошибке: "Не удалось добавить\<ссылку на ссылочное имя >" в текущее приложение. Убедитесь, что другая версия "\<Reference Name >" еще не указана. "
...
Сообщение об ошибке: "Не удалось извлечь текущий файл. Файл может быть заблокирован или может потребоваться извлечь его вручную. "
...
### <a name="unable-to-find-page-named-options-dialog-box-tab-name"></a>Не удается найти страницу "Параметры\<" имя вкладки диалогового окна > "
...
### <a name="unable-to-find-property-property-name-on-page-options-dialog-box-tab-name"></a>Не удалось найти свойство "\<имя свойства >" в диалоговом\<окне "Параметры страницы" имя вкладки > "
