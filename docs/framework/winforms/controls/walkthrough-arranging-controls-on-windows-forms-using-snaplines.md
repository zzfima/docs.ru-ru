...
Точное расположение элементов управления на форме является важным для многих приложений. Конструктор Windows Forms предоставляет множество инструментов макета для решения этой задачи. Одним из наиболее важных является <xref:System.Windows.Forms.Design.Behavior.SnapLine> функция.
 ...
 Линии привязки точно показывают, куда выровнять элементы управления с другими элементами управления. Они также показывают Рекомендуемые расстояния для полей между элементами управления, как указано в руководстве по пользовательскому интерфейсу Windows. Дополнительные сведения см. в разделе [проектирование и разработка пользовательского интерфейса](https://go.microsoft.com/FWLink/?LinkId=83878).
...
- Использование отступов и выравнивание элементов управления с помощью линий привязки

- Выровняйте до полей формы и контейнера

- Выровняйте по сгруппированным элементам управления

- Использование линий привязки для размещения элемента управления путем структурирования его размера
...
- Совмещение метки с текстом элемента управления
...
- Отключение линий привязки
 ...
 По завершении вы получите представление о роли макета, которая воспроизводится функцией привязки.
...
1. Создайте проект приложения на основе Windows с именем "снаплиниксампле" (**файл** > **создать** > **проект** >  > **визуальный C#**  элемент или классическую **Visual Basic**Desktop > **Windows Forms приложение**).

2. Выберите форму в конструкторе форм.

## <a name="spacing-and-aligning-controls-using-snaplines"></a>Использование отступов и выравнивание элементов управления с помощью линий привязки
...
## <a name="aligning-to-form-and-container-margins"></a>Выровняйте до полей формы и контейнера
 Линии привязки позволяют согласовать элементы управления с полями формы и контейнера согласованным образом.

### <a name="to-align-controls-to-form-and-container-margins"></a>Выровняйте элементы управления по полям формы и контейнера
...
2. <xref:System.Windows.Forms.Button> Переместить элемент управления ближе к правой границе формы, пока не появится линия привязки. Это расстояние теперь задается значением <xref:System.Windows.Forms.Control.Padding%2A> свойства формы.

3. Перетащите элемент управления <xref:System.Windows.Forms.GroupBox> из **панели элементов** в свою форму.

4. Измените значение <xref:System.Windows.Forms.GroupBox> <xref:System.Windows.Forms.Control.Padding%2A> свойства элемента управления, развернув <xref:System.Windows.Forms.Control.Padding%2A> <xref:System.Windows.Forms.Padding.All%2A> запись в окне " **Свойства** " и установив свойство равным 10.

5. Перетащите элемент управления из <xref:System.Windows.Forms.GroupBox> **панели элементов** в элемент управления. <xref:System.Windows.Forms.Button>

6. Переместить элемент управления ближе к правой границе <xref:System.Windows.Forms.GroupBox> элемента управления, пока не появится линия привязки. <xref:System.Windows.Forms.Button> Переместите элемент управления <xref:System.Windows.Forms.GroupBox> в элементе управления и обратите внимание на то, где появляются линии привязки. <xref:System.Windows.Forms.Button>

## <a name="aligning-to-grouped-controls"></a>Выровняйте по сгруппированным элементам управления
...
### <a name="to-align-to-grouped-controls"></a>Выровняйте по сгруппированным элементам управления
...
## <a name="using-snaplines-to-place-a-control-by-outlining-its-size"></a>Использование линий привязки для размещения элемента управления путем структурирования его размера
 Линии привязки позволяют выстроить элементы управления при их первом помещении в форму.

### <a name="to-use-snaplines-to-place-a-control-by-outlining-its-size"></a>Использование линий привязки для размещения элемента управления путем структурирования его размера
...
## <a name="using-snaplines-when-dragging-a-control-from-the-toolbox"></a>Использование линий привязки при перетаскивании элемента управления из панели элементов
 Линии привязки позволяют выровняйте элементы управления при перетаскивании их с **панели инструментов** на форму.

### <a name="to-use-snaplines-when-dragging-a-control-from-the-toolbox"></a>Использование линий привязки при перетаскивании элемента управления из панели элементов
...
## <a name="resizing-controls-using-snaplines"></a>Изменение размеров элементов управления с помощью линий привязки
 Линии привязки позволяют выстроить элементы управления при изменении их размера.

### <a name="to-resize-a-control-using-snaplines"></a>Изменение размера элемента управления с помощью линий привязки
...
## <a name="aligning-a-label-to-a-controls-text"></a>Совмещение метки с текстом элемента управления
 Некоторые элементы управления предлагают линию привязки для согласования других элементов управления с отображаемым текстом.

### <a name="to-align-a-label-to-a-controls-text"></a>Выровняйте метку по тексту элемента управления
...
## <a name="using-snaplines-with-keyboard-navigation"></a>Использование линий привязки с навигацией с помощью клавиатуры
 Линии привязки позволяют выстроить элементы управления при их размещении с помощью клавиш со стрелками клавиатуры.

### <a name="to-use-snaplines-with-keyboard-navigation"></a>Использование линий привязки с навигацией с помощью клавиатуры
...
7. Нажмите Shift + Any клавиша со стрелкой, чтобы <xref:System.Windows.Forms.Button> изменить размер элемента управления с шагом в 1 пиксель.

8. Нажмите сочетание клавиш CTRL + SHIFT + Any клавиша со стрелкой <xref:System.Windows.Forms.Button> , чтобы изменить размер элемента управления в приращениях линии привязки.

## <a name="snaplines-and-layout-panels"></a>Панели линий привязки и макета
 Линии привязки отключены в панелях макета.

### <a name="to-selectively-disable-snaplines"></a>Выборочное отключение линий привязки
...
## <a name="disabling-snaplines"></a>Отключение линий привязки
 Линии привязки включены по умолчанию. Можно отключить линии привязки выборочно или отключить их в среде разработки.

### <a name="to-selectively-disable-snaplines"></a>Выборочное отключение линий привязки

- Нажмите клавишу ALT и при перемещении элемента управления вокруг формы.

     Обратите внимание, что линии привязки не отображаются, и элемент управления не привязывается к потенциальным позициям выравнивания.

### <a name="to-disable-snaplines-in-the-design-environment"></a>Отключение линий привязки в среде разработки

1. В меню " **Сервис** " Откройте диалоговое окно " **Параметры** ". Откройте диалоговое окно конструктор Windows Forms. Дополнительные сведения см. в разделе [Общие, конструктор Windows Forms, диалоговое окно "Параметры"](https://docs.microsoft.com/previous-versions/visualstudio/visual-studio-2010/5aazxs78(v=vs.100)).
...
- Создайте столбец <xref:System.Windows.Forms.TextBox> элементов управления и соответствующий <xref:System.Windows.Forms.Label> столбец элементов управления. Присвойте <xref:System.Windows.Forms.Label> <xref:System.Windows.Forms.Control.AutoSize%2A> свойству`true`элементов управления значение. Используйте линии привязки для перемещения <xref:System.Windows.Forms.Label> элементов управления, чтобы их отображаемый текст был согласован с текстом <xref:System.Windows.Forms.TextBox> в элементах управления.
 ...
 Сведения о проектировании пользовательского интерфейса Windows см. в книге *взаимодействие с пользователем Microsoft Windows, официальные руководства для разработчиков пользовательского интерфейса и конструкторов* Redmond, WA: Microsoft Press, 1999. (УСБН: 0-7356-0566-1).
