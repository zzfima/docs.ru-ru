...
5.  В конструкторе компонентов щелкните **Timer1**и задайте <xref:System.Windows.Forms.Timer.Interval%2A> свойства `1000` и <xref:System.Windows.Forms.Timer.Enabled%2A> свойства `True`.  
     ...
     <xref:System.Windows.Forms.Timer.Interval%2A> Свойство управляет частотой, с которой такты компонента timer. При каждом такте `Timer1` оно запускает код в событии `Timer1_Tick`. Интервал представляет время между тактами в миллисекундах.  
...
9. В **элементов**, найдите и дважды щелкните **DateTimePicker** добавление <xref:System.Windows.Forms.DateTimePicker> управления в форму, а затем добавьте <xref:System.Windows.Forms.Label> управления, дважды щелкнув **метка**.  
...
16. Нажмите кнопку <xref:System.Windows.Forms.DateTimePicker> которой отображаются минуты часа.  
