...
2. В списке проектов Visual Basic выберите шаблон проект **библиотеки элементов управления Windows** , введите `ctlClockLib` в поле **имя** и нажмите кнопку **ОК**.
...
5. В конструкторе компонентов щелкните **timer1**, а затем <xref:System.Windows.Forms.Timer.Interval%2A> присвойте <xref:System.Windows.Forms.Timer.Enabled%2A> свойству `1000` значение, а свойству `True`— значение.

     <xref:System.Windows.Forms.Timer.Interval%2A> Свойство определяет частоту, с которой компонент Timer тикает. При каждом такте `Timer1` оно запускает код в событии `Timer1_Tick`. Интервал представляет время между тактами в миллисекундах.
...
9. На **панели элементов**найдите и дважды щелкните элемент **DateTimePicker** , чтобы добавить его <xref:System.Windows.Forms.DateTimePicker> в <xref:System.Windows.Forms.Label> форму, а затем добавьте элемент управления, дважды щелкнув надпись.
...
16. Щелкните, <xref:System.Windows.Forms.DateTimePicker> где отображаются минуты часа.
