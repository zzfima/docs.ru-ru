...
## <a name="example"></a>Пример  
 Во втором примере кода процедура выполняется каждые 600 миллисекунд до завершения цикла. В следующем примере кода требуется форма с элементом управления <xref:System.Windows.Forms.Button> с именем `Button1`, <xref:System.Windows.Forms.Timer>ным элементом управления с именем `Timer1`и элементом управления <xref:System.Windows.Forms.Label> с именем `Label1`.  
