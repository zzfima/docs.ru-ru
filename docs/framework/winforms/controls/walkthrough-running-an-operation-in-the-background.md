...
## <a name="run-an-operation-in-the-background"></a>Выполнение операции в фоновом режиме

1. Форма будет открыта в конструкторе Windows Forms в Visual Studio, перетащите два <xref:System.Windows.Forms.Button> управляет из **элементов** форму, а затем задайте `Name` и <xref:System.Windows.Forms.Control.Text%2A> свойства кнопок согласно следующие таблицы.
...
15. Нажмите клавишу **F6** для построения решения, а затем нажмите клавишу **Ctrl**+**F5** для запуска приложения вне отладчика.
...
16. Нажмите кнопку **запустить** для запуска асинхронной операции, а затем нажмите кнопку **отменить** кнопка для остановки асинхронной операции.
