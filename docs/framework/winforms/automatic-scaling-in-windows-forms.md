...
-   Так как свойство <xref:System.Windows.Forms.Form.AutoScaleBaseSize%2A> представляет базовый размер шрифта в виде целого значения, погрешности округления становились заметны, когда форма проходила через ряд разрешений.  
...
4.  Метод <xref:System.Windows.Forms.ContainerControl.PerformAutoScale%2A> также вызывается автоматически в перечисленных ниже ситуациях.  
    ...
    -   В ответ на событие <xref:System.Windows.Forms.Control.OnFontChanged%2A>, если используется режим масштабирования <xref:System.Windows.Forms.AutoScaleMode>.  
