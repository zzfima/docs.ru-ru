...
- интерфейс <xref:System.Collections.IList>
  ...
  Класс, реализующий интерфейс <xref:System.Collections.IList>, может быть <xref:System.Array>, <xref:System.Collections.ArrayList> или <xref:System.Collections.CollectionBase>. Это индексированные списки элементов типа <xref:System.Object>. Эти списки должны содержать однородные типы, так как первый элемент индекса определяет тип. <xref:System.Collections.IList> будет доступен для привязки только во время выполнения.

  > [!NOTE]
  > Если необходимо создать список бизнес-объектов для привязки с Windows Forms, рекомендуется использовать <xref:System.ComponentModel.BindingList%601>. @No__t-0 — это расширяемый класс, реализующий основные интерфейсы, необходимые для двусторонней Windows Forms привязки данных.
...
- интерфейс <xref:System.ComponentModel.IBindingList>
  ...
  > [!NOTE]
  > @No__t-0 предоставляет универсальную реализацию интерфейса <xref:System.ComponentModel.IBindingList>.
...
- интерфейс <xref:System.ComponentModel.IEditableObject>
  ...
  Класс, реализующий интерфейс <xref:System.ComponentModel.IEditableObject>, позволяет объекту управлять тем, когда изменения в этом объекте сделаны постоянными. Эта реализация предоставляет методы <xref:System.ComponentModel.IEditableObject.BeginEdit%2A>, <xref:System.ComponentModel.IEditableObject.EndEdit%2A> и <xref:System.ComponentModel.IEditableObject.CancelEdit%2A>, которые позволяют откатить изменения, внесенные в объект. Ниже приведено краткое описание работы методов <xref:System.ComponentModel.IEditableObject.BeginEdit%2A>, <xref:System.ComponentModel.IEditableObject.EndEdit%2A> и <xref:System.ComponentModel.IEditableObject.CancelEdit%2A> и способов их совместного использования, чтобы обеспечить возможность отката изменений, внесенных в данные:

  - Метод <xref:System.ComponentModel.IEditableObject.BeginEdit%2A> сигнализирует о начале редактирования объекта. Объект, реализующий этот интерфейс, должен сохранить все обновления после вызова метода <xref:System.ComponentModel.IEditableObject.BeginEdit%2A> таким образом, чтобы обновления можно было отменять при вызове метода <xref:System.ComponentModel.IEditableObject.CancelEdit%2A>. В Windows Forms привязки данных можно вызвать <xref:System.ComponentModel.IEditableObject.BeginEdit%2A> несколько раз в области действия одной операции изменения (например, <xref:System.ComponentModel.IEditableObject.BeginEdit%2A>, <xref:System.ComponentModel.IEditableObject.BeginEdit%2A>, <xref:System.ComponentModel.IEditableObject.EndEdit%2A>). Реализации <xref:System.ComponentModel.IEditableObject> должны отследить, был ли уже вызван <xref:System.ComponentModel.IEditableObject.BeginEdit%2A>, и пропустить последующие вызовы <xref:System.ComponentModel.IEditableObject.BeginEdit%2A>. Поскольку этот метод может вызываться несколько раз, важно, чтобы последующие вызовы этого метода были необратимыми; то есть последующие вызовы <xref:System.ComponentModel.IEditableObject.BeginEdit%2A> не могут уничтожить произведенные обновления или изменить данные, сохраненные при первом вызове <xref:System.ComponentModel.IEditableObject.BeginEdit%2A>.

  - Метод <xref:System.ComponentModel.IEditableObject.EndEdit%2A> помещает все изменения с момента вызова <xref:System.ComponentModel.IEditableObject.BeginEdit%2A> в базовый объект, если объект находится в режиме редактирования.
...
- интерфейс <xref:System.ComponentModel.IDataErrorInfo>
  ...
  - Свойство <xref:System.ComponentModel.IDataErrorInfo.Item%2A> возвращает строку с конкретным сообщением об ошибке из столбца (например, "значение в столбце `State` недопустимо").
...
- интерфейс <xref:System.ComponentModel.ITypedList>
  ...
  Класс коллекций, реализующий интерфейс <xref:System.ComponentModel.ITypedList>, дает возможность управлять порядком и набором свойств, предоставляемых привязанному элементу управления.
...
- интерфейс <xref:System.ComponentModel.ICustomTypeDescriptor>
  ...
  Класс, реализующий интерфейс <xref:System.ComponentModel.ICustomTypeDescriptor>, предоставляет динамическую информацию о себе. Этот интерфейс аналогичен <xref:System.ComponentModel.ITypedList>, но используется для объектов, а не списков. Этот интерфейс используется <xref:System.Data.DataRowView> для проецирования схемы базовых строк. Простая реализация <xref:System.ComponentModel.ICustomTypeDescriptor> предоставляется классом <xref:System.ComponentModel.CustomTypeDescriptor>.
...
- интерфейс <xref:System.ComponentModel.IListSource>
  ...
  Класс, реализующий интерфейс <xref:System.ComponentModel.IListSource>, включает привязку на основе списка для объектов, не являющихся списками. Метод <xref:System.ComponentModel.IListSource.GetList%2A> класса <xref:System.ComponentModel.IListSource> используется для возврата списка с возможностью привязки из объекта, который не наследуется от <xref:System.Collections.IList>. <xref:System.ComponentModel.IListSource> используется классом <xref:System.Data.DataSet>.

- интерфейс <xref:System.ComponentModel.IRaiseItemChangedEvents>

  Класс, реализующий интерфейс <xref:System.ComponentModel.IRaiseItemChangedEvents>, является связываемым списком, который также реализует интерфейс <xref:System.ComponentModel.IBindingList>. Этот интерфейс используется для указания, вызывает ли тип события <xref:System.ComponentModel.IBindingList.ListChanged> типа <xref:System.ComponentModel.ListChangedType.ItemChanged> через его свойство <xref:System.ComponentModel.IRaiseItemChangedEvents.RaisesItemChangedEvents%2A>.

  > [!NOTE]
  > Необходимо реализовать <xref:System.ComponentModel.IRaiseItemChangedEvents>, если источник данных предоставляет свойство для перечисления событий, описанное ранее, и взаимодействует с компонентом <xref:System.Windows.Forms.BindingSource>. В противном случае <xref:System.Windows.Forms.BindingSource> также будет выполнять преобразование свойства в список, что приводит к снижению производительности.

- интерфейс <xref:System.ComponentModel.ISupportInitialize>

  Компонент, реализующий интерфейс <xref:System.ComponentModel.ISupportInitialize>, использует преимущества пакетной оптимизации для настройки свойств и инициализации свойств, зависящих от друга. @No__t-0 содержит два метода:
...
- интерфейс <xref:System.ComponentModel.ISupportInitializeNotification>
  ...
  Компонент, реализующий интерфейс <xref:System.ComponentModel.ISupportInitializeNotification>, также реализует интерфейс <xref:System.ComponentModel.ISupportInitialize>. Этот интерфейс позволяет уведомлять другие компоненты <xref:System.ComponentModel.ISupportInitialize> о завершении инициализации. Интерфейс <xref:System.ComponentModel.ISupportInitializeNotification> содержит два члена:

  - <xref:System.ComponentModel.ISupportInitializeNotification.IsInitialized%2A> возвращает значение `boolean`, указывающее, инициализирован ли компонент.

  - <xref:System.ComponentModel.ISupportInitializeNotification.Initialized> происходит при вызове <xref:System.ComponentModel.ISupportInitialize.EndInit%2A>.

- интерфейс <xref:System.ComponentModel.INotifyPropertyChanged>

  Класс, реализующий этот интерфейс, представляет собой тип, который вызывает событие при изменении любого значения его свойств. Этот интерфейс предназначен для замены шаблона, в рамках которого имеется отдельное событие изменения для каждого свойства элемента управления. При использовании в <xref:System.ComponentModel.BindingList%601> бизнес-объект должен реализовать интерфейс <xref:System.ComponentModel.INotifyPropertyChanged>, а BindingList @ no__t-21 будет преобразовывать события <xref:System.ComponentModel.INotifyPropertyChanged.PropertyChanged> в <xref:System.ComponentModel.BindingList%601.ListChanged> типа <xref:System.ComponentModel.ListChangedType.ItemChanged>.

  > [!NOTE]
  > Чтобы уведомление об изменениях происходило в привязке между привязанным клиентом и источником данных, тип привязанного источника данных должен либо реализовать интерфейс <xref:System.ComponentModel.INotifyPropertyChanged> (предпочтительно), либо предоставить *propertyName*`Changed` события для связанного типа, но вы не следует выполнять оба действия.
...
- интерфейс <xref:System.Windows.Forms.IBindableComponent>
  ...
  > [!NOTE]
  > Если компонент наследуется от <xref:System.Windows.Forms.Control>, реализовывать интерфейс <xref:System.Windows.Forms.IBindableComponent> не требуется.
...
- интерфейс <xref:System.Windows.Forms.ICurrencyManagerProvider>
  ...
  > [!NOTE]
  > Класс, который наследует от <xref:System.Windows.Forms.Control>, управляет привязками автоматически с помощью свойства <xref:System.Windows.Forms.Control.BindingContext%2A>, поэтому в случаях, когда необходимо реализовать <xref:System.Windows.Forms.ICurrencyManagerProvider>, достаточно редко.

## <a name="see-also"></a>См. также

- [Привязка данных и Windows Forms](data-binding-and-windows-forms.md)
- [Практическое руководство. Создание элемента управления с простой привязкой в форме Windows @ no__t-0
