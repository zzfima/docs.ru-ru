...
CCW представляет public, интерфейсах, видимых для COM, типы данных и возвращаемые значения COM-клиентам таким способом, который согласуется с реализацией средствами COM взаимодействия на основе интерфейса. Для клиента COM вызов методов в объекте .NET Framework идентичен вызову методов в COM-объекте.  
...
|Интерфейс|Описание|  
|---------------|-----------------|  
|(\_*Classname*) интерфейс класса|Интерфейс, предоставляемый средой выполнения и не определенный явным образом, который обеспечивает доступ ко всем открытым интерфейсам, методам, свойствам и полям, явно предоставляемым в управляемом объекте.|  
