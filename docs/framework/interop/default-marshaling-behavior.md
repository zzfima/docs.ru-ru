...
> [!NOTE]
>  Маршалинг универсальных типов не поддерживается.  Подробнее см. в разделе [Interoperating Using Generic Types](http://msdn.microsoft.com/ru-ru/26b88e03-085b-4b53-94ba-a5a9c709ce58).  
...
3.  Если упаковщик все еще не может идентифицировать класс, то он инкапсулирует интерфейс в оболочку универсального класса с именем **System.\_\_ComObject**.  
...
-   Для COM\-взаимодействия делегат по умолчанию маршалируется как COM\-интерфейс типа **\_Delegate**.  Интерфейс **\_Delegate** определяется в библиотеке типов Mscorlib.tlb и содержит метод <xref:System.Delegate.DynamicInvoke%2A?displayProperty=fullName>, который позволяет вызывать метод, на который ссылается делегат.  
...
## Маршалинг по умолчанию для типов значений  
 Большинство типов значений, такие как целые числа и числа с плавающей запятой, являются [непреобразуемыми](../../../docs/framework/interop/blittable-and-non-blittable-types.md) и не требуют маршалинга.  [Преобразуемые](../../../docs/framework/interop/blittable-and-non-blittable-types.md) типы представлены в управляемой и неуправляемой памяти по\-разному и требуют маршалинга.  Другие типы требуют явного форматирования при пересечении границы взаимодействия.  
...
-   [Типы значений, используемые во COM\-взаимодействии](#cpcondefaultmarshalingforvaluetypesanchor3)  
...
### Системные типы значений  
 Пространство имен <xref:System> содержит несколько типов значений, представляющих собой упакованную форму простых типов среды выполнения.  Например, структура типов значений <xref:System.Int32?displayProperty=fullName> представляет собой упакованную форму **ELEMENT\_TYPE\_I4**.  Вместо маршалинга этих типов в качестве структур, как в случае с другими форматированными типами, их следует маршалировать так же, как и соответствующие простые типы.  Соответственно, **System.Int32** маршалируется как **ELEMENT\_TYPE\_I4**, а не как структура, содержащая один член типа **long**.  В таблице ниже приведен список типов значений в пространстве имен **System**, являющихся упакованными представлениями простых типов.  
...
|<xref:System.Drawing.Color?displayProperty=fullName>|**OLE\_COLOR**|  
 ...
 В приведенном ниже коде показано определение неуправляемых типов **DATE**, **GUID**, **DECIMAL** и **OLE\_COLOR** в библиотеке типов Stdole2.  
