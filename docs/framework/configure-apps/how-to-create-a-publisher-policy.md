...
Возможность ориентироваться на конкретную архитектуру процессора доступна начиная с версии .NET Framework 2.0. Следующая команда создает сборку `policy.1.0.myAssembly` политики издателя с именем из `pub.config`файла политики издателя, присваивает строгое имя сборка, использующая пару ключей в `sgKey.snk` файле, и указывает, что сборка предназначена для архитектуры процессора x86.
