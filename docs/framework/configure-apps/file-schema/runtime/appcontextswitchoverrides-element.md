...
|`Switch.System.Runtime.InteropServices.`<br/>`DoNotMarshalOutByrefSafeArrayOnInvoke`|Управляет ли параметры ByRef SafeArray на COM-взаимодействия событий маршалинга обратно в машинном коде (`false`) или отключено ли маршалинг обратно в машинный код (`true`).|.NET Framework 4.8|
