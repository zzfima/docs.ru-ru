...
|instanceEncodingOption|Необязательное значение, которое указывает, производится ли сжатие информации о состоянии экземпляра при использовании алгоритма GZip, прежде чем она будет сохранена в хранилище сохраняемости. Это значение имеет тип `System.Activities.DurableInstancing.InstanceEncodingAction`. Допустимые значения для этого свойства — «None», который указывает отсутствие сжатия и «GZip», который указывает этот экземпляр, данные сжимаются с использованием алгоритма gzip.|  
