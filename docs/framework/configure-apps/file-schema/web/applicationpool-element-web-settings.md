...
---
# <a name="applicationpool-element-web-settings"></a>Элемент > @no__t 0applicationPool (веб-параметры)
...
> [!IMPORTANT]
> При запуске IIS 7,0 на [!INCLUDE[win7](../../../../../includes/win7-md.md)] можно настроить отдельный файл ASPNET. config для каждого пула приложений. Это позволяет адаптировать производительность потоков для каждого пула приложений.  
  
Для параметра `maxConcurrentRequestsPerCPU` значение по умолчанию "5000" в .NET Framework 4 эффективно регулирует регулирование запросов, контролируемое ASP.NET, если на самом деле не будет 5000 или больше запросов на ЦП. Значение по умолчанию зависит от того, что в пуле потоков CLR для автоматического управления параллелизмом на каждый ЦП. Приложения, которые активно используют асинхронную обработку запросов или которые имеют много долго выполняющихся запросов, заблокированы в операциях ввода-вывода сети, получат преимущество от увеличения предельного значения по умолчанию в .NET Framework 4. Установка `maxConcurrentRequestsPerCPU` в ноль отключает использование управляемых потоков для обработки запросов ASP.NET. Когда приложение запускается в пуле приложений IIS, запросы остаются в потоке ввода-вывода IIS, поэтому параллелизм регулируется параметрами потока IIS.  
  
Параметр `requestQueueLimit` работает так же, как атрибут `requestQueueLimit` элемента [processModel](https://docs.microsoft.com/previous-versions/dotnet/netframework-4.0/7w2sway1(v=vs.100)) , который задается в файлах Web. config для приложений ASP.NET. Однако параметр `requestQueueLimit` в файле aspnet. config переопределяет параметр `requestQueueLimit` в файле Web. config. Иными словами, если заданы оба атрибута (по умолчанию это значение равно true), приоритет имеет параметр `requestQueueLimit` в файле aspnet. config.  
