...
|serializationFormat|Задает модуль форматирования, используемый для сериализации объектов, отправляемых как часть сообщения MSMQ. Допустимы следующие значения:<br /><br /> -ActiveX: Средство форматирования ActiveX используется при сериализации COM-объектов.<br />-Двоичный:  Сериализует объект в двоичный пакет.<br />-ByteArray:  Сериализует объект в массив байтов.<br />-Stream:  Выполняет сериализацию объекта в поток.<br />-Xml:  Сериализует объект в пакет XML. Значение по умолчанию - XML.<br /><br /> Это атрибут типа <xref:System.ServiceModel.MsmqIntegration.MsmqMessageSerializationFormat>.|  
