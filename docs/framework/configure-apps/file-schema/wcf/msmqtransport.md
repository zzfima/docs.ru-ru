...
|queueTransferProtocol|Задает использующий очереди транспорт коммуникационного канала, используемый этой привязкой. Допустимы следующие значения:<br /><br /> -Native: Используется собственный протокол MSMQ.<br />-Srmp: Используйте надежный протокол обмена сообщениями Soap (SRMP).<br />-SrmpSecure: Используется Soap надежного обмена сообщениями протокола Secure (srmps).<br /><br /> Это атрибут типа <xref:System.ServiceModel.QueueTransferProtocol>.<br /><br /> Поскольку MSMQ не поддерживает адресацию Active Directory при использовании протокола SOAP Reliable Messaging, не устанавливайте этот атрибут в значение Srmp или Srmps при `useActiveDirectory` присваивается `true`.|  
