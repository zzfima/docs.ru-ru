---
title: "Разработка больших, быстро реагирующих приложений .NET Framework | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## Как производительность нового компилятора затрагивает ваше приложение  
 ...
 Если вы хотите подробнее узнать о новых компиляторах, посетите [проект с открытым исходным кодом — платформа компиляторов .NET \("Roslyn"\)](http://roslyn.codeplex.com/).  
...
### Факт 1. Не проводите преждевременную оптимизацию  
...
### Факт 2. Если вы не измеряете, значит, вы угадываете  
 Профили и измерения не врут.  Профили показывают, полностью ли загружен ЦП или выполняется ли блокировка на уровне операций ввода\-вывода диска.  Профили сообщают вам, какой тип и объем памяти вы выделяете, и тратит ли ЦП слишком много времени на [сборку мусора](../../../docs/standard/garbage-collection/index.md).  
...
### Факт 3. Значение хороших средств трудно переоценить  
 Хорошие средства позволяют вам быстро выявлять наиболее важные проблемы с производительностью \(ЦП, память или диск\) и помогают локализовать код, вызывающий такие узкие места.  Корпорация Майкрософт предоставляет разнообразные средства оценки производительности, такие как [профилировщик Visual Studio](../Topic/Beginners%20Guide%20to%20Performance%20Profiling.md), [средство анализа Windows Phone](http://msdn.microsoft.com/ru-ru/e67e3199-ea43-4d14-ab7e-f7f19266253f) и [PerfView](http://www.microsoft.com/download/details.aspx?id=28567).  
  
 PerfView — это бесплатное и крайне эффективное средство, помогающее вам сконцентрироваться на самых важных проблемах — операциях ввода\-вывода диска, событиях сборки мусора и памяти.  Вы можете перехватывать связанные с производительностью события [трассировки событий Windows](../../../docs/framework/wcf/samples/etw-tracing.md) и просматривать информацию для отдельных приложений, процессов, стеков и потоков.  PerfView показывает, какой объем и тип памяти выделяет ваше приложение, а также какие функции или стеки вызовов чаще всего осуществляют выделение памяти.  Дополнительные сведения см. в разделах справки, демонстрациях и видеозаписях, поставляемых вместе с этим средством \(например, [учебники PerfView](http://channel9.msdn.com/Series/PerfView-Tutorial) в Channel 9\).  
 ...
 **Исправление для примера 3**  
 ...
 Группа по обеспечению производительности компиляторов решила проблему всех этих выделений с помощью следующего кода:  
...
```  
 ...
 В первой строчке [лямбда\-выражение](../Topic/Lambda%20Expressions%20\(C%23%20Programming%20Guide\).md) `s => s.Name == name` [удерживает](http://blogs.msdn.com/b/ericlippert/archive/2003/09/17/53028.aspx) локальную переменную `name`.  Это означает, что в дополнение к выделению объекта для [делегата](../Topic/delegate%20\(C%23%20Reference\).md), содержащегося в `predicate`, код выделяет статический класс для среды, которая перехватывает значение `name`.  Компилятор формирует код, аналогичный следующему:  
...
### Кэширование асинхронных методов  
 В следующем примере показана распространенная проблема, возникающая при попытке использования кэшированных результатов в [асинхронном](../Topic/Asynchronous%20Programming%20with%20Async%20and%20Await%20\(C%23%20and%20Visual%20Basic\).md) методе.  
...
-   Выделения памяти значат больше всего — именно на них группа разработчиков платформы компиляторов потратила больше всего времени, стремясь повысить производительность новых компиляторов.  
  
## См. также  
 [Видеозапись презентации по данному разделу](http://channel9.msdn.com/Events/TechEd/NorthAmerica/2013/DEV-B333)   
 [Руководство по профилированию производительности для начинающих](../Topic/Beginners%20Guide%20to%20Performance%20Profiling.md)   
 ...
 [Учебники PerfView на Channel 9](http://channel9.msdn.com/Series/PerfView-Tutorial)   
 ...
 [Проект платформы компиляции .NET Compiler Platform \(Roslyn\) с открытым исходным кодом](http://roslyn.codeplex.com/)