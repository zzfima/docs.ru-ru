...
## <a name="why-the-new-compiler-performance-applies-to-your-app"></a>Как производительность нового компилятора затрагивает ваше приложение  
 ...
 Для получения дополнительной информации о рослин компиляторов, [см.](../../csharp/roslyn-sdk/index.md)
...
### <a name="fact-3-good-tools-make-all-the-difference"></a>Факт 3. Значение хороших средств трудно переоценить  
 Хорошие средства позволяют вам быстро выявлять наиболее важные проблемы с производительностью (ЦП, память или диск) и помогают локализовать код, вызывающий такие узкие места. Корпорация Майкрософт поставляет различные инструменты производительности, такие как [Visual Studio Profiler](/visualstudio/profiling/beginners-guide-to-performance-profiling) и [PerfView.](https://www.microsoft.com/download/details.aspx?id=28567)
...
```  
 ...
 Первая версия `WriteFormattedDocComment()` выделяла массив, несколько подстрок и усеченную подстроку, а также пустой массив `params`. Она также проверила на наличие "///". Переработанный код использует только индексирование и ничего не выделяет. Он находит первый символ, который не является белым пространством, а затем проверяет символ по символу, чтобы увидеть, если строка начинается с "//". Новый код `IndexOfFirstNonWhiteSpaceChar` использует <xref:System.String.TrimStart%2A> вместо того, чтобы вернуть первый индекс (после указанного стартового индекса), где происходит небело-пространственный символ. Такое исправление нельзя назвать окончательным, но из него можно понять, как применять аналогичные исправления для всего решения. Используя такой подход для всего кода, можно удалить все выделения в `WriteFormattedDocComment()`.
...
Языковой интегрированный запрос (LINЗ), в сочетании с выражениями лямбды, является примером функции производительности. Тем не менее, его использование может иметь значительное влияние на производительность с течением времени, и вы можете обнаружить, что вам нужно переписать свой код.
 ...
 **Исправление для примера 6**  
 ...
 Чтобы удалить <xref:System.Threading.Tasks.Task> завершенное распределение, можно кэшировать объект Задачи с завершенным результатом:  
...
- [Руководство для начинающих по профилированию производительности](/visualstudio/profiling/beginners-guide-to-performance-profiling)
- [Производительность](index.md)
- [Советы по производительности .NET](https://docs.microsoft.com/previous-versions/dotnet/articles/ms973839(v%3dmsdn.10))
...
- [Dotnet/roslyn репо на GitHub](https://github.com/dotnet/roslyn)
