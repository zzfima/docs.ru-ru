...
Использование блокировки в неуправляемом коде вместо управляемого может стать причиной атаки типа "отказ в обслуживании", поскольку в этом случае среде CLR не удастся прервать поток.  Из-за заблокированного потока среда CLR не сможет выгрузить <xref:System.AppDomain>, не используя некоторые крайне небезопасные операции.  Примитив синхронизации, блокировка с использованием Windows является примером такой кое-что мы не можем позволить.  Блокировки при обращении к `ReadFile` на сокете следует избегать, если это возможно, в идеале Windows API должны обеспечивать механизм для времени ожидания операции следующим образом.
