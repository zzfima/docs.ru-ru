...
### <a name="iprincipalisinrole-method"></a>Метод IPrincipal.IsInRole  
 Чтобы реализовать подход RBAC в приложениях, поддерживающих утверждения, используйте метод **IsInRole ()** в интерфейсе **IPrincipal** так же, как и в приложениях, не поддерживающих утверждения. Существует несколько способов использования метода **IsInRole()** .  
