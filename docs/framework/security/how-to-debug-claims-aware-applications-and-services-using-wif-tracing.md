...
3. Приведенная выше конфигурация предписывает WIF создавать подробные события трассировки и регистрировать их в файле *WIFTrace.e2e*. Полный список значений для **switchValue** см. в таблице уровень трассировки в следующем разделе: [Настройка трассировки](../wcf/diagnostics/tracing/configuring-tracing.md).  
...
1. Средство Trace Viewer (SvcTraceViewer.exe) поставляется в составе пакета Windows SDK. Если вы еще не установили пакет Windows SDK, его можно загрузить здесь: [Windows SDK](https://www.microsoft.com/download/en/details.aspx?id=8279).  
...
5. Перейдите на вкладку **Действия**. Вы увидите подробные записи трассировки в главной области средства. Используйте **уровень** раскрывающегося списка в меню, чтобы отфильтровать конкретный уровень трассировки, например: **Все**, **предупреждение**, **ошибки**, **сведения**и т. д.  
