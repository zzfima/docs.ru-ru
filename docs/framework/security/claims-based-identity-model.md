...
- [Вводные сведения об удостоверениях на основе утверждений](claims-based-identity-model.md#BKMK_1)  
  
- [Базовый сценарий для модели удостоверений на основе утверждений](claims-based-identity-model.md#BKMK_2)  
...
## <a name="basic-scenario-for-a-claims-based-identity-model"></a>Базовый сценарий для модели удостоверения на основе утверждений  
 ...
 ![Поток аутентификации с проверяющей стороной](./media/conc-relying-partner-processc.png "conc_relying_partner_processc")  
...
1. Когда пользователь, не прошедший проверку подлинности, запрашивает страницу, его браузер перенаправляется на страницы поставщика удостоверений (IdP).  
  
2. IdP требует, чтобы пользователь представлю свои учетные данные, такие как имя пользователя, пароль или проверка подлинности Kerberos.  
  
3. IdP выдает маркер обратно, возвращаемый в браузер.  
