...
`pCtx`\
окне Как правило, это значение `null`равно. В противном случае он является указателем на экземпляр [ивбемконтекст](/windows/desktop/api/wbemcli/nn-wbemcli-iwbemcontext) , который может использоваться поставщиком, который предоставляет запрошенные события.
...
| `WBEMESS_E_REGISTRATION_TOO_BROAD` | 0x80042001 | Запрос запрашивает больше информации, чем может предоставить система управления Windows. Это `HRESULT` значение возвращается, когда запрос события приводит к запросу на опрос всех объектов в пространстве имен. |
...
После того как функция возвращает значение, вызывающий объект периодически `ppEnum` передает возвращенные объекты [следующей](next.md) функции, чтобы определить, доступны ли какие либо события.
