...
`ppCopy`  
[out] Новый объект, который имеет полный одиночному из `ptr`. Этот аргумент не может быть `null` при получении копия текущего объекта.
...
Эта функция создает оболочку для вызова [IWbemClassObject::Clone](https://msdn.microsoft.com/library/aa391436(v=vs.85).aspx) метод.
