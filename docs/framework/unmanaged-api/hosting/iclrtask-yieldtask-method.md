...
## <a name="remarks"></a>Примечания  
 Узел вызывает `YieldTask` для запроса ресурсов процессора для других задач или процессов. Этот метод предназначен главным образом, разрешающий коду выполняющейся длительное время освобождать время ЦП. Среда выполнения пытается перевести задачу, текущий `ICLRTask` представляет экземпляр в состоянии, когда он может дать время обработки, но не гарантирует успеха.  
