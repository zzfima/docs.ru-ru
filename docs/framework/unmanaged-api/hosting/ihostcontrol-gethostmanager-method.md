...
## <a name="parameters"></a>Параметры  
 `riid`  
 окне `IID` интерфейса, для которого запрашиваются общеязыковая среда выполнения (CLR).  
...
|E_INVALIDARG|Запрошенный `IID` недопустим.|  
...
- [Метод ihostsyncmanager](../../../../docs/framework/unmanaged-api/hosting/ihostsyncmanager-interface.md)  
...
- [IHostSecurityManager](../../../../docs/framework/unmanaged-api/hosting/ihostsecuritymanager-interface.md)  
 ...
 Если узел поддерживает указанный интерфейс, он устанавливает `ppObject` в его реализацию этого интерфейса. В противном случае задается `ppObject` null.  
  
 Среда CLR не вызывает `Release` для диспетчеров узлов даже при завершении работы.  
