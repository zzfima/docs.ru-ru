...
Останавливает выполнение кода средой CLR (CLR).  
  
> [!IMPORTANT]
>  Этот метод не освобождать ресурсы на узле, выгрузки доменов приложений и не удаляет потоки. Необходимо завершить процесс для высвобождения ресурсов.  
