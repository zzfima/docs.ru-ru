...
---
# <a name="iclrtaskabort-method"></a>Метод ICLRTask::Abort
Запрашивает прерывание задачи общеязыковой среды выполнения (CLR), текущий [ICLRTask](../../../../docs/framework/unmanaged-api/hosting/iclrtask-interface.md) представляет экземпляр.  
...
## <a name="remarks"></a>Примечания  
 Среда CLR вызывает <xref:System.Threading.ThreadAbortException> Если узел вызывает метод `Abort`. Возвращается сразу после инициализации сведения об исключении, не ожидая пользовательский код, например методы завершения или механизмы обработки исключений, для выполнения. Вызовы `Abort` тем самым быстро вернуться.  
