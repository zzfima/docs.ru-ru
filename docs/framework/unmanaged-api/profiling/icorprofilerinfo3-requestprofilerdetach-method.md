...
|E_ CORPROF_E_CALLBACK3_REQUIRED|Профилировщику не удалось выполнить попытку [IUnknown:: QueryInterface](https://go.microsoft.com/fwlink/?LinkID=144867) для интерфейса [ICorProfilerCallback3](../../../../docs/framework/unmanaged-api/profiling/icorprofilercallback3-interface.md) , который он должен реализовать для поддержки операции отсоединения. Попытка отсоединения не выполнялась.|  
