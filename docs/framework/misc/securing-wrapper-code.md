...
- Размещение тех же проверок безопасности в вызывающем коде, который появляется в вызываемом коде, и принуждение вызывающих его объектов делать это. Например, при написании кода, который вызывает метод, защищенный с помощью **LinkDemand** <xref:System.Security.Permissions.SecurityPermission> для <xref:System.Security.Permissions.SecurityPermissionFlag.UnmanagedCode> с указанным флагом, метод должен также сделать **LinkDemand** (или **Demand**, который является более надежным) для этого PermissionSet. Исключением является то, что в коде используется метод, защищенный с помощью **LinkDemand**, ограниченным способом, который можно выбрать в качестве безопасного, учитывая другие механизмы защиты (например, требования) в коде. В этом исключительном случае вызывающий объект несет ответственность за понижение степени защиты в базовом коде.  
