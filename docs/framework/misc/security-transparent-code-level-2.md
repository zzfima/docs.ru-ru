...
- [Примеры использования и варианты поведения](#examples)

- [Шаблоны переопределения](#override)
...
Если сборка не замечается, то по умолчанию используются правила .NET Framework 4. Однако рекомендуется использовать <xref:System.Security.SecurityRulesAttribute> атрибут, а не в зависимости от значения по умолчанию.
...
- Нет атрибутов: Если не указать какие-либо атрибуты, среда выполнения интерпретирует весь код как критический с точки зрения безопасности, за исключением случаев, когда критический уровень безопасности нарушает правило наследования (например, при переопределении или реализации прозрачного виртуального метода или интерфейса). В таких случаях методы являются надежными с точки зрения безопасности. Если атрибут не указан, то определение правил прозрачности выполняется средой CLR.

- `SecurityTransparent`: Весь код является прозрачным; Вся сборка не будет выполнять никаких привилегированных или ненадежных.
...
- `SecurityCritical`: Тип или член является критическим и может вызываться только кодом с полным доверием. Методы, представленные в критическом с точки зрения безопасности коде, также являются критическими.
...
- `SecuritySafeCritical`: Тип или член является безопасным. В то же время этот тип или член может вызываться из прозрачного кода (с частичным доверием) и имеет те же возможности, что и любой другой критический код. Код необходимо проверить на безопасность.
...
- Правила для типов: При переходе слева направо доступ станет более ограниченным. Производные типы должны иметь как минимум те же ограничения, что и базовый тип.

- Правила для методов: Производные методы не могут изменять доступность базового метода. По умолчанию все производные методы без заметок являются методами `Transparent`. Типы, производные от критических, вызывают возникновение исключения, если переопределенный метод явным образом не помечен как `SecurityCritical`.
...
> [!NOTE]
> `SafeCritical` Метод <xref:System.Reflection.MethodBase.IsSecuritySafeCritical%2A>возвращает `true` для<xref:System.Type.IsSecurityCritical%2A> и, поскольку`SafeCritical` действительно является критически важным (он имеет те же возможности, что и важный код, но он может быть вызван из прозрачного кода).
