---
title: "Обнаружение сборок в среде выполнения | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
dev_langs: 
  ...
  - "jsharp"
helpviewer_keywords: 
  - "развертывание приложений .NET Framework, расположение сборок"
  - "файлы app.config, расположения сборок"
  - "файлы конфигурации приложений, расположение сборок"
  - "сборки [платформа .NET Framework], расположение"
  - "развертывание приложений [платформа .NET Framework], расположения сборок"
  - "расположение сборок"
...
> [!NOTE]
>  Сведения о привязке можно просмотреть в файле журнала с помощью [средства просмотра журнала привязок сборки \(Fuslogvw.exe\)](../../../docs/framework/tools/fuslogvw-exe-assembly-binding-log-viewer.md), которое включено в [!INCLUDE[winsdklong](../../../includes/winsdklong-md.md)].  
...
4.  [Проверяет наличие сборки](#step4), выполняя указанные ниже действия.  
    ...
    1.  Если конфигурация и политика издателя не влияют на исходную ссылку и запрос привязки создан с помощью метода <xref:System.Reflection.Assembly.LoadFrom%2A?displayProperty=fullName>, среда выполнения пытается найти косвенные упоминания о расположении сборки.  
...
```  
 ...
 Чтобы создать сборку, можно воспользоваться [Al.exe \(компоновщик сборок\)](../../../docs/framework/tools/al-exe-assembly-linker.md) и выполнить команду следующего вида:  
...
1.  Если элемент [\<codeBase\>](../../../docs/framework/configure-apps/file-schema/runtime/codebase-element.md) найден в файле конфигурации приложения, среда выполнения проверяет указанное местоположение. Если найдено совпадение, используется эта сборка, и дальнейший поиск не выполняется. Если сборку не удается обнаружить в этом файле, происходит сбой запроса привязки.  
