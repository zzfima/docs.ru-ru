---
title: "Рекомендации для загрузки сборок | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
helpviewer_keywords: 
  - "сборки, привязка"
  - "сборки, загрузка"
  - "ошибки загрузки сборок"
  - "контекст загрузки по умолчанию"
  - "контексты загрузки"
  - "контекст, из которого ведется загрузка"
  - "LoadFrom - метод"
  - "LoadWithPartialName - метод"
  - "TypeLoadException - класс, причины"
...
-   [Необходимость понимания преимуществ и недостатков контекстов загрузки.](#load_contexts)  
  
-   [Избежание привязки к именам частичных сборок.](#avoid_partial_names)  
  
-   [Избежание загрузки сборок в нескольких контекстах.](#avoid_loading_into_multiple_contexts)  
  
-   [Избежание загрузки нескольких версий сборки в одном и тот же контексте.](#avoid_loading_multiple_versions)  
  
-   [Необходимость рассмотрения возможности перехода к использованию контекста загрузки по умолчанию.](#switch_to_default)  
  
 Первая рекомендация, [касающаяся преимуществ и недостатков контекстов загрузки](#load_contexts), предоставляет базовые сведения для всех прочих рекомендаций, поскольку все они полагаются на понимание сути контекстов загрузки.  
  
<a name="load_contexts"></a>   
## Преимущества и недостатки контекстов загрузки  
...
-   Метод <xref:System.Reflection.Assembly.LoadFrom%2A> требует наличия флагов <xref:System.Security.Permissions.FileIOPermissionAccess?displayProperty=fullName> или <xref:System.Security.Permissions.FileIOPermissionAccess?displayProperty=fullName>, либо наличия объекта <xref:System.Net.WebPermission> для заданного пути.  
  
-   Если для сборки существует машинный образ, то он не используется.  
  
-   Сборка не может быть загружена как доменно\-нейтральная.  
  
-   В платформе .NET Framework версий 1.0 и 1.1 политика не применяется.  
...
-   Необходимые сборки не загружаются автоматически.  Их можно предварительно загрузить без контекста, загрузить их в контексте загрузки по умолчанию, или загрузить их в обработчике событий <xref:System.AppDomain.AssemblyResolve?displayProperty=fullName>.  
...
-   Фактически загружаемая сборка может быть несовместимой с последующими версиями.  Например, приложение могло строиться и тестироваться с последней версией сборки, но при частичной привязке могла загрузиться существенно более ранняя ее версия, не поддерживающая функции, которые использует приложение.  
...
-   При загрузке необходимых сборок может возникать непредвиденное поведение.  Загрузка двух сборок, совместно зависящих от одной сборки, с использованием частичной привязки имен может привести к тому, что одна из сборок будет использовать компонент, с которым она не строилась и не тестировалась.  
 ...
 Ввиду потенциальных проблем метод <xref:System.Reflection.Assembly.LoadWithPartialName%2A> был помечен как устаревший.  Вместо него рекомендуется использовать метод <xref:System.Reflection.Assembly.Load%2A?displayProperty=fullName>, указывая полные отображаемые имена сборок.  См. разделы [Преимущества и недостатки контекстов загрузки](#load_contexts) и [Возможность перехода к использованию контекста загрузки по умолчанию](#switch_to_default).  
  
 Если вы все же хотите использовать метод <xref:System.Reflection.Assembly.LoadWithPartialName%2A>, поскольку он упрощает загрузку сборок, учтите, что сбой приложения с сообщением, содержащим полный идентификатор необходимой сборки, — это лучше для пользователя, чем автоматическое использование неизвестной версии сборки, способной вызвать непредвиденное поведение и бреши в системе безопасности.  
...
## Избежание загрузки сборок в нескольких контекстах  
 ...
 Например, предположим, что в сборке `Utility` объявлен интерфейс `ICommunicate`, который используется в программе и в других сборках, загружаемых программой.  Эти другие сборки содержат типы, реализующие интерфейс `ICommunicate`, позволяя программе их использовать.  
 ...
 В разделе [Возможность перехода к использованию контекста загрузки по умолчанию](#switch_to_default) описаны альтернативы использованию загрузки по файловому пути, например <xref:System.Reflection.Assembly.LoadFile%2A> и <xref:System.Reflection.Assembly.LoadFrom%2A>.  
  
<a name="avoid_loading_multiple_versions"></a>   
## Избежание загрузки нескольких версий сборки в одном и том же контексте  
 Загрузка нескольких версий сборки в одном контексте загрузки может вызывать проблемы с идентификацией типов.  Загрузка одного и того же типа из двух версий одной и той же сборки аналогична загрузке двух разных типов с одним и тем же именем.  При попытке приведения из одного такого типа в другой возникнет исключение <xref:System.InvalidCastException>, а в сообщении об ошибке при этом будет несколько туманно сказано, что тип `MyType` нельзя привести к типу `MyType`.  
  
 Предположим, программа загружает одну версию сборки `Utility` напрямую, а потом загружает другую сборку, загружающую другую версию сборки `Utility`.  Приложение также может загрузить две разные версии сборки, если в коде приложения имеется ошибка.  
...
### Использование модели надстроек платформы .NET Framework  
 При использовании контекста, из которого ведется загрузка, для реализации надстроек, которые обычно устанавливаются вне базовой папки приложения, используйте модель надстроек платформы .NET Framework.  Она обеспечивает изоляция на уровне доменов приложений и процессов, не требуя самостоятельно управлять доменами приложений.  Сведения о модели надстроек см. в разделе [Надстройки и расширения среды](../../../ml/index.xml).  
...
### Использование доменов приложений  
 ...
 Обратите внимание, что для загрузки таких сборок можно использовать метод <xref:System.Reflection.Assembly.LoadFrom%2A?displayProperty=fullName>.  Поскольку они теперь расположены в пути поиска, они будут загружены в контексте загрузки по умолчанию, а не в контексте, из которого ведется загрузка.  Тем не менее, рекомендуется перейти к использованию метода <xref:System.Reflection.Assembly.Load%2A?displayProperty=fullName> и полных отображаемых имен сборок, чтобы гарантировать, что всегда используются только правильные их версии.  
