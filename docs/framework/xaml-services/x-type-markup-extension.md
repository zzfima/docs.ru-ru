...
- `x:TypeArguments` и элемент связанного объекта для создания экземпляра универсального объекта могут быть в элементах, отличном от корневого. Дополнительные сведения см. в разделе «XAML 2009» в статье [x: TypeArguments Directive](x-typearguments-directive.md).  
