...
## <a name="services-for-a-markup-extension"></a>Службы для расширения разметки  
 <xref:System.Windows.Markup.MarkupExtension> имеет только один виртуальный метод <xref:System.Windows.Markup.MarkupExtension.ProvideValue%2A>. Входной параметр `serviceProvider` определяет, как службы взаимодействуют с реализациями, когда расширение разметки вызывается обработчиком XAML. В следующем псевдокоде показано, как реализация расширения разметки может запросить службы в <xref:System.Windows.Markup.MarkupExtension.ProvideValue%2A>:  
 ...
 **Справочная документация по**: <xref:System.IServiceProvider>  
 ...
 **Справочная документация по**: <xref:System.ComponentModel.ITypeDescriptorContext>  
 ...
 **Справочная документация по**: <xref:System.Windows.Markup.IValueSerializerContext>  
 ...
 **Справочная документация по**: <xref:System.Windows.Markup.IXamlTypeResolver>  
 ...
 **API службы:**  <xref:System.Windows.Markup.IXamlTypeResolver.Resolve%2A>  
 ...
 Может влиять на сопоставление типов XAML в CLR, необходимое, когда средство записи XAML создает объект среды CLR в графе объектов. <xref:System.Windows.Markup.IXamlTypeResolver.Resolve%2A> обрабатывает потенциально содержащую префикс строку, соответствующий имени типа XAML (<xref:System.Xaml.XamlType.Name%2A?displayProperty=nameWithType>) и возвращает CLR <xref:System.Type>. Разрешение типов обычно сильно зависит от контекста схемы XAML. Только контекст схемы языка XAML учитывает, какие сборки загружаются и какие из них можно или нужно использовать для разрешения типов.  
  
### <a name="iuricontext"></a>IUriContext  
 **Справочная документация по**: <xref:System.Windows.Markup.IUriContext>  
 ...
 **Справочная документация по**: <xref:System.Xaml.IAmbientProvider>  
 ...
 **Справочная документация по**: <xref:System.Xaml.IXamlSchemaContextProvider>  
 ...
 **Справочная документация по**: <xref:System.Xaml.IRootObjectProvider>  
 ...
 **Справочная документация по**: <xref:System.Xaml.IXamlNamespaceResolver>  
 ...
 **API службы:** <xref:System.Xaml.IXamlNamespaceResolver.GetNamespace%2A> для пути загрузки, <xref:System.Xaml.IXamlNamespaceResolver.GetNamespacePrefixes%2A> для пути сохранения.  
 ...
 <xref:System.Xaml.IXamlNamespaceResolver> — Это служба, которая может вернуть идентификатор пространства имен XAML / URI, используя его префикс, сопоставленный в исходной разметке XAML.  
  
### <a name="iprovidevaluetarget"></a>IProvideValueTarget  
 **Справочная документация по**: <xref:System.Windows.Markup.IProvideValueTarget>  
 ...
 **API службы:**  <xref:System.Windows.Markup.IProvideValueTarget.TargetObject%2A>, <xref:System.Windows.Markup.IProvideValueTarget.TargetProperty%2A>.  
 ...
 <xref:System.Windows.Markup.IProvideValueTarget> включает расширение типа преобразователя или разметки получать контекст выполнения во время загрузки. Реализации могут применять этот контекст, чтобы сделать использование недействительным. Например, WPF располагает логикой внутри некоторых расширений разметки, таких как <xref:System.Windows.DynamicResourceExtension>. Эта логика проверяет <xref:System.Windows.Markup.IProvideValueTarget.TargetProperty%2A> , чтобы убедиться в том, что расширение используется только для задания свойств зависимостей (или краткого списка других свойств, не относящихся к зависимостям).  
  
### <a name="ixamlnameresolver"></a>IXamlNameResolver  
 **Справочная документация по**: <xref:System.Xaml.IXamlNameResolver>  
 ...
 **Справочная документация по**: <xref:System.Xaml.IDestinationTypeProvider>  
