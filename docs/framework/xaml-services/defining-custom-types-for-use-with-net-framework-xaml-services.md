...
## Перечисления  
 ...
 XAML поддерживает использование перечислений с примененным атрибутом <xref:System.FlagsAttribute> подобно флагам.  Дополнительные сведения см. в разделе [Подробное описание синтаксиса XAML](../../../docs/framework/wpf/advanced/xaml-syntax-in-detail.md).  \(Раздел [Подробное описание синтаксиса XAML](../../../docs/framework/wpf/advanced/xaml-syntax-in-detail.md) адресован пользователям WPF, но большая часть информации этого раздела имеет отношение к XAML в части, не относящейся к какой\-либо конкретной платформе реализации.\)  
