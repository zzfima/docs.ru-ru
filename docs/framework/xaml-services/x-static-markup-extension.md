...
|||  
|-|-|  
|`prefix`|Необязательно. Префикс, который ссылается на сопоставленном, не по умолчанию пространство имен XAML. `prefix`явно приведен в использовании, так как редко создаются ссылки на статические свойства, полученные из пространства имен XAML по умолчанию. См. заметки.|  
