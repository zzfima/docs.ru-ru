---
title: "Примеры XML-сериализации | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
helpviewer_keywords: 
  ...
  - "DataSet, класс, сериализация"
  - "интерфейс ICollection, сериализация"
...
## Сериализация XmlElement и XmlNode  
...
## Сериализация класса, который содержит поле, возвращающее сложный объект  
 Если свойство или поле возвращает сложный объект \(например, массив или экземпляр класса\), [XmlSerializer](https://msdn.microsoft.com/en-us/library/system.xml.serialization.xmlserializer.aspx) преобразует его в элемент, вложенный в рамках основного документа XML.Например, первый класс в следующем примере кода возвращает экземпляр второго класса.  
...
## Сериализация класса, реализующего интерфейс ICollection  
...
## Пример заказа на покупку  
 ...
 Метод `CreatePO` создает `PurchaseOrder`, `Address` и объекты класса `OrderedItem` и задает значения открытых полей.Метод также создает экземпляр класса <xref:System.Xml.Serialization.XmlSerializer>, который используется для сериализации и десериализации `PurchaseOrder`.Обратите внимание, что код передает тип класса, который будет сериализован в конструктор.Код также создает [FileStream](frlrfSystemIOFileStreamClassTopic), который используется для записи потока XML в документ XML.  
