...
## Заметки  
 ...
 Обычно используется лишь один из параметров **\/list** и **\/remove**, однако в случае указания нескольких параметров они будут обрабатываться в порядке их указания в командной строке.  
