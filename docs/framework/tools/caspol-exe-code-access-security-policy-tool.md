...
|**-e**[**xecution**] {**on** &#124; **off**}|Включает и отключает механизм проверки разрешений до выполнения кода. **Примечание.**  Этот параметр не поддерживается в .NET Framework 4 и более поздних версиях. Дополнительные сведения см. в разделе [Изменения системы безопасности](../../../docs/framework/security/security-changes.md).|  
