...
> [!IMPORTANT]
> RESX-файлы следует загружать и использовать только из надежных источников. Эти файлы могут содержать исполняемый код и должны рассматриваться как исполняемые файлы (EXE) с точки зрения безопасности.
