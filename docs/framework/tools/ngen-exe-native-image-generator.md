...
-   Были устранены некоторые причины недействительности образов.  
 ...
 Для Windows 8 см. раздел [Задача образов в машинном коде](http://msdn.microsoft.com/en-us/9b1f7590-4e0d-4737-90ef-eaf696932afb).  
  
 Подробнее об использовании программы Ngen.exe и службы образов в машинном коде см. в разделе [Служба образов в машинном коде](http://msdn.microsoft.com/en-us/b15e0e32-59cb-4ae4-967c-6c9527781309).  
  
> [!NOTE]
>  Синтаксис Ngen.exe для .NET Framework версий 1.0 и 1.1 см. в разделе [Генератор образов в машинном коде (Ngen.exe), традиционный синтаксис](http://msdn.microsoft.com/en-us/5a69fc7a-103f-4afc-8ab4-606adcb46324).  
...
|Аргумент|Описание|  
|--------------|-----------------|  
|`assemblyName`|Полное отображаемое имя сборки. Например, `"myAssembly, Version=2.0.0.0, Culture=neutral, PublicKeyToken=0038abc9deabfle5"`. **Примечание**. Для действий `display` и `uninstall` разработчик может предоставить имя частичной сборки, например `myAssembly`. <br /><br /> В командной строке программы Ngen.exe может быть указана только одна сборка.|  
...
|`3`|Образы в машинном коде устанавливаются, когда служба образов в машинном коде обнаружит, что компьютер находится в режиме простоя. См. раздел [Служба образов в машинном коде](http://msdn.microsoft.com/en-us/b15e0e32-59cb-4ae4-967c-6c9527781309).|  
...
```  
 ...
 Примеры, связанные со службой образов в машинном коде, см. в разделе [Служба образов в машинном коде](http://msdn.microsoft.com/en-us/b15e0e32-59cb-4ae4-967c-6c9527781309).  
  
## <a name="native-image-task"></a>Задача образов в машинном коде  
 Задача образов в машинном коде — это задача Windows, которая создает и поддерживает образы в машинном коде. Задача образов в машинном коде автоматически создает и освобождает образы в машинном коде в поддерживаемых сценариях. (См. раздел [Создание образов в машинном коде](http://msdn.microsoft.com/en-us/2bc8b678-dd8d-4742-ad82-319e9bf52418).) Она также позволяет установщикам использовать программу [Ngen.exe (генератор образов в машинном коде)](../../../docs/framework/tools/ngen-exe-native-image-generator.md) для отложенного создания и обновления образов в машинном коде.  
...
|NET Framework NGEN v4.0.30319 64|Нет|Да|  
 ...
 Задача образов в машинном коде доступна в .NET Framework версии 4.5 и более поздних версий в ОС Windows 8 или более поздних версий. В более ранних версиях Windows платформа .NET Framework использует [службу образов в машинном коде](http://msdn.microsoft.com/en-us/b15e0e32-59cb-4ae4-967c-6c9527781309).  
