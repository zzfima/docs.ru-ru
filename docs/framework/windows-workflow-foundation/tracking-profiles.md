...
Профили отслеживания сами являются XML-элементами в стандартном .NET Framework файле конфигурации или задаются в коде. В следующем примере показан профиль отслеживания [!INCLUDE[netfx_current_long](../../../includes/netfx-current-long-md.md)] в файле конфигурации, который позволяет участнику отслеживания подписаться на события рабочих процессов `Started` и `Completed`.
...
Например, предположим, что *MyActivity* является составным действием, реализация которого содержит два действия: *Activity1* и *Activity2*. При добавлении этого действия в рабочий процесс и включении отслеживания с помощью профиля отслеживания с `implementationVisibility`, для которого задано значение `RootScope`, записи отслеживания создаются только для *MyActivity*. Однако для действий *Activity1* и *Activity2*не создаются никакие записи.

Однако если атрибут `implementationVisibility` для профиля отслеживания имеет значение `All`, то отслеживание записей создается не только для *MyActivity*, но и для действий *Activity1* и *Activity2*.
