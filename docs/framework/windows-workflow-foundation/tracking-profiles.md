...
Профили отслеживания ведут себя как XML-элементы в стандартный файл конфигурации .NET Framework, или указать в коде. В следующем примере показан профиль отслеживания [!INCLUDE[netfx_current_long](../../../includes/netfx-current-long-md.md)] в файле конфигурации, который позволяет участнику отслеживания подписаться на события рабочих процессов `Started` и `Completed`.
...
Тем не менее если `implementationVisibility` атрибут задается профиль отслеживания `All`, то записи отслеживания создаются не только для *MyActivity*, но также для действий *Activity1* и  *Activity2*.
