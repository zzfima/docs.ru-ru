...
> [!NOTE]
>  Дополнительные сведения о транзакциях и компенсации см. в разделе [транзакций](workflow-transactions.md) и [компенсации](compensation.md).  
...
### <a name="cancellation-using-nativeactivity"></a>Отмена с помощью NativeActivity  
 ...
 В следующем примере метод <xref:System.Activities.NativeActivity.Cancel%2A> переопределяет пользовательское действие <xref:System.Activities.NativeActivity>, основанное на действии `ParallelForEach`. Если действие отменяется, это переопределение отрабатывает логику отмены для действия. Этот пример является частью [ParallelForEach неуниверсальные](./samples/non-generic-parallelforeach.md) образца.  
