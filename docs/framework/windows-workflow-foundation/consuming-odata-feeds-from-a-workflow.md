...
ms.locfileid: "65592979"
---
# <a name="consuming-odata-feeds-from-a-workflow"></a>Использование каналов OData из рабочего процесса

Службы данных WCF — это компонент платформы .NET Framework, которая позволяет создавать службы, использующие Open Data Protocol (OData) для передачи и получения данных через Интернет или интрасеть с помощью семантики передачи репрезентативного состояния (REST). OData предоставляет доступ к данным в виде ресурсов, которые адресуются по URI. Со службами данных, основанными на OData, может взаимодействовать любое приложение, которое способно отправить HTTP-запрос и обработать канал OData, возвращаемый службой данных. Кроме того службы данных WCF включает в себя клиентские библиотеки, предоставляющие более широкие программные возможности при каналам OData из приложения .NET Framework. В этом разделе представлены общие сведения об использовании каналов OData в рабочем процессе как с клиентскими библиотеками, так и без них.

## <a name="using-the-sample-northwind-odata-service"></a>С помощью службы OData примера Northwind

В примерах этого раздела используется образец службы данных Northwind в [ http://services.odata.org/Northwind/Northwind.svc/ ](https://go.microsoft.com/fwlink/?LinkID=187426). Эта служба поставляется в составе [пакета SDK OData](https://go.microsoft.com/fwlink/?LinkID=185248) и предоставляет доступ только для чтения к образцу базы данных "Борей". Если необходим доступ для чтения или локальная служба данных WCF, то можно выполнить действия, описанные в [кратком руководстве по службам данных WCF](https://go.microsoft.com/fwlink/?LinkID=131076) , чтобы создать локальную службу OData, обеспечивающую доступ к базе данных "Борей". При выполнении инструкций, описанных в кратком руководстве, необходимо заменить локальный URI на тот, который указан в коде в этом разделе.

## <a name="consuming-an-odata-feed-using-the-client-libraries"></a>Использование OData веб-канала, с помощью клиентских библиотек

Службы WCF Data Services включает в себя клиентские библиотеки, которые позволяют упростить использование каналов OData из приложения .NET Framework и клиента. Эти библиотеки упрощают отправку и получение сообщений HTTP. Кроме того, они преобразуют полезные данные сообщений в объекты CLR, представляющие данные сущностей. Клиентские библиотеки содержат два базовых класса: <xref:System.Data.Services.Client.DataServiceContext> и <xref:System.Data.Services.Client.DataServiceQuery%601>. Эти классы позволяют отправлять запросы к службе данных и работать с возвращенными данными сущностей как с объектами CLR. В этом разделе описаны два подхода к созданию действий, в которых используются клиентские библиотеки.

### <a name="adding-a-service-reference-to-the-wcf-data-service"></a>Добавление ссылки на службу в службу данных WCF

Чтобы сформировать клиентские библиотеки Northwind, можно использовать **Add Service Reference** диалогового окна в Visual Studio 2012, чтобы добавить ссылку на службу Northwind OData.

![Снимок экрана: диалоговое окно Add Service Reference.](./media/consuming-odata-feeds-from-a-workflow/add-service-reference-dialog.gif)
...
> [!NOTE]
> Дополнительные сведения см. в разделе [Создание клиентской библиотеки службы данных (службы данных WCF)](../data/wcf/generating-the-data-service-client-library-wcf-data-services.md).

### <a name="using-asynchronous-methods"></a>Использование асинхронных методов

Для разрешения возможных проблем с задержками, которые могут произойти при обращении к ресурсам через Интернет, рекомендуется производить обращения к службам данных WCF асинхронно. Библиотеки клиента служб данных WCF включают в себя асинхронные методы для вызова запросов, и Windows Workflow Foundation (WF) предоставляет <xref:System.Activities.AsyncCodeActivity> класс для создания асинхронных действий. <xref:System.Activities.AsyncCodeActivity> Чтобы воспользоваться преимуществами классы .NET Framework, имеющие асинхронные методы могут записываться действия, производные от, или код, выполняемый асинхронно можно поместить в метод и вызывать с помощью делегата. В этом разделе приводятся два примера действия, являющегося производным от <xref:System.Activities.AsyncCodeActivity> , - одно из них использует асинхронные методы клиентских библиотек служб данных WCF, а второе - делегат.

> [!NOTE]
> Дополнительные сведения см. в разделе [асинхронные операции (службы данных WCF)](../data/wcf/asynchronous-operations-wcf-data-services.md) и [создание асинхронных действий](creating-asynchronous-activities-in-wf.md).
...
>
> необработанное исключение: "System.InvalidOperationException: Произошла ошибка при обработке этого запроса. ---> System.Net.WebException: Не удается подключиться к удаленному серверу---> System.Net.Sockets.SocketException: Попытка подключения, не удалось, так как подключенная сторона не ответила правильно после определенного периода времени, или установленное соединение, так как подключенный узел не отвечает.
...
### <a name="using-a-delegate"></a>С помощью делегата

Помимо вызова асинхронного метода класса .NET Framework, <xref:System.Activities.AsyncCodeActivity>-действие, основанное на можно также определить асинхронной логики в одном из этих методов. Этот метод указывается с помощью делегата в переопределении действия <xref:System.Activities.AsyncCodeActivity.BeginExecute%2A> . Когда метод возвращает управление, среда выполнения вызывает переопределения действия <xref:System.Activities.AsyncCodeActivity.EndExecute%2A> . При вызове службы OData из рабочего процесса этот метод можно использовать для запроса к службе и выполнения дополнительной обработки.
...
> [!NOTE]
> Дополнительные сведения о разбиении на страницы в службах WCF Data Services см. в разделе [как: Загрузка разбитых на страницы результатов (службы данных WCF)](../data/wcf/how-to-load-paged-results-wcf-data-services.md).
...
> [!NOTE]
> Если метод, выполняющий асинхронные операции, не имеет возвращаемого значения, то используется <xref:System.Action> вместо <xref:System.Func%601>. Примеры создания асинхронного образца с использованием обоих методов, см. в разделе [создание асинхронных действий](creating-asynchronous-activities-in-wf.md).
...
## <a name="consuming-an-odata-feed-without-using-the-client-libraries"></a>Использование OData в веб-канала без использования клиентских библиотек
...
В этом примере представлен один метод, который разработчики приложений рабочих процессов могут использовать для работы с необработанными данными, полученными от службы OData. Дополнительные сведения о доступе к службам данных WCF с использованием URI см. в разделе [ресурсы службы доступа к данным (службы данных WCF)](../data/wcf/accessing-data-service-resources-wcf-data-services.md) и [OData: Соглашения об URI](https://go.microsoft.com/fwlink/?LinkId=185564).
