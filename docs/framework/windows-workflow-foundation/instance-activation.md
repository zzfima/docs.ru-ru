 ...
 **Период обнаружения запускаемых экземпляров** свойство Store экземпляра рабочего процесса SQL определяет период времени, после чего Store экземпляра рабочего процесса SQL выполняет задачу обнаружения для обнаружения любой рабочий процесс или активированы экземпляры в базе данных сохраняемости после предыдущего цикла обнаружения. См. в разделе [период обнаружения запускаемых экземпляров](../../../docs/framework/windows-workflow-foundation/runnable-instances-detection-period.md) Дополнительные сведения об этом свойстве.
