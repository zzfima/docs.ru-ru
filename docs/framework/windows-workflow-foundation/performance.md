...
### <a name="wf-runtime"></a>Среда выполнения WF  
 ...
 Планировщик среды выполнения WF4 более эффективен по сравнению с WF3. Он использует тот же пул потоков ввода-вывода, который используется для WCF, который очень эффективен при выполнении пакетных рабочих элементов. Внутренняя очередь рабочих элементов оптимизирована для самых распространенных шаблонов использования. Среда выполнения WF4 также обеспечивает значительно облегченное управление состоянием выполнения с минимальной степенью синхронизации и логикой обработки событий, тогда как в WF3 это делается с помощью громоздкой регистрации событий и их вызова для выполнения сложной синхронизации при смене состояний.  
...
### <a name="performance-test-results"></a>Результаты теста производительности  
 ![Результаты теста производительности](../../../docs/framework/windows-workflow-foundation/media/performancedata.gif "опубликованных")  
...
#### <a name="workflow-activation-latency"></a>Задержки активации рабочего процесса  
 В приложении службы рабочего процесса WCF задержки для запуска нового рабочего процесса или загрузке существующего рабочего процесса важна, поскольку она может заблокировать.  В данном проверочном варианте производится измерение узла WF3 XOML в сравнении с узлом WF4 XAMLX в типичном сценарии.  
...
##### <a name="test-setup"></a>Настройка теста  
 В этом сценарии клиентский компьютер обращается службы рабочего процесса WCF, с помощью корреляции на основе контекста.  Корреляция контекста требует специальной привязки контекста и использует заголовок контекста или файл cookie для связывания сообщений с нужным экземпляром рабочего процесса.  Преимущество в производительности объясняется тем, что идентификатор корреляции располагается в заголовке сообщения, поэтому не требуется синтаксический анализ его текста. Дополнительные сведения о корреляции контекста в разделе [корреляцию обмена контекстом](../../../docs/framework/wcf/feature-details/context-exchange-correlation.md)  
...
#### <a name="test-setup"></a>Настройка теста  
 ...
 Рабочий процесс, показанный выше совпадает с календарем, используемым в приведенном ниже разделе «Сохраняемость».  Для тестов корреляции без сохраняемости в среде выполнения поставщик сохраняемости не устанавливается.  Корреляция происходит в двух методах: CreateOrder и CompleteOrder.  
...
|Службы рабочих процессов, размещаемые в IIS|446 MB|364 MB|  
 ...
 Размещение определения рабочего процесса в IIS будет занято больше памяти из-за <xref:System.ServiceModel.WorkflowServiceHost>, подробных артефактов службы WCF и связанные с узлом логики обработки сообщений.  
...
-   Физический диск\\% дискового времени записи  
...
-   События трассировки событий Windows легко сочетаются с событий WCF ETW или другими поставщиками ETW, включая поставщик SQL Server или ядра.  
...
## <a name="interop"></a>Interop  
 WF4 представляет собой почти полностью переписанный [!INCLUDE[wf1](../../../includes/wf1-md.md)], поэтому рабочие процессы и действия WF3 не являются непосредственно совместимыми c WF4.  Многие клиенты, которые реализуют Windows Workflow Foundation раньше будут иметь определения собственные или сторонние рабочих процессов и пользовательских действий для WF3.  Одним из способов облегчить переход на WF4 является использование действия Interop, которое способно выполнять действия WF3 в рабочих процессах WF4.  Рекомендуется использовать действие <xref:System.Activities.Statements.Interop> только при необходимости. Дополнительные сведения о миграции на WF4 извлечь [руководство по миграции для WF4](http://go.microsoft.com/fwlink/?LinkID=153313).  
