...
ms.locfileid: "79182978"
---
# <a name="windows-workflow-foundation-4-performance"></a>Производительность Windows Workflow Foundation 4

 .NET Framework 4 включает в себя крупный пересмотр Фонда рабочего процесса Windows (WF) с большими инвестициями в производительность. Этот новый пересмотр вводит значительные изменения [!INCLUDE[wf1](../../../includes/wf1-md.md)] в дизайне по сравнению с предыдущими версиями, которые поставляются в рамках .NET Framework 3.0 и .NET Framework 3.5. Переработка множества аспектов от ядра модели программирования и среды выполнения до инструментария позволила значительно повысить производительность и удобство использования. В этом разделе описаны важные характеристики производительности этих версий по сравнению с аналогичными характеристиками предыдущей версии.

 Производительность компонентов отдельных рабочих процессов повысилась за счет разницы в скорости выполнения операций умножения между WF3 и WF4.  Это оставляет разрыв между ручной кодированной Windows Communication Foundation (WCF) услуг и WCF служб рабочего процесса, чтобы быть довольно небольшим.  Время ожидания рабочего процесса в WF4 значительно сократилось.  Производительность сохранения данных повысилась в 2,5–3,0 раза.  Заметно снизились издержки при наблюдении за работоспособностью при отслеживании рабочего процесса.  Все это делает применение WF4 в приложениях или переход на эту платформу очень привлекательным.

## <a name="terminology"></a>Терминология

 Версия, [!INCLUDE[wf1](../../../includes/wf1-md.md)] представленная в .NET Framework 4, будет называться WF4 для остальной части этой темы. [!INCLUDE[wf1](../../../includes/wf1-md.md)]был введен в .NET Framework 3.0 и имел несколько незначительных изменений через .NET Framework 3.5 SP1. Версия Фонда рабочего процесса .NET Framework 3.5 будет называться WF3 для остальной части этой темы. WF3 поставляется в .NET Framework 4 бок о бок с WF4. Для получения дополнительной информации о миграции артефактов WF3 в WF4 см.: [Windows Workflow Foundation 4 Миграционное руководство.](migration-guidance.md)

 Windows Communication Foundation (WCF) — унифицированная модель программирования корпорации Майкрософт для создания приложений, ориентированных на обслуживание. Он был впервые представлен как часть .NET 3.0 вместе с WF3 и в настоящее время является одним из ключевых компонентов .NET Framework.
...
### <a name="wf-runtime"></a>Среда выполнения WF
 ...
 Планировщик среды выполнения WF4 более эффективен по сравнению с WF3. Он использует тот же пул потоков вв/o, который используется для WCF, который очень эффективен при выполнения пакетированных рабочих элементов. Внутренняя очередь рабочих элементов оптимизирована для самых распространенных шаблонов использования. Время выполнения WF4 также управляет состояниями выполнения очень легким способом с минимальной логикой синхронизации и обработки событий, в то время как WF3 зависит от регистрации событий большого веса и вызова для выполнения сложной синхронизации для переходов состояния.

### <a name="data-storage-and-flow"></a>Хранение и поток данных
 В WF3 данные, связанные с действием, моделируется через свойства зависимостей, которые реализуются типом <xref:System.Windows.DependencyProperty>. Шаблон свойств зависимости был введен в Фонд презентации Windows (WPF). В целом этот шаблон стал более гибким и обеспечивает простоту привязки данных и другие возможности пользовательского интерфейса. Однако при этом требуется объявление свойств как статических полей в определении рабочего процесса. Когда среда выполнения [!INCLUDE[wf1](../../../includes/wf1-md.md)] устанавливает или возвращает значения свойств, задействуется громоздкая логика поиска.

 В WF4 используются прозрачные правила видимости, которые значительно повышают эффективность обработки данных в рабочем процессе. При этом данные, хранимые внутри действия, отделяются от данных, проходящих через границы действия, за счет разделения двух понятий: переменные и аргументы. Используя четкую иерархическую область для переменных и аргументов "In/Out/InOut", сложность использования данных для действий резко снижается, а срок службы данных также автоматически осуществляется. В аргументах действий имеются строгие определения сигнатур. Простой проверкой действия можно определить, какие данные оно ожидает получить и какие данные будут получены в результате выполнения.
...
### <a name="control-flow"></a>Поток управления
 ...
 В WF4 это делается несколько иначе. Берется шаблон действия, и создается новый объект ActivityInstance, который затем добавляется в очередь планировщика. Весь этот процесс включает в себя только явное создание объектов и очень легкий.
...
```
 ...
 Обработчик неисправности нацелен на `WorkScope` деятельность. Рабочий процесс WF4 не менее упрощен.  <xref:System.Activities.Statements.CompensableActivity> имеет основной код и обработчик компенсации.  Далее в последовательности выполняется явная компенсация.  Действие текста и действие обработчика компенсации - пустые реализации:
...
На следующей диаграмме показан основной процесс компенсации. Рабочий процесс WF3 слева, а рабочий процесс WF4 справа.

![Основные компенсационные рабочие процессы WF3 и WF4](./media/performance/basic-compensation-workflows-wf3-wf4.gif)

### <a name="performance-test-results"></a>Результаты теста производительности

 ![Таблица, показывающая данные о результатах тестирования производительности](./media/performance/performance-test-data.gif)

 ![Диаграмма столбцов, сравнивающая данные теста производительности WF3 и WF4](./media/performance/performance-test-chart.gif)
...
## <a name="service-composition-scenario"></a>Сценарий композиции служб
 Как показано в предыдущем разделе "Сравнение производительности на уровне компонентов", произошло значительное сокращение накладных расходов между WF3 и WF4.  Службы рабочего процесса WCF теперь могут почти соответствовать производительности сервисов [!INCLUDE[wf1](../../../includes/wf1-md.md)] WCF с ручным кодом, но все еще имеют все преимущества времени выполнения.  Этот сценарий тестирования сравнивает службу WCF с службой рабочего процесса WCF в WF4.

### <a name="online-store-service"></a>Служба интернет-магазина
 Одной из сильных сторон Windows Workflow Foundation является возможность составления процессов с помощью нескольких служб.  Например, имеется служба интернет-магазина, которая координирует два вызова служб при размещении заказа.  На первом этапе производится проверка заказа с помощью службы проверки заказа.  На втором этапе производится заполнение заказа с помощью службы склада.

 Обе серверные службы - проверки заказа и склада - одинаковы в обоих тестах.  Изменяющийся компонент - служба интернет-магазина, которая выполняет координацию.  В одном случае услуга кодируется вручную как услуга WCF.  В другом случае услуга написана как служба рабочего процесса WCF в WF4. Возможности, относящиеся к [!INCLUDE[wf1](../../../includes/wf1-md.md)], например отслеживание сохраняемости, для этого теста будут отключены.

### <a name="environment"></a>Среда
![Установка среды для измерения производительности](./media/performance/performance-test-environment.gif)

 Клиентские запросы поступают к службе интернет-магазина по протоколу HTTP от множества компьютеров.  Все три службы размещены на одном компьютере.  Транспортный уровень между службой интернет-магазина и серверными службами - TCP или HTTP.  Измерения операций производятся по числу обработанных вызовов `PurchaseOrder` к службе интернет-магазина.  Пул каналов - новая возможность, появившаяся в WF4.  В WCF часть этого тестового канала объединения не предусмотрена из коробки, поэтому в сервисе Интернет-магазина была использована закодированная вручную реализация простого метода объединения.

### <a name="performance"></a>Производительность
![Диаграмма столбцов, показывающая производительность службы онлайн-магазина](./media/performance/online-store-performance-graph.gif)
...
#### <a name="workflow-activation-latency"></a>Задержки активации рабочего процесса
 В приложении службы обработки рабочего процесса WCF важна задержка для запуска нового рабочего процесса или загрузки существующего рабочего процесса, поскольку он может быть блокирован.  В данном проверочном варианте производится измерение узла WF3 XOML в сравнении с узлом WF4 XAMLX в типичном сценарии.
...
##### <a name="test-setup"></a>Настройка теста
 В сценарии клиентский компьютер связывается с службой рабочего процесса WCF, используя контекстную корреляцию.  Корреляция контекста требует специальной привязки контекста и использует заголовок контекста или файл cookie для связывания сообщений с нужным экземпляром рабочего процесса.  Преимущество в производительности объясняется тем, что идентификатор корреляции располагается в заголовке сообщения, поэтому не требуется синтаксический анализ его текста.
 ...
 ![Рабочий процесс с областью корреляции WF4](./media/performance/wf4-correlationscope-workflow.gif)

 Действие <xref:System.ServiceModel.Activities.Receive> создает экземпляр рабочего процесса.  Значение, переданное в полученном сообщении, повторяется в ответном сообщении.  Последовательность после ответа содержит оставшуюся часть рабочего процесса.  В приведенном выше примере показано только одно действие комментария.  Число действий комментариев изменено, чтобы имитировать сложный рабочий процесс.  Действие комментария эквивалентно действию WF3 <xref:System.Workflow.Activities.CodeActivity>, которое ничего не делает. Для получения дополнительной информации об деятельности по комментариям смотрите раздел «Сравнение производительности на уровне компонентов» ранее в этой статье.

##### <a name="test-results"></a>Результаты теста

 Холодная и теплая задержка для служб рабочего процесса WCF:

 ![Диаграмма столбцов, показывающая холодную и теплую задержку для служб рабочего процесса WCF с помощью WF3 и WF4](./media/performance/latency-results-graph.gif)

 В предыдущем графике холод относится к случаю, <xref:System.ServiceModel.WorkflowServiceHost> когда нет существования для данного рабочего процесса.  Другими словами, «холодная» задержка - это когда рабочий процесс используется в первый раз и для его компиляции нужен XOML или XAML.  «Теплая» задержка - это время, необходимое для создания нового экземпляра рабочего процесса, когда его тип уже скомпилирован.  При использовании WF4 сложность рабочего процесса почти не влияет на результат, но при использовании WF3 линейно возрастает.

#### <a name="correlation-throughput"></a>Пропускная способность корреляции
 В WF4 появилась новая возможность корреляции на основе содержимого.  В WF3 реализована только корреляция на основе контекста.  Контекстная корреляция может быть выполнена только по конкретным привязками каналов WCF.  Идентификатор рабочего потока при использовании этих привязок вставляется в заголовок сообщения.  Время выполнения WF3 может идентифицировать рабочий процесс только по идентификатору.  При корреляции на основе содержимого автор рабочего процесса может создать ключ корреляции из соответствующей части данных, такой как номер счета или идентификатор клиента.
...
#### <a name="environment-setup"></a>Настройка среды
![Настройка среды для проверки производительности рабочего процесса](./media/performance/performance-test-environment.gif)

#### <a name="test-setup"></a>Настройка теста
![Тест пропускной способности корреляции рабочего процесса](./media/performance/correlation-throughput-workflow.gif "Установка теста на обработку рабочего процесса корреляции")

 Предыдущий рабочий процесс является тем же самым, что используется в разделе [Persistence.](#persistence) Для корреляционных тестов без сохранения, нет настойчивости провайдера, установленного в времени выполнения. Корреляция происходит в двух методах: CreateOrder и CompleteOrder.

#### <a name="test-results"></a>Результаты теста
![Пропускная способность корреляции](./media/performance/correlation-throughput-graph.gif "График пропускной связи")
...
#### <a name="correlation-with-persistence"></a>Корреляция с сохраняемостью
 ...
 ![Диаграмма строки, показывающая результаты корреляции и настойчивости](./media/performance/correlation-persistence-graph.gif)
...
### <a name="test-setup"></a>Настройка теста
 ...
 ![Уравнение для вычисления количества действий WF3](./media/performance/wf3-number-activities-equation.gif)
 ...
 ![Таблица, отображив ажиотажие действия в каждом тесте](./media/performance/workflow-variation-compare-table.gif)
 ...
 ![Flowchart сложного рабочего процесса пропускной связи](./media/performance/complex-workflow-throughput-workflow.gif)
 ...
 ![Фрагмент Flowchart рабочего процесса WF3](./media/performance/wf3-workflow-snippet.gif)
 ...
 ![Flowchart вложенной последовательности](./media/performance/nested-sequence-workflow.gif)

 Отслеживание и сохраняемость в этом тесте не используются.

### <a name="test-results"></a>Результаты теста
 ![Диаграмма столбцов, показывающая результаты производительности пропускной способности](./media/performance/throughput-performance-results.gif)
...
### <a name="workflow-complexity-test"></a>Тест сложности рабочего процесса
 ...
 ![Сложный рабочий процесс как для WF3, так и для WF4](./media/performance/complex-workflow-wf3-wf4.gif)
 ...
 ![Диаграмма столбцов, показывающая сложное использование памяти рабочего процесса для рабочих процессов WF3 и WF4](./media/performance/complex-memory-usage-wf3-wf4.gif)
...
### <a name="multiple-workflow-definitions-test"></a>Тест нескольких определений рабочих процессов
 ...
 При размещении рабочих процессов в IIS можно с помощью <xref:System.Web.Hosting.VirtualPathProvider> создать новый <xref:System.ServiceModel.WorkflowServiceHost> вместо формирования всех файлов XAMLX или XOML.  Обработка <xref:System.Web.Hosting.VirtualPathProvider> входящего запроса и отвечает "виртуальным файлом", который может быть загружен из базы данных или, в данном случае, сгенерирован на лету.  Поэтому нет необходимости создавать 1000 физических файлов.
...
На следующем изображении показан рабочий процесс WF3 с ReceiveActivity и рабочий процесс WF4 с шаблоном запроса/ответа:

 ![Службы рабочих процессов в WF3 и WF4](./media/performance/workflow-receive-activity.gif)

 В следующей таблице показана дельта в рабочем наборе между определением одного рабочего процесса и определениями 1001:
...
|Службы рабочих процессов, размещаемые в IIS|446 MB|364 MB|
 ...
 Хостинг определения рабочего процесса в IIS потребляет <xref:System.ServiceModel.WorkflowServiceHost>гораздо больше памяти из-за, подробные артефакты службы WCF, и логика обработки сообщений, связанных с хостом.
...
### <a name="test-setup"></a>Настройка теста
 ...
 ![Устойчивый рабочий процесс в WF3 слева и WF4 справа](./media/performance/persist-workflow-wf3-wf4.gif)

 Оба рабочих процесса создаются при получении сообщения.  После отправки первого ответа рабочие процессы сохраняются.  В случае WF3 для сохранения используется пустой объект <xref:System.Workflow.ComponentModel.TransactionScopeActivity>.  То же самое можно было бы достичь и в WF3, пометив деятельность как «сохраняемую на близком насесь».  Второе, связанное сообщение завершает рабочий процесс.  Рабочие процессы сохраняются, но не выгружаются.

### <a name="test-results"></a>Результаты теста
 ![Диаграмма столбцов, показывающая сохранение пропускной высостойкости](./media/performance/throughput-persistence-graph.gif)
...
- Физическийдиск\\%Диск Время чтения

- Физическийдиск\\% Время диска

- PhysicalDisk\\% Время записи диска

- Физическийдиск\\% Avg. Длина очереди диска

- ФизическийДиск-Avg. Длина очереди диска

- PhysicalDisk-Avg. Диск Написать Длина очереди

- PhysicalDisk\Current Disk Queue Length

- Процессор\\Информация % Время процессора
...
- События отслеживания ETW легко сочетаются с событиями WCF ETW или другими поставщиками ETW, такими как поставщик сервера или ядра S'L.
...
- Администратор может включать и выключать трассировку без перезапуска хост-процесса.
 ...
 События трассировки событий Windows могут регистрироваться с помощью таких средств, как logman или xperf.  Компактный файл ETL можно просмотреть с помощью такого средства, как xperfview, или преобразовать в более удобочитаемый формат, например XML, с помощью tracerpt.  В WF3 единственная возможность получения событий трассировки без базы данных SQL - создание пользовательской службы трассировки. Для получения дополнительной информации о ETW см. [WCF Services и отслеживание событий для Windows](../wcf/samples/wcf-services-and-event-tracing-for-windows.md) и [отслеживания событий - Windows-приложений](/windows/desktop/etw/event-tracing-portal).
...
### <a name="test-results"></a>Результаты теста
 ...
 ![Диаграмма столбцов, показывающая затраты на отслеживание рабочего процесса](./media/performance/workflow-tracking-costs.gif)
...
## <a name="interop"></a>Interop
 WF4 представляет собой почти полностью переписанный [!INCLUDE[wf1](../../../includes/wf1-md.md)], поэтому рабочие процессы и действия WF3 не являются непосредственно совместимыми c WF4.  Многие клиенты, которые приняли Windows Workflow Foundation на раннем этапе будут иметь внутренние или сторонние определения рабочего процесса и пользовательские действия для WF3.  Одним из способов облегчить переход на WF4 является использование действия Interop, которое способно выполнять действия WF3 в рабочих процессах WF4.  Рекомендуется использовать действие <xref:System.Activities.Statements.Interop> только при необходимости. Для получения дополнительной информации о миграции в WF4 проверить [WF4 Миграция Руководство](migration-guidance.md).
