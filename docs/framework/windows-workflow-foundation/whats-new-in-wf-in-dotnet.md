---
title: "Новые возможности Windows Workflow Foundation в .NET 4.5 | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
>   
>  -   Выражения на языке C\# заменяются в конструкторе сообщением **Значение задано в языке XAML**.  
...
-   Введено динамическое обновление, которое позволяет обновить определение сохраненного экземпляра рабочего процесса.Дополнительные сведения см. в разделах [Динамическое обновление](../../../docs/framework/windows-workflow-foundation//dynamic-update.md) и [Как обновить определение выполняющегося экземпляра рабочего процесса](../../../docs/framework/windows-workflow-foundation//how-to-update-the-definition-of-a-running-workflow-instance.md).  
  
-   Предоставляется скрипт базы данных SqlWorkflowInstanceStoreSchemaUpgrade.sql для обновления баз данных сохраняемости, созданных с помощью скриптов базы данных [!INCLUDE[netfx40_short](../../../includes/netfx40-short-md.md)].Этот скрипт обновляет базы данных сохраняемости [!INCLUDE[netfx40_short](../../../includes/netfx40-short-md.md)] для поддержки новых возможностей управления версиями, представленных в [!INCLUDE[net_v45](../../../includes/net-v45-md.md)].Сохраняемые экземпляры рабочих процессов в базе данных получают значения управления версиями по умолчанию и могут участвовать в параллельном выполнении и динамическом обновлении.[Обновление баз данных сохраняемости .NET Framework 4 для управления версиями рабочего процесса](../../../docs/framework/windows-workflow-foundation//using-workflowidentity-and-versioning.md#UpdatingWF4PersistenceDatabases) [!INCLUDE[crdefault](../../../includes/crdefault-md.md)].  
...
###  <a name="BKMK_NewFlowchartCapabilities"></a> Новые возможности блок\-схемы  
...
4.  Создатели действия должны переопределить метод <xref:System.Activities.Activity.CacheMetadata%2A>, чтобы в среде выполнения рабочего процесса отражение не использовалось автоматически для этого типа.Аргументы и дочерние действия не должны иметь значение NULL, а метод <xref:System.Activities.ActivityMetadata.Bind%2A> должен вызываться явным образом.Дополнительные сведения о переопределении метода <xref:System.Activities.Activity.CacheMetadata%2A> см. в разделе [Предоставление данных с помощью CacheMetadata](../../../docs/framework/windows-workflow-foundation//exposing-data-with-cachemetadata.md).Кроме того, экземпляры аргументов типа `internal` или **private** необходимо создавать явно в <xref:System.Activities.Activity.CacheMetadata%2A>, чтобы они не создавались путем отражения.  
  
5.  Типы не будут использовать <xref:System.SerializableAttribute> или <xref:System.Runtime.Serialization.ISerializable> для сериализации. Типы, которые должны быть сериализованы, должны поддерживать <xref:System.Runtime.DataContractSerializer>.  
...
7.  В режиме частичного уровня доверия выражения не могут компилироваться с помощью <xref:System.Activities.XamlIntegration.WorkflowExpressionCompiler> или размещенного компилятора Visual Basic, но ранее скомпилированные выражения будут работать.  
  
8.  Отдельная сборка, использующая [прозрачность уровня 2](http://aka.ms/Level2Transparency), не может использоваться в [!INCLUDE[netfx40_short](../../../includes/netfx40-short-md.md)], [!INCLUDE[netfx_current_short](../../../includes/netfx-current-short-md.md)] в режиме полного доверия и в [!INCLUDE[netfx_current_short](../../../includes/netfx-current-short-md.md)] в режиме частичного уровня доверия.  
...
-   Быстрый поиск, запускаемый сочетанием клавиш **CTRL\+F** или командами **Правка**, **Найти и заменить** и **Быстрый поиск**.  
  
-   Поиск в файлах, запускаемый сочетанием клавиш **CTRL\+SHIFT\+F** или командами **Правка**, **Найти и заменить** и **Найти в файлах**.  
...
###  <a name="BKMK_VariableDeleteContextMenu"></a> Пункт контекстного меню «Удалить» в конструкторе переменных и аргументов  
...
###  <a name="BKMK_MultiSelect"></a> Выбор нескольких действий  
...
###  <a name="BKMK_CSharpExpressions"></a> Выражения C\#  
 ...
 [!INCLUDE[crabout](../../../includes/crabout-md.md)] выражениях на C\# см. в разделе [Выражения C\#](../../../docs/framework/windows-workflow-foundation//csharp-expressions.md).  
  
###  <a name="BKMK_Visibility"></a> Дополнительные возможности управления видимостью панели оболочки и элементов заголовка  
...
###  <a name="BKMK_BackwardCompatRehostedDesigner"></a> Включаемые по требованию функции рабочего процесса 4.5 в повторно размещенном конструкторе  
...
###  <a name="BKMK_StateMachine"></a> Рабочие процессы конечного автомата  
 Рабочие процессы конечного автомата появились в составе платформы .NET Framework 4.0.1 в [обновлении 1 для платформы Microsoft .NET Framework 4](http://go.microsoft.com/fwlink/?LinkID=215092).Это обновление включило несколько новых классов и действий, которые дали возможность разработчикам создавать рабочие процессы конечного автомата.Для [!INCLUDE[net_v45](../../../includes/net-v45-md.md)] эти классы и действия были обновлены.Обновления включают следующее:  
...
4.  Действия для создания рабочих процессов конечного автомата, включая <xref:System.Activities.Statements.StateMachine>, <xref:System.Activities.Statements.State> и <xref:System.Activities.Statements.Transition>  
 ...
 Дополнительные сведения о создании рабочих процессов конечного автомата см. в разделе [Рабочие процессы конечного автомата](../../../docs/framework/windows-workflow-foundation//state-machine-workflows.md).  
