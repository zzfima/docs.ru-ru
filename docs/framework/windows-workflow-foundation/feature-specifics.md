...
## <a name="simplified-configuration"></a>Упрощенная конфигурация  
 Схема конфигурации WCF сложна и содержит много трудно найти компоненты. В [!INCLUDE[netfx_current_short](../../../includes/netfx-current-short-md.md)], мы постарались помочь пользователям WCF при настройке служб с помощью следующих средств:  
...
-   Опытному разработчику ASMX необходимо начать работу с WCF. Однако WCF кажется чересчур сложным! Что означает вся информация, которую нужно указать в файле конфигурации? В .NET 4 можно вообще не использовать файл конфигурации.  
