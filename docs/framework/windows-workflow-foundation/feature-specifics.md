...
Служба маршрутизации должен быть универсальный программный маршрутизатор, который позволяет управлять как сообщения WCF передавать между клиентами и службами. Служба маршрутизации позволяет отделить клиенты из служб, дающий гораздо большую гибкость с точки зрения конфигурации, можно поддерживать, и гибкость, имеется рассматривая способы размещения служб. В .NET 3.5 клиенты и службы были тесно связаны; клиент должны были быть известны обо всех служб, необходимо обратиться к и где они были найдены. Кроме того, служба WCF в .NET Framework 3.5 имела следующие ограничения.
