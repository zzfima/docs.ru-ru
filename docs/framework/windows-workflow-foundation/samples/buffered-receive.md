...
```  
 ...
 Конечная точка клиента настраивается аналогично, с использованием <xref:System.ServiceModel.NetMsmqBinding>. Код клиента и его конфигурация формируются с помощью **Add Service Reference** функции Visual Studio. В следующем примере в файле App.config формируется конечная точка клиента.  
...
4.  При запросе, если вы хотите создать виртуальные каталоги для проекта LoanService, выберите **Да**.  
...
3.  В **Управление компьютером** консоли, разверните узел **службы**, **приложений**, **Message Queuing**, **частные очереди** .  
  
4.  Щелкните правой кнопкой мыши очередь loanservice/service1.xamlx и выберите **свойства**.  
  
5.  Выберите **безопасности** вкладка и добавить **сообщение получают все**, **Peek Message** и **Отправка сообщения** разрешения.  
  
6.  Откройте диспетчер служб [!INCLUDE[iis60](../../../../includes/iis60-md.md)].  
  
7.  Перейдите к **Server**, **сайты**, **по умолчанию веб-сайт**, **частного**, **LoanService** и выберите  **Дополнительные параметры**  
...
2.  Разверните **службы** и **приложений**, **Message Queuing**, **частные очереди**.  
