---
title: "Процесс корпоративных закупок | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
>   
>  `<ДискУстановки>:\WF_WCF_Samples\WF\Application\PurchaseProcess`  
  
## Описание процесса  
 В этом образце показана реализация программы [!INCLUDE[wf](../../../../includes/wf-md.md)] для сбора предложений от поставщиков в интересах типичной компании.  
...
|RfpRepository|Репозиторий запросов предложений.  Эта реализация содержит примеры использования Linq to XML для запроса XML\-файла запросов предложений, созданного схематизированными средствами обеспечения сохраняемости.  Этот класс реализует <xref:System.Runtime.Persistence.IDataViewMapper>.|  
...
|ShowProposals|Показывает все активные и завершенные запросы предложений. **Note:**  Может потребоваться нажать кнопку **Обновить** в пользовательском интерфейсе для просмотра изменений на этом экране после создания или изменения запроса предложений.|  
...
2.  Чтобы выполнить проект веб\-клиента, откройте **Обозреватель решений** и щелкните правой кнопкой проект **Web Client**.  Выберите команду **Назначить запускаемым проектом**.  
  
3.  Чтобы выполнить проект клиента WinForms, откройте **Обозреватель решений** и щелкните правой кнопкой проект **WinForms Client**.  Выберите команду **Назначить запускаемым проектом**.  
...
-   **Создание нового запроса предложений** \- создает новый запрос предложений и запускает рабочий процесс покупки.  
...
-   Просматривать как \- пользователь может обращаться к запросу предложений, используя другие идентификаторы, путем выбора желаемого участника в поле со списком **Просматривать как** активной таблицы запросов предложений.  
  
### Параметры клиента WinForms  
  
-   **Создание запроса предложений** \- создает новый запрос предложений и запускает рабочий процесс покупки.  
...
-   **Подключиться как** \- пользователь может обращаться к запросу предложений, используя другие идентификаторы, путем выбора желаемого участника в поле со списком **Просматривать как** активной таблицы запросов предложений.  
