---
title: "Процесс найма | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
>   
>  `<диск_установки>:\WF_WCF_Samples\WF\Application\HiringProcess`  
  
## Описание процесса  
 Компании Contoso, Inc.требуется постоянно управлять количеством сотрудников во всех своих отделах.Поэтому каждый раз, когда какой\-нибудь сотрудник хочет нанять нового сотрудника, он должен выполнить процесс одобрения запроса о найме до того, как объявление о вакансии будет опубликовано.Этот процесс называется запросом о найме \(он определен в проекте HiringRequestService\), он состоит из двух этапов.  
...
|Действия обмена сообщениями|На блок\-схеме действия обмена сообщениями используются двумя способами.<br /><br /> -   Для получения сведений от пользователя \(получение решений и связанных с ними данных на каждом этапе одобрения\).<br />-   Для взаимодействия с другими существующими службами \(InboxService и OrgDataService, используемыми с помощью ссылок на службы\).|HiringRequestService|  
|Корреляция по содержимому|Сообщения об одобрении связываются со свойством ID запроса о найме.<br /><br /> -   При запуске процесса дескриптор взаимосвязи инициализируется с идентификатором запроса.<br />-   Входящие сообщения об одобрении связываются со своими идентификаторами \(первым параметром каждого сообщения об одобрении является идентификатор запроса\).|HiringRequestService \/ ResumeRequestService|  
|Настраиваемые действия \(декларативные и основанные на коде\)|В этом образце присутствует несколько пользовательских действий.<br /><br /> -   `SaveActionTracking`: это действие выдает настраиваемую запись <xref:System.Activities.Tracking.TrackingRecord> \(с помощью <xref:System.Activities.NativeActivityContext.Track%2A>\).Это действие создано с помощью императивного кода, расширяющего <xref:System.Activities.NativeActivity>.<br />-   `GetEmployeesByPositionTypes`: это действие получает список идентификаторов типов должностей и возвращает список людей, которые занимают эту должность в компании Contoso.Это действие создано декларативно \(с помощью конструктора действий\).<br />-   `SaveHiringRequestInfo`: это действие сохраняет сведения о `HiringRequest` \(с помощью `HiringRequestRepository.Save`\).Это действие создано с помощью императивного кода, расширяющего <xref:System.Activities.CodeActivity>.|HiringRequestService|  
...
|Параллельные действия|-   <xref:System.Activities.Statements.ParallelForEach%601> используется для параллельной регистрации в папке «Входящие» главы компании и менеджеров по персоналу \(этап «Ожидание одобрения двумя менеджерами по персоналу»\).<br />-   <xref:System.Activities.Statements.Parallel> используется для выполнения задач по очистке на этапах «Завершено» и «Отклонено».|HiringRequestService|  
...
2.  Щелкните решение в **обозревателе решений** правой кнопкой мыши и выберите **Свойства**.  
  
3.  Выберите параметр **Несколько запускаемых проектов** и задайте параметрам **CareersWebSite**, **InternalClient**, **HiringRequestService** и **ResumeRequestService** значение **Пуск**.Параметрам **ContosoHR**, **InboxService** и **OrgService** оставьте значение «None».  
  
4.  Постройте решение, нажав клавиши CTRL\+SHIFT\+B.Удостоверьтесь, что построение выполнено успешно.  
...
2.  В решении щелкните правой кнопкой мыши **InternalClient** и выберите **Просмотр в браузере**.Будет отображена страница `InternalClient` по умолчанию.Удостоверьтесь, что службы работают, и щелкните ссылку.  
  
3.  Будет отображен модуль **HiringRequest**.Далее можно выполнить описанный здесь сценарий.  
...
6.  В решении щелкните правой кнопкой мыши **CareersWebSite** и выберите **Просмотр в браузере**.  
  
7.  Вернитесь назад в `InternalClient`, щелкнув в решении **InternalClient** правой кнопкой мыши и выбрав **Просмотр в браузере**.  
  
8.  Перейдите в раздел **JobPostings**, щелкнув ссылку **Объявления о вакансиях** в верхнем меню папки «Входящие».Далее можно выполнить описанный здесь сценарий.  
...
2.  После создания запрос появится в папке «Входящие» Майкла \(если запрос не отображается, нажмите кнопку **Обновить**\) в ожидании одобрения Питера Брема, который является менеджером Майкла.  
...
1.  Теперь вакансию необходимо разместить на внешнем веб\-узле, с которого люди смогут присылать свои резюме \(чтобы увидеть ее, перейдите по ссылке **Вакансии**\).В настоящий момент задача по размещению вакансии лежит на представителе отдела кадров, который отвечает за ее размещение.  
  
2.  Сотрудник отдела кадров хочет изменить объявление \(щелкнув ссылку **Правка**\), задав время ожидания 60 минут \(в жизни это могут быть дни или недели\).Значение времени ожидания позволяет убрать объявление о вакансии с внешнего веб\-узла по истечении указанного времени.  
  
3.  После сохранения измененного объявления оно появится на вкладке **Получение резюме** \(обновите веб\-страницу, чтобы отобразить новое объявление о вакансии\).  
...
2.  Если вернуться к службе Job Postings List, то можно будет просмотреть собранные на данный момент резюме.  
...
2.  Если решение не было построено, то проверьте следующее.  
    ...
    -   Ссылка на `ContosoHR` присутствует в проектах `InternalClient` и `CareersWebSite`.  
...
3.  Если решение не выполняется, убедитесь в следующем.  
    ...
    2.  Ссылки на службы обновлены.  
        ...
        1.  Откройте папку App\_WebReferences.  
  
        2.  Щелкните правой кнопкой мыши **Contoso** и выберите **Обновить веб\-ссылки\/ссылки на службы**.  
