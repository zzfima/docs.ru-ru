...
|Действия обмена сообщениями|На блок-схеме действия обмена сообщениями используются двумя способами.<br /><br /> -Для получения сведений от пользователя (получение решений и связанные данные на каждом этапе одобрения).<br />-Для взаимодействия с другими существующими службами (InboxService и OrgDataService, используемыми с помощью ссылки на службу).|HiringRequestService|  
|Корреляция по содержимому|Сообщения об одобрении связываются со свойством ID запроса о найме.<br /><br /> — При запуске процесса дескриптор взаимосвязи инициализируется с Идентификатором запроса.<br />-Входящего сообщения об одобрении связываются со своими Идентификаторами (первый параметр каждого сообщения об одобрении является идентификатор запроса).|HiringRequestService / ResumeRequestService|  
...
3.  Выберите параметр **несколько запускаемых проектов** и задайте **CareersWebSite**, **InternalClient**, **HiringRequestService**, и **ResumeRequestService** для **запустить**. Оставить **ContosoHR**, **InboxService**, и **OrgService** значение None.  
...
2.  Щелкните правой кнопкой мыши **InternalClient** в решении, а затем выберите **Просмотр в браузере**. Будет отображена страница `InternalClient` по умолчанию. Удостоверьтесь, что службы работают, и щелкните ссылку.  
...
6.  Щелкните правой кнопкой мыши **CareersWebSite** в решение и выберите **Просмотр в браузере**.  
  
7.  Вернитесь к `InternalClient` щелкните правой кнопкой мыши **InternalClient** в решении и выберите команду **Просмотр в браузере**.  
  
8.  Последовательно выберите пункты **JobPostings** , щелкнув **вакансии** ссылку в верхнем меню папки "Входящие". Далее можно выполнить описанный здесь сценарий.  
...
2.  После создания запроса отображается в «входящие» Майкла (щелкните **обновление** Если запрос не отображается) ожидает Питера брема, который является менеджером Майкла.  
...
1.  Теперь вакансию необходимо разместить на внешний веб-узел которого люди смогут присылать (вы увидите это щелкнув **вакансии** связи). В настоящий момент задача по размещению вакансии лежит на представителе отдела кадров, который отвечает за ее размещение.  
  
2.  Сотрудник отдела Кадров хочет изменить объявление (щелкнув **изменить** ссылку), задав время ожидания, равное 60 минут (на практике, это может быть дни или недели). Значение времени ожидания позволяет убрать объявление о вакансии с внешнего веб-узла по истечении указанного времени.  
  
3.  После сохранения измененного объявления, он отображается в **получение резюме** (обновите веб-страницы, чтобы увидеть новое объявление о вакансии).  
