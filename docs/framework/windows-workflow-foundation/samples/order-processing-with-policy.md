---
title: "Обработка заказов с помощью политики | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# Обработка заказов с помощью политики
...
> [!NOTE]
>  Для этого образца необходимо установить и запустить [!INCLUDE[netfx35_long](../../../../includes/netfx35-long-md.md)].[!INCLUDE[vs_current_long](../../../../includes/vs-current-long-md.md)] необходимо, чтобы открыть файлы проекта и решения.  
  
 В данном образце демонстрируется проект `OrderProcessingPolicy`, в котором вводится заказ клиента, состоящий из нумерованного списка доступных элементов и почтового индекса.Обработка заказа выполняется успешно, если обе записи верны, в противном случае политика создает объекты ошибок, используя перегруженный оператор `+` и заранее заданный метод расширения, чтобы информировать пользователей об ошибках.  
...
-   `OrderProcessingPolicy`  
    ...
    -   `displayErrors`  
        ...
        1.  Вызывает перегруженный оператор `+` для копирования содержимого `invalidItemNumErrorCollection` и `invalidZipCodeErrorCollection` в экземпляр `invalidOrdersCollection``OrderErrorCollection`.  
...
>   
>  `<диск_установки>:\WF_WCF_Samples\WF\Basic\Rules\Policy\OrderProcessingPolicy`  
