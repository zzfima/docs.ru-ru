...
## <a name="sample-details"></a>Подробные сведения об образце  
 В этом примере две службы Windows Communication Foundation (WCF), реализованные с помощью Windows Workflow Foundation настраиваются для устойчивый дуплексный обмен сообщениями. Устойчивый дуплексный обмен сообщениями состоит из двух односторонних обменов сообщениями по MSMQ, сопоставляются с помощью [обмена контекстом .NET](https://go.microsoft.com/fwlink/?LinkID=166059). Сообщения отправляются посредством действий по обмену сообщениями <xref:System.ServiceModel.Activities.Send> и <xref:System.ServiceModel.Activities.Receive>. Обмен контекстом .NET используется для указания адреса обратного вызова в отправленных сообщениях. Обе службы размещаются при помощи служб активации процессов Windows WAS и настраиваются на включение сохраняемости экземпляров служб.  
...
> [!NOTE]
>  Используемые в данном образце очереди не являются транзакционными. Пример, в котором показано, как настроить обмен сообщениями WCF помощи транзакционных очередей, см. в разделе [активации MSMQ](../../../../docs/framework/wcf/samples/msmq-activation.md) образца.  
...
3.  Настройте очереди обслуживания.  
    ...
    3.  Разверните **служба и приложения**, **Message Queuing**. **Частные очереди**.  
  
    4.  Щелкните правой кнопкой мыши очереди durableduplex/service1.xamlx и durableduplex/service2.xamlx и выберите **свойства**.  
  
    5.  Выберите **безопасности** вкладку и разрешить **получение сообщения**, **Peek Message** и **Отправка сообщения** обоим очередям права.  
  
    6.  Откройте диспетчер служб IIS.  
  
    7.  Перейдите к **Server**, **сайты**, **по умолчанию веб-сайт**, **частного**, **устойчивый дуплекс** и выберите пункт **Дополнительные параметры**.  
  
    8.  Изменение **включенные протоколы** для **http,net.msmq**.  
  
4.  Выполните образец.  
  
    1.  Перейдите к http://localhost/private/durableduplex/service1.xamlx и http://localhost/private/durableduplex/service2.xamlx чтобы убедиться, что обе службы выполняются.  
...
2.  Удалите виртуальное приложение служб.  
    ...
    1.  Откройте диспетчер служб Internet Information Services (IIS), выполнив `Inetmgr.exe` из командной строки.  
  
    2.  Перейдите на веб-сайт по умолчанию и удалите **частного** виртуального каталога.  
