...
> [!NOTE]
>  Используемые в данном образце очереди не являются транзакционными. Пример, в котором показано, как настроить [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] обмен сообщениями с использованием транзакций очередей сообщений см. в разделе [активации MSMQ](../../../../docs/framework/wcf/samples/msmq-activation.md) образца.  
