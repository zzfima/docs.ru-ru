 ...
 **Экземпляров** содержит общую информацию обо всех экземплярах рабочего процесса в базе данных.  
...
|ExecutionStatus|Nvarchar(450)|Указывает текущее состояние выполнения рабочего процесса. Возможные состояния: **выполнение**, **Idle**, **закрыто**.|  
...
|IsCompleted|Разряд|Указывает, завершено ли выполнение экземпляра рабочего процесса. **Примечание:** Iif **атрибута instancecompletionaction классе** свойству **DeleteAll**, экземпляры удаляются из представления после завершения.|  
 ...
 **ServiceDeployments** представление содержит сведения о развертывании для всех веб-приложений (IIS / WAS) размещенной службы рабочего процесса. Каждый экземпляр рабочего процесса, размещенной на веб-сервере будет содержать **ServiceDeploymentId** , ссылается на строку в этом представлении.  
  
|Имя столбца|Тип столбца|Описание|  
|-----------------|-----------------|-----------------|  
|ServiceDeploymentId|BigInt|Первичный ключ для этого представления.|  
|SiteName|Nvarchar(max)|Представляет имя узла, который содержит службу рабочего процесса (например **веб-сайт по умолчанию**).|  
...
> [!NOTE]
>  Для индексированного представления требуется больше памяти и увеличиваются затраты на обработку. Обратитесь к [повышение производительности с помощью индексированных представлений SQL Server 2008](http://go.microsoft.com/fwlink/?LinkId=179529) для получения дополнительной информации.
