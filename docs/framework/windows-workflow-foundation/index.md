---
title: Windows Workflow Foundation
ms.date: 03/30/2017
helpviewer_keywords:
- WF [WF]
- Windows Workflow Foundation [WF]
ms.assetid: 9a23ea6b-d600-483e-89cd-8889cfec5f66
ms.openlocfilehash: 37c988e18090848ce1eb444bb9735aeadcdb566e
ms.sourcegitcommit: 160a88c8087b0e63606e6e35f9bd57fa5f69c168
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 03/09/2019
ms.locfileid: "57720288"
---
# <a name="windows-workflow-foundation"></a>Windows Workflow Foundation
В этом разделе описывается модель программирования, примеры и средства Windows Workflow Foundation (WF).  
  
## <a name="in-this-section"></a>В этом разделе  
 [Руководство по документации Windows Workflow Foundation](guide-to-the-documentation.md)  
 Список тем для ознакомления с учетом степени подготовленности (от новичка до хорошо ознакомленного с предметом разработчика) и требований.  
  
 [Новые возможности Windows Workflow Foundation](whats-new.md)  
 Описываются изменения некоторых парадигм разработки по сравнению с предыдущими версиями.  
  
 [Новые возможности Windows Workflow Foundation в .NET 4.5](whats-new-in-wf-in-dotnet.md)  
 Описывает новые возможности Windows Workflow Foundation в [!INCLUDE[netfx_current_long](../../../includes/netfx-current-long-md.md)].  
  
 [Подробное описание возможностей Windows Workflow Foundation](feature-specifics.md)  
 Описывает новые возможности Windows Workflow Foundation в [!INCLUDE[netfx40_short](../../../includes/netfx40-short-md.md)].  
  
 [Концептуальный обзор Windows Workflow Foundation](conceptual-overview.md)  
 Набор разделов, обсуждаются общие понятия и принципы Windows Workflow Foundation.  
  
 [Руководство по началу работы](getting-started-tutorial.md)  
 Набор пошаговые руководства, в которых рассматриваются программирования приложений Windows Workflow Foundation.  
  
 [Программирование в Windows Workflow Foundation](programming.md)  
 Основные темы, которые нужно изучить, чтобы стать профессиональным программистом [!INCLUDE[wf1](../../../includes/wf1-md.md)].  
  
 [Расширение Windows Workflow Foundation](extend.md)  
 Набор разделов, в которых обсуждается позволяют расширить и настроить Windows Workflow Foundation в соответствии с потребностями.  
  
 [Глоссарий Windows Workflow Foundation для .NET Framework 4.5](glossary.md)  
 Определяет список терминов WF.  
  
 [Примеры для Windows Workflow Foundation](./samples/index.md)  
 Содержит образцы приложений, демонстрирующих функции и сценарии WF.
