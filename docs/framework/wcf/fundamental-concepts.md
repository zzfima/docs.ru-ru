...
## <a name="wcf-fundamentals"></a>Основы WCF  
 WCF не среды выполнения и набор интерфейсов API для создания систем, используемых при отправке сообщений между службами и клиентами. Те же инфраструктура и интерфейсы API используются для создания приложений, обменивающихся данными с другими приложениями на данном компьютере или на компьютере, который находится в другой компании, и доступ к которому можно получить через Интернет.  
  
### <a name="messaging-and-endpoints"></a>Обмен сообщениями и конечные точки  
 WCF основана на связи на основе сообщений и любого другого действия понятие, моделируемые в виде сообщения (например, HTTP-запроса или сообщения Message Queuing (MSMQ)) можно представить единым образом в модели программирования. Это обеспечивает универсальный интерфейс API для разных транспортных механизмов.  
...
```  
 ...
 Клиент WCF может создаваться автоматически с помощью [ServiceModel Metadata Utility Tool (Svcutil.exe)](../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) и нацеливания его на работающую службу, которая публикует метаданные.  
 ...
 безопасность  
 В WCF включает в себя конфиденциальность (шифрование сообщений для исключения перехвата), целостности (средства обнаружения подделки сообщения), проверки подлинности (средства проверки серверов и клиентов) и авторизацию (Управление доступом к ресурсы). Эти функции предоставляются либо путем использования существующих механизмов обеспечения безопасности, таких как TLS по HTTP (также называемого HTTPS), либо путем реализации одной или нескольких различных спецификаций безопасности WS-*.  
