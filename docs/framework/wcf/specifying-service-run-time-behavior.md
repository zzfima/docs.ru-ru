...
-   <xref:System.ServiceModel.Description.ServiceDebugBehavior>. Справка сведения для службы WCF и включает возможность отладки.  
...
-   <xref:System.ServiceModel.Description.MustUnderstandBehavior>. Указывает, что WCF для отключения `MustUnderstand` обработки.  
