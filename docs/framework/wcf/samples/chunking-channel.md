...
### <a name="onclose"></a>OnClose  
 `OnClose` сначала задает `stopReceive` значение `true`, чтобы указать, что требуется остановить выполняющийся `ReceiveChunkLoop`. Затем метод ожидает события `receiveStopped`					<xref:System.Threading.ManualResetEvent>, которое задается при остановке `ReceiveChunkLoop`. Предполагая, что `ReceiveChunkLoop` остановится в течение заданного времени ожидания, `OnClose` вызывает `innerChannel.Close` с остающимся временем ожидания.  
