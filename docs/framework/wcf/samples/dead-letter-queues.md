---
title: "Очереди недоставленных сообщений | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
В этом образце показано, как обрабатывать недоставленные сообщения.  Он основан на образце [Привязка MSMQ с поддержкой транзакций](../../../../docs/framework/wcf/samples/transacted-msmq-binding.md).  В этом примере используется привязка `netMsmqBinding`.  Служба представляет собой резидентное консольное приложение, позволяющее наблюдать за получением службой сообщений из очереди.  
...
```  
 ...
 В этом образце используется код службы из примера [Привязка MSMQ с поддержкой транзакций](../../../../docs/framework/wcf/samples/transacted-msmq-binding.md).  
...
4.  Чтобы выполнить образец на одном или нескольких компьютерах, соответственно измените имена очередей, заменив «localhost» полным именем компьютера, и выполните инструкции в разделе [Выполнение примеров Windows Communication Foundation](../../../../docs/framework/wcf/samples/running-the-samples.md).  
...
>   
>  `<диск_установки>:\WF_WCF_Samples\WCF\Basic\Binding\Net\MSMQ\DeadLetter`  
