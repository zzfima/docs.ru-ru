...
- `System`: Системная очередь недоставленных сообщений используется для хранения недоставленного сообщения. Системная очередь недоставленных сообщений общая для всех приложений, работающих на компьютере.

- `Custom`: Пользовательская очередь недоставленных сообщений, указанная с помощью <xref:System.ServiceModel.MsmqBindingBase.CustomDeadLetterQueue%2A> свойства, используется для хранения недоставленного сообщения. Эта возможность доступна только в [!INCLUDE[wv](../../../../includes/wv-md.md)]. Используется, если приложению нужна собственная очередь недоставленных сообщений, независимая от других приложений, работающих на данном компьютере.
...
## <a name="comments"></a>Комментарии
 По умолчанию с транспортом привязки `netMsmqBinding` безопасность включена. Тип безопасности транспорта определяется двумя свойствами: `MsmqAuthenticationMode` и `MsmqProtectionLevel`. По умолчанию устанавливается режим проверки подлинности `Windows` и уровень защиты `Sign`. Чтобы служба MSMQ обеспечивала возможности проверки подлинности и подписывания, она должна входить в домен. Если запустить этот пример на компьютере, который не входит в домен, появится следующее сообщение об ошибке: "Внутренний сертификат очереди сообщений пользователя не существует".
