...
4.  Для запуска образца в очередь на изменение конфигурации одним или несколькими компьютерами имена соответствующим образом, замените имя localhost на полное имя компьютера и следуйте инструкциям в [выполнение образцов Windows Communication Foundation](../../../../docs/framework/wcf/samples/running-the-samples.md).  
