---
title: "Использование моникера WCF с клиентами COM | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
>   
>  `<диск_установки>:\WF_WCF_Samples\WCF\Basic\Services\Interop\COM`  
...
## Типизированный контракт  
 Чтобы использовать моникер с типизированным контрактом, в COM следует зарегистрировать типы с правильными атрибутами для контракта службы.  Сначала с помощью [Служебное средство ServiceModel Metadata Utility Tool \(Svcutil.exe\)](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) должен быть создан клиент.  Чтобы создать типизированную учетную запись\-посредник, выполните следующую команду из командной строки в каталоге клиента.  
...
```  
 ...
 При запуске образца в окне сообщений сервера скриптов Windows отображается ответ операции.  Это свидетельствует о том, что клиент COM осуществляет вызовы COM с использованием типизированного моникера для связи со службой [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)].  Несмотря на использование COM в клиентском приложении, взаимодействие со службой состоит только из вызовов веб\-службы.  
...
-   Имя и пространство имен контракта.  Эта идентификация требуется, поскольку WSDL может содержать не один контракт.  
    ...
    > [!NOTE]
    >  По умолчанию службы [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] создают отдельные файлы WSDL для каждого используемого пространства имен.  Они связаны с использованием конструктора импорта WSDL.  Поскольку моникер ожидает одного определения WSDL, в службе должно использоваться либо одно пространство имен \(как показано в этом образце\), либо требуется объединение отдельных файлов вручную.  
...
```  
 ...
 При запуске образца в окне сообщений сервера скриптов Windows отображается ответ операции.  Это свидетельствует о том, что клиент COM осуществляет вызовы COM с использованием моникера с контрактом WSDL для связи со службой [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)].  
  
## Контракт обмена метаданными  
...
```  
 ...
 При запуске образца в окне сообщений сервера скриптов Windows отображается ответ операции.  Это свидетельствует о том, что клиент COM осуществляет вызовы COM с использованием моникера с контрактом MEX для связи со службой [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)].  
...
3.  В командной строке [!INCLUDE[vsprvs](../../../../includes/vsprvs-md.md)] откройте папку \\client\\bin\\ в языковой папке.  
...
4.  Наберите `tlbexp.exe client.dll /out:CalcProxy.tlb`, чтобы экспортировать DLL\-библиотеку в файл TLB.  Появится сообщение "Предупреждение программы экспорта библиотек типов", однако это не свидетельствует о наличии проблем, поскольку универсальный тип не требуется.  
  
5.  Наберите `regasm.exe /tlb:CalcProxy.tlb client.dll`, чтобы зарегистрировать типы в COM.  Появится сообщение "Предупреждение программы экспорта библиотек типов", однако это не свидетельствует о наличии проблем, поскольку универсальный тип не требуется.  
  
6.  Наберите `gacutil.exe /i client.dll`, чтобы добавить сборку в глобальный кэш сборок.  
...
8.  Наберите `tlbexp.exe client.dll /out:CalcProxy.tlb`, чтобы экспортировать DLL\-библиотеку в файл TLB.  Появится сообщение "Предупреждение программы экспорта библиотек типов", однако это не свидетельствует о наличии проблем, поскольку универсальный тип не требуется.  
  
9. Наберите `regasm.exe /tlb:CalcProxy.tlb client.dll`, чтобы зарегистрировать типы в COM.  Перед запуском команды убедитесь, что в пути указана папка, содержащая `regasm.exe`.  
  
10. Наберите `gacutil.exe /i client.dll`, чтобы добавить сборку в глобальный кэш сборок.  Перед запуском команды убедитесь, что в пути указана папка, содержащая `gacutil.exe`.  
