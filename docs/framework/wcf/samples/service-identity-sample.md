...
1. Для операционной системы [!INCLUDE[wxp](../../../../includes/wxp-md.md)] или [!INCLUDE[wv](../../../../includes/wv-md.md)] импортируйте файл сертификата Identity.pfx в папку решения для идентификации в хранилище сертификатов LocalMachine/My (Personal) с помощью средства оснастки MMC. Этот файл защищен паролем. Во время импорта будет запрошен пароль. Введите `xyz` в поле пароль. Дополнительные сведения см. в разделе [Практическое руководство. Просмотр сертификатов с помощью оснастки](../../../../docs/framework/wcf/feature-details/how-to-view-certificates-with-the-mmc-snap-in.md) MMC. После этого запустите Setup. bat в Командная строка разработчика для Visual Studio с правами администратора, который скопирует этот сертификат в хранилище CurrentUser/Trusted Peoples для использования на клиенте.

2. В [!INCLUDE[ws2003](../../../../includes/ws2003-md.md)]запустите программу Setup. bat из папки примера установки в командной строке Visual Studio 2012 с правами администратора. При этом устанавливаются все сертификаты, необходимые для выполнения образца.
...
3. Запустите программу Service.exe из каталога \service\bin. Убедитесь, что служба указывает, что она готова, и отобразится запрос на \<нажатие клавиши Ввод > для завершения работы службы.  
...
6. В службе запустите `setup.bat service` в Командная строка разработчика для Visual Studio, открытой с правами администратора. При запуске `setup.bat` с аргументомсоздаетсясертификатслужбысполнымдоменнымименемкомпьютераиэкспортируетсясертификатслужбывфайлсименемService.cer.`service`  
