...
1.  Установите службы IIS из **панель управления**, **Установка и удаление программ**, **компоненты Windows**.  
  
2.  Установка **проверки подлинности Windows** в **компоненты Windows**, **Internet Information Services**, **World Wide Web Services**,  **Безопасность**, и **проверки подлинности Windows**.  
  
3.  Установка **активация Windows Communication Foundation HTTP** в **компоненты Windows**, **Microsoft .NET Framework 3.5.1**, и **связи Windows Активация Foundation HTTP**.  
...
5.  Откройте **действия** области для веб-сайта по умолчанию. Нажмите кнопку **изменение узла**, **привязки**. Добавьте HTTPS в качестве типа, если пока этот протокол отсутствует, с номером порта 443. Назначьте SSL-сертификат, созданный на предыдущем шаге.  
...
9. Откройте **Дополнительные параметры** под **проверки подлинности Windows** для этого виртуального каталога и присвойте ему значение **необходимые**.  
