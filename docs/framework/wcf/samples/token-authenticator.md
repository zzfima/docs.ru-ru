...
-   Как код службы WCF привязывается проверки подлинности пользовательского маркера.  
  
-   как сервер может проходить проверку подлинности с помощью сертификата X.509 сервера.  
  
 В этом примере также показано, как можно получить доступ из WCF удостоверения вызывающего пользователя после завершения процесса проверки подлинности пользовательского маркера.  
