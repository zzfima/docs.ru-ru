...
Этот образец основан на образце [Transport: UDP.](../../../../docs/framework/wcf/samples/transport-udp.md) Он расширяет образец [Transport: UDP](../../../../docs/framework/wcf/samples/transport-udp.md) для поддержки активации процесса с помощью службы активации процессов Windows (WAS).  
...
## <a name="udp-protocol-activator"></a>Активатор протокола UDP  
 Активатор протокола UDP является мостом между клиентом WCF и службой WCF. Он обеспечивает передачу данных через протокол UDP на транспортном уровне. У него две основные функции.  
...
- `WebhostCloseAllListenerChannelInstances`  
 ...
 Адаптер слушателя зарегистрирован в разделе %SystemRoot% -System32-ApplicationHost.config в `listenerAdapters` разделе <> следующим образом:  
...
### <a name="control-service"></a>Служба управления  
 В этом примере мы используем WCF для связи между активатором и рабочим процессом WAS. Служба, находящаяся в активаторе, называется службой управления.  
  
## <a name="protocol-handlers"></a>Обработчики протоколов  
 После того как адаптер прослушивателя вызывает `WebhostOpenListenerChannelInstance`, диспетчер процесса WAS запускает рабочий процесс, если он еще не запущен. Затем диспетчер приложения внутри рабочего процесса загружает обработчик протокола процесса (PPH) UDP с запросом этого `ListenerChannelId`. PPH в поворотах вызывает `IAdphManager`.`StartAppDomainProtocolListenerChannel` для запуска обработчика протокола UDP AppDomain (ADPH).  
...
3. Запустите приложение пользовательского интерфейса "WasNetActivator.exe". Нажмите на вкладку **Настройка,** проверьте следующие флажки, а затем нажмите **Установить,** чтобы установить их:  
  
    - Адаптер прослушивателя UDP  
  
    - Обработчики протокола UDP  
  
4. Нажмите на вкладку **активации** приложения пользовательского интерфейса "WasNetActivator.exe". Нажмите кнопку **"Пуск",** чтобы запустить адаптер-слушатель. Теперь все готово для запуска программы.  
...
2. Затем запустите активатор WasNetActivator.exe. В рамках вкладки **Активации** в списке выпадающих списков выбирается единственный **протокол, UDP.** Нажмите кнопку **«Пуск»,** чтобы запустить активатор.  
