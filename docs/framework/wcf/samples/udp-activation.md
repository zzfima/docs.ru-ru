...
## <a name="udp-protocol-activator"></a>Активатор протокола UDP  
 Активатор протокола UDP представляет собой мост между клиентом WCF и службы WCF. Он обеспечивает передачу данных через протокол UDP на транспортном уровне. У него две основные функции.  
...
4.  Нажмите кнопку **активации** вкладке приложение пользовательского интерфейса «WasNetActivator.exe». Нажмите кнопку **запустить** кнопку для запуска адаптера прослушивателя. Теперь все готово для запуска программы.  
