...
```  
 ...
 [ \<Безопасности >](../../../../docs/framework/configure-apps/file-schema/wcf/security-of-custombinding.md) элемент привязки берет на себя изначальной вызывающей стороны передачу и извлечение имени. [ \<Windowsstreamsecurity инициирует >](../../../../docs/framework/configure-apps/file-schema/wcf/windowsstreamsecurity.md) и [ \<tcpTransport >](../../../../docs/framework/configure-apps/file-schema/wcf/tcptransport.md) обеспечения проверки подлинности фасадной и серверной служб, а также защиту сообщения.  
  
 Для передачи запроса фасадной службе необходимо предоставить имя пользователя изначальной вызывающей стороны, эту инфраструктуру безопасности WCF можно размещать это в перенаправленного сообщения. Фасадная служба предоставляет имя пользователя изначальной вызывающей стороны в виде значения свойства `ClientCredentials` экземпляра клиентского прокси-класса, используемого для обмена информацией между фасадной и серверной службами.  
...
```  
 ...
 Как показано в предыдущем примере кода, в свойстве `ClientCredentials` не задается пароль (в нем содержится только имя пользователя). Инфраструктура безопасности WCF создает токен безопасности имени пользователя без пароля в этом случае именно то, что необходимо в этом сценарии.  
  
 В серверной службе должна быть проверена подлинность информации, содержащейся в маркере безопасности. По умолчанию безопасность WCF пытается сопоставить пользователя к учетной записи Windows, с помощью указанного пароля. В данном случае пароль не предоставляется, и серверной службе не требуется проверять имя пользователя, поскольку эта процедура уже выполнена фасадной службой. Чтобы реализовать эту функциональность в WCF, пользовательский `UserNamePasswordValidator` предоставляется, обеспечивающий принудительное требование, имя пользователя, указанный в токене и не выполняет дополнительную проверку подлинности.  
...
6.  Если клиент и служба не может взаимодействовать, см. в разделе [советы по устранению неполадок для образцов WCF](https://docs.microsoft.com/previous-versions/dotnet/netframework-3.5/ms751511(v=vs.90)).  
