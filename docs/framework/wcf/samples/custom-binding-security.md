...
1.  На компьютере службы выполните следующее.  
    ...
    4.  Выполните следующую команду в командную строку разработчика для Visual Studio, открытой с правами администратора: `Setup.bat service`. При этом создается сертификат службы с именем субъекта, соответствующим имени компьютера, на котором запущен пакетный файл.  
...
2.  На клиентском компьютере выполните следующее.
    ...
    3.  Экспорт сертификата службы, открыв командную строку разработчика для Visual Studio с правами администратора и выполнив следующую команду на компьютере службы (Замените `%SERVER_NAME%` с полное доменное имя компьютера, где служба запущена):
    ...
    5.  Импортировать сертификат службы, открыв командную строку разработчика для Visual Studio с правами администратора и выполнив следующую команду на клиентском компьютере (Замените % SERVER_NAME % полное доменное имя компьютера, где служба запущена):
