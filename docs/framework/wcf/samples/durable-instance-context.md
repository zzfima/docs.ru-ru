...
```  
 ...
 [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] позволяет расширить компонент среды выполнения InstanceContext путем добавления нового состояния и поведения с помощью шаблона расширяемого объекта. Шаблон расширяемого объекта используется в [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] для расширения существующих классов среды выполнения с помощью новых функциональных возможностей или добавления новых функций состояния к объекту. Предусмотрено три интерфейса в шаблоне расширяемого объекта - IExtensibleObject\<T >, IExtension\<T > и IExtensionCollection\<T >:  
...
-   Экземпляр IExtension\<T > интерфейс реализуется объектами, которые являются расширениями классов типа T.  
  
-   IExtensionCollection\<T > интерфейс представляет собой коллекцию расширений iextension, позволяющую извлекать расширения iextension заданного типа.  
