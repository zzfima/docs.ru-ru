...
- если строится пользовательский маркер.
 ...
 В этом примере показано, как построить пользовательский поставщик маркеров, который позволяет токена SAML, полученного от за пределами инфраструктура клиента WCF для использования.
...
- Как маркер SAML предоставляется инфраструктура клиента WCF.
...
```
 ...
 Ниже показано, как разработать пользовательский поставщик маркеров SAML и интегрировать его с WCF: платформа безопасности:
...
1. Запустите файл Setup.bat из папки установки образца с правами администратора Запустите командную строку Visual Studio 2012. При этом устанавливаются все сертификаты, необходимые для выполнения образца.
...
3. Необходимо иметь сертификат сервера с именем субъекта, содержащим полное имя домена компьютера. Для отображения этого нового имени сертификата необходимо обновить файл Service.exe.config. Сертификат сервера можно создать путем изменения пакетного файла Setup.bat. Обратите внимание на то, что файл setup.bat нужно запускать в командной строке разработчика для окна Visual Studio, открытой с правами администратора. Необходимо установить переменную `%SERVER_NAME%` равной полному имени узла компьютера, используемого для размещения службы.  
