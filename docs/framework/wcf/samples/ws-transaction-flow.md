...
3.  Для запуска образца в конфигурации одного или нескольких компьютерах, следуйте инструкциям в [выполнение образцов Windows Communication Foundation](../../../../docs/framework/wcf/samples/running-the-samples.md).  
 ...
 Запускается ли образец на одном компьютере или на разных компьютерах, необходимо настроить распределенного координатора транзакций (Майкрософт) для включения поток сетевых транзакций и с помощью средства WsatConfig.exe для поддержки сетевых транзакций WCF.  
