...
## <a name="discussion"></a>Обсуждение  
 Поставщик маркеров в Windows Communication Foundation (WCF) используется для предоставления учетных данных инфраструктуре безопасности. Поставщик токенов осуществляет общую проверку цели и выдает соответствующие учетные данные, чтобы инфраструктура безопасности смогла обеспечить защиту сообщения. WCF поставляется с поставщиком токена CardSpace. Пользовательские поставщики маркеров полезны в следующих случаях:  
  
- если используется хранилище учетных данных с которым не работает встроенный поставщик маркеров;  
  
- Если вы хотите предоставить собственный настраиваемый механизм для преобразования учетных данных с момента предоставления пользователю сведений о том, когда клиент WCF использует учетные данные.  
...
- Как выданные маркеры могут быть кэшированы и предоставлены клиенту WCF.  
...
## <a name="custom-client-credentials-and-token-provider"></a>Пользовательские учетные данные клиента и поставщик маркера  
 Ниже показано, как разработать пользовательский поставщик маркеров, который кэширует выданные маркеры и интегрирует их с WCF: Security.  
...
2. Чтобы выполнить сборку решения, следуйте инструкциям в разделе [Создание примеров Windows Communication Foundation](../../../../docs/framework/wcf/samples/building-the-samples.md). Убедитесь, что все проекты в решении построены (Shared, RSTRSTR, Service, SecurityTokenService и Client).  
