...
2.  При использовании Svcutil.exe для создания клиентского прокси-сервера, убедитесь, что включает `/async` параметр.  
