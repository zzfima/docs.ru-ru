...
В этом образце показано, как управлять веб-каналами синдикации, содержащими большое число элементов. На стороне сервера этот пример показывает, как откладывать создание отдельных объектов <xref:System.ServiceModel.Syndication.SyndicationItem> в веб-канале до момента записи элемента в сетевой поток.  
 ...
 Демонстрационный делает использование Visual C# итераторы (с помощью `yield``return` слова). Дополнительные сведения об итераторах см. в разделе «Использование итераторов» в библиотеке MSDN.  
...
```  
 ...
 В результате поток элементов никогда не помещается в буфер памяти целиком. Это поведение можно наблюдать можно установить точку останова на `yield``return` инструкции внутри `ItemGenerator.GenerateItems()` метод и отметить, что это останова в первый раз после служба возвратила результат `StreamedFeed()` метод.  
...
```  
 ...
 В результате элемент не будет прочитан из сети, пока клиентское приложение, через которое проходят результаты вызова метода `ReadItems()`, не будет готово к использованию этого элемента. Это поведение можно наблюдать можно установить точку останова на `yield``return` инструкции внутри `StreamedAtom10FeedFormatter.DelayReadItems()` и убедившись, что это останова в первый раз после вызова `ReadFrom()` завершения.  
