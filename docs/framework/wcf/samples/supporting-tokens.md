...
## <a name="running-the-sample"></a>Запуск примера  
 При выполнении образца клиент сначала запрашивает имя пользователя и пароль для маркера имени пользователя. Обязательно предоставьте правильные значения для системной учетной записи, так как WCF в службе сопоставляет значения в маркере имени пользователя с идентификацией, предоставляемые системой. После этого клиент отображает ответ от службы. Чтобы закрыть клиент, нажмите клавишу ВВОД в окне клиента.  
...
> [!NOTE]
>  Этот скрипт не удаляет сертификаты службы на клиенте при выполнении примера на нескольких компьютерах. При запуске образцов WCF, которые используют сертификаты на нескольких компьютерах, обязательно удалите сертификаты службы, которые были установлены в хранилище CurrentUser - trustedpeople. Для этого воспользуйтесь следующей командой: `certmgr -del -r CurrentUser -s TrustedPeople -c -n <Fully Qualified Server Machine Name>`. Например: `certmgr -del -r CurrentUser -s TrustedPeople -c -n server1.contoso.com`.  
