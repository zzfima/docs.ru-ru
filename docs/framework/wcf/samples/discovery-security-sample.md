...
-   **DiscoverySecurityChannels**: это библиотека, которая предоставляет безопасную привязку. Библиотека вычисляет и проверяет компактную сигнатуру для исходящих и входящих сообщений.  
  
-   **Служба**: резидентная служба, доступ к контракту ICalculatorService,. Служба помечена как доступная для обнаружения. Пользователь указывает данные сертификата, используемого для подписания сообщения, указывая место хранения, имя и имя темы или другой уникальный идентификатор сертификата, а также хранилище, где находятся сертификаты клиента (сертификаты, используемые для проверки сигнатуры входящих сообщений). С учетом этих данных создается и используется точка UdpDiscoveryEndpoint с функциями безопасности.  
...
-   **AnnouncementListener**: резидентная служба, которая прослушивает объявления сетевой и автономный режим и использует защищенную конечную точку объявления.  
