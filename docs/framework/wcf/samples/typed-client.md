...
В выборке показано, как получить информацию от набранного клиента, генерируемого [Инструментом утилиты ServiceModel Metadata (Svcutil.exe).](../../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) Этот пример основан на [getting Started,](../../../../docs/framework/wcf/samples/getting-started-sample.md) который реализует услугу калькулятора. В этом образце клиентом является консольное приложение (EXE), а служба размещается в службах IIS.  
