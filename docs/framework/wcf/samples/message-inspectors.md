---
title: "Инспекторы сообщений | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## Поведение  
 Инспекторы сообщений являются расширениями среды выполнения клиента или среды выполнения распределения.Эти расширения настраиваются с помощью *поведений*.Поведение — это класс, изменяющий поведение среды выполнения модели служб путем изменения конфигурации по умолчанию или добавления в нее расширений \(например, инспекторов сообщений\).  
...
## Добавление инспектора сообщений с помощью конфигурации  
 Согласно требованиям модели служб для настройки пользовательского поведения в конечной точке в файле конфигурации приложения необходимо создать *элемент расширения* конфигурации, представленный классом, который наследуется от <xref:System.ServiceModel.Configuration.BehaviorExtensionElement>.Это расширение затем необходимо добавить в раздел конфигурации модели служб для расширений, как показано для следующего расширения, рассматриваемого в данном разделе.  
...
>   
>  `<диск_установки>:\WF_WCF_Samples\WCF\Extensibility\MessageInspectors`  
