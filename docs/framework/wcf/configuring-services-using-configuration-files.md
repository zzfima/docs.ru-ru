---
title: "Настройка служб с использованием файлов конфигурации | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
-   [\<endpoint\> element reference](http://msdn.microsoft.com/ru-ru/13aa23b7-2f08-4add-8dbf-a99f8127c017)  
...
### Элемент \<binding\>  
 Элементы `binding`, содержащиеся в элементе `bindings`, могут являться одной из предоставленных системой привязок \(см. раздел [Привязки, предоставляемые системой](../../../docs/framework/wcf/system-provided-bindings.md)\) или пользовательской привязкой \(см. раздел [Пользовательские привязки](../../../docs/framework/wcf/extending/custom-bindings.md)\). Элемент `binding` имеет атрибут `name`, сопоставляющий привязку с конечной точкой, заданной в атрибуте `bindingConfiguration` элемента `endpoint`. Если имя не указано, то привязка будет соответствовать значению по умолчанию для этого типа привязки.  
  
 [!INCLUDE[crabout](../../../includes/crabout-md.md)] настройке служб и клиентов см. в разделе [Configuring Windows Communication Foundation Applications](http://msdn.microsoft.com/ru-ru/13cb368e-88d4-4c61-8eed-2af0361c6d7a).  
...
```  
 ...
 `name` для `bindingConfiguration` задано в элементе `<binding>`. Атрибут `name` должен содержать уникальную строку в области типа привязки \(в данном случае — [\<basicHttpBinding\>](../../../docs/framework/configure-apps/file-schema/wcf/basichttpbinding.md)\) или пустое значение, указывающее на привязку по умолчанию. Конечная точка содержит ссылки на конфигурацию, задавая для этой строки атрибут `bindingConfiguration`.  
