...
### <a name="the-services-element"></a>Элемент \<> служб  
...
- [\<> конечной точки](../configure-apps/file-schema/wcf/endpoint-element.md)  
  
### <a name="the-bindings-element"></a>\<Привязка > элемент  
...
### <a name="the-behaviors-element"></a>Элемент \<Behaviors >  
...
### <a name="the-behavior-element"></a>Элемент \<> поведения  
 Каждый `behavior` элемент определяется `name` атрибутом и предоставляет либо предоставляемое системой поведение, например <`throttling`>, либо пользовательское поведение. Если имя не задано, то элемент behavior будет соответствовать поведению по умолчанию для службы или конечной точки.  
  
 [\<> поведения](../../../docs/framework/configure-apps/file-schema/wcf/behavior-of-servicebehaviors.md)  
...
```  
 ...
 `name` для `bindingConfiguration` задано в элементе `<binding>` . Должна быть уникальной строкой в области действия типа привязки — в данном случае [< BasicHttpBinding\>](../../../docs/framework/configure-apps/file-schema/wcf/basichttpbinding.md)или пустое значение для ссылки на привязку по умолчанию. `name` Конечная точка содержит ссылки на конфигурацию, задавая для этой строки атрибут `bindingConfiguration` .  
