...
### <a name="the-services-element"></a>Элемент \<services >  
...
### <a name="the-service-element"></a>Элемент \<service >  
...
### <a name="the-endpoint-element"></a>Элемент \<endpoint >  
...
### <a name="the-bindings-element"></a>Элемент \<bindings >  
...
### <a name="the-binding-element"></a>Элемент \<binding >  
...
### <a name="the-behavior-element"></a>Элемент \<behavior >  
...
```  
 ...
 `name` для `bindingConfiguration` задано в элементе `<binding>` . @No__t-0 должен быть уникальной строкой в области действия типа привязки — в данном случае [< BasicHttpBinding @ no__t-2](../configure-apps/file-schema/wcf/basichttpbinding.md)или пустое значение для ссылки на привязку по умолчанию. Конечная точка содержит ссылки на конфигурацию, задавая для этой строки атрибут `bindingConfiguration` .  
...
## <a name="behavior-merge"></a>Слияние поведений  
 ...
 А файл `~\Web.config` содержит следующее содержимое:  
