...
При вызове [ \<метода (](../../../docs/framework/configure-apps/file-schema/wcf/service.md) т. е. когда приложение размещения пытается запустить службу) система ищет элемент > службы с атрибутом Name, указывающим "UE. <xref:System.ServiceModel.Channels.CommunicationObject.Open%2A> Samples. Хеллосервице ". Если элемент Service > найден, система загружает указанный класс и создает конечные точки, используя определения конечных точек, предоставленные в файле конфигурации. [ \<](../../../docs/framework/configure-apps/file-schema/wcf/service.md) Благодаря такому механизму можно загружать и запускать службу с двумя строками кода, при этом не указывая привязку и адрес в коде. Преимуществом этого подхода является отсутствие необходимости в повторной компиляции или повторном развертывании приложения при внесении этих изменений.

Необязательные заголовки объявляются в [ \<заголовках >](../../../docs/framework/configure-apps/file-schema/wcf/headers-element.md). Ниже приведен пример элементов, используемых для указания конечных точек службы в файле конфигурации, который различает два заголовка: "Золотых" клиентов `http://tempuri1.org/` и "стандартных" клиентов из `http://tempuri2.org/`. Клиент, вызывающий эту службу, должен иметь соответствующие [ \<заголовки, >](../../../docs/framework/configure-apps/file-schema/wcf/headers-element.md) в файле конфигурации.
