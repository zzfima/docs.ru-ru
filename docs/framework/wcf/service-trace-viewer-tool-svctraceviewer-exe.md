...
|-----------------|-----------------|  
|Critical|— Регистрирует в быстром прекращением и записи журнала событий и сведений о корреляции трассировки. Ниже приводится несколько примеров ситуаций, в которых может потребоваться использовать уровень Critical.<br />-AppDomain вышел из строя из-за необработанного исключения.<br />-Не удается запустить приложения.<br />-Сообщение, являющееся причиной сбоя, произошло из процесса MyApp.exe.|  
|Error|-Заносит в журнал все исключения. Уровень Error можно использовать в следующих ситуациях.<br />-Произошел сбой кода из-за исключения недопустимого приведения.<br />-Исключение «не удалось создать конечную точку» происходит сбой приложения при запуске.|  
|Предупреждение|-Существует условие, которое впоследствии может привести к ошибке или критическому сбою. Этот уровень можно использовать в следующих ситуациях.<br />-Приложение получает больше запросов, чем его параметрами регулирования.<br />-Принимающей очереди — на 98% заполнена.|  
|Сведения|-Создаются сообщения, полезные для мониторинга и диагностики состояния системы, измерения производительности или профилирования. Эту информацию можно использовать для планирования загрузки и управления производительностью. Этот уровень можно использовать в следующих ситуациях.<br />-Ошибка после достижения AppDomain сообщения и был десериализован.<br />-Ошибка при создании привязки HTTP.|  
|Verbose|Отладка на уровне трассировки как для пользовательского кода и обслуживания. Этот уровень используется в следующих ситуациях.<br />— Вы не уверены, какой метод в коде был вызван, когда произошел сбой.<br />-У вас есть неверная конечная точка настроена и службе не удалось запустить из-за блокировки записи в хранилище резервирования.|  
...
1.  Запустите Service Trace Viewer, используя окно командной строки перейдите к папке установки WCF (C:\Program Files\Microsoft SDKs\Windows\v6.0\Bin), а затем введите `SvcTraceViewer.exe`.  
...
1.  После запуска программы Service Trace Viewer щелкните **файл** и выберите команду **откройте**. Перейдите в папку, где находятся файлы трассировки.  
...
3.  Чтобы добавить дополнительные файлы трассировки в открытую коллекцию, нажмите кнопку **файл** и выберите команду **добавить**. В открывшемся окне перейдите в расположение файлов трассировки и дважды щелкните левой кнопкой мыши файл, который нужно добавить.  
...
#### <a name="event-tracing-and-crimson-tracing"></a>Трассировка событий и трассировка Crimson  
 Средство просмотра собственный формат может формата трассировки действий, создающего WCF. Трассировки, создаваемые в различных форматах, должны конвертироваться перед их отображением в средстве просмотра. На данный момент кроме формата трассировки действий средство просмотра поддерживает трассировку событий и трассировку Crimson.  
...
-   В **проекта** вкладку, можно добавить файлы в проект.  
  
### <a name="viewing-wcf-traces"></a>Просмотр трассировок WCF  
 WCF создает трассировки с использованием формата трассировки действий. В модели трассировки действий отдельные трассировки группируются в действиях в соответствии со своим назначением. Процесс логического контроля передается между действиями. Например, во время работы приложения появляется и исчезает множество действий "отправка сообщения". Дополнительные сведения о просмотре трассировки, действия и пользовательский интерфейс программы Service Trace Viewer слишком см. в разделе [с помощью программы Service Trace Viewer для просмотра корреляцию трассировок и устранения неполадок](../../../docs/framework/wcf/diagnostics/tracing/using-service-trace-viewer-for-viewing-correlated-traces-and-troubleshooting.md).  
...
##### <a name="activity-view"></a>Представление действия  
 После открытия файлов трассировки видно трассировок группируется в действия и отображается в **действия** представление на панели слева.  
 ...
 **Действия** представления отображаются имена действий, количество трассировок в действии, продолжительность, время начала и окончания.  
 ...
 Можно выбрать несколько действий, нажав клавишу **Ctrl** или **Shift** ключ и выбора нужных действий. Панель трассировок отображает все трассировки выбранных действий.  
  
 Можно дважды щелкнуть действие для отображения на **Graph** представления. Другой способ: выберите действие и переключитесь в **Graph** представления.  
...
##### <a name="graph-view"></a>Представление диаграммы  
 Одним из самых мощных функциональных возможностей программы Service Trace Viewer является **Graph** представление, в котором данные трассировки для заданного действия отображаются в виде диаграммы. Диаграмма позволяет просматривать последовательные этапы выполнения событий и взаимосвязи между множеством действий при перемещении данных между ними.  
  
 Чтобы переключиться в **Graph** просмотра, выберите действие в **действия** просмотра и нажмите кнопку **действия** tab или трассировку журнала сообщений в **сообщения**Представления. Если загружено несколько файлов трассировки и действие включает трассировки из нескольких файлов, в представлении графика отображаются все соответствующие трассировки. Двойной щелчок действия или трассировки журнала сообщений также приведет к **Graph** представления.  
...
2.  Нажмите кнопку «+», или нажмите клавишу **Ctrl** и «+» с помощью клавиатуры.  
  
3.  На графике отображается следующее действие.  
  
4.  Значение «-» отображается слева от значка перенаправления. Нажмите кнопку «-» или нажмите сочетание клавиш Ctrl и «-», чтобы свернуть перенаправление действия.  
...
###### <a name="layout-mode"></a>Режим макета  
 В средстве просмотра предусмотрено два режима макета: **процесс** и **потоков**. Этот параметр определяет самый большой элемент организации. По умолчанию используется режим макета **процесс**, что означает, что действия группируются по процессам в графе.  
...
##### <a name="trace-pane"></a>Панель трассировки  
 ...
 Необработанные XML-данные трассировки можно скопировать в буфер обмена, щелкнув правой кнопкой мыши трассировку и выбрав **Копировать трассировку в буфер обмена**.  
 ...
 **XML** представление отображается XML-код, соответствующий выбранной трассировке. Это представление поддерживает выделение цветом и раскраску синтаксических конструкций. При использовании **найти** для поиска строк, в нем описаны результаты поиска.  
...
-   Предварительно определенные фильтра средства просмотра можно выборочно фильтровать части трассировок WCF. По умолчанию он настроен на пропускание всех трассировок инфраструктуры. Определенные параметры этот фильтр в **параметры фильтра** подменю в **представление** меню.  
...
#### <a name="using-the-filter-toolbar"></a>Использование панели инструментов фильтра  
 Панель фильтра отображается в верхней части инструмента. Если он отсутствует, включите ее в **представление** меню. Панель содержит три компонента.  
  
-   Искать: **искать** определяет нужно искать при фильтрации. Например, если вы хотите найти все трассировки, которые были созданы в контексте процесса X, значение этого поля X и **поиска в** поле «Имя процесса». Это поле заменяется элементом управления выбора даты и времени, если выбран фильтр времени.  
 ...
 **Применить фильтр сейчас** кнопка запускает операцию фильтрации. Некоторые фильтры, особенно если они применяются к большим объемам данных, работают достаточно долго. Операцию фильтрации можно отменить, нажав клавишу **остановить** кнопки, которая отображается в строке состояния в разделе **операции** меню.  
  
 **Снимите** сбрасывает встроенные и настраиваемые фильтры, чтобы разрешить все трассировки для передачи данных через.  
...
#### <a name="filter-options"></a>Параметры фильтрации  
 ...
 Определенные параметры этот фильтр в **параметры фильтра** подменю в **представление** меню.  
...
2.  Нажмите кнопку **создать настраиваемый фильтр** кнопку, расположенную в верхней части панели трассировки.  
  
3.  В появившемся диалоговом окне введите имя фильтра. В этом примере введите `Thread ID`. Также можно ввести описание фильтра.  
  
4.  В древовидном представлении слева представлена структура записи трассировки, выбранной на шаге 1. Перейдите к элементу, для которого нужно создать условие. В этом примере перейдите к атрибуту ThreadID, расположенному в XPath: /E2ETraceEvent/System/Execution/@ThreadID узла. Дважды щелкните атрибут ThreadID в древовидном представлении. Будет создано выражение для атрибута в правой части диалогового окна.  
  
5.  Измените поле параметра условия ThreadID нет "{0}". В результате выполнения этого действия при применении фильтра станет возможной настройка значения ThreadID. (См. раздел "Применение фильтра".) Можно определить до четырех параметров. Условия комбинируются с помощью оператора "ИЛИ".  
...
1.  В меню «Вид» выберите **настраиваемые фильтры** элемента меню.  
  
2.  В диалоговом окне, которое отображается, нажмите кнопку **New.**  
...
1.  Укажите значение в поиске **найти** поля. В данном примере введите идентификатор потока, который требуется найти.  
  
2.  Нажмите кнопку **применить фильтр сейчас**и просмотрите результаты операции.  
  
 Если в фильтре используется несколько параметров, введите их с помощью «;» в качестве разделителя в **найти** поля. Например, следующая строка определяет три параметра: "1;findValue;text". Средство просмотра применяет "1" для {0} параметр фильтра. «findValue» и «text» применяются к {1} и {2} соответственно.  
...
1.  В **представление** меню, нажмите кнопку **настраиваемые фильтры**.  
  
2.  В открывшемся диалоговом окне, щелкните **импорта** кнопки.  
  
3.  Перейдите к файлу пользовательского фильтра (.stvcf), выберите файл и нажмите кнопку **откройте** кнопки.  
...
4.  Укажите имя и расположение файла определения настраиваемого фильтра (.stvcf) и нажмите кнопку **Сохранить** кнопки.  
...
-   Диалоговое окно поиска предоставляет дополнительные параметры поиска. Он доступен с помощью **изменить** меню или сочетания клавиш Ctrl + f.  
  
 Панель инструментов поиска отображается в верхней части средства просмотра. Если он отсутствует, включите ее в **представление** меню. Панель содержит два компонента.  
...
#### <a name="step-forward-or-backward"></a>Шаг вперед или назад  
 Если представить каждую трассировку как строку кода в программе, шаг вперед похож на «Шаг с обходом» в Visual Studio интегрированной среде разработки (IDE). Единственное отличие заключается в том, что при работе с трассировками можно также совершать шаги назад. Шаг вперед означает переход к следующей трассировке в действии.  
  
-   Шаг вперед: Используйте **действия** меню или нажмите клавишу «F10». Также можно использовать клавиши со стрелкой «вниз» на панели трассировки.  
  
-   Шаг назад: Используйте **действия** меню или нажмите клавишу «F9». Также можно использовать клавиши со стрелкой «вверх» на панели трассировки.  
  
> [!NOTE]
>  Это можно перейти к действию, выполняющемуся в другом процессе или даже на другом компьютере, поскольку сообщения WCF могут содержаться идентификаторы действий, охватывающие несколько компьютеров.  
  
#### <a name="follow-transfer"></a>Отслеживание перенаправлений  
 Трассировки перенаправлений — это специальные трассировки в файле трассировки. Посредством трассировки перенаправления одно действие может перенаправляться другому действию. Например «Действие А» может передавать «Действии Б». В этом случае нет трассировки перенаправления на значке «Действию А» с именем «Чтобы действие:» и передачи. Эта трассировка перенаправления является связью между двумя трассировками. В «Действии Б» могут существовать трассировка перенаправления при завершении действия для перенаправления обратно к «Действию А». Это похоже на вызовы функций в программах: А вызывает Б, затем Б возвращает значение.  
  
 «Отслеживание перенаправлений» похоже на «Шаг с заходом» в отладчике. Отслеживается перенаправление от "Действия А" к "Действию Б". Это не оказывает какого-либо воздействия на другие трассировки.  
...
-   С помощью клавиатуры: Выберите трассировку перенаправления и используйте «Следовать за перенаправлением» в **действия** меню или нажмите клавишу «F11»  
...
#### <a name="jump-to-next-or-previous-transfer"></a>Переход к следующему или предыдущему перенаправлению  
 При анализе текущего действия (или нескольких выбранных действий) может потребоваться быстро найти действия, к которым оно перенаправляется. «Перейти к следующему перенаправлению» позволяет найти следующую трассировку перенаправления в действие. Найдя трассировку перенаправления, можно использовать «Отслеживание перенаправлений» к шагу к следующему действию.  
...
#### <a name="navigate-in-graph-view"></a>Навигация в представлении графика  
 Несмотря на то, что переход в область действия и трассировки аналогична отладке, с помощью **Graph** представление предоставляет гораздо более широкие возможности навигации. См. раздел «Представления диаграммы» для получения дополнительной информации.  
  
### <a name="loading-large-trace-files"></a>Загрузка больших файлов трассировки  
 Файлы трассировки могут быть очень большими. Например если трассировка выполняется на уровне «Verbose», результирующий файл трассировки за несколько минут работы легко включить быть сотен мегабайт или более, в зависимости от сети скорость и схемы обмена данными.  
...
## <a name="understanding-trace-icons"></a>Значки трассировок  
 Ниже приведен список значков, используемых в программе Service Trace Viewer **действия** представление, **Graph** представление и **трассировки** области для представления различных элементов.  
  
> [!NOTE]
>  Некоторые трассировки, которые не относятся к категории (например, «сообщение закрыто») не имеют значков.  
  
### <a name="activity-tracing-traces"></a>Трассировки действий  
  
|Значок|Описание|  
|----------|-----------------|  
|![Трассировка предупреждения](../../../docs/framework/wcf/media/7457c4ed-8383-4ac7-bada-bcb27409da58.gif "7457c4ed-8383-4ac7-bada-bcb27409da58")|Трассировка предупреждения: трассировка, создаваемая на пороге предупреждений|  
|![Ошибка трассировки](../../../docs/framework/wcf/media/7d908807-4967-4f6d-9226-d52125db69ca.gif "7d908807-4967-4f6d-9226-d52125db69ca")|Трассировка Error: трассировка, создаваемая на уровне Error.|  
...
|![Корневого действия](../../../docs/framework/wcf/media/5dc8e0eb-1c32-4076-8c66-594935beaee9.gif "5dc8e0eb-1c32-4076-8c66-594935beaee9")|Действие Root: обозначает корневое действие процесса.|  
  
### <a name="wcf-activities"></a>Действия WCF  
  
|Значок|Описание|  
|----------|-----------------|  
|![Действие Environment](../../../docs/framework/wcf/media/29fa00ac-cf78-46e5-822d-56222fff61d1.gif "29fa00ac-cf78-46e5-822d-56222fff61d1")|Действие Environment: действие, которое создается, открывает или закрывает узел WCF или клиента. Ошибки, произошедшие в ходе этих этапов, отображаются в данном действии.|  
...
## <a name="troubleshooting"></a>Устранение неполадок  
 Если нет разрешения на запись в реестр, вы получаете сообщение об ошибке «Microsoft Service Trace Viewer не зарегистрирован в системе» при использовании «`svctraceviewer /register`» команду для регистрации программы. В этом случае необходимо войти в систему с использованием учетной записи, у которой имеется право записи в реестр.  
 ...
 Если открыть журнал трассировки в операционной системе на арабском языке, можно заметить, что фильтр времени не работает. Например, 2005 год соответствует 1427 году в арабском календаре. Однако диапазон времени, поддерживаемый фильтром в программе «Просмотр трассировки службы», не включает даты ранее 1752 г. В результате может оказаться невозможным выбрать правильную дату в фильтре. Чтобы устранить эту проблему, можно создать настраиваемый фильтр (**вид/настраиваемые фильтры**) с помощью выражения XPath для включения конкретного диапазона времени.  
...
## <a name="see-also"></a>См. также  
 ...
 [Трассировка и распространение для корреляции трассировок конца в конец действий](http://msdn.microsoft.com/library/2c11a905-64f8-47b5-bae5-a74fc666137e)
