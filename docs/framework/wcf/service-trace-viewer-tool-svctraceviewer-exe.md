...
|Ошибка|-Заносит в журнал все исключения. Уровень Error можно использовать в следующих ситуациях.<br />-Произошел сбой кода из-за исключения недопустимого приведения.<br />-Исключение «не удалось создать конечную точку» происходит сбой приложения при запуске.|  
...
2.  Нажмите кнопку **применить фильтр сейчас**и просмотрите результаты операции.  
 ...
 Если в фильтре используется несколько параметров, введите их с помощью «;» в качестве разделителя в **найти** поля. Например, следующая строка определяет три параметра: "1;findValue;text". Средство просмотра применяет "1" к параметру {0} фильтра. «findValue» и «text» применяются к параметрам {1} и {2}, соответственно.  
...
|Значок|Описание|  
|----------|-----------------|  
|![Действие](../../../docs/framework/wcf/media/wcfc-defaultactivityc.gif "wcfc_defaultActivityc")|Activity: указывает, что текущее действие является общим действием.|  
...
## <a name="see-also"></a>См. также  
 ...
 [Трассировка и распространение для корреляции трассировок конца в конец действий](http://msdn.microsoft.com/en-us/2c11a905-64f8-47b5-bae5-a74fc666137e)
