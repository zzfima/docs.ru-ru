...
```  
 ...
 В любом случае приложения могут вызывать операцию асинхронно, даже если служба реализована синхронно, так же, как приложение может использовать один и тот же шаблон для асинхронного вызова и вызова локального синхронного метода. Как операция реализована не имеет значения для клиента; когда сообщение ответа поступает, его содержимое отправляется в асинхронный <методклиента> `End` и клиент получает информацию.  
  
### <a name="one-way-message-exchange-patterns"></a>Шаблоны одностороннего обмена сообщениями  
 Можно создать асинхронный шаблон обмена сообщениями, в котором односторонние операции (операции, у которых свойство <xref:System.ServiceModel.OperationContractAttribute.IsOneWay%2A?displayProperty=nameWithType> имеет значение `true` и нет согласованного ответа) могут отправляться в любом направлении клиентом или службой независимо от противоположной стороны. (При этом используется шаблон обмена дуплексными сообщениями с односторонними сообщениями.) В этом случае в контракте службы указывается односторонний обмен сообщениями, который любая сторона может реализовать в качестве асинхронных вызовов или реализаций, или нет, в случае необходимости. Обычно, если контракт определяет односторонний обмен сообщениями, реализации бывают преимущественно асинхронными, поскольку после отправки сообщения приложение не дожидается ответа и продолжает выполнять другие операции.  
