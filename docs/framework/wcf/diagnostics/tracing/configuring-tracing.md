...
-   System.ServiceModel: Записывает все этапы обработки WCF, при изменении конфигурации доступен для чтения, каждое сообщение обрабатывается в процессе передачи, обработка событий безопасности, сообщение отправляется в пользовательском коде и т. д.  
