...
В этом разделе перечислены все действия, определенные в Windows Communication Foundation (WCF).  
...
|-----------|-------------------|-------------------|-----------------|  
|A, M|Внешнее действие|Н/П (не контролируется ServiceModel)|Действие, идентификатор которого задается в протоколе TLS до каких-либо вызовов кода ServiceModel (сторона клиента или сторона сервера).<br /><br /> Пример Вызывается мероприятие, где open вызывается для клиента WCF или serviceHost.open.|  
...
|-------------------|-------------------|-----------------|  
|Создание безопасного сеанса|SetupSecurity|Существует только на стороне клиента. Содержит все передаваемые данные RST*/SCT для проверки подлинности и установки контекста безопасности. Если `propagateActivity` = `true`, это действие объединяется с помощью службы соответствующий процесс действия RST\*/SCT действий.|  
