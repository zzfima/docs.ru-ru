---
title: "Исключения IdentityModel | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
|SAMLUnableToLoadAdvice|Не удалось загрузить элемент \<saml:advice\>.|  
...
|SspiWrapperEncryptDecryptAssert1|SSPIWrapper::EncryptDecryptHelper&#124;"offset" выходит за пределы допустимого диапазона.|  
|SspiWrapperEncryptDecryptAssert2|SSPIWrapper::EncryptDecryptHelper&#124;"size" выходит за пределы допустимого диапазона.SecurityTokenManagerCannotCreateAuthenticatorForRequirement\=Диспетчер маркера безопасности не может создать структуру проверки подлинности для указанного требования.|  
...
|SAMLUnableToLoadAssertion|Не удалось загрузить элемент \<saml:assertion\>.|  
...
|SAMLUnableToLoadCondtions|Не удалось загрузить элемент \<saml:conditions\>.|  
...
|SAMLUnableToLoadCondtion|Не удалось загрузить элемент \<saml:conditions\>.|  
...
|TraceCodeSecurityClientSessionCloseSent|Клиентским сеансом безопасности было отправлено сообщение закрытия.|  
...
|IncorrectUserNameFormat|Имя пользователя имеет недопустимый формат.Имя пользователя должно быть в формате "имя\_пользователя" или "домен\\\\имя\_пользователя".|  
