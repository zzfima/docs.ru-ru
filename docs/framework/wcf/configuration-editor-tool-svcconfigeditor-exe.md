...
## <a name="using-the-configuration-editor"></a>Использование редактора конфигураций  
 ...
 После запуска редактора конфигураций служб, можно использовать **файл/открыть** меню, чтобы найти службы или сборки, которые вы хотите управлять. Можно непосредственно открывать файлы конфигурации, выбирать службы WCF/COM+ и открывать файлы конфигурации для служб, размещенных на веб-сервере.  
...
1. Запустите редактор конфигураций служб, используя окно командной строки перейдите в папку установки WCF, а затем введите `SvcConfigEditor.exe`.  
  
2. Из **файл** меню, выберите **откройте** и выберите тип файла, которым требуется управлять.  
 ...
 **Служб** узел отображает все службы, назначенные в файле конфигурации. Каждый вложенный узел в дереве соответствует вложенному элементу элемента <`services`> в файле конфигурации.  
  
 При нажатии кнопки **служб** узел, можно просматривать или выполнять задачи на ее странице сводных данных в **подробно** области.  
...
-   С помощью мастера: Щелкните ссылку **создать службу...** в области задач или странице сводных сведений, чтобы запустить мастер. Вы также можете выполнить в **файл** -> меню **Добавление нового элемента**.  
  
-   Создание вручную: Щелкнуть правой кнопкой мыши **служб** узел и выберите **новую службу**.  
...
-   Создание с помощью мастера: щелкните ссылку **создать конечную точку службы...** в области задач или странице сводных сведений, чтобы запустить мастер. Вы также можете выполнить в **файл** -> меню **Добавление нового элемента**.  
  
-   Создание вручную: После создания службы можно щелкнуть **конечные точки** узел и выберите пункт "**новую конечную точку службы**«.  
...
1. Нажмите кнопку **конечная точка службы** узла.  
...
2. Нажмите кнопку **New...** кнопки в **базовые адреса** раздел.  
...
> [!NOTE]
>  Не удается изменить значение [ \<baseAddressPrefixFilters >](../../../docs/framework/configure-apps/file-schema/wcf/baseaddressprefixfilters.md) в этом средстве. Чтобы добавить или изменить данный элемент, используйте текстовый редактор или среду Visual Studio.  
  
### <a name="client"></a>"Клиент";  
 **Клиента** узел отображает все конечные точки клиента в файле конфигурации. Каждый вложенный узел в дереве соответствует вложенному элементу элемента <`client`> в файле конфигурации.  
  
 При нажатии кнопки **клиента** узел, вы можете просматривать или выполнять задачи на стороне клиента **Сводная страница** в **панель подробностей**.  
...
-   Создание с помощью мастера: Щелкните ссылку **создать новый клиент...** на **область задач** в левой нижней части окна, или **Сводная страница** для запуска мастера. Вы также можете выполнить в **файл** -> меню **Добавление нового элемента**. Мастер предложит указать расположение конфигурации службы, из которой создается конфигурация клиента. Затем можно выбрать конечную точку службы для подключения.  
...
### <a name="standard-endpoint"></a>Стандартная конечная точка  
 ...
 Такие параметры конфигурации хранятся в **стандартной конечной точки** узла. **Стандартной конечной точки** узел отображает все параметры стандартной конечной точки в файле конфигурации. Каждый вложенный узел в дереве соответствует вложенному элементу `<standardEndpoints>` элемент в файле конфигурации.  
  
 При нажатии кнопки **стандартной конечной точки** узел, вы можете просматривать или выполнять задачи со стандартной конечной точкой **Сводная страница** в **панель подробностей**.  
...
-   Щелкните правой кнопкой мыши **стандартной конечной точки** узел и выберите **создать конфигурацию стандартной конечной точки...** Выберите тип привязки в диалоговом окне и нажмите кнопку **ОК**.  
  
-   Выберите **стандартной конечной точки** узел и нажмите кнопку **создать конфигурацию стандартной конечной точки...** в **область задач** в левой нижней части окна.  
  
 **Создание новую стандартную конечную точку** отображает диалоговое окно, а также список все зарегистрированные типы стандартных конечных точек.  
...
-   Щелкните, чтобы развернуть **стандартной конечной точки** узел и щелкните вложенный узел соответствующей конечной точке.  
...
-   Щелкните, чтобы развернуть **стандартной конечной точки** узел и щелкните правой кнопкой мыши вложенный узел соответствующей конечной точке. Используйте команду контекста **удалить конфигурацию стандартной конечной точки** удалить конечную точку.  
  
-   Нажмите кнопку **стандартной конечной точки** узла. В **задачи** панели щелкните **удалить конфигурацию стандартной конечной точки**.  
  
 Если стандартная конечная точка в данный момент используется, предупреждающее сообщение отображается при попытке удалить его: **Стандартная конечная точка уже используется. Если сейчас ее удалить, также необходимо удалить все ссылки на нее в других частях конфигурации (например, в конечной точке службы или клиентской конечной точке). В противном случае конфигурация станет недопустимой, и в следующий раз ее будет невозможно открыть. Вы действительно хотите удалить стандартную конечную точку?»**  
 ...
 **Привязки** узла отображаются все параметры привязки в файле конфигурации. Каждый вложенный узел в дереве соответствует вложенному элементу <`bindings`> в файле конфигурации.  
  
 При нажатии кнопки **привязки** узел, вы можете просматривать или выполнять задачи в привязке **Сводная страница** в **панель подробностей**.  
...
-   Щелкните правой кнопкой мыши **привязки** узел и выберите **создать конфигурацию привязки**... Выберите тип привязки в диалоговом окне и нажмите кнопку **ОК**.  
  
-   Выберите **привязки** узел и нажмите кнопку **создать конфигурацию привязки**... в **область задач** в левой нижней части окна.  
  
-   На странице сводки службы или клиента, щелкните **щелкните, чтобы создать** в **конфигурацию привязки** поля, чтобы создать конфигурацию привязки для соответствующей конечной точки.  
...
2. Выберите один элемент расширения привязки в **положение расширения элемента привязки** раздел.  
  
3. Используйте **вверх** или **вниз** кнопку слева от списка, чтобы изменить положение выбранного элемента.  
 ...
 **Диагностики** узле отображаются все параметры диагностики, заданные в файле конфигурации. Он позволяет включить счетчики производительности, или отключить, включить или отключать инструментарий управления Windows (WMI), Настройка трассировки WCF и ведение журнала сообщений WCF. Параметры в **диагностики** соответствуют узел <`system.diagnostics`> разделе и `<diagnostics>` статьи `<system.serviceModel>` в файле конфигурации.  
  
 При нажатии кнопки **диагностики** узел, можно просматривать или выполнять задачи на диагностики **Сводная страница** в **панель подробностей**.  
...
2. Нажмите кнопку **переключение счетчиков производительности**. Счетчик производительности имеет три состояния: OFF (по умолчанию) ServiceOnly и All. Для переключения между этими тремя состояниями следует щелкнуть ссылку соответствующее число раз.  
...
3. Нажмите кнопку **уровень трассировки** ссылку, чтобы настроить уровень трассировки. Существует шесть уровней трассировки: OFF, Critical, ошибка, предупреждение, сведения и многословными. **Трассировка действий** и **распространять действие** позволяют использовать функцию трассировки активности WCF.  
...
3. Нажмите кнопку **уровень ведения журнала** ссылку, чтобы настроить уровень ведения журнала. Существует три уровня журнала. Неверно сформирован, службы и транспорта.  
  
4. Выберите имя прослушивателя, чтобы задать файл журнала и параметры.  
  
> [!NOTE]
>  Если вы хотите, чтобы журналы трассировки и сообщений автоматически очищались при закрытии приложения, включите **Автоматическая очистка** параметр.  
  
 **Диагностики** **Сводная страница** позволяет выполнять наиболее распространенные задачи с настройкой диагностики. Тем не менее, если вы хотите вручную изменить параметры прослушивателей и источников, необходимо развернуть **диагностики** узла и измените параметры в **ведение журнала сообщений**, **прослушиватели** и **Источников** узла.  
...
2. Щелкните правой кнопкой мыши **прослушиватели** узел и выберите **новой прослушиватель**.  
  
3. Введите имя файла трассировки в **исходные данные** поля. Можно щелкнуть «…» Чтобы выбрать путь.  
  
4. Щелкнув **TypeName** строки отображается «...» кнопка. Нажмите эту кнопку, чтобы открыть **браузер типов прослушивателя трассировки**, который можно использовать, чтобы найти предварительно настроенные прослушиватели трассировки, которые уже установлены.  
  
5. Примечание **источника** раздел. Нажмите кнопку **добавить** в этом разделе, чтобы открыть диалоговое окно, в раскрывающемся меню, в котором перечислены доступные источники трассировки. Выберите источник трассировки и нажмите кнопку **ОК**.  
 ...
 **Поведения** узла отображаются поведения, которые в настоящее время определены в файле конфигурации.  
  
 Конфигурации поведения используются для настройки поведений конечных точек и служб. Такие параметры конфигурации хранятся в **Дополнительно** узле **поведения служб** и **поведения конечной точки**. Поведения служб используются службами, а поведения конечных точек - конечными точками.  
...
3. Выберите элемент расширения поведения в **позиция расширения элемента поведения** раздел.  
  
4. Используйте **вверх** или **вниз** кнопку слева от списка, чтобы изменить положение выбранного элемента.  
...
### <a name="creating-a-configuration-file-using-the-wizard"></a>Создание файла конфигурации с помощью мастера  
 Один из способов создания нового файла конфигурации — использование мастера создания элементов службы. Мастер находит типы установленной службы и другие элементы совместимы с WCF на компьютере, включая COM + и виртуальные каталоги, размещенные в Интернете и загружает их, чтобы ускорить создание конфигурации.  
  
#### <a name="creating-a-configuration-file"></a>Создание файла конфигурации  
  
1. Запустите редактор конфигураций служб, используя окно командной строки перейдите в папку установки WCF, а затем введите `SvcConfigEditor.exe`.  
  
2. Из **файл** меню, выберите **откройте** и нажмите кнопку **исполняемый файл**, **службы COM +**, или **размещенная в Интернете службы**в зависимости от типа файла конфигурации, которые вы хотите создать.  
  
3. В **откройте** диалоговом окне перейдите к нужному файлу, чтобы создать файл конфигурации для и дважды щелкните его.  
...
## <a name="configuring-com"></a>Настройка COM+  
 Редактор конфигураций служб позволяет создавать новые файлы конфигурации для существующих приложений COM+, а также изменять имеющиеся конфигурации COM+. **COM-контракт** узел отображается только тогда, когда <`comContract`> в файле конфигурации существует раздел.  
...
1. Выберите **файл** -> меню **интегрировать** -> **приложение COM +.** В результате этого действия открытый в данный момент файл закрывается. Если в текущем файле есть несохраненные данные, то появится диалоговое окно сохранения. **Мастер интеграции COM +** запустится.  
...
## <a name="security"></a>Безопасность  
 Редактор конфигураций, с помощью которого создаются файлы конфигурации служб, не гарантирует безопасность. Обратитесь к [безопасности](../../../docs/framework/wcf/feature-details/security.md) документации, чтобы узнать, как обеспечить безопасность служб WCF.  
