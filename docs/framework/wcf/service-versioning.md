...
- Изменения контракта: Например, можно добавить операцию или добавить или изменить элемент данных в сообщении.  
  
- Изменения адреса: Например, служба перемещается в другое расположение, где конечные точки имеют новые адреса.  
  
- Изменения привязки: Например, изменяется механизм безопасности или изменяются его параметры.  
  
- Изменения в реализации: Например, при изменении реализации внутреннего метода.  
...
### <a name="lax-versioning"></a>Нестрогое управление версиями  
 ...
 Подробные рекомендации по нестрогим и строгим версиям контрактов данных см. [в разделе рекомендации: Управление версиями](../../../docs/framework/wcf/best-practices-data-contract-versioning.md)контракта данных.  
...
## <a name="routing-service-and-versioning"></a>Служба маршрутизации и управление версиями  
 Если внесенные в службу изменения являются критическими и необходимо наличие двух или нескольких версий службы, запущенных одновременно, можно воспользоваться службой маршрутизации WCF для направления сообщений соответствующему экземпляру службы. Служба маршрутизации WCF использует маршрутизацию на основе содержимого, то есть на основе информации в самом сообщении, чтобы определить, куда направить это сообщение. Дополнительные сведения о службе маршрутизации WCF см. в разделе [Служба маршрутизации](../../../docs/framework/wcf/feature-details/routing-service.md). Пример использования службы маршрутизации WCF для управления версиями службы см [. в статье как: Управление версиями](../../../docs/framework/wcf/feature-details/how-to-service-versioning.md)службы.  
