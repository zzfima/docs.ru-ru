...
-   Если служба требует, минимальным `ProtectionLevel` все сообщения является `Sign`, клиент (возможно, он создан с помощью технологии не WCF) может шифровать и подписывать все сообщения (то есть больше минимума). В этом случае WCF не вызовет исключение, так как клиент выполнил более, чем минимально. Обратите внимание, что не будут чрезмерно защищать часть сообщения по возможности приложений WCF (службы или клиенты), но будет соответствовать минимальным уровнем. Обратите также внимание, что при использовании режима `Transport` в качестве режима безопасности транспорт может чрезмерно защищать поток сообщений, поскольку он не способен обеспечить защиту на более детальном уровне.  
