---
title: "Суррогаты контрактов данных | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "контракты данных [WCF], суррогаты"
...
#### Метод GetCustomDataToExport \(Type, Type\)  
 Метод <xref:System.Runtime.Serialization.IDataContractSurrogate.GetCustomDataToExport%28System.Type%2CSystem.Type%29?displayProperty=fullName> вызывается во время экспорта схемы для каждого определения типа.Этот метод добавляет информацию в типы схемы при экспорте.Каждый определенный тип отправляется этому методу с целью определить, есть ли дополнительные данные, которые необходимо включить в схему.  
  
#### Метод GetCustomDataToExport \(MemberInfo, Type\)  
 Метод <xref:System.Runtime.Serialization.IDataContractSurrogate.GetCustomDataToExport%28System.Reflection.MemberInfo%2CSystem.Type%29?displayProperty=fullName> вызывается во время экспорта для каждого элемента в экспортируемых типах.Эта функция дает возможность настроить для элементов любые комментарии, которые будут включены в схему при экспорте.Сведения для каждого элемента класса отправляются в этот метод для проверки, не нужно ли включить в схему какие\-либо дополнительные данные.  
...
1.  Создайте экземпляр <xref:System.ServiceModel.ServiceHost> для службы.Полные указания см. в разделе [Базовое программирование WCF](../../../../docs/framework/wcf/basic-wcf-programming.md).  
...
2.  Чтобы проверить, определен ли объект <xref:System.Runtime.Serialization.XsdDataContractImporter>, воспользуйтесь методом <xref:System.Collections.Generic.Dictionary%602.TryGetValue%2A>.  
...
2.  Чтобы проверить, определен ли объект <xref:System.Runtime.Serialization.XsdDataContractExporter>, воспользуйтесь функцией <xref:System.Collections.Generic.Dictionary%602.TryGetValue%2A>.  
