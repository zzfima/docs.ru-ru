...
Рис. 1. Конечный автомат ICommunicationObject.  
 ...
 Windows Communication Foundation (WCF) предоставляет абстрактный базовый класс с именем <xref:System.ServiceModel.Channels.CommunicationObject>, который реализует <xref:System.ServiceModel.ICommunicationObject> и конечный автомат канала. Ниже приведена схема изменившегося состояния, относящаяся к <xref:System.ServiceModel.Channels.CommunicationObject>. Кроме конечного автомата <xref:System.ServiceModel.ICommunicationObject>, на схеме также показано время, когда вызываются дополнительные методы <xref:System.ServiceModel.Channels.CommunicationObject>.  
  
 ![схемы потока данных изменений состояния реализации Коммуникатионобжект.](./media/understanding-state-changes/communicationobject-implementation-state-machine.gif)
...
### <a name="derived-object-callbacks"></a>Обратные вызовы производного объекта  
 ...
 <xref:System.ServiceModel.Channels.CommunicationObject.OnOpening%2A?displayProperty=nameWithType>, <xref:System.ServiceModel.Channels.CommunicationObject.OnClosing%2A?displayProperty=nameWithType> и <xref:System.ServiceModel.Channels.CommunicationObject.OnFaulted%2A?displayProperty=nameWithType> вызывают соответствующие события <xref:System.ServiceModel.Channels.CommunicationObject.Opening?displayProperty=nameWithType>, <xref:System.ServiceModel.Channels.CommunicationObject.Closing?displayProperty=nameWithType> и <xref:System.ServiceModel.Channels.CommunicationObject.Faulted?displayProperty=nameWithType>. <xref:System.ServiceModel.Channels.CommunicationObject.OnOpened%2A?displayProperty=nameWithType> и <xref:System.ServiceModel.Channels.CommunicationObject.OnClosed%2A?displayProperty=nameWithType> установить состояние объекта для открытия и закрытия соответственно, затем запустить соответствующие события <xref:System.ServiceModel.Channels.CommunicationObject.Opened?displayProperty=nameWithType> и <xref:System.ServiceModel.Channels.CommunicationObject.Closed?displayProperty=nameWithType>.  
  
### <a name="state-transition-methods"></a>Методы перехода состояния  
 <xref:System.ServiceModel.Channels.CommunicationObject> предоставляет реализации метода Abort, закрытия и открытия. Он также предоставляет метод Fault, вызывающий переход в состояние Faulted. На рисунке 2 показан конечный автомат <xref:System.ServiceModel.ICommunicationObject>, где каждый переход помечен методом, который его вызвал (непомеченные переходы выполняются внутри реализации метода, вызвавшего последний помеченный переход).  
...
> Все реализации <xref:System.ServiceModel.Channels.CommunicationObject> для получения или задания состояния связи синхронизированы с потоком.  
 ...
 <xref:System.ServiceModel.Channels.CommunicationObject> предоставляет три конструктора, каждый из которых оставляет объект в созданном состоянии. Конструкторы определяются следующим образом.  
...
### <a name="throwifxxx-methods"></a>Методы ThrowIfXxx  
 ...
 <xref:System.ServiceModel.Channels.CommunicationObject.ThrowIfDisposed%2A> создает исключение, если состояние закрывается, закрывается или произошел сбой.  
  
 <xref:System.ServiceModel.Channels.CommunicationObject.ThrowIfDisposedOrImmutable%2A> создает исключение, если состояние не создано.  
  
 <xref:System.ServiceModel.Channels.CommunicationObject.ThrowIfDisposedOrNotOpen%2A> создает исключение, если состояние не открыто.  
...
|Закрыто|Да|<xref:System.ServiceModel.CommunicationObjectAbortedException?displayProperty=nameWithType> в случае, если объект был закрыт предыдущим и явным вызовом метода Abort. При вызове метода Close для объекта выдается исключение <xref:System.ObjectDisposedException?displayProperty=nameWithType>.|  
...
### <a name="timeouts"></a>Время ожидания  
 ...
 Исключениями являются перегрузки Open() и Close(), которые не используют параметр времени ожидания. Они используют значение времени ожидания по умолчанию, предоставленное производным классом. <xref:System.ServiceModel.Channels.CommunicationObject> предоставляет два защищенных абстрактных свойства с именами <xref:System.ServiceModel.Channels.CommunicationObject.DefaultCloseTimeout%2A> и <xref:System.ServiceModel.Channels.CommunicationObject.DefaultOpenTimeout%2A>, определенные как:  
