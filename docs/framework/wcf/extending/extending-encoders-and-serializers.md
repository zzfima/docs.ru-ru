---
title: "Расширение кодировщиков и сериализаторов"
ms.custom: 
ms.date: 03/30/2017
ms.prod: .net-framework
ms.reviewer: 
ms.suite: 
ms.technology: dotnet-clr
ms.tgt_pltfrm: 
ms.topic: article
ms.assetid: 97aec970-d73c-469c-9b93-8b86be701c44
caps.latest.revision: "10"
author: dotnet-bot
ms.author: dotnetcontent
manager: wpickett
ms.openlocfilehash: 274efaec0efa2aed12689013db2ad70a2e32c1f9
ms.sourcegitcommit: ce279f2d7fe2220e6ea0a25a8a7a5370ddf8d9f0
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 12/02/2017
---
# <a name="extending-encoders-and-serializers"></a>Расширение кодировщиков и сериализаторов
В настоящем разделе описывается, как расширить систему сериализации [!INCLUDE[indigo1](../../../../includes/indigo1-md.md)].  
  
## <a name="in-this-section"></a>Содержание  
 [Суррогаты контрактов данных](../../../../docs/framework/wcf/extending/data-contract-surrogates.md)  
 Описывается, как сериализация, десериализация, экспорт и импорт схемы могут быть настроены при помощи заменяющего класса контракта данных.  
  
 [Пользовательские модули форматирования сообщения](../../../../docs/framework/wcf/extending/custom-message-formatters.md)  
 Описываются пользовательские модули форматирования сообщения, которые могут использоваться для управления сериализацией и десериализацией сообщения [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] в типы среды CLR.
