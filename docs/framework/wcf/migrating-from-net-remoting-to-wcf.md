...
## <a name="comparing-net-remoting-to-wcf"></a>Сравнение удаленного взаимодействия .NET и WCF  
 В этом разделе сравниваются основные структурные элементы системы удаленного взаимодействия .NET с аналогичными элементами WCF. Мы будем использовать эти строительные блоки позже для создания некоторых общих сценариев клиент-сервера в WCF. На следующем графике кратко излагаются основные сходства и различия между .NET Remoting и WCF.  
...
|Ошибки и исключения|Любые сериализуемые исключения|НеисправностьПодрядная\<>|  
|Прокси-объекты клиента|Строго типизированные прозрачные прокси-объекты создаются автоматически из MarshalByRefObjects.|Сильно набранные прокси генерируются по\<требованию с помощью ChannelFactory TChannel>|  
...
```  
 ...
 Это пример программирования на уровне канала, так как он обладает максимальным сходством с примером удаленного взаимодействия. Также доступен подход **Add Service Reference** в Visual Studio, который генерирует код для упрощения клиентского программирования. Дополнительные сведения см. в следующих разделах:  
...
- *По значению* значения значения объекта сериализуются через границы яруса, и новый экземпляр этого объекта создается на другом ярусе. Любые вызовы методов и свойств этого нового экземпляра выполняются только на локальном компьютере и не влияют на исходный объект или уровень.  
  
- *По ссылке* - специальная "объектная ссылка" сериализуется через границы яруса. Когда один уровень взаимодействует с методами или свойствами данного объекта, он устанавливает обратную связь с исходным объектом на исходном уровне. Объекты по ссылке могут передаваться в любом направлении — с сервера на клиент или с клиента на сервер.  
...
```  
 ...
 Когда WCF отправляет объект между уровнями, он сериализует только значения и создает новый экземпляр объекта на другом уровне. Любое взаимодействие со значениями объекта осуществляется только на локальном уровне — они не взаимодействуют с другим уровнем в отличие от объектов по ссылке системы удаленного взаимодействия .NET. Для получения дополнительной информации [см.](./feature-details/serialization-and-deserialization.md)  
...
- **Удаленное взаимодействие .NET — это устаревший продукт.** Как описано в [.NET Remoting,](https://docs.microsoft.com/previous-versions/dotnet/netframework-4.0/72x4h507%28v=vs.100%29)он считается устаревшим продуктом и не рекомендуется для новой разработки. Для новых и существующих приложений рекомендуется использовать платформу WCF или веб-интерфейс API ASP.NET.  
  
- **WCF использует кроссплатформенные стандарты.** Решение WCF было разработано с учетом кроссплатформенного взаимодействия и поддерживает различные отраслевые стандарты (SOAP, WS-Security, WS-Trust, и т. д.). Служба WCF может взаимодействовать с клиентами, работающими под управлением других операционных систем, отличных от Windows. Remoting был разработан в первую очередь для сред, где как сервер, так и клиентские приложения работают с помощью .NET Framework на операционной системе Windows.
  
- **WCF включает встроенные функции обеспечения безопасности.** WCF был разработан с учетом безопасности и предлагает множество вариантов проверки подлинности, безопасности транспортного уровня, безопасности уровня сообщений и т.д. Remoting был разработан, чтобы сделать его легким для приложений для работы, но не был разработан, чтобы быть безопасным в ненадежных средах. Решение WCF было разработано для работы как в доверенных, так и в недоверенных средах.  
...
- **Создайте контракт на передачу данных.** Определите типы данных, которыми будут обмениваться сервер и клиент, и отметьте их атрибутом [DataContract]. Отметьте все поля и свойства, доступные для использования клиентом, атрибутом [DataMember].  
...
- **Настройте и разместите службу.** После создания контракта службы необходимо настроить привязку для предоставления службы в конечной точке. Для получения дополнительной [Endpoints: Addresses, Bindings, and Contracts](./feature-details/endpoints-addresses-bindings-and-contracts.md)информации см.  
...
5. После этого можно запустить службу WCF с помощью следующего кода:  
   ...
   ```  
     ...
     При запуске метод ServiceHost использует файл web.config для установления надлежащего контракта, привязки и конечной точки. Для получения дополнительной информации о файлах [конфигурации](./configuring-services-using-configuration-files.md)см. Этот метод запуска сервера называется резидентным размещением. Чтобы узнать больше о других вариантах [Hosting Services](./hosting-services.md)размещения wCF-сервисов, см.  
  
6. В файле app.config клиентского проекта должны быть объявлены соответствующие сведения о привязке для конечной точки службы. Самый простой способ сделать это в Visual Studio является использование **Add Service Reference**, который будет автоматически обновлять файл app.config. Эти же изменения также можно добавить вручную.  
    ...
    ```  
     ...
     Для получения дополнительной информации об использовании Справки об использовании **услуг,** см. [How to: Add, Update, or Remove a Service Reference](/visualstudio/data-tools/how-to-add-update-or-remove-a-wcf-data-service-reference)  
...
4. Нам нужно изменить файл конфигурации сервера, выполнив два действия, представленные в следующем примере:  
    ...
    1. Объявить \<раздел клиента>, описывающий конечную точку для объекта сеанса. Это необходимо, поскольку сервер также выступает в качестве клиента в данной ситуации.  
...
2. Используйте клиент для создания нового объекта по значению (Customer), создайте канал для взаимодействия со службой ICustomerService и отправьте в нее объект.  
 ...
 Стандартные обмены данными в службе WCF осуществляются по значению. Это гарантирует, что методы вызова этих объектов данных реализуются только локально без вызова кода на другом уровне. Хотя можно достичь чего-то вроде объектов, возвращаювых *с* сервера по ссылке, клиент не может передать серверу *объект* по ссылке. Сценарий, при котором клиенту и серверу необходимо обмениваться сообщениями друг с другом, можно реализовать в WCF с помощью дуплексной службы. Для получения дополнительной информации [см.](./feature-details/duplex-services.md)  
