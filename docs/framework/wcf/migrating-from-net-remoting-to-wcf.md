...
-   [Сериализация в Windows Communication Foundation](https://msdn.microsoft.com/magazine/cc163569.aspx)  
...
-   **Удаленное взаимодействие .NET — это устаревший продукт.** Как описано в разделе [удаленного взаимодействия .NET](https://msdn.microsoft.com/library/vstudio/72x4h507\(v=vs.100\).aspx), он считается это устаревший продукт и не рекомендуется для разработки новых приложений. Для новых и существующих приложений рекомендуется использовать WCF или веб-API ASP.NET.  
...
6.  В файле app.config клиентского проекта должны быть объявлены соответствующие сведения о привязке для конечной точки службы. Самый простой способ сделать это в Visual Studio является использование **Add Service Reference**, которая автоматически обновит файл app.config. Эти же изменения также можно добавить вручную.  
    ...
    ```  
     ...
     Дополнительные сведения об использовании **Add Service Reference**, см. в разделе [как: Добавление, обновление или удаление ссылки на службу](/visualstudio/data-tools/how-to-add-update-or-remove-a-wcf-data-service-reference).  
...
4.  Нам нужно изменить файл конфигурации сервера, выполнив два действия, представленные в следующем примере:  
    ...
    1.  Объявите \<клиента > раздел, описывающий конечную точку для объекта сеанса. Это необходимо, поскольку сервер также выступает в качестве клиента в данной ситуации.  
...
2.  Используйте клиент для создания нового объекта по значению (Customer), создайте канал для взаимодействия со службой ICustomerService и отправьте в нее объект.  
 ...
 Стандартные обмены данными в службе WCF осуществляются по значению. Это гарантирует, что методы вызова этих объектов данных реализуются только локально без вызова кода на другом уровне. Хотя и существует возможность аналогичному возврату по ссылке объекты, возвращаемые *из* сервера, он не поддерживается для клиента передать объект по ссылке *для* сервера. Сценарий, при котором клиенту и серверу необходимо обмениваться сообщениями друг с другом, можно реализовать в WCF с помощью дуплексной службы. Дополнительные сведения см. в разделе [дуплексные службы](./feature-details/duplex-services.md).  
