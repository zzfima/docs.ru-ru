---
title: "Перенос из .NET на платформу WCF | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# Перенос из .NET на платформу WCF
...
-   [Сравнение удаленного взаимодействия .NET и WCF](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Compare_Top)  
  
    -   [Сравнение реализации сервера](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Server_Comp)  
  
    -   [Сравнение реализации клиента](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Client_Comp)  
  
    -   [Использование сериализации](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Serialization_Usage)  
  
    -   [Возможности обработки исключений](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Exception_Handling)  
  
    -   [Вопросы безопасности](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Security_Considerations)  
  
-   [Миграция в WCF](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Migrating_Top)  
  
    -   [Почему необходимо выполнить миграцию с переходом от удаленного взаимодействия на WCF?](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Why_Migrate)  
  
    -   [Рекомендации по миграции](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Migration_Recommendations)  
  
    -   [Сценарии миграции](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Migration_Scenarios)  
  
-   [Сводка](../../../docs/framework/wcf/migrating-from-net-remoting-to-wcf.md#Summary)  
  
<a name="Compare_Top"></a>   
## Сравнение удаленного взаимодействия .NET и WCF  
 В этом разделе сравниваются основные структурные элементы системы удаленного взаимодействия .NET с аналогичными элементами WCF.  Мы будем использовать эти структурные элементы в дальнейшем для создания некоторых наиболее распространенных сценариев взаимодействия «клиент — сервер» в среде WCF. На следующей схеме показаны основные сходства и различия между системой удаленного взаимодействия .NET и WCF.  
  
||Удаленное взаимодействие .NET|WCF|  
|-|-----------------------------------|---------|  
|Тип сервера|Подкласс MarshalByRefObject|Отметка атрибутом \[ServiceContract\]|  
|Операции служб|Открытые методы для типа сервера|Отметка атрибутом \[OperationContract\]|  
...
|Ошибки и исключения|Любые сериализуемые исключения|FaultContract\<TDetail\>|  
|Прокси\-объекты клиента|Строго типизированные прозрачные прокси\-объекты создаются автоматически из MarshalByRefObjects.|Строго типизированные прокси\-объекты создаются по требованию с помощью ChannelFactory\<TChannel\>.|  
|Требуемая платформа|Клиент и сервер должны использовать ОС Microsoft и .NET.|Кроссплатформенный|  
|Формат сообщений|Закрытый|Отраслевые стандарты \(SOAP, WS\-\* и т. п.\)|  
  
<a name="Server_Comp"></a>   
### Сравнение реализации сервера  
  
#### Создание сервера в системе удаленного взаимодействия .NET  
...
### Сравнение реализации клиента  
  
#### Создание клиента в системе удаленного взаимодействия .NET  
...
```  
 ...
 Это пример программирования на уровне канала, так как он обладает максимальным сходством с примером удаленного взаимодействия.  Также доступен подход **Добавить ссылку на службу** в Visual Studio, который создает код для упрощения программирования клиента.  Дополнительные сведения см. в следующих разделах:  
  
-   [Программирование клиентов на уровне канала](../../../docs/framework/wcf/extending/client-channel-level-programming.md)  
...
#### Сериализация в среде удаленного взаимодействия .NET  
...
-   *По значению* — значения объекта сериализуются вне границ уровня, и на другом уровне создается новый экземпляр этого объекта.  Любые вызовы методов и свойств этого нового экземпляра выполняются только на локальном компьютере и не влияют на исходный объект или уровень.  
  
-   *По ссылке* — вне границ уровня сериализуется специальная «ссылка на объект».  Когда один уровень взаимодействует с методами или свойствами данного объекта, он устанавливает обратную связь с исходным объектом на исходном уровне.  Объекты по ссылке могут передаваться в любом направлении — с сервера на клиент или с клиента на сервер.  
...
```  
 ...
 Когда WCF отправляет объект между уровнями, он сериализует только значения и создает новый экземпляр объекта на другом уровне.  Любое взаимодействие со значениями объекта осуществляется только на локальном уровне — они не взаимодействуют с другим уровнем в отличие от объектов по ссылке системы удаленного взаимодействия .NET.  Дополнительные сведения см. в следующих разделах:  
  
-   [Сериализация и десериализация](../../../docs/framework/wcf/feature-details/serialization-and-deserialization.md)  
  
-   [Сериализация в Windows Communication Foundation](http://msdn.microsoft.com/magazine/cc163569.aspx)  
  
<a name="Exception_Handling"></a>   
### Возможности обработки исключений  
  
#### Исключения в системе удаленного взаимодействия .NET  
...
#### Безопасность в системе удаленного взаимодействия .NET  
...
-   [Руководство по безопасности WCF](http://wcfsecurity.codeplex.com/)  
  
<a name="Migrating_Top"></a>   
## Миграция в WCF  
  
<a name="Why_Migrate"></a>   
### Почему необходимо выполнить миграцию с переходом от удаленного взаимодействия на WCF?  
  
-   **Удаленное взаимодействие .NET — это устаревший продукт.** Как описано в статье [Удаленное взаимодействие .NET](http://msdn.microsoft.com/en-us/library/vstudio/72x4h507\(v=vs.100\).aspx), это решение считается устаревшим и не рекомендовано для новых разработок.  Для новых и существующих приложений рекомендуется использовать WCF или веб\-API ASP.NET.  
  
-   **WCF использует кроссплатформенные стандарты.** Решение WCF было разработано с учетом кроссплатформенного взаимодействия и поддерживает различные отраслевые стандарты \(SOAP, WS\-Security, WS\-Trust, и т. д.\).  Служба WCF может взаимодействовать с клиентами, работающими под управлением других операционных систем, отличных от Windows.  Решение удаленного взаимодействия было предназначено в первую очередь для сред, где клиентские и серверные приложения выполнялись с использованием платформы .NET в операционной системе Windows.  
  
-   **WCF включает встроенные функции обеспечения безопасности.** Решение WCF было разработано с учетом требований безопасности и предоставляет различные функции проверки подлинности, уровня безопасности транспорта, безопасности на уровне сообщений и т. д.  Решение удаленного взаимодействия было разработано для упрощения взаимодействия приложений, но не предназначено для обеспечения безопасности в средах, не заслуживающих доверия.  Решение WCF было разработано для работы как в доверенных, так и в недоверенных средах.  
...
-   **Настройте и разместите службу.** После создания контракта службы необходимо настроить привязку для предоставления службы в конечной точке.  Дополнительные сведения см. в разделе [Конечные точки: адреса, привязки и контракты](../../../docs/framework/wcf/feature-details/endpoints-addresses-bindings-and-contracts.md).  
...
5.  После этого можно запустить службу WCF с помощью следующего кода:  
    ...
    ```  
     ...
     При запуске метод ServiceHost использует файл web.config для установления надлежащего контракта, привязки и конечной точки.  Дополнительные сведения о файлах конфигурации см. в разделе [Настройка служб с использованием файлов конфигурации](../../../docs/framework/wcf/configuring-services-using-configuration-files.md).  Этот метод запуска сервера называется резидентным размещением.  Дополнительные сведения о других вариантах размещения служб WCF см. в разделе [Размещение служб](../../../docs/framework/wcf/hosting-services.md).  
  
6.  В файле app.config клиентского проекта должны быть объявлены соответствующие сведения о привязке для конечной точки службы.  В Visual Studio в этих целях проще всего использовать функцию **Добавить ссылку на службу**, которая автоматически обновит файл app.config.  Эти же изменения также можно добавить вручную.  
    ...
    ```  
     ...
     Дополнительные сведения об использовании функции **Добавить ссылку на службу** см. в разделе [How to: Add, Update, or Remove a Service Reference](../Topic/How%20to:%20Add,%20Update,%20or%20Remove%20a%20Service%20Reference.md).  
...
#### Сценарий 2. Служба возвращает объект по ссылке  
...
4.  Нам нужно изменить файл конфигурации сервера, выполнив два действия, представленные в следующем примере:  
    ...
    1.  Объявите раздел \<client\>, который описывает конечную точку для объекта, связанного с сеансом.  Это необходимо, поскольку сервер также выступает в качестве клиента в данной ситуации.  
...
#### Сценарий 3. Клиент отправляет серверу экземпляр по значению  
...
2.  Используйте клиент для создания нового объекта по значению \(Customer\), создайте канал для взаимодействия со службой ICustomerService и отправьте в нее объект.  
 ...
 Стандартные обмены данными в службе WCF осуществляются по значению.  Это гарантирует, что методы вызова этих объектов данных реализуются только локально без вызова кода на другом уровне.  Хотя существует возможность возврата объектов *с* сервера по методу, аналогичному возврату по ссылке, клиент не может передавать объекты по ссылке *на* сервер.  Сценарий, при котором клиенту и серверу необходимо обмениваться сообщениями друг с другом, можно реализовать в WCF с помощью дуплексной службы.  Дополнительные сведения см. в разделе [Дуплексные службы](../../../docs/framework/wcf/feature-details/duplex-services.md).  
