...
#### <a name="setting-a-clientcredentials-value-in-configuration"></a>Параметр \<clientCredentials > в конфигурации  
 Значения учетных данных указываются с помощью поведения конечной точки, как дочерние элементы элемента [ \<clientCredentials >](../../../docs/framework/configure-apps/file-schema/wcf/clientcredentials.md) элемента. Используемый элемент зависит от типа учетных данных клиента. Например, в следующем примере показано конфигурации задается сертификат X.509 с помощью <[\<clientCertificate >](../../../docs/framework/configure-apps/file-schema/wcf/clientcertificate-of-clientcredentials-element.md).  
