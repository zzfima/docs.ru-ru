...
## <a name="enabling-transaction-flow-at-the-endpoint-level"></a>Включение потока транзакций на уровне конечной точки  
 ...
 Большинство имеющихся привязок WCF предоставляет содержат `transactionFlow` и `transactionProtocol` атрибуты, позволяющие настроить прием входящих транзакций для конкретной привязки. Дополнительные сведения о настройке элементов конфигурации см. в разделе [ \<привязки >](../../../../docs/framework/misc/binding.md).  
...
## <a name="security"></a>Безопасность  
 ...
 При создании WCF-клиентам неизвестных или ненадежных веб-служб посредством обмена метаданными, вызовы операций в эти веб-службы следует по возможности отключать текущую транзакцию. В следующем примере показано, как это сделать.  
