...
1.  Предоставьте учетной записи, под которой WCF выполняется доступ на чтение к файлу, содержащему закрытый ключ, связанный с сертификатом X.509.  
    ...
    4.  Определите учетную запись, которой запущена WCF.  
  
         В следующей таблице представлены учетной записи, под которой выполняется WCF для данного сценария.  
    ...
    5.  Право на чтение файла, содержащего закрытый ключ для учетной записи с WCF, с помощью средства, например, cacls.exe.  
