...
- Службы WCF необходимо запустить в Windows Server 2003 и использовать транспорт, отличный от HTTP. В Windows Server 2003 в среде размещения IIS 6.0 предназначена только для взаимодействия только протокол HTTP. Приложения служб Windows не распространяются на это ограничение и можно использовать любой транспорт поддерживает WCF, включая net.tcp, net.pipe и net.msmq.  
