...
## <a name="definition-of-federated-security"></a>Определение федеративной безопасности  
 ...
 WCF предоставляет поддержку для создания и развертывания распределенных систем, в которых используется Федеративная безопасность.  
...
## <a name="support-for-federated-security-in-wcf"></a>Поддержка федеративной безопасности в WCF  
 WCF предоставляет готовое к использованию поддержка развертывания архитектуры федеративной безопасности через [ \<wsFederationHttpBinding >](../../../../docs/framework/configure-apps/file-schema/wcf/wsfederationhttpbinding.md).  
...
### <a name="phase-2-run-time-phase"></a>Фаза 2: фаза времени выполнения  
 На этапе выполнения клиент создает экземпляр объекта класса клиента WCF, а затем вызывает метод с помощью клиента WCF. Базовая архитектура WCF обрабатывает упомянутые выше шаги схемы обмена данными федеративной безопасности и позволяет клиенту легко воспользоваться службой.  
...
> [!NOTE]
>  Опять же `userAuthenticated` утверждения является тип утверждения, которые требуются для STS б. — Полное имя этого типа утверждения http://tempuri.org:userAuthenticated (а также связанное пространство имен), который используется в файле конфигурации службы маркеров безопасности. Значение этого утверждения указывает на присутствие этого утверждения, и предполагается, что служба STS A установила его равным `true`.  
...
## <a name="summary"></a>Сводка  
 Федеративная безопасность обеспечивает четкое разделение ответственности и позволяет выстраивать безопасные, масштабируемые архитектуры служб. Платформа для построения и развертывания распределенных приложений WCF предоставляет встроенную поддержку для реализации федеративной безопасности.  
