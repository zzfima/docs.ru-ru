...
## <a name="security-functions-of-bindings"></a>Функции безопасности привязок  
 WCF содержит ряд предоставляемых системой привязок, которые удовлетворяют большинству потребностей. Если определенная привязка не удовлетворяет всем необходимым требованиям, можно также создать пользовательскую привязку. Список привязок, предоставляемых системой см. в разделе [привязка, предоставляемая системой](../../../../docs/framework/wcf/system-provided-bindings.md). Дополнительные сведения о пользовательских привязок см. в разделе [пользовательские привязки](../../../../docs/framework/wcf/extending/custom-bindings.md).  
  
 Каждая привязка в WCF имеет две формы: виде API и в виде элемента XML, используемого в файле конфигурации. Например `WSHttpBinding` (API) имеет аналога в [ \<wsHttpBinding >](../../../../docs/framework/configure-apps/file-schema/wcf/wshttpbinding.md).  
...
### <a name="msmqintegrationbinding"></a>MsmqIntegrationBinding  
 ...
 Эта привязка оптимизирована для создания клиентов WCF и службы, поддерживающие взаимодействие с конечными точками, не являющихся - WCF очереди сообщений Майкрософт.  
...
### <a name="netmsmqbinding"></a>NetMsmqBinding  
 ...
 Эта привязка предназначена для использования при создании служб WCF, требующих MSMQ поддержка очереди сообщений.  
...
|UserName|Позволяет службе запрашивать проверку подлинности клиента на основе учетных данных типа "имя пользователя". Обратите внимание, что если режиму безопасности присвоено `TransportWithMessageCredential`, WCF не поддерживает отправку пароля хэш-кода или получение ключей с использованием пароля и использование таких ключей для режима безопасности Message. Таким образом WCF обеспечивает безопасность транспорта при использовании учетных данных имени пользователя.|  
