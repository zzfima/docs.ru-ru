---
title: "Рекомендации по частичному уровню доверию | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## Использование WCF из кода полностью доверенной платформы, который разрешает вызовы от частично доверенных вызывающих  
 В модели безопасности с частичным доверием системы [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] предполагается, что любой ызов открытого метода или свойства [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] происходит в контексте кода управления доступом \(CAS\) ведущего приложения.В [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] также предполагается, что существует только один контекст безопасности приложения для каждого домена <xref:System.AppDomain> и что этот контекст задается во время создания домена <xref:System.AppDomain> доверенным узлом \(например, вызовом метода <xref:System.AppDomain.CreateDomain%2A> или диспетчером приложения ASP.NET\).  
