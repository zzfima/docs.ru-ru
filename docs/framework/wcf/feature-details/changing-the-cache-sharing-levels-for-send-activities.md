---
title: "Изменение уровней совместного использования кэша для действий &#171;Send&#187; | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
---
# Изменение уровней совместного использования кэша для действий &#171;Send&#187;
...
## Уровни совместного использования кэша  
...
-   **Уровень узла**: на уровне совместного использования узла кэш доступен только экземплярам рабочих процессов, размещенным на узле службы рабочего процесса.Кэш может также совместно использоваться узлами служб рабочих процессов в кэше всего процесса.  
  
-   **Уровень экземпляра** — на уровне совместного использования экземпляра кэш доступен определенному экземпляру рабочего процесса во время его существования и недоступен другим экземплярам рабочих процессов.  
  
-   **Без кэширования** — кэширование отключено по умолчанию, если у вас будет рабочий процесс, использующий конечные точки, определенные в конфигурации.В этом случае также рекомендуется отключить кэширование, поскольку его использование может быть небезопасным.Например, если для каждой операции отправки требуется отдельный идентификатор \(другие учетные данные или олицетворение\).  
  
## Изменение уровня совместного использования кэша для клиентского рабочего процесса  
...
```  
 ...
 Затем добавьте модуль кэша каждому экземпляру клиентского рабочего процесса.  
...
## Изменение уровня совместного использования кэша для размещенной службы рабочего процесса  
...
```  
 ...
 Затем добавьте статический модуль кэша каждому узлу службы рабочего процесса.  
...
```  
 ...
 В размещенной службе рабочего процесса в файле конфигурации приложения можно указать параметры кэша фабрики и канала.Для этого необходимо создать поведение службы, содержащее параметры для кэша фабрики и канала, и добавить это поведение в службу.Следующий пример показывает содержание файла конфигурации, который содержит поведение службы `MyChannelCacheBehavior` с пользовательскими настройками кэша фабрики и канала.Это поведение добавляется в службу через атрибут `behaviorConfiguarion`.  
