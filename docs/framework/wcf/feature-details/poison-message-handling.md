...
> Для успешно доставленного сообщения повторных попыток доставки не выполняется.  
 ...
 Чтобы отслеживать число попыток чтения сообщения, в [!INCLUDE[wv](../../../../includes/wv-md.md)] есть устойчивое свойство сообщения, которое подсчитывает число прекращений, и свойство подсчета перемещений, считающее, сколько раз сообщение перемещалось между очередью приложения и вложенными очередями. Канал WCF использует их для вычисления числа повторных попыток получения и числа циклов повторных попыток. В [!INCLUDE[ws2003](../../../../includes/ws2003-md.md)] и[!INCLUDE[wxp](../../../../includes/wxp-md.md)]счетчик прерываний сохраняется в памяти каналом WCF и сбрасывается в случае сбоя приложения. Кроме того, канал WCF может содержать счетчики прерываний для до 256 сообщений в памяти в любое время. Когда прочитывается 257-е сообщение, подсчет числа прекращений для самого старого сообщения сбрасывается.  
...
## <a name="best-practice-handling-msmqpoisonmessageexception"></a>Рекомендации: Обработка Мсмкпоисонмессажеексцептион  
