...
|<xref:System.ServiceModel.WebHttpBinding>|Платформе|Нет, транспорт, TransportCredentialOnly|(Нет)|(Нет)|Н/Д|  
...
|Безопасность|Указывает способ защиты канала.<br /><br /> None Сообщение SOAP не защищено, и клиент не прошел проверку подлинности.<br />Перемещения Требования к безопасности удовлетворяются на транспортном уровне.<br />Письма Требования к безопасности удовлетворяются на уровне сообщений.<br />Типов Этот режим безопасности называется `TransportWithMessageCredentials`. В этом режиме учетные данные обрабатываются на уровне сообщений, а требования целостности и конфиденциальности выполняются на транспортном уровне.<br />Как Используются как уровень сообщений, так и безопасность на транспортном уровне. Эта возможность уникальна для привязки <xref:System.ServiceModel.NetMsmqBinding>.|  
