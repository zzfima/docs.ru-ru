...
Подробное пошаговое руководство по созданию службы WCF, размещенной в [IIS, см. в разделе Практическое руководство. Размещение службы WCF в IIS](how-to-host-a-wcf-service-in-iis.md).
...
- Windows 7: Наконец, необходимо убедиться, что ASP.NET настроен для использования .NET Framework версии 4 или более поздней. Это можно сделать, запустив инструмент ASPNET_Regiis с `–i` параметром. Дополнительные сведения см. в разделе [ASP.NET IIS Registration Tool](https://go.microsoft.com/fwlink/?LinkId=201186).
...
Можно также создать дополнительную конфигурацию размещения, например список базовых адресов службы. Кроме того, можно применить пользовательскую фабрику <xref:System.ServiceModel.Activation.ServiceHostFactory> , чтобы расширить директиву для использования с пользовательскими решениями по размещению. Приложения IIS, в которых размещаются службы WCF, не отвечают за управление созданием <xref:System.ServiceModel.ServiceHost> и временем существования экземпляров. Управляемая инфраструктура размещения WCF создает необходимый <xref:System.ServiceModel.ServiceHost> экземпляр динамически при получении первого запроса для SVC-файла. Этот экземпляр не освобождается, пока он не будет явно закрыт кодом или не перезапустится приложение.
...
Дополнительные сведения о модели компиляции ASP.NET 2,0 см. в разделе [Обзор компиляции ASP.NET](https://go.microsoft.com/fwlink/?LinkId=94773).
...
- Приложения, в которых размещены службы WCF вне служб IIS, могут управлять базовым адресом размещенных в них служб путем передачи набора универсальных <xref:System.ServiceModel.ServiceHost> кодов ресурса (URI) базового адреса конструктору или путем [ \<предоставления элемента > узла](../../../../docs/framework/configure-apps/file-schema/wcf/host.md) в службе Настройка. Службы, размещенные в IIS, не могут управлять своими базовыми адресами; базовым адресом службы, размещенной в IIS, является адрес ее SVC-файла.
...
Это предоставляет конечную точку, к которой можно `http://localhost/Application1/MyService.svc/anotherEndpoint`получить доступ по адресу.

Аналогично, элемент конфигурации конечной точки, использующий пустую строку в качестве относительного адреса, предоставляет `http://localhost/Application1/MyService.svc`конечную точку, доступную по адресу, который является базовым.
