---
title: "Односторонние службы | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
ms.topic: "article"
helpviewer_keywords: 
  - "контракты служб [WCF], определение односторонних"
  - "WCF [WCF], односторонние контракты служб [WCF]"
  - "Windows Communication Foundation [WCF], односторонние контракты служб [WCF]"
...
-   При использовании класса <xref:System.ServiceModel.NetMsmqBinding> или <xref:System.ServiceModel.MsmqIntegration.MsmqIntegrationBinding>.\([!INCLUDE[crabout](../../../../includes/crabout-md.md)] этом сценарии см. в разделе [Очереди в WCF](../../../../docs/framework/wcf/feature-details/queues-in-wcf.md).\)  
...
```  
 ...
 Полный пример см. на странице [Одностороннее взаимодействие](../../../../docs/framework/wcf/samples/one-way.md).  
  
## Блокировка клиентов с помощью односторонних операций  
 Важно понимать, что хотя некоторые односторонние приложения возобновляют работу, как только исходящие данные будут переданы по сетевому подключению, в некоторых случаях реализация привязки или службы может вызвать блокировку клиента [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] при использовании односторонних операций.В клиентских приложениях [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] клиентский объект [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] не возвращает управление, пока исходящие данные не будут переданы по сетевому подключению.Это относится ко всем шаблонам обмена сообщениями, включая односторонние операции; это означает, что любая проблема, связанная с передачей данных через транспортный канал, не позволит клиенту вернуть управление.В зависимости от проблемы ее результатом может быть исключение или задержка при отправке сообщения службе.  
  
 Например, если транспорту не удается найти конечную точку, исключение <xref:System.ServiceModel.EndpointNotFoundException?displayProperty=fullName> будет создано без большой задержки.Однако также возможна ситуация, при которой службе по какой\-либо причине не удается считать данные из сети, в результате чего операция отправки на стороне клиента не может быть завешена.В этих случаях, если превышается время <xref:System.ServiceModel.Channels.Binding.SendTimeout%2A?displayProperty=fullName> привязки транспорта клиента, создается исключение <xref:System.TimeoutException?displayProperty=fullName> — но не раньше истечения времени ожидания.Кроме того, службе может быть отправлено так много сообщений, что в какой\-то момент служба больше не сможет их обрабатывать.В этом случае односторонний клиент также блокируется, пока служба не сможет обработать сообщения или пока не будет создано исключение.  
  
 Подобная блокировка также может происходить в том случае, если свойство <xref:System.ServiceModel.ServiceBehaviorAttribute.ConcurrencyMode%2A?displayProperty=fullName> службы имеет значение <xref:System.ServiceModel.ConcurrencyMode>, а привязка использует сеансы.В этом случае диспетчер принудительно упорядочивает входящие сообщения \(требование сеансов\), что не допускает чтения из сети следующего сообщения, пока служба не обработала предыдущее сообщение для этого сеанса.Клиент в этом случае также блокируется, но появление исключения зависит от того, успеет ли служба обработать ожидающие данные, пока не истечет установленное на стороне клиента время ожидания.  
