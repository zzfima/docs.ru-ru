---
title: "Конечные точки служб и адресация очереди | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## Адресация к очереди сообщений MSMQ  
 ...
 Имена путей сопоставляются именам «FormatName», чтобы определить дополнительные аспекты адреса, включая маршрутизацию и протокол передачи данных диспетчера очередей.Диспетчер очередей поддерживает два протокола передачи данных: собственный протокол MSMQ и протокол SRMP.  
  
 [!INCLUDE[crabout](../../../../includes/crabout-md.md)] путях и именах форматов MSMQ см. в разделе [Сведения об очередях сообщений](http://go.microsoft.com/fwlink/?LinkId=94837).  
...
## NetMsmqBinding и адресация к службе  
 ...
 Транспорт очередей MSMQ в [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] предоставляет схему net.msmq.Любое сообщение, адресованное с помощью схемы net.msmq, отправляется с помощью `NetMsmqBinding` через транспортный канал очередей MSMQ.  
  
 Адресация очереди в [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] основана на следующем шаблоне:  
...
-   \<*queue\-name*\> — имя очереди.Имя очереди также может относиться к вложенной очереди.Поэтому \<*queue\-name*\> \= \<*name\-of\-queue*\>\[;*sub\-queue\-name*\].  
 ...
 Конечная точка, выполняющая чтение из очереди, должна указывать адрес очереди с помощью указанной выше схемы при открывании узла службы \(ServiceHost\).Примеры см. в разделах [Привязка MSMQ на платформе .NET](../../../../docs/framework/wcf/samples/net-msmq-binding.md) и [Message Queuing Integration Binding Samples](http://msdn.microsoft.com/ru-ru/997d11cb-f2c5-4ba0-9209-92843d4d0e1a).  
...
|------------------------------------------|--------------------------------------------|-------------------------------------------------|----------------------------------------|  
|Net.msmq:\/\/\<имя компьютера\>\/private\/abc|False \(по умолчанию\)|Native \(по умолчанию\)|DIRECT\=OS:имя\_компьютера\\private$\\abc|  
|Net.msmq:\/\/\<имя компьютера\>\/private\/abc|False|SRMP|DIRECT\=http:\/\/компьютер\/msmq\/private$\/abc|  
|Net.msmq:\/\/\<имя компьютера\>\/private\/abc|True|Native|PUBLIC\=guid \(идентификатор GUID очереди\)|  
...
### Чтение сообщений из очереди недоставленных сообщений или из очереди подозрительных сообщений  
 ...
 Служба [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] проверяет, что все полученные ей сообщения адресованы в определенную очередь, которую прослушивает эта служба.Если очередь назначения сообщения не соответствует очереди, в которой оно содержится, служба не обрабатывает сообщение.Если служба прослушивает очередь недоставленных сообщений, эту проблему необходимо решить обязательно, поскольку сообщения в такой очереди всегда адресованы в другое расположение.Для чтения сообщений из очереди недоставленных или подозрительных сообщений необходимо использовать `ServiceBehavior` и параметр <xref:System.ServiceModel.AddressFilterMode>.Пример см. в разделе [Очереди недоставленных сообщений](../../../../docs/framework/wcf/samples/dead-letter-queues.md).  
  
## MsmqIntegrationBinding и адресация к службе  
 `MsmqIntegrationBinding` используется для обмена информацией с традиционными приложениями MSMQ.Чтобы упростить взаимодействие с существующими приложениями MSMQ, в [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] предусмотрена поддержка адресации только по именам форматов.Таким образом, сообщения, отправленные с помощью этой привязки, должны соответствовать следующей схеме URI:  
  
 msmq.formatname:\<*MSMQ\-format\-name*\>\>  
  
 Имя формата MSMQ имеет форму, указанную для MSMQ в разделе [Сведения об очередях сообщений](http://go.microsoft.com/fwlink/?LinkId=94837).  
