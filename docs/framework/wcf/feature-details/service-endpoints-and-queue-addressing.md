...
В этом разделе рассматриваются принципы обращения клиентов к службам, выполняющим чтение из очередей, и сопоставления конечных точек служб с очередями. Напоминаем на следующем рисунке классической Windows Communication Foundation (WCF) в очереди развертывания приложения.  
 ...
 Чтобы отправить службе сообщение, клиент адресует это сообщение целевой очереди. Чтобы прочитать сообщение из очереди, служба задает целевую очередь в качестве своего адреса прослушивания. Адресация в WCF на основе универсальный код ресурса URI пока имена очередей сообщений (MSMQ) не основаны на URI. Поэтому важно понять, как обращаться очередям, созданным в MSMQ, с помощью WCF.  
...
-   \<*Имя очереди*> — имя очереди. Имя очереди также может относиться к вложенной очереди. Таким образом \< *имя очереди*> = \< *name-of-queue*> [; *Имя подопераций queue*].  
 ...
 Конечная точка, выполняющая чтение из очереди, должна указывать адрес очереди с помощью указанной выше схемы при открывании узла службы (ServiceHost). Примеры см. в разделе [Net привязки MSMQ](../../../../docs/framework/wcf/samples/net-msmq-binding.md) и [сообщений очереди привязки образцов интеграции](http://msdn.microsoft.com/library/997d11cb-f2c5-4ba0-9209-92843d4d0e1a).  
...
### <a name="reading-messages-from-the-dead-letter-queue-or-the-poison-message-queue"></a>Чтение сообщений из очереди недоставленных сообщений или из очереди подозрительных сообщений  
 ...
 NET.MSMQ: //localhost/ [закрытый /] \< *-сообщений буква очереди имя пользовательского*>.  
  
 Служба WCF проверяет, что всех сообщений, которые он получает адресованы в определенную очередь, выполняет прослушивание. Если очередь назначения сообщения не соответствует очереди, в которой оно содержится, служба не обрабатывает сообщение. Если служба прослушивает очередь недоставленных сообщений, эту проблему необходимо решить обязательно, поскольку сообщения в такой очереди всегда адресованы в другое расположение. Для чтения сообщений из очереди недоставленных или подозрительных сообщений необходимо использовать `ServiceBehavior` и параметр <xref:System.ServiceModel.AddressFilterMode.Any>. Пример см. в разделе [очередей недоставленных сообщений](../../../../docs/framework/wcf/samples/dead-letter-queues.md).  
  
## <a name="msmqintegrationbinding-and-service-addressing"></a>MsmqIntegrationBinding и адресация к службе  
 `MsmqIntegrationBinding` используется для обмена информацией с традиционными приложениями MSMQ. Чтобы облегчить взаимодействие с существующим приложением MSMQ, WCF поддерживает только адресацию имен форматов. Таким образом, сообщения, отправленные с помощью этой привязки, должны соответствовать следующей схеме URI:  
  
 msmq.formatname:\<*MSMQ-format-name*>>  
  
 MSMQ-формат name — формы, заданные MSMQ в [об очереди сообщений](http://go.microsoft.com/fwlink/?LinkId=94837).  
 ...
 Обратите внимание, что нельзя использовать адресацию net.msmq:// вместе с `MsmqIntegrationBinding`. Поскольку `MsmqIntegrationBinding` поддерживает произвольный адресацию имен форматов MSMQ, можно воспользоваться службой WCF, использующей эту привязку для использования в MSMQ и многоадресной рассылки составления списка функций. Единственное исключение: необходимо указать `CustomDeadLetterQueue` при использовании `MsmqIntegrationBinding`. Требуемая форма: net.msmq://, как и при указании с помощью `NetMsmqBinding`.  
