...
В этом разделе рассматриваются различные аспекты, относящиеся к федеративным приложениям, границы доверия и конфигурации и использовать выданные маркеры в Windows Communication Foundation (WCF).  
...
## <a name="services-security-token-services-and-trust"></a>Службы, службы маркеров безопасности и доверие  
 ...
 Службы WCF используют значение <xref:System.ServiceModel.Channels.LocalServiceSecuritySettings.MaxClockSkew%2A> свойство [SecurityBindingElement](../../../../docs/framework/wcf/diagnostics/wmi/securitybindingelement.md) разрешающее для часов наклона между клиентом и службой. В федерации параметр `MaxClockSkew` применяется к рассогласованию часов как для клиента, так и для службы маркеров безопасности, от которой клиент получил выданный маркер. Поэтому службам маркеров безопасности при задании времени начала и истечения срока действия выданных маркеров не требуется учитывать рассогласование часов.  
