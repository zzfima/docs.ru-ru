---
title: "Федерация и доверие | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## Службы, службы маркеров безопасности и доверие  
 ...
 Службы [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] используют значение свойства <xref:System.ServiceModel.Channels.LocalServiceSecuritySettings.MaxClockSkew%2A> элемента [SecurityBindingElement](../../../../docs/framework/wcf/diagnostics/wmi/securitybindingelement.md) для разрешения рассогласования часов в клиенте и службе.В федерации параметр `MaxClockSkew` применяется к рассогласованию часов как для клиента, так и для службы маркеров безопасности, от которой клиент получил выданный маркер.Поэтому службам маркеров безопасности при задании времени начала и истечения срока действия выданных маркеров не требуется учитывать рассогласование часов.  
...
## Время существования маркера и его обновление  
 Клиенты [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] не проверяют выданные маркеры перед передачей исходного запроса службе.[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] полагает, что служба маркеров безопасности выдала маркер с соответствующим временем начала и окончания срока действия.Если клиент закэшировал и повторно использует токен, при последующих запросах проверяется время существования токена и в случае необходимости клиент автоматически обновляет токен.[!INCLUDE[crabout](../../../../includes/crabout-md.md)] о кэшировании токена см. в разделе [Как создавать федеративный клиент](../../../../docs/framework/wcf/feature-details/how-to-create-a-federated-client.md).  
