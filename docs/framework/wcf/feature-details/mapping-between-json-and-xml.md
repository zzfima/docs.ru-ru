...
На внутреннем уровне JSON представлены в виде информационного набора XML при обработке WCF. Обычно у вас должно заботить это внутреннее представление, поскольку сопоставление является только логично совмещая: JSON является физически обычно не преобразуются в XML в памяти или преобразовать JSON из XML. Сопоставление означает, что для обращения к JSON-содержимому используются интерфейсы API XML.
...
Пример Следующий документ может быть не сопоставлен JSON, так как [children] содержит Персональные данные и комментарий.
...
Пример Следующий документ может быть не сопоставлен JSON, так как [пространство имен attributes] не является пустым.
