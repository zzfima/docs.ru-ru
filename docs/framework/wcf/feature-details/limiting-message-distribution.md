---
title: "Ограничение распространения сообщений | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
-   Фрагменты кода и сопутствующие сведения см. в разделе [Блог об одноранговых каналах](http://go.microsoft.com/fwlink/?LinkID=114531) \(http:\/\/go.microsoft.com\/fwlink\/?LinkID\=114531\).  
...
-   Фрагменты кода и сопутствующие сведения см. в разделе [Блог об одноранговых каналах](http://go.microsoft.com/fwlink/?LinkID=114532) \(http:\/\/go.microsoft.com\/fwlink\/?LinkID\=114532\).  
...
-   **Куда** требуется доставить сообщение?Только на соседний узел?На узел в другой части сетки?На половину узлов сетки?  
  
-   **Как часто** будут отправляться сообщения?  
  
-   Какая **пропускная способность** потребуется для этих сообщений?  
...
-   **Куда**  
    ...
    -   *большой набор узлов*: MessagePropagationFilter.  
  
-   **Как часто**  
  
    -   *очень часто*: прямое подключение, PeerHopCount, MessagePropagationFilter;  
  
    -   *изредка*: локальный фильтр.  
  
-   **Потребляемая пропускная способность**  
  
    -   *высокая*: прямое подключение, менее предпочтительно использовать MessagePropagationFilter или локальный фильтр;  
  
    -   *низкая*: любой способ, прямое подключение скорее всего не требуется.  
