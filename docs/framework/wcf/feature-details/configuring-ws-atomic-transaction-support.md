...
[!INCLUDE[wxp](../../../../includes/wxp-md.md)] Если вы используете или [!INCLUDE[ws2003](../../../../includes/ws2003-md.md)], доступ к оснастке MMC можно получить, перейдя в раздел **Панель управления/Администрирование/службы компонентов**, щелкнув правой кнопкой мыши **Мой компьютер**и выбрав пункт **Свойства**. Здесь же можно настроить координатор распределенных транзакций (Майкрософт). Параметры, доступные для конфигурации, группируются на вкладке **WS-AT** . Если вы [!INCLUDE[lserver](../../../../includes/lserver-md.md)]используете Windows Vista или, оснастку MMC можно найти, нажав кнопку **Пуск** и введя `dcomcnfg.exe` в поле **поиска** . Когда откроется консоль MMC, перейдите к узлу **Мои Компутер\дистрибутед Transaction КУРДИНАТОР\ЛОКАЛ DTC** , щелкните правой кнопкой мыши и выберите пункт **свойства**. Параметры, доступные для конфигурации, группируются на вкладке **WS-AT** .
...
Чтобы установить доверенные отношения, необходимо уже иметь в наличии необходимые сертификаты. Сведения о создании и установке новых сертификатов перед выполнением следующих шагов см. в разделе [как Создание и Установка временных сертификатов клиента в WCF во время](https://go.microsoft.com/fwlink/?LinkId=158925)разработки.
...
3. Щелкните правой кнопкой мыши значениенаправойпанелиивыберите`ServiceModelDiagnosticTracing` изменить.
