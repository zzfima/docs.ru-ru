...
2.  Определите фильтры, используемые для маршрутизации сообщений до конечных точек назначения.  В этом примере фильтр XPath используется для обнаружения значения пользовательского заголовка «CalcVer» для определения нужной версии должно быть передано сообщение. Фильтр XPath используется также для определения сообщений, которые не содержат заголовка «CalcVer». В следующем примере определяются необходимые фильтры и таблица пространства имен.  
