---
title: "Служба маршрутизации | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
> [!NOTE]
>  Служба маршрутизации в настоящее время не поддерживает маршрутизацию служб [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] REST.  Попробуйте реализовать маршрутизацию вызовов REST с помощью <xref:System.Web.Routing> или [маршрутизации запросов приложений](http://go.microsoft.com/fwlink/?LinkId=164589) \(http:\/\/go.microsoft.com\/fwlink\/?LinkId\=164589\).  
...
-   Динамическая конфигурация  
 ...
 Дополнительные сведения о фильтрах сообщений и таблицах фильтров см. в разделах [Введение в маршрутизацию](../../../../docs/framework/wcf/feature-details/routing-introduction.md) и [Фильтры сообщений](../../../../docs/framework/wcf/feature-details/message-filters.md).  
...
## Динамическая конфигурация  
 ...
 Дополнительные сведения о динамической настройке службы маршрутизации см. в разделе [Введение в маршрутизацию](../../../../docs/framework/wcf/feature-details/routing-introduction.md).  
...
## Обработка SOAP  
 Общее требование \- возможность маршрутизации сообщений между конечными точками с различными требованиями SOAP.  В поддержку этого требования служба маршрутизации реализует класс <xref:System.ServiceModel.Routing.SoapProcessingBehavior>, который автоматически создает новый объект **MessageVersion**, отвечающий требованиям целевой конечной точки, прежде чем к ней производится маршрутизация.  При этом также создается новый объект **MessageVersion** для всех ответных сообщений, прежде чем они будут возвращены запрашивающему клиентскому приложению. Это гарантирует, что объект класса **MessageVersion** в ответе соответствует исходному запросу.  
  
 Дополнительные сведения об обработке SOAP см. в разделе [Введение в маршрутизацию](../../../../docs/framework/wcf/feature-details/routing-introduction.md).  
...
## Обработка ошибок  
 ...
 Если служба маршрутизации обнаружила исключение <xref:System.ServiceModel.CommunicationException> при попытке отправки сообщения, то будет выполнена обработка ошибок.  Эти исключения обычно указывают на то, что при попытке связаться с определенной конечной точкой клиента была обнаружена проблема, например <xref:System.ServiceModel.EndpointNotFoundException>, <xref:System.ServiceModel.ServerTooBusyException> или <xref:System.ServiceModel.CommunicationObjectFaultedException>.  Код обработки ошибок также перехватит и попытается повторить отправку при возникновении исключения **TimeoutException**. Это еще одно общее исключение, которое не является производным от **CommunicationException**.  
  
 [!INCLUDE[crabout](../../../../includes/crabout-md.md)] обработке ошибок см. в разделе [Введение в маршрутизацию](../../../../docs/framework/wcf/feature-details/routing-introduction.md).  
...
## Резервные конечные точки  
 ...
 [!INCLUDE[crabout](../../../../includes/crabout-md.md)] резервных конечных точках см. в разделах [Введение в маршрутизацию](../../../../docs/framework/wcf/feature-details/routing-introduction.md) и [Фильтры сообщений](../../../../docs/framework/wcf/feature-details/message-filters.md).  
