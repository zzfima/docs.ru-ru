...
Не все пользователи знакомы с терминологией, которая используется при обсуждении вопросов безопасности. В данном разделе кратко объясняются некоторые термины, относящиеся к безопасности, однако его целью не является исчерпывающее объяснение каждого термина.  
 ...
 Определяет центр сертификации, который выдает сертификаты проверки подлинности сервера и клиента серверам и клиентам, запрашивающим эти сертификаты. Так как он содержит открытый ключ, используемый в цифровых подписях, его также называют *сертификат подписи*. Если центр сертификации является корневым центром сертификации, сертификат ЦС может называться *корневой сертификат*. Иногда также называют *сертификат узла*.  
  
 Иерархия ЦС  
 В иерархии ЦС содержится несколько центров сертификации. Его структуру, чтобы каждый ЦС Сертифицирован другим ЦС на более высоком уровне иерархии до верхней части иерархии, также называется *корневой центр*, будет достигнут.  
 ...
 данные с хэшем  
 Тип данных содержимого, определенный стандартом PKCS 7 и состоящий из любого типа данных и хэш-кода сообщения содержимого.  
 ...
 открытый текст  
 Сообщение в незашифрованном виде. Сообщения с открытым текстом, иногда называют *текстом* сообщений.  
 ...
 алгоритм SHA  
 Хэш-алгоритм, формирующий хэш. SHA используется в различных алгоритмах; в частности, он используется совместно с алгоритмом DSA в алгоритме DSS. Существует четыре разновидности алгоритма SHA: SHA-1, SHA-256, SHA-384 и SHA-512. SHA-1 формирует хэш-значение длиной 160 бит. SHA-256, SHA-384 и SHA-512 формируют хэш-коды длиной 256, 384 и 512 бит соответственно. Алгоритм SHA был разработан специалистами Национального института стандартов и технологий США (NIST) и Агентства национальной безопасности США (NSA).  
