...
## <a name="protect-configuration-files-with-acls"></a>Защита файлов конфигурации с помощью списков управления доступом  
 Обязательные и необязательные утверждения можно указать в коде и файлах конфигурации для CardSpace, выданные маркеры. В результате в сообщениях `RequestSecurityToken`, отправляемых в службу маркеров безопасности, будут создаваться соответствующие элементы. Злоумышленник может изменить код или файл конфигурации, удалив обязательные или необязательные утверждения, в результате чего возникнет вероятность того, что служба маркеров безопасности выдаст маркер, который запрещает доступ к целевой службе.  
