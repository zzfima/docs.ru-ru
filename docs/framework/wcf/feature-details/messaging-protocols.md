...
- R3515: Сообщения запроса отправляются в конечную точку должны содержать `ReplyTo` заголовок с `[address]` равным `http://www.w3.org/2005/08/addressing/anonymous`, или нет `ReplyTo` заголовок вообще, если эта конечная точка использует привязку WSDL 1.1 SOAP 1.x HTTP и вариант политики с `wsap10:UsingAddressing` утверждение и нет `cdp:CompositeDuplex` присоединенного утверждения.

- R3516: Сообщения запроса отправляются в конечную точку должны содержать `ReplyTo` заголовок с `[address]` равным `http://www.w3.org/2005/08/addressing/anonymous` Если эта конечная точка использует привязку WSDL 1.1 SOAP 1.x HTTP и вариант политики с `wsap:UsingAddressing` утверждения и не `cdp:CompositeDuplex`присоединенного утверждения.
