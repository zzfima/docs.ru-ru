---
title: "Протоколы обмена сообщениями | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
#### Ссылки на конечные точки и версии WS\-Addressing  
 В [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] реализован ряд протоколов инфраструктуры, использующих WS\-Addressing — в частности, элемент `EndpointReference` и класс `W3C.WsAddressing.EndpointReferenceType` \(например, WS\-ReliableMessaging, WS\-SecureConversation и WS\-Trust\).[!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] поддерживает использование обеих версий WS\-Addressing с других протоколов инфраструктуры.Конечные точки [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] поддерживают одну версию WS\-Addressing для каждой конечной точки.  
...
#### Ссылки на конечные точки и метаданные  
 ...
 Рассмотрим сценарий, в котором служба [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] требует проверку подлинности с использованием токен SAML \(Security Assertions Markup Language\), выданного издателем токена по адресу http:\/\/sts.fabrikam123.com.Конечная точка [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] описывает это требование проверки подлинности с помощью утверждения `sp:IssuedToken` с вложенным утверждением `sp:Issuer`, указывающим на издателя токена.Клиентское приложение, обращающееся к утверждению `sp:Issuer`, должно знать, как взаимодействовать с конечной точкой издателя токена.Клиенту требуются метаданные об издателе токена.Используя ссылку на конечную точку расширения метаданных, определенную в MEX, [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] предоставляет ссылку на метаданные издателя токена.  
...
#### Заголовки сообщения  
 Для обеих версий WS\-Addressing [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] использует следующие заголовки сообщения, указанные в спецификациях `wsa:To`, `wsa:ReplyTo`, `wsa:Action`, `wsa:MessageID`и  `wsa:RelatesTo`.  
...
|Код|Причина|  
|---------|-------------|  
|wsa10:InvalidAddressingHeader|Повторяющийся `wsa:To`, `wsa:ReplyTo`, `wsa:From` или `wsa:MessageID`.Повторяющийся `wsa:RelatesTo` с тем же `RelationshipType`.|  
...
#### Определение действия  
 ...
 B3521: независимо от версии WS\-Addressing, используемой конечной точкой, для задания универсального кода ресурса \(URI\) действия `Action` для соответствующих сообщений [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] использует в элементах `wsdl:portType/wsdl:operation/[wsdl:input | wsdl:output | wsdl:fault]` атрибут `wsaw10:Action`, определенный в спецификации WS\-ADDR10\-WSDL.  
...
```  
 ...
 В ряде реализаций MIME ослаблены требования к тому, чтобы значение внутри «\<» и «\>» было адресом электронной почты, и, помимо адреса электронной почты, допускается использование `absoluteURI` в угловых скобках «\<» и «\>».В этой версии [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] используются значения заголовка Content\-ID MIME в виде:  
