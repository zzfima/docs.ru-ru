...
Приложения, созданные с помощью Windows Communication Foundation (WCF), могут регистрировать события безопасности (успех, сбой или и то, и другое) с функцией аудита. События записываются в журнал системных событий Windows, и их можно просматривать при помощи средства просмотра событий.  
 ...
 Для получения дополнительной информации [Security Overview](../../../../docs/framework/wcf/feature-details/security-overview.md)о безопасности WCF см. Для получения дополнительной информации о программировании WCF, [см.](../../../../docs/framework/wcf/basic-wcf-programming.md)  
...
- Уровень сообщения, в котором WCF проверяет достоверность сообщения и проверяет подлинность вызываемого абонента.  
  
 Вы можете проверить оба уровня аудита на успех или неудачу, что известно как *поведение аудита.*  
...
## <a name="audit-log-location"></a>Расположение журнала аудита  
 ...
 В отличие от этого авторизованные пользователи могут производить чтение и запись в журнал приложений. Windows XP по умолчанию записывает события аудита в журнал приложения. Этот журнал может также содержать персональный сведения, видимые всем авторизованным пользователям.  
...
|<xref:System.ServiceModel.Description.ServiceSecurityAuditBehavior.SuppressAuditFailure%2A>|Задает, что происходит с запросом клиента в случае сбоя аудита. Например, если служба пытается произвести запись в журнал безопасности, но не имеет привилегий `SeAuditPrivilege`. Значение по умолчанию `true` означает, что сбои игнорируются и запрос клиента обрабатывается обычным образом.|  
 ...
 Например, настройка приложения для регистрации событий аудита [см.](../../../../docs/framework/wcf/feature-details/how-to-audit-wcf-security-events.md)  
  
### <a name="configuration"></a>Конфигурация  
 Можно также использовать конфигурацию для указания поведения аудита, добавив [ \<>serviceSecurityAudit](../../../../docs/framework/configure-apps/file-schema/wcf/servicesecurityaudit.md) под [ \<поведением>. ](../../../../docs/framework/configure-apps/file-schema/wcf/behaviors.md) Необходимо добавить элемент под [ \<поведением>,](../../../../docs/framework/configure-apps/file-schema/wcf/behavior-of-endpointbehaviors.md) как показано в следующем коде.  
...
## <a name="security-considerations"></a>Соображения безопасности  
 ...
 События аудита, запинанные в журнал приложений windows XP, видны любому пользователю, удостоверяем подлинность.  
...
|------------|---------------------|------------------|  
|Windows XP SP2 или позже|Поддерживается|Не поддерживается|  
...
- [\<поведение>](../../../../docs/framework/configure-apps/file-schema/wcf/behaviors.md)
