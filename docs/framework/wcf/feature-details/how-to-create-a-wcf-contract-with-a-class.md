...
> [!WARNING]
>  `[ServiceContract]` и `[ServiceContractAttribute]` сделать то же самое. То же самое справедливо для `[OperationContract]` и `[OperationContractAttribute]`. В каждом случае первое является сокращением для второго.  
