...
|------------------|-----------------|-------------------------|--------------------|  
|Действие|Использует класс <xref:System.ServiceModel.Dispatcher.ActionMessageFilter> для поиска сообщений, содержащих определенное действие.|Действие, по которому производится фильтрация.|\<Имя фильтра = filterType «action1» = «Действие» filterData = «http://namespace/contract/operation» / >|  
