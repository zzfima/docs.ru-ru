...
|Значение `Null` для любого типа|NULL|Недействительные типы значений также поддерживаются и отображаются на JSON так же, как и типы значения, не связанные с нейнулируемыми.|
