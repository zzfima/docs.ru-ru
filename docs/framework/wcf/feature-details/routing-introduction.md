---
title: "Введение в маршрутизацию | Microsoft Docs"
ms.custom: ""
ms.date: "03/30/2017"
...
## Конфигурация  
 Служба маршрутизация представляет собой службу WCF, которая обнаруживает одну или несколько конечных точек службы, получающих сообщения от клиентских приложений и перенаправляющих их в одну или несколько целевых конечных точек.  Эта служба предоставляет поведение <xref:System.ServiceModel.Routing.RoutingBehavior>, которое применяется к обнаруженным конечным точкам службы.  Это поведение используется для настройки различных аспектов работы службы.  Для упрощения настройки с использованием файла конфигурации параметры указываются в разделе **RoutingBehavior**.  В сценариях, основанных на программном коде, эти параметры указываются как часть объекта <xref:System.ServiceModel.Routing.RoutingConfiguration>, который затем можно передать в **RoutingBehavior**.  
  
 При запуске этого поведения к клиентским конечным точкам добавляется параметр <xref:System.ServiceModel.Routing.SoapProcessingBehavior>, который используется для обработки сообщений SOAP.  Такой подход позволяет службе маршрутизации передавать сообщения в конечные точки, требующие версии **MessageVersion**, отличающейся от версии конечной точки, через которую было получено сообщение.  Поведение **RoutingBehavior** также регистрирует расширение службы <xref:System.ServiceModel.Routing.RoutingExtension>, которое предоставляет точку доступа, используемую для изменения конфигурации службы во время выполнения.  
  
 Класс **RoutingConfiguration** позволяет согласованным образом настраивать и обновлять конфигурацию службы маршрутизации.  Этот класс содержит параметры, используемые для настройки службы маршрутизации. Он используется для настройки поведения \(**RoutingBehavior**\) при запуске службы или передается в расширение службы \(**RoutingExtension**\) для изменения конфигурации во время выполнения.  
  
 Логика маршрутизации на основе содержимого определяется путем группирования нескольких объектов <xref:System.ServiceModel.Dispatcher.MessageFilter> в таблицы фильтрации \(объекты <xref:System.ServiceModel.Dispatcher.MessageFilterTable%601>\).  Входящие сообщения анализируются с использованием фильтров из таблицы фильтрации. Каждое сообщение, совпадающее с фильтром **MessageFilter**, отправляется в целевую конечную точку.  Таблица фильтров, которая будет использоваться для маршрутизации сообщений, определяется поведением **RoutingBehavior**, указанным в конфигурации или в программном коде \(с использованием объекта **RoutingConfiguration**\).  
...
>  При работе с контрактами, задействующими несколько шаблонов взаимодействия \(например со смешением односторонних и двусторонних операций\), правильным решением будет использование в службе маршрутизации дуплексного контракта \(например <xref:System.ServiceModel.Routing.IDuplexSessionRouter>\).  Это значит, что привязка должна быть совместима с дуплексным взаимодействием, однако это неприменимо ко всем сценариям.  В сценариях, для которых это неприменимо, может понадобиться разбиение модели взаимодействия на несколько конечных точек либо изменение приложения.  
 ...
 Дополнительные сведения о контрактах маршрутизации см. в разделе [Контракты маршрутизации](../../../../docs/framework/wcf/feature-details/routing-contracts.md).  
  
 После определения конечной точки службы можно использовать поведение **RoutingBehavior** для связывания конкретной конфигурации **RoutingConfiguration** с этой конечной точкой.  Если служба маршрутизации настраивается с помощью файла конфигурации, то для определения таблицы фильтров, содержащей логику маршрутизации для обработки сообщений, получаемых в этой конечной точке, используется поведение **RoutingBehavior**.  Если служба маршрутизации настраивается с помощью программного кода, таблицу фильтров можно определить в объекте **RoutingConfiguration**.  
...
```  
 ...
 В этом примере служба маршрутизации настроена на обнаружение единственной конечной точки с адресом «http:\/\/localhost:8000\/routingservice\/router», которая используется для приема перенаправляемых сообщений.  Сообщения перенаправляются в конечные точки типа «запрос\-ответ», поэтому конечная точка службы использует контракт <xref:System.ServiceModel.Routing.IRequestReplyRouter>.  Эта конфигурация также определяет единственную клиентскую конечную точку, куда перенаправляются сообщения \(«http:\/\/localhost:8000\/servicemodelsample\/service»\).  Таблица фильтров \(не показана\) с именем «routingTable1» содержит логику маршрутизации, используемую для перенаправления сообщений. Она связывается с конечной точкой службы с помощью **RoutingBehavior** \(файл конфигурации\) либо с помощью **RoutingConfiguration** \(программируемая конфигурация\).  
...
### Логика маршрутизации  
 ...
 В службе маршрутизации реализовано несколько фильтров **MessageFilter**, которые проверяют сообщение на наличие конкретных значений \(таких как адрес, действие, имя конечной точки или запрос XPath\).  Если ни один из этих фильтров не отвечает потребностям, можно создать пользовательскую реализацию фильтра **MessageFilter**.  Дополнительные сведения о фильтрах сообщений и сравнение различных реализаций, используемых службой маршрутизации, см. в разделах [Фильтры сообщений](../../../../docs/framework/wcf/feature-details/message-filters.md) и [Выбор фильтра](../../../../docs/framework/wcf/feature-details/choosing-a-filter.md).  
  
 Множество фильтров сообщений организовано в таблицы фильтров, связывающие каждый фильтр **MessageFilter** с целевой конечной точкой.  Кроме того, в таблице фильтров можно привести список резервных конечных точек, в которые служба маршрутизации отправляет сообщение в случае сбоя соединения.  
  
 По умолчанию все фильтры сообщений в таблице оцениваются одновременно. Однако, если нужно, чтобы фильтры оценивались в определенном порядке, можно указать приоритет оценки \- <xref:System.ServiceModel.Routing.Configuration.FilterTableEntryElement.Priority%2A>.  Сначала оцениваются все записи с наивысшим приоритетом. Если найдено сопоставление на более высоком уровне, фильтры сообщений с низким приоритетом не оцениваются.  Дополнительные сведения о таблицах фильтров см. в разделе [Фильтры сообщений](../../../../docs/framework/wcf/feature-details/message-filters.md).  
  
 В следующем примере использован фильтр <xref:System.ServiceModel.Dispatcher.MatchAllMessageFilter>, который оценивает все сообщения как `true`.  Этот фильтр **MessageFilter** добавлен в таблицу «routingTable1», которая связывает фильтр **MessageFilter** с клиентской конечной точкой «CalculatorService».  В поведении **RoutingBehavior** определено, что эта таблица должна использоваться для перенаправления сообщений, обрабатываемых конечной точкой службы.  
...
-   При оценке сообщения несколько фильтров должны возвращать значение `true`.  
 ...
 Если эти условия выполняются, сообщение перенаправляется во все конечные точки всех фильтров, обнаруживших значение `true`.  В следующем примере определена конфигурация маршрутизации, при которой сообщения перенаправляются в обе конечные точки, если адрес конечной точки в сообщении \- «http:\/\/localhost:8000\/routingservice\/router\/rounding».  
...
### Обработка SOAP  
 В целях поддержки маршрутизации сообщений между разнородными протоколами **RoutingBehavior** по умолчанию добавляет <xref:System.ServiceModel.Routing.SoapProcessingBehavior> ко всем клиентским конечным точкам, в которые перенаправляются сообщения.  Это поведение автоматически создает новую версию **MessageVersion** перед перенаправлением сообщения в конечную точку. Кроме того, создается подходящая версия **MessageVersion** для любого ответного документа \(перед его возвратом запрашивающему клиентскому приложению\).  
  
 Далее перечислены действия, которые нужно выполнить, чтобы создать новую версию **MessageVersion** для исходящего сообщения.  
  
 **Обработка запроса**  
  
-   Получение версии **MessageVersion** исходящей привязки или канала.  
  
-   Получение модуля чтения текста для первоначального сообщения.  
  
-   Создание нового сообщения с тем же действием и модулем чтения текста и новой версией сообщения **MessageVersion**.  
  
-   Если <xref:System.ServiceModel.Channels.MessageVersion.Addressing%2A> \!\= **Addressing.None**, скопируйте заголовки To, From, FaultTo и RelatesTo в новое сообщение.  
...
-   Получение версии **MessageVersion** первоначального сообщения с запросом.  
  
-   Получение модуля чтения текста для принятого ответного сообщения.  
  
-   Создание нового ответного сообщения с тем же действием и модулем чтения текста и версией **MessageVersion** первоначального сообщения запроса.  
...
-   Возврат нового ответного сообщения.  
 ...
 По умолчанию при запуске службы <xref:System.ServiceModel.Routing.RoutingBehavior> автоматически добавляет к клиентским конечным точкам **SoapProcessingBehavior**. Однако с помощью свойства <xref:System.ServiceModel.Routing.RoutingConfiguration.SoapProcessingEnabled%2A> можно управлять добавлением обработки SOAP ко всем клиентским точкам.  Можно напрямую добавить поведение к конкретной клиентской точке, а также включить или отключить это поведение на уровне конечной точки, если требуется более точное управление обработкой SOAP.  
...
>  Если обработка SOAP отключена для точки, требующей версии \(MessageVersion\), отличной от версии первоначального сообщения с запросом, то перед отправкой сообщения в целевую конечную точку необходимо указать пользовательский механизм для выполнения любых требуемых изменений SOAP.  
 ...
 В следующих примерах свойство **soapProcessingEnabled** используется, чтобы предотвратить автоматическое добавление **SoapProcessingBehavior** ко всем клиентским конечным точкам.  
...
### Динамическая конфигурация  
 ...
 Программным образом можно изменить только **RoutingConfiguration**.  Изначально можно настроить службу, используя файл конфигурации. Однако во время выполнения изменить конфигурацию можно только путем создания нового объекта **RoutingConfigution** и передачи его в качестве параметра методу <xref:System.ServiceModel.Routing.RoutingExtension.ApplyConfiguration%2A> расширения службы <xref:System.ServiceModel.Routing.RoutingExtension>.  При маршрутизации сообщений, передаваемых в настоящий момент, будет использована предыдущая конфигурация. Для всех сообщений, принятых после вызова метода **ApplyConfiguration**, будет использована новая конфигурация.  В следующем примере показано создание экземпляра службы маршрутизации с последующим изменением конфигурации.  
...
## Обработка ошибок  
 Если при попытке отправки сообщения возникнет исключение <xref:System.ServiceModel.CommunicationException>, будет запущена обработка ошибок.  Эти исключения обычно указывают на то, что при попытке связаться с определенной конечной точкой клиента была обнаружена проблема, например <xref:System.ServiceModel.EndpointNotFoundException>, <xref:System.ServiceModel.ServerTooBusyException> или <xref:System.ServiceModel.CommunicationObjectFaultedException>.  При возникновении исключения <xref:System.TimeoutException> можно обработать код ошибки и попытаться повторить отправку. Это общее исключение, которое не является производным от исключения **CommunicationException**.  
...
> [!NOTE]
>  Функция обработки ошибок перехватывает и обрабатывает исключения, возникающие при отправке сообщения или при закрытии канала.  Код обработки не предназначен для обнаружения или обработки исключений, созданных конечными точками приложения, с которым взаимодействует код. Исключение <xref:System.ServiceModel.FaultException>, сформированное службой, отображается в службе маршрутизации как **FaultMessage** и передается обратно клиенту.  
>   
...
|Одностороннее взаимодействие||![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "Checkmark")||Нет|Возникает исключение, и выполняется откат транзакции.|  
|Одностороннее взаимодействие|||![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "Checkmark")|Да|Пытается повторно отправить сообщение резервной конечной точке.  После успешного получения сообщения нужно закрыть все контексты получения.  Если сообщение не получено любой конечной точкой, завершать контекст получения не нужно.<br /><br /> В случае многоадресной рассылки сообщения контекст получения завершается, если сообщение получено хотя бы одной конечной точкой \(основной или резервной\).  Если ни одна из конечных точек многоадресной рассылки не получила сообщение, завершать контекст получения не нужно.|  
|Одностороннее взаимодействие||![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "Checkmark")|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "Checkmark")|Да|Отмените предыдущую транзакцию, создайте новую транзакцию и повторно отправьте все сообщения.  Сообщения, обнаруживающие ошибку, передаются в резервное назначение.<br /><br /> После создания транзакции, в которой все операции по передаче были успешными, можно завершить контексты получения и зафиксировать транзакцию.|  
|Одностороннее взаимодействие|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "Checkmark")|||Да|Пытается повторно отправить сообщение резервной конечной точке.  В случае многоадресного сценария в резервные назначения повторно отправляются только сообщения, в сеансах которых обнаружена ошибка или сбой при закрытии.|  
|Одностороннее взаимодействие|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "Checkmark")|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "Checkmark")||Нет|Возникает исключение, и выполняется откат транзакции.|  
|Одностороннее взаимодействие|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "Checkmark")||![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "Checkmark")|Да|Пытается повторно отправить сообщение резервной конечной точке.  После того как все сообщения отправлены без ошибок, сеанс указывает на то, что сообщений не осталось, служба маршрутизации закрывает все исходящие каналы сеанса, завершаются все контексты получения, а затем закрывается входящий канал сеанса.|  
|Одностороннее взаимодействие|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "Checkmark")|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "Checkmark")|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "Checkmark")|Да|Отмена текущей транзакции и создание новой.  Повторная отправка всех предыдущих сообщений в сеансе.  После создания транзакции, в которой все сообщения отправлены без ошибок, а также после того, как сеанс указывает на то, что сообщений не осталось, закрываются все исходящие каналы сеанса, завершаются все контексты получения, закрывается входящий канал и происходит фиксация транзакции.<br /><br /> Если в сеансах использовалась многоадресная рассылка, сообщения без ошибок повторно отправляются в то же назначение, а сообщения с ошибками \- в резервное назначение.|  
...
|Двусторонний|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "Checkmark")|||Да|Отправка всех сообщений в канале в резервное назначение.  После того как канал вернет ответное сообщение, верните ответ исходному клиенту.|  
|Двусторонний||![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "Checkmark")||Нет|Возникает исключение, и выполняется откат транзакции.|  
|Двусторонний|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "Checkmark")|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "Checkmark")||Нет|Возникает исключение, и выполняется откат транзакции.|  
|Дуплекс||||Нет|В настоящий момент дуплексная связь вне сеанса не поддерживается.|  
|Дуплекс|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "Checkmark")|||Да|Отправьте резервному назначению.|  
...
## См. также  
 ...
 [Выбор фильтра](../../../../docs/framework/wcf/feature-details/choosing-a-filter.md)