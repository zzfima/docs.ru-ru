...
## <a name="configuration"></a>Конфигурация  
 Служба маршрутизация представляет собой службу WCF, которая обнаруживает одну или несколько конечных точек службы, получающих сообщения от клиентских приложений и перенаправляющих их в одну или несколько целевых конечных точек. Эта служба предоставляет поведение <xref:System.ServiceModel.Routing.RoutingBehavior>, которое применяется к обнаруженным конечным точкам службы. Это поведение используется для настройки различных аспектов работы службы. Для упрощения настройки при помощи файла конфигурации, параметры заданы на **RoutingBehavior**. В сценариях, основанная на коде, эти параметры указываются как часть <xref:System.ServiceModel.Routing.RoutingConfiguration> объекта, который может быть передан **RoutingBehavior**.  
  
 При запуске этого поведения к клиентским конечным точкам добавляется параметр <xref:System.ServiceModel.Routing.SoapProcessingBehavior>, который используется для обработки сообщений SOAP. Это позволяет службе маршрутизации передавать сообщения в конечные точки, требующие **MessageVersion** дальше конечной точки, через которую было получено сообщение. **RoutingBehavior** также регистрирует расширение службы <xref:System.ServiceModel.Routing.RoutingExtension>, которое предоставляет точку доступа, используемую для изменения конфигурации службы во время выполнения.  
  
 **RoutingConfiguration** класса позволяет согласованным образом настраивать и обновлять конфигурацию службы маршрутизации.  Этот класс содержит параметры, которые действуют как параметры для службы маршрутизации и используется для настройки **RoutingBehavior** при запуске службы или передается **RoutingExtension** для изменения маршрутизации конфигурации во время выполнения.  
  
 Логика маршрутизации на основе содержимого определяется путем группирования нескольких объектов <xref:System.ServiceModel.Dispatcher.MessageFilter> в таблицы фильтрации (объекты <xref:System.ServiceModel.Dispatcher.MessageFilterTable%601>). Входящие сообщения анализируются с использованием фильтров сообщений, содержащихся в таблице фильтров, а также для каждого **MessageFilter** , соответствующего сообщения перенаправляются в целевую конечную точку. Таблица фильтров, который должен использоваться для маршрутизации сообщений задается с помощью либо **RoutingBehavior** в конфигурации или посредством кода с помощью **RoutingConfiguration** объекта.  
...
>  При работе с контрактами, задействующими несколько шаблонов взаимодействия (например со смешением односторонних и двусторонних операций), правильным решением будет использование в службе маршрутизации дуплексного контракта (например <xref:System.ServiceModel.Routing.IDuplexSessionRouter>). Это значит, что привязка должна быть совместима с дуплексным взаимодействием, однако это неприменимо ко всем сценариям. В сценариях, для которых это неприменимо, может понадобиться разбиение модели взаимодействия на несколько конечных точек либо изменение приложения.  
 ...
 Дополнительные сведения о маршрутизации контрактах см. в разделе [контракты маршрутизации](../../../../docs/framework/wcf/feature-details/routing-contracts.md).  
  
 После определения конечной точки службы можно использовать **RoutingBehavior** для связывания конкретной **RoutingConfiguration** с конечной точкой. При настройке службы маршрутизации с помощью файла конфигурации, **RoutingBehavior** используется для указания фильтра таблицы, содержащей логику маршрутизации для обработки сообщений, получаемых этой конечной точки. При настройке службы маршрутизации программным образом можно указать таблицу фильтров с помощью **RoutingConfiguration**.  
...
### <a name="routing-logic"></a>Логика маршрутизации  
 ...
 Служба маршрутизации предоставляет несколько **MessageFilter** реализации, которые проверяют наличие конкретных значений в сообщении, такие как адрес, действие, имя конечной точки или запрос XPath. Если ни один из этих реализаций отвечают вашим требованиям, можно создать пользовательский **MessageFilter** реализации. Дополнительные сведения о фильтры сообщений и сравнения реализаций, используемых службой маршрутизации, в разделе [фильтры сообщений](../../../../docs/framework/wcf/feature-details/message-filters.md) и [Выбор фильтра](../../../../docs/framework/wcf/feature-details/choosing-a-filter.md).  
 ...
 По умолчанию все фильтры сообщений в таблице оцениваются одновременно. Однако, если нужно, чтобы фильтры оценивались в определенном порядке, можно указать приоритет оценки - <xref:System.ServiceModel.Routing.Configuration.FilterTableEntryElement.Priority%2A>. Сначала оцениваются все записи с наивысшим приоритетом. Если найдено сопоставление на более высоком уровне, фильтры сообщений с низким приоритетом не оцениваются. Дополнительные сведения о таблицах фильтра см. в разделе [фильтры сообщений](../../../../docs/framework/wcf/feature-details/message-filters.md).  
  
 В следующем примере использован фильтр <xref:System.ServiceModel.Dispatcher.MatchAllMessageFilter>, который оценивает все сообщения как `true`. Это **MessageFilter** добавляется в таблицу фильтров «routingTable1», которая связывает **MessageFilter** с клиентской конечной точкой «CalculatorService». **RoutingBehavior** указывает, что в этой таблице следует использовать для маршрутизации сообщений, обрабатываемых этой конечной точкой службы.  
...
### <a name="soap-processing"></a>Обработка SOAP  
 Для поддержки маршрутизации сообщений между разнородными протоколами **RoutingBehavior** по умолчанию добавляет <xref:System.ServiceModel.Routing.SoapProcessingBehavior> для всем клиентским конечным точкам, которые направляются сообщения. Это поведение автоматически создает новую **MessageVersion** до маршрутизации сообщения в конечную точку, а также создается подходящая **MessageVersion** для любого ответного документа перед его возвратом запрашивающему клиентскому приложению.  
  
 Шаги, необходимые для создания нового **MessageVersion** для исходящего сообщения, как показано ниже:  
  
 **Обработка запросов**  
  
-   Получить **MessageVersion** исходящей привязки или канала.  
  
-   Получение модуля чтения текста для первоначального сообщения.  
  
-   Создайте новое сообщение с одним действием, модуля чтения текста и новый **MessageVersion**.  
  
-   Если <xref:System.ServiceModel.Channels.MessageVersion.Addressing%2A> ! = **Addressing.None**, скопируйте, From, FaultTo и RelatesTo в новое сообщение.  
...
-   Возврат нового ответного сообщения.  
 ...
 По умолчанию **SoapProcessingBehavior** автоматически добавляется к клиентским конечным точкам, <xref:System.ServiceModel.Routing.RoutingBehavior> при запуске службы; Однако можно управлять, добавляется ли обработка SOAP для всех клиентских конечных точек с помощью <xref:System.ServiceModel.Routing.RoutingConfiguration.SoapProcessingEnabled%2A> свойство. Можно напрямую добавить поведение к конкретной клиентской точке, а также включить или отключить это поведение на уровне конечной точки, если требуется более точное управление обработкой SOAP.  
...
>  Если обработка SOAP отключена для точки, требующей версии (MessageVersion), отличной от версии первоначального сообщения с запросом, то перед отправкой сообщения в целевую конечную точку необходимо указать пользовательский механизм для выполнения любых требуемых изменений SOAP.  
 ...
 В следующих примерах **soapProcessingEnabled** свойство используется для предотвращения **SoapProcessingBehavior** автоматическое добавление ко всем конечным точкам клиента.  
...
### <a name="dynamic-configuration"></a>Динамическая конфигурация  
 ...
 Можно изменять только **RoutingConfiguration** программными средствами. Изначально можно настроить службу с помощью файла конфигурации, конфигурации во время выполнения можно изменить только путем создания нового **RoutingConfigution** и передачи его в качестве параметра <xref:System.ServiceModel.Routing.RoutingExtension.ApplyConfiguration%2A> метод предоставляемые <xref:System.ServiceModel.Routing.RoutingExtension> расширения службы. Наличие сообщений, передаваемых в настоящий момент по-прежнему будет использована предыдущую конфигурацию при маршрутизации сообщений, полученных после вызова **ApplyConfiguration** использовать новую конфигурацию. В следующем примере показано создание экземпляра службы маршрутизации с последующим изменением конфигурации.  
...
## <a name="error-handling"></a>Обработка ошибок  
 Если при попытке отправки сообщения возникнет исключение <xref:System.ServiceModel.CommunicationException>, будет запущена обработка ошибок. Эти исключения обычно указывают на то, что при попытке связаться с определенной конечной точкой клиента была обнаружена проблема, например <xref:System.ServiceModel.EndpointNotFoundException>, <xref:System.ServiceModel.ServerTooBusyException> или <xref:System.ServiceModel.CommunicationObjectFaultedException>. Код обработки ошибок также перехватит и попытается повторить отправку при <xref:System.TimeoutException> происходит, это другой общее исключение, которое не является производным от **CommunicationException**.  
...
|Одностороннее взаимодействие||![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "флажком")||Нет|Возникает исключение, и выполняется откат транзакции.|  
|Одностороннее взаимодействие|||![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "флажком")|Да|Пытается повторно отправить сообщение резервной конечной точке. После успешного получения сообщения нужно закрыть все контексты получения. Если сообщение не получено любой конечной точкой, завершать контекст получения не нужно.<br /><br /> В случае многоадресной рассылки сообщения контекст получения завершается, если сообщение получено хотя бы одной конечной точкой (основной или резервной). Если ни одна из конечных точек многоадресной рассылки не получила сообщение, завершать контекст получения не нужно.|  
|Одностороннее взаимодействие||![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "флажком")|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "флажком")|Да|Отмените предыдущую транзакцию, создайте новую транзакцию и повторно отправьте все сообщения. Сообщения, обнаруживающие ошибку, передаются в резервное назначение.<br /><br /> После создания транзакции, в которой все операции по передаче были успешными, можно завершить контексты получения и зафиксировать транзакцию.|  
|Одностороннее взаимодействие|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "флажком")|||Да|Пытается повторно отправить сообщение резервной конечной точке. В случае многоадресного сценария в резервные назначения повторно отправляются только сообщения, в сеансах которых обнаружена ошибка или сбой при закрытии.|  
|Одностороннее взаимодействие|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "флажком")|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "флажком")||Нет|Возникает исключение, и выполняется откат транзакции.|  
|Одностороннее взаимодействие|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "флажком")||![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "флажком")|Да|Пытается повторно отправить сообщение резервной конечной точке. После того как все сообщения отправлены без ошибок, сеанс указывает на то, что сообщений не осталось, служба маршрутизации закрывает все исходящие каналы сеанса, завершаются все контексты получения, а затем закрывается входящий канал сеанса.|  
|Одностороннее взаимодействие|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "флажком")|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "флажком")|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "флажком")|Да|Отмена текущей транзакции и создание новой. Повторная отправка всех предыдущих сообщений в сеансе. После создания транзакции, в которой все сообщения отправлены без ошибок, а также после того, как сеанс указывает на то, что сообщений не осталось, закрываются все исходящие каналы сеанса, завершаются все контексты получения, закрывается входящий канал и происходит фиксация транзакции.<br /><br /> Если в сеансах использовалась многоадресная рассылка, сообщения без ошибок повторно отправляются в то же назначение, а сообщения с ошибками - в резервное назначение.|  
...
|Двусторонний|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "флажком")|||Да|Отправка всех сообщений в канале в резервное назначение.  После того как канал вернет ответное сообщение, верните ответ исходному клиенту.|  
|Двусторонний||![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "флажком")||Нет|Возникает исключение, и выполняется откат транзакции.|  
|Двусторонний|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "флажком")|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "флажком")||Нет|Возникает исключение, и выполняется откат транзакции.|  
|Дуплекс||||Нет|В настоящий момент дуплексная связь вне сеанса не поддерживается.|  
|Дуплекс|![Флажок](../../../../docs/framework/wcf/feature-details/media/checkmark.gif "флажком")|||Да|Отправьте резервному назначению.|  
