...
- Возврат нового ответного сообщения.  
 ...
 По умолчанию **соаппроцессингбехавиор** автоматически добавляется в конечные точки <xref:System.ServiceModel.Routing.RoutingBehavior> клиента при запуске службы. Однако можно контролировать, добавлена ли обработка SOAP ко всем <xref:System.ServiceModel.Routing.RoutingConfiguration.SoapProcessingEnabled%2A> конечным точкам клиента с помощью свойства. . Можно напрямую добавить поведение к конкретной клиентской точке, а также включить или отключить это поведение на уровне конечной точки, если требуется более точное управление обработкой SOAP.  
