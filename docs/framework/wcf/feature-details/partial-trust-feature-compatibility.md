...
ms.locfileid: "33496790"
---
# <a name="partial-trust-feature-compatibility"></a>Совместимость возможностей частичного доверия
Windows Communication Foundation (WCF) поддерживает ограниченный набор функциональности при выполнении в среде с частичным доверием. Дополнительные сведения о различных функциях, поддерживаемых при работе с частичным доверием, которые разработаны на базе определенного набора сценариев, см. в разделе [Supported Deployment Scenarios](../../../../docs/framework/wcf/feature-details/supported-deployment-scenarios.md) .  
...
### <a name="security"></a>Безопасность  
 Частично доверенные приложения можно использовать возможности безопасности транспортного уровня WCF для обеспечения безопасности своего взаимодействия. Безопасность уровня сообщений не поддерживается. Настройка привязки на использование безопасности уровня сообщений вызовет исключение в среде выполнения.  
...
-   Все сериализуемые поля и свойства `[DataMember]` в типе `[DataContract]` должны быть открытыми и доступными для чтения и записи. Сериализация и десериализация [readonly](http://go.microsoft.com/fwlink/?LinkID=98854) поля не поддерживается при выполнении в частично доверенного приложения WCF.  
...
### <a name="collection-types"></a>Типы коллекций  
 ...
 Для типов коллекций, WCF работает в среде с частичным доверием, если ни одна из `GetEnumerator()` реализации являются `public`, или ни один из них не явные реализации интерфейса, то возникает исключение безопасности.  
...
## <a name="configuration"></a>Конфигурация  
 За одним исключением, частично доверенный код может загружать только разделы конфигурации WCF в локальном `app.config` файла. Для загрузки разделов конфигурации WCF, которые ссылаются на разделы WCF в файле machine.config или в корневом файле web.config требуется разрешение ConfigurationPermission(Unrestricted). Без этого разрешения ссылок на WCF разделов конфигурации (поведения, привязки) за пределами локальной конфигурации файла приводят к возникновению исключения при загрузке конфигурации.  
...
## <a name="other-limitations"></a>Другие ограничения  
 WCF обычно ограничивается вопросами безопасности, установленными ведущим приложением. Например, если WCF размещается в приложения браузера XAML (XBAP), она есть ограничения XBAP, как описано в [Foundation частичного доверия безопасность платформы Windows Presentation](http://go.microsoft.com/fwlink/?LinkId=89138).  
...
-   Счетчики производительности.  
 ...
 Использование функции WCF, которые не поддерживаются в среде с частичным доверием может привести к исключениям во время выполнения.  
