...
-   `TransactionScopeRequired`. Если при обработке пакета сообщений WCF находит тот, который имеет `TransactionScopeRequired`  =  `false`, он фиксирует пакет и открывается новый пакет при получении первого сообщения со значениями `TransactionScopeRequired`  =  `true` и `TransactionAutoComplete`  = `true`.  
