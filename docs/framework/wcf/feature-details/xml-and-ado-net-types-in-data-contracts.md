...
-   Все средства записи XML, поставляемых вместе с [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)] отклоняют инструкции по обработке XML (\<? … ? >) и определения типа документа (\<! … >), так как они не разрешены в сообщениях SOAP. И, опять же, чтобы обойти данное ограничение, можно использовать собственный механизм кодирования. Если нужно включить их в готовый XML-код, можно написать пользовательский кодировщик, использующий поддерживающие их средства записи XML.  
