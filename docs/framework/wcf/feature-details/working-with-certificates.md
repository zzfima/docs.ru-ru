...
## <a name="viewing-certificates"></a>Просмотр сертификатов  
 При работе с сертификатами зачастую необходимо просматривать их и проверять определенные свойства. Это легко выполняется с помощью консоли управления (MMC). Дополнительные сведения см. в разделе [Практическое руководство. Просмотр сертификатов с помощью оснастки](how-to-view-certificates-with-the-mmc-snap-in.md)MMC.  
...
## <a name="chain-trust-and-certificate-authorities"></a>Цепочка доверия и центры сертификации  
 ...
 Цифровые сертификаты используются для удостоверения подлинности сущности на основе этой иерархии, которая также называется *цепочкой сертификатов*. Цепочку любого сертификата можно просмотреть с помощью оснастки MMC, дважды щелкнув сертификат и выбрав вкладку **Путь сертификата**. Дополнительные сведения об импорте цепочек сертификатов для центра сертификации см. [в разделе как Укажите цепочку сертификатов центра сертификации, используемую для](specify-the-certificate-authority-chain-verify-signatures-wcf.md)проверки подписей.  
...
4. Пошаговые инструкции см. в разделе [как Создание временных сертификатов для использования во время](how-to-create-temporary-certificates-for-use-during-development.md)разработки.  
