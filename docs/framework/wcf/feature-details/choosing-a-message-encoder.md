...
|------------|-----------------|---------------------------------------|  
|Поддерживаемые кодировки|<xref:System.ServiceModel.Channels.TextMessageEncodingBindingElement> и <<!--zz xref:System.ServiceModel.Channels.MTOMMessageEncodingBindingElement --> `System.ServiceModel.Channels.MTOMMessageEncodingBindingElement`> поддерживает только UTF8 и UTF16 в Юникоде (*обратный* и *с прямым порядком байтов*) кодировки. Если требуются другие кодировки, такие как UTF7 или ASCII, необходимо использовать пользовательский кодировщик. Пример пользовательского кодировщика, см. в разделе [пользовательский кодировщик сообщений](https://go.microsoft.com/fwlink/?LinkId=119857).|Text|  
...
|Взаимодействие|Под этим фактором понимается способность кодировщика WCF для взаимодействия со службами не WCF.|Text<br /><br /> MTOM (частично)|  
