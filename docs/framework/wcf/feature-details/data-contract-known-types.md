...
- Контракт отправляемых данных унаследован из контракта ожидаемых данных. Дополнительные сведения см. в разделе, посвященном наследованию, в [эквивалентность контрактов данных](../../../../docs/framework/wcf/feature-details/data-contract-equivalence.md)). В этом случае контракт передаваемых данных отличается от контракта данных, ожидаемого принимающей конечной точкой;  
...
## <a name="known-types-using-open-generic-methods"></a>Известные типы и открытые универсальные методы  
 ...
 Эту проблему можно решить с помощью альтернативного механизма: Написание метода, который возвращает список типов, добавляемый в коллекцию известных типов. После этого имя метода задается в качестве строкового аргумента атрибута `KnownTypeAttribute` с некоторыми ограничениями.  
...
## <a name="additional-ways-to-add-known-types"></a>Дополнительные способы добавления известных типов  
 Кроме того, известные типы можно добавлять с помощью файла конфигурации. Это полезно в том случае, если вы не управляете тип, который требуется для правильной сериализации, например когда библиотеки с помощью Windows Communication Foundation (WCF) с помощью сторонних типов известных типов.  
