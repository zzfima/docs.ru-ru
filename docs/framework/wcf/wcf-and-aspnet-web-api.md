...
|Поддерживает шаблоны обмена сообщениями «запрос-ответ», «односторонний» и «дуплексный».|HTTP — запрос/ответ, но дополнительные шаблоны, которые могут поддерживаться через [SignalR](https://github.com/SignalR/SignalR) и интеграции WebSockets.|  
