...
2. После создания клиента WCF задайте для свойства <xref:System.ServiceModel.Security.WindowsClientCredential.AllowedImpersonationLevel%2A> класса <xref:System.ServiceModel.Security.WindowsClientCredential> одно из значений перечисления <xref:System.Security.Principal.TokenImpersonationLevel>.  
