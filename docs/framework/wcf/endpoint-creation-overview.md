...
- Определите контракт для типа службы `IEcho`, который принимает имя пользователя и эхо с ответом "Hello \<name >!".
...
> [!NOTE]
> Используйте [служебную программу метаданных ServiceModel (Svcutil. exe)](servicemodel-metadata-utility-tool-svcutil-exe.md) с параметром `/config:`*имя_файла*`[,`*filename*`]`, чтобы быстро создать файлы конфигурации.
