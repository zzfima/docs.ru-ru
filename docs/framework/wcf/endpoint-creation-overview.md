...
- Определите контракт для `IEcho` тип службы, принимающей чужое имя и выведите на экран с ответ «Hello \<имя >!».  
  
- Реализуйте службу `Echo` типа, определенного контрактом `IEcho`.  
  
- Укажите адрес конечной точки из `http://localhost:8000/Echo` для службы.  
...
> [!NOTE]
>  Используйте [ServiceModel Metadata Utility Tool (Svcutil.exe)](../../../docs/framework/wcf/servicemodel-metadata-utility-tool-svcutil-exe.md) с `/config:` *filename*`[,`*filename* `]` переключиться в режим быстрого создания файлов конфигурации.  
